				...
				$html .= "<div id='top_of_first_post'>";
					...
					$html .= "<div class='topic-post-header-with-fav'>";
						...
						$html .= "<div class='topic-post-header'>";
							...
							if ( ($post->topic_owner == $current_user->ID && $seconds_left > 0) || (current_user_can('level_10')) ) {
								...
								if ( (get_option(WPS_OPTIONS_PREFIX.'_use_votes_remove') == $score) && (__wps__get_current_userlevel() < 5) && ($score != 0) ) {
									...
									$reply_html .= "<div class='topic-edit-delete-icon'>";
										...
										if ( ($child->topic_owner == $current_user->ID) || (current_user_can('level_10')) ) {
											...
											if ( ($child->topic_owner == $current_user->ID && $seconds_left > 0) || (current_user_can('level_10')) ) {
				...
				if ($limit_from == 0) {
					...
					foreach ($query as $topic) {
						...
						$html .= $color_check.'">';
							...
							if (current_user_can('level_10') || ($current_user->ID == $topic->topic_owner && $seconds_left > 0)) {
									...
									if  ( ($comment->type != 'poke') || ($comment->type == 'poke' && ($comment->author_uid == $uid2 || $comment->subject_uid == $uid2 )) ) {	
										...
										$comment_div = "<div class='wall_post_div' id='post_".$comment->cid."'>";
											...
											if ($comment->author_uid != $comment->subject_uid && !$comment->is_group) {
												$comment_inner_div .= ' &rarr; <a href="'.$profile_page.$q.'uid='.$comment->subject_uid.'">'.stripslashes($comment->subject_name).'</a> ';
											}
												...
												) 
											{
												if ($comment->type != 'gallery') {
													...
													if (get_option(WPS_OPTIONS_PREFIX.'_show_buttons')) {
														$comment_inner_div .= '<br /><input title="'.$comment->cid.'" type="submit" style="width:75px" class="__wps__button symposium_add_reply" value="'.__('Add', WPS_TEXT_DOMAIN).'" />';
													}
