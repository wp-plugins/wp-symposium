			...
			if ($can_reply) {
				...
				if ($group_id == 0) {
					...
					if (get_option(WPS_OPTIONS_PREFIX.'_forum_login') && !is_user_logged_in()) {
						$html .= ' '.sprintf(__("<a href='%s'>Login...</a>", WPS_TEXT_DOMAIN), site_url().'/wp-login.php?redirect_to='.urlencode(__wps__pageURL()));
					}
		...
		if ($limit_from == 0) {
			...
			if (is_user_logged_in()) {
				...
				if (get_option(WPS_OPTIONS_PREFIX.'_forum_login') == "on") {
					$new_topic_form .= "<p>".__("Until you login, you can only view the forum.", WPS_TEXT_DOMAIN);
...
function __wps__create_usermeta($uid) {
	...
	if ($uid > 0) {
		...
		if ($wpdb->get_var($wpdb->prepare("SELECT meta_key FROM ".$wpdb->base_prefix."usermeta WHERE meta_key = 'symposium_extended_city' AND user_id = %d", $uid)) != 'symposium_extended_city') {
			...
			__wps__audit('usermeta', $uid, 'symposium_share', 'Friends only', 'create');
			...
			__wps__audit('usermeta', $uid, 'symposium_wall_share', 'Friends only', 'create');
			...
			__wps__audit('usermeta', $uid, 'symposium_last_login', '[null]', 'create');
...
//	if ($uid1 > 0) {
		...
		if (is_user_logged_in() || $privacy == 'public') {	
			...
			return sprintf(__("Please <a href='%s'>login</a> to view this member's profile.", WPS_TEXT_DOMAIN), site_url().'/wp-login.php?redirect_to='.__wps__pageURL());		
					...
					if ($show_add_comment) {
						...
						$html .= '<div id="symposium_add_status">';
							...
							if (get_option(WPS_OPTIONS_PREFIX."_activity_youtube")) {
								$html .= '<input type="submit" id="activity_youtube_embed_button" onclick="return false;" class="__wps__button" value="'.__('YouTube', WPS_TEXT_DOMAIN).'">';
									...
									if  ( ($comment->type != 'poke') || ($comment->type == 'poke' && ($comment->author_uid == $uid2 || $comment->subject_uid == $uid2 )) ) {	
										...
										$html .= "<div class='wall_post_div' id='post_".$comment->cid."'>";
											...
											if ($comment->is_group) {
												...
												$html .= __("Group post in", WPS_TEXT_DOMAIN)." <a href='".$url."'>".stripslashes($group_name)."</a>: ".__wps__make_url($text);
											} else {
												$html .= __wps__make_url($text);
											}
