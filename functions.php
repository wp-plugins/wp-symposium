...
function __wps__getTopic($topic_id, $group_id=0) {
	...
	if (strpos($viewer, __('everyone', WPS_TEXT_DOMAIN)) !== FALSE) $can_view = true;
	...
	if ($group_id > 0) {
		$sql = "SELECT COUNT(*) FROM ".$wpdb->prefix."symposium_group_members WHERE group_id=%d AND valid='on' AND member_id=%d";
		$member_count = $wpdb->get_var($wpdb->prepare($sql, $group_id, $current_user->ID));
		if ($member_count == 0) { $can_reply = false; } else { $can_comment = true; }
	}
	...
	if ( $can_view ) {
		...
		if ($post) {
			...
			if (is_user_logged_in()) {
				$html .= "<div id='__wps__topic_options' class='floatleft label'>";

					// Add Quickjump drop-down list
					$html .= '<div style="padding-right:15px; float:left;">'.__wps__forum_dropdown($cat_id, $topic_id, $group_id).'</div>';				
						...
						if (get_option(WPS_OPTIONS_PREFIX.'_forum_info')) {
							...
							if (get_option(WPS_OPTIONS_PREFIX.'_use_answers') == 'on') {
								$html .= "<div class='forum_info'>";
									...
									if (WPS_DEBUG) $html .= __wps__displayArray($settings);
					...
					$html .= "<div class='topic-post-header-with-fav'>";
						...
						$html .= "<div class='topic-post-header'>";
							...
							if (get_option(WPS_OPTIONS_PREFIX.'_allow_reports') == 'on') {
								$html .= "<a href='javascript:void(0)' title='forum_".$post->tid."' class='report label symposium_report' style='display:none; cursor:pointer'><div class='topic-edit-icon'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/warning.png' /></a></div>";
							}
							if ( ($post->topic_owner == $current_user->ID) || (current_user_can('level_10')) ) {
								...
								if ( (get_option(WPS_OPTIONS_PREFIX.'_forum_lock')==0) || ($post->topic_started > $now) || (current_user_can('level_10')) ) {
									$html .= "<a href='javascript:void(0)' title='".$post->tid."' id='edit-this-topic' class='edit_topic edit label' style='cursor:pointer'><div class='topic-edit-icon'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/edit.png' /></a></div>";
								}
							}
						...
						$post_text = __wps__make_url(stripslashes($post->topic_post));
						$has_code = strpos($post_text, '[code]') ? true : false;
						$post_text = __wps__bbcode_replace($post_text);
						if (!$has_code) $post_text = __wps__buffer($post_text);
							...
							if (file_exists($targetPath)) {
								...
								foreach($file_list as $file) {
									...
									if (strpos(get_option(WPS_OPTIONS_PREFIX.'_image_ext'), $ext[sizeof($ext)-1]) > 0) {
										...
										// Video
										if (strpos(get_option(WPS_OPTIONS_PREFIX.'_video_ext'), $ext[sizeof($ext)-1]) > 0) {
				...
				if ($child_query) {
					...
					foreach ($child_query as $child) {
						...
						$reply_html .= "<div id='reply".$child->tid."' class='child-reply";
								...
								if ( (get_option(WPS_OPTIONS_PREFIX.'_use_votes_remove') == $score) && (__wps__get_current_userlevel() < 5) && ($score != 0) ) {
									...
									$reply_html .= "<div class='topic-edit-delete-icon'>";
										...
										if ( ($child->topic_owner == $current_user->ID) || (current_user_can('level_10')) ) {
											$now = date('Y-m-d H:i:s', time() - get_option(WPS_OPTIONS_PREFIX.'_forum_lock') * 60);
											if ( (get_option(WPS_OPTIONS_PREFIX.'_forum_lock')==0) || ($child->topic_started > $now) || (current_user_can('level_10')) ) {
												$reply_html .= "<a href='javascript:void(0)' class='floatright link_cursor delete_forum_reply' style='display:none' id='".$child->tid."'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/delete.png' /></a>";
												$reply_html .= "<a href='javascript:void(0)' class='floatright link_cursor edit_forum_reply' style='display:none; margin-right: 5px' id='".$child->tid."'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/edit.png' /></a>";
											}
										}
									$reply_html .= "</div>";
									...
									$reply_html .= "<div id='child_".$child->tid."' class='child-reply-post'>";
										...
										if (!$has_code) $reply_text = __wps__buffer($reply_text);
										if (!get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg')) {
							...
							if (WPS_DEBUG) $reply_html .= $wpdb->last_query;
							if ($comments)
								$reply_html .= "<div style='position:relative;top:57px;left:-53px;'><img style='border:0;box-shadow:none;width:32px;height:32px;' src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/comment.png' /></div>";
								...
								if ($comments) {
									foreach ($comments AS $comment) {
										$reply_html .= "<div id='comment".$comment->tid."' class='reply-comments-reply'>";
											...
											$reply_html .= "<div style='margin-left: 45px;'>";
												...
												$reply_html .= "<div class='topic-edit-delete-icon'>";
													...
													// Delete comment
													if ( ($comment->topic_owner == $current_user->ID) || (current_user_can('level_10')) ) {
			...
			if ($can_reply) {
				...
				if ($wpdb->get_var($wpdb->prepare("SELECT allow_replies FROM ".$wpdb->prefix."symposium_topics WHERE tid = %d", $post->tid)) == "on")
				{
					...
					if (get_option(WPS_OPTIONS_PREFIX.'_use_wp_editor')) {
						...
						$html .= '<div><textarea class="textarea_Editor reply-topic-text'.$elastic.'" style="width:'.get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_width').'; height:'.get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_height').'" id="__wps__reply_text"></textarea></div>';
						$html .= '<div class="sep"></div>';
					}

					// For admin's only set this as the answer
					if (get_option(WPS_OPTIONS_PREFIX.'_use_answers') == 'on' && __wps__get_current_userlevel() == 5) {
						$html .= '<input type="checkbox" id="quick-reply-answer" /> '.__('Set this as the answer', WPS_TEXT_DOMAIN).'<br />';
					}
				...
				if ($group_id == 0) {
					...
					if (__wps__get_current_userlevel() == 5) $html .= sprintf(__('<br />Permissions are set via the WordPress admin dashboard->%s->Forum.', WPS_TEXT_DOMAIN), WPS_WL_SHORT);	
					$html .= "</p>";
					...
					if (get_option(WPS_OPTIONS_PREFIX.'_forum_login') && !is_user_logged_in()) {
						...
						$html .= wp_login_form(array(
   					 		'echo' => false,
						    'redirect' => get_option(WPS_OPTIONS_PREFIX.'_forum_url').'#cid='.$post->topic_category.',tid='.$post->tid,
		...
		if ($wpdb->get_var($wpdb->prepare($sql, $topic_id))) {
			...
			if (__wps__get_current_userlevel() == 5) $html .= '<br /><br />'.sprintf(__('Permissions are set via the WordPress admin dashboard->%s->Forum.', WPS_TEXT_DOMAIN), WPS_WL_SHORT);
		} else {
			...
			if (is_user_logged_in()) {
				...
				if ( ($can_edit) && ($allow_new == 'on') ) {
					...
					if ($group_id == 0 && $allow_new == 'on') {
						...
						if (__wps__get_current_userlevel() == 5) $new_topic_form .= sprintf(__('<br />Permissions are set via the WordPress admin dashboard->%s->Forum.', WPS_TEXT_DOMAIN), WPS_WL);	
						$new_topic_forum .= "</p>";
					}
				...
				if (get_option(WPS_OPTIONS_PREFIX.'_forum_login') == "on") {
					$new_topic_form .= "<div class='sep'>".__("Until you login, you can only view the forum.", WPS_TEXT_DOMAIN);
					$new_topic_form .= " <a href=".wp_login_url( get_permalink() )." class='simplemodal-login' title='".__("Login", WPS_TEXT_DOMAIN)."'>".__("Login", WPS_TEXT_DOMAIN).".</a></div><br />";
				}
				...
				if ($categories) {
					...
					$html .= '<div id="__wps__table" class="__wps__forum_table">';
						...
						foreach($categories as $category) {
							...
							if (WPS_DEBUG) $html .= 'Checking forum category role '.$user_role.' against '.$cat_roles.'<br />'.strpos($cat_roles, $user_role.',').'<br />';
							...
							if ( ($rank_ok) && (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos($cat_roles, $user_role.',') !== FALSE) ) {
								...
								$html .= '<div class="__wps__table_row_sep"></div>';
				...
				if ($limit_from == 0) {
					...
					foreach ($query as $topic) {
						...
						$html .= $color_check.'">';
							...
							// Views
							if (!get_option(WPS_OPTIONS_PREFIX.'_wps_lite')) {
								$views_html = "<div class='post_count' style='color:".get_option(WPS_OPTIONS_PREFIX.'_text_color').";'>".$reply_views."</div>";
								if ($reply_views != 1) { 
							...
							// Replies
							if (!get_option(WPS_OPTIONS_PREFIX.'_wps_lite')) {
								$replies_html = "<div class='post_count' style='color:".get_option(WPS_OPTIONS_PREFIX.'_text_color').";'>".$replies."</div>";
							...
							if (current_user_can('level_10') || $current_user->ID == $topic->topic_owner) {
								...
								$topic_title_html .= "<a class='floatright delete_topic link_cursor' id='".$topic->tid."'style='width:16px;'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/delete.png' style='width:16px; height:16px;' /></a>";
...
function __wps__show_profile_menu_tabs($uid1, $uid2) {
	...
	global $wpdb, $current_user;
		...
		$privacy = __wps__get_meta($uid1, 'wall_share');		
		$is_friend = __wps__friend_of($uid1, $current_user->ID);
		if ( $wpdb->get_results( $wpdb->prepare("SELECT meta_key FROM ".$wpdb->base_prefix."usermeta WHERE user_ID = %d AND meta_key LIKE '%symposium_extended_%' AND meta_value != ''", $uid) ) > 0 ) { $extended = "on"; } else { $extended = ""; }
			...
			foreach($str_arr as $item) {
				...
				if (strpos($item, '=') !== false) {
					...
					case 'settings':
						...
						break;
					case 'avatar' :
						if ( ($uid1 == $uid2 || __wps__get_current_userlevel() == 5) && (get_option(WPS_OPTIONS_PREFIX.'_profile_avatars') == "on") )
...
function __wps__friend_of($from, $to) {
	...
	if ( $wpdb->get_var($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix."symposium_friends WHERE (friend_accepted = 'on') AND (friend_from = %d AND friend_to = %d)", $from, $to)) ) {
		return true;
	} else {
		return false;
	}
...
function __wps__is_following($uid, $following) {
	...
	if ( $wpdb->get_var($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix."symposium_following WHERE uid = %d AND following = %d", $uid1, $following)) ) {
		return true;
	} else {
		return false;
	}
...
function __wps__profile_link($uid) {
	global $wpdb;
...
function __wps__profile_body($uid1, $uid2, $post, $version, $limit_from, $exclude_info_box=true) {
...
//	if ($uid1 > 0) {
		...
		if (is_user_logged_in() || $privacy == 'public') {	
						...
						if ($fields) {
							...
							foreach ($fields as $field) {
								...
								if ( ($split[0] != '') && ($split[1] != '') ) {
									...
									$extension = $wpdb->get_row($wpdb->prepare("SELECT extended_name,extended_order FROM ".$wpdb->prefix."symposium_extended WHERE eid = %d", $split[0]));
			...
			return __("Please login to view this member's profile.", WPS_TEXT_DOMAIN);
		...
		if ( ($uid1 == $uid2) || (is_user_logged_in() && strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'public') || (strtolower($privacy) == 'friends only' && $is_friend) || __wps__get_current_userlevel() == 5) {
			...
			$html .= "<div id='__wps__wall'>";
						...
						$html .= '<div id="symposium_add_status">';
							...
							if (get_option(WPS_OPTIONS_PREFIX."_activity_images")) {
								$html .= '<div class="uploader" id="symposium_activity_uploader" style="display:none;">';
							        $html .= '<img src="'.get_option(WPS_OPTIONS_PREFIX.'_images').'/add_small.png" /> <a id="pickfiles" href="#">'.__('Attach an image', WPS_TEXT_DOMAIN).'</a>';
								    $html .= '<div id="progressbar" style="border:1px;"></div>';
								$html .= '</div>';
							    $html .= '<div id="symposium_filelist" class="cb">';
							    if (get_option(WPS_OPTIONS_PREFIX.'_activity_images_old')) $html .= __('Install a <a href="http://www.google.com/chrome" target="_blank">modern browser</a> to attach an image', WPS_TEXT_DOMAIN);
							    $html .= '</div>';
							}
							...
							if (get_option(WPS_OPTIONS_PREFIX.'_show_buttons')) {
								$html .= '<input id="__wps__add_comment" type="submit" class="__wps__button" value="'.__('Post', WPS_TEXT_DOMAIN).'" /><br />';
							} else {
								$html .= '<br />';
							}
				...
				if ($comments) {
								...
								if ($private_group != 'on') {
									...
									if  ( ($comment->type != 'poke') || ($comment->type == 'poke' && ($comment->author_uid == $uid2 || $comment->subject_uid == $uid2 )) ) {	
										...
										$html .= "<div class='wall_post_div' id='post_".$comment->cid."'>";
											...
											$html .= __wps__time_ago($comment->comment_timestamp).".";
											...
											if (get_option(WPS_OPTIONS_PREFIX.'_activity_likes') && is_user_logged_in() && $comment->author_uid != $uid2 ) {
												...
												if (!$already_liked) {
													$html .= "<div class='wall_post_like delete_post_top'>";
														$html .= "<img class='wall_add_like' title='".__('You like this.', WPS_TEXT_DOMAIN)."' data-action='like' rel='".$comment->cid."' style='width:20px;height:20px;' src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/smilies/good.png' />";
														$html .= "<img class='wall_add_like' title='".__('You do not like this.', WPS_TEXT_DOMAIN)."'data-action='dislike' rel='".$comment->cid."' style='width:20px;height:20px' src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/smilies/bad.png' />";
													$html .= "</div>";
												}
											}	
											...
											if (get_option(WPS_OPTIONS_PREFIX.'_allow_reports') == 'on') {
												$html .= " <a title='post_".$comment->cid."' href='javascript:void(0);' class='report_post report_post_top symposium_report'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/warning.png' style='width:16px;height:16px' /></a>";
											}
											if (__wps__get_current_userlevel() == 5 || $comment->subject_uid == $uid2 || $comment->author_uid == $uid2) {
												$html .= " <a title='".$comment->cid."' rel='post' href='javascript:void(0);' class='delete_post delete_post_top'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/delete.png' style='width:16px;height:16px' /></a>";
											}
											$html .= '</div>';
											...
											if (get_option(WPS_OPTIONS_PREFIX.'_activity_likes')) {
												...
												$html .= "<div id='__wps__likes_".$comment->cid."'>";
													if ($likes) {
														$link = '<a id="symposium_show_likes" href="javascript:void(0)" rel="'.$comment->cid.'">';
														$html .= "<img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/smilies/good.png' style='width:16px;height:16px' /> ";
														if ($youlike) {
															$html .= __('You', WPS_TEXT_DOMAIN);
															$likes--;
															if ($likes > 1) {
																$html .= ' '.sprintf(__('and %s%d others</a> like this.', WPS_TEXT_DOMAIN), $link, $likes);
															}
															if ($likes == 1) {
																$html .= ' '.sprintf(__('and %s1 other</a> person likes this.', WPS_TEXT_DOMAIN), $link);
															}
															if ($likes == 0) {
																$html .= ' '.__('like this.', WPS_TEXT_DOMAIN);
															}
														} else {
															if ($likes > 1) {
																$html .= sprintf(__('%s%d people</a> like this.', WPS_TEXT_DOMAIN), $link, $likes);
															}
															if ($likes == 1) {
																...
																$html .= __wps__profile_link($uid).' '.__('likes this.', WPS_TEXT_DOMAIN);
															}															
														}
													}
													if ($dislikes) {
														if ($start_likes) $html .= '<br />';
														$link = '<a id="symposium_show_likes" href="javascript:void(0)" rel="'.$comment->cid.'">';
														$html .= "<img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/smilies/bad.png' style='width:16px;height:16px' /> ";
														if ($youdislike) {
															$html .= __('You', WPS_TEXT_DOMAIN);
															$dislikes--;
															if ($dislikes > 1) {
																$html .= ' '.sprintf(__('and %s%d others</a> don\'t like this.', WPS_TEXT_DOMAIN), $link, $dislikes);
															}
															if ($dislikes == 1) {
																$html .= ' '.sprintf(__('and %s1 other</a> person don\'t like this.', WPS_TEXT_DOMAIN), $link);
															}
															if ($dislikes == 0) {
																$html .= ' '.__('don\'t like this.', WPS_TEXT_DOMAIN);
															}
														} else {
															if ($dislikes > 1) {
																$html .= sprintf(__('%s%d people</a> don\'t like this.', WPS_TEXT_DOMAIN), $link, $dislikes);
															}
															if ($dislikes == 1) {
																$sql = "SELECT uid FROM ".$wpdb->base_prefix."symposium_likes WHERE cid = %d AND type = 'dislike'";
																$uid = $wpdb->get_var($wpdb->prepare($sql, $comment->cid));
																$html .= __wps__profile_link($uid).' '.__('doesn\'t like this.', WPS_TEXT_DOMAIN);
															}															
														}
													}
												$html .= "</div>";
											...
											if ($comment->type == 'gallery' && strpos($text, '[]')) {
												...
												// Get images
												$sql = "SELECT * FROM ".$wpdb->prefix."symposium_gallery_items WHERE gid = %d ORDER BY photo_order";
											...
											$directory = WP_CONTENT_DIR."/wps-content/members/".$comment->author_uid.'/activity';
											if (file_exists($directory)) {
												$handler = opendir($directory);
												while ($image = readdir($handler)) {
													$path_parts = pathinfo($image);
													if ($path_parts['filename'] == $comment->cid) {
														$directoryURL = WP_CONTENT_URL."/wps-content/members/".$comment->author_uid.'/activity/'.$image;
														$text = '<img class="symposium-dialog" style="cursor:pointer" rel="attachment_'.$comment->cid.'" src="'.$directoryURL.'" width="100%" /><br />' . $text;
														$text .= '<div style="display:none" title="'.get_bloginfo('name').'" id="attachment_'.$comment->cid.'"><img src="'.$directoryURL.'"></div>';
													}
												}
											}											
											
											// Finally show comment...!
											$html .= '<div class="next_comment '.$show_class.'" id="'.$comment->cid.'">';
											if ($comment->is_group) {
												...
												$html .= __("Group post in", WPS_TEXT_DOMAIN)." <a href='".$url."'>".stripslashes($group_name)."</a>: ".__wps__make_url(stripslashes($text));
											} else {
												$html .= __wps__make_url(stripslashes($text));
											}
											...
											if ($replies) {
												if (count($replies) > 4) {
													...
													$html .= "<a title='".$comment->cid."' class='view_all_comments' href='javascript:void(0);'>".__(sprintf("View all %d comments", count($replies)), WPS_TEXT_DOMAIN)."</a>";
													$html .= "</div>";
												}
												foreach ($replies as $reply) {
													...
													$html .= "<div id='".$reply->cid."' class='reply_div' style='".$reply_style."'>";
														$html .= "<div class='__wps__wall_reply_div'>";
															$html .= "<div class='wall_reply'>";
																$html .= '<a href="'.$profile_page.$q.'uid='.$reply->author_uid.'">'.stripslashes($reply->display_name).'</a> ';
																$html .= __wps__time_ago($reply->comment_timestamp).".";
																$html .= '<div style="width:50px; float:right;">';
																if (get_option(WPS_OPTIONS_PREFIX.'_allow_reports') == 'on') {
																	$html .= " <a title='post_".$reply->cid."' href='javascript:void(0);' style='padding:0px' class='report_post symposium_report reply_warning'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/warning.png' style='width:14px;height:14px' /></a>";
																}
																...
																if (get_option(WPS_OPTIONS_PREFIX.'_activity_likes') && is_user_logged_in() && $reply->author_uid != $uid2 ) {
																	...
																	if (!$already_liked) {
																		$html .= "<div class='wall_post_like delete_reply' style='margin:0;padding:0;'>";
																			$html .= "<img class='wall_add_like' title='".__('You like this.', WPS_TEXT_DOMAIN)."' data-action='like' rel='".$reply->cid."' style='padding:0;width:20px;height:20px;' src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/smilies/good.png' />";
																			$html .= "<img class='wall_add_like' title='".__('You do not like this.', WPS_TEXT_DOMAIN)."'data-action='dislike' rel='".$reply->cid."' style='padding:0;width:20px;height:20px' src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/smilies/bad.png' />";
																		$html .= "</div>";
																	}
																}	

																if (__wps__get_current_userlevel($uid2) == 5 || $reply->subject_uid == $uid2 || $reply->author_uid == $uid2) {
																	$html .= " <a title='".$reply->cid."' rel='reply' href='javascript:void(0);' style='padding:0px' class='delete_post delete_reply'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/delete.png' style='width:14px;height:14px' /></a>";
																}
																$html .= '</div>';
																$html .= "<br />";
																...
																if (get_option(WPS_OPTIONS_PREFIX.'_activity_likes')) {
																	...
																	$html .= "<div id='__wps__likes_".$reply->cid."'>";
																		if ($likes) {
																			...
																			if ($youlike) {
																				...
																				if ($likes == 1) {
																					$html .= ' '.sprintf(__('and %s1 other person</a> likes this.', WPS_TEXT_DOMAIN), $link);
																				}
																				if ($likes == 0) {
																					$html .= ' '.__('like this.', WPS_TEXT_DOMAIN);
																				}
																...
																$html .= __wps__make_url(stripslashes($reply->comment));
															$html .= "</div>";
														$html .= "</div>";
												...
												) 
											{
												if ($comment->type != 'gallery') {
													$html .= '<div style="margin-top:5px;'.$show_field.'" id="__wps__reply_div_'.$comment->cid.'" >';
	
													$html .= '<textarea title="'.$comment->cid.'" class="__wps__reply';
													...
													if (get_option(WPS_OPTIONS_PREFIX.'_show_buttons')) {
														$html .= '<br /><input title="'.$comment->cid.'" type="submit" style="width:75px" class="__wps__button symposium_add_reply" value="'.__('Add', WPS_TEXT_DOMAIN).'" />';
													}
					...
					if ($post == '' && $cnt > 0) {
						if (is_user_logged_in()) $html .= "<a href='javascript:void(0)' id='".$id."' class='showmore_wall' title='".($limit_from+$cnt+1)."'>".__("more...", WPS_TEXT_DOMAIN)."</a>";
					} else {
						if ($post == '') {
...
function __wps__do_members_Widget($__wps__members_count) {
	...
	$members = $wpdb->get_results("
		...
		ORDER BY user_registered DESC LIMIT 0,".$__wps__members_count); 
	...
	if ($members) {
		...
		$html .= "<div id='__wps__new_members'>";
			...
			foreach ($members as $member)
			{
				$html .= "<div class='__wps__new_members_row' style='clear:both;'>";		
					$html .= "<div class='__wps__new_members_row_avatar' style='width:32px;float:left; margin-bottom:6px;'>";
						$html .= "<a href='".$profile_url.$q."uid=".$member->ID."'>";
							$html .= get_avatar($member->ID, 32);
						$html .= "</a>";
					$html .= "</div>";
					$html .= "<div class='__wps__new_members_row_member'>";
						$html .= __wps__profile_link($member->ID)." ".__('joined', WPS_TEXT_DOMAIN)." ";
...
function __wps__do_Forumrecentposts_Widget($postcount,$preview,$cat_id,$show_replies,$incl_cat,$incl_parent,$just_own) {
	...
	$posts = $wpdb->get_results($wpdb->prepare($sql, 0, 100)); 
	$count = 0;
	$html = '';
								...
								$html .= "<div class='__wps__latest_forum_row_post'>";
									if ($post->topic_parent > 0) {
										...
										if ($preview > 0) {
											...
											if ($incl_cat) {
												if (get_option(WPS_OPTIONS_PREFIX.'_permalink_structure') && $group_id == 0) {
													$cat = $wpdb->get_var($wpdb->prepare('SELECT title FROM '.$wpdb->prefix.'symposium_cats WHERE cid = %d', $post->parent_category));
													$perma_cat = __wps__get_forum_category_part_url($post->parent_category);
										...
										if ($preview > 0) {
											...
											if ($incl_cat) {
												if (get_option(WPS_OPTIONS_PREFIX.'_permalink_structure') && $group_id == 0) {
													$cat = $wpdb->get_var($wpdb->prepare('SELECT title FROM '.$wpdb->prefix.'symposium_cats WHERE cid = %d', $post->topic_category));
													$perma_cat = __wps__get_forum_category_part_url($post->topic_category);
													$html .= ' '.__('in', WPS_TEXT_DOMAIN)." <a href='".$forum_url.'/'.$perma_cat."'>".$cat."</a> ";
												} else {
...
function __wps__do_summary_Widget($show_loggedout,$show_form,$login_url,$show_avatar,$login_username,$login_password,$login_remember_me,$login_button,$login_forgot,$login_register) {
	...
	if (is_user_logged_in()) {
		...
		if (function_exists('__wps__profile')) {
			...
			echo "<li>";
				if ($show_avatar) {
					echo "<div id='__wps__summary_widget_avatar' style='float:left;margin-right:6px;'>";
...
function has_bronze_plug_actived() {
	...
	if (function_exists('__wps__news_main') || 
		function_exists('__wps__facebook') || 
		function_exists('__wps__gallery') || 
		function_exists('__wps__profile_plus') || 
		function_exists('__wps__events_main') || 
		function_exists('__wps__group') || 
		function_exists('__wps__rss_main') ||
		get_option(WPS_OPTIONS_PREFIX.'__wps__wysiwyg_activated')
	) {
...
function __wps__bronze_countdown() {
	...
	if (substr($saved_code,0,3) == 'vip' || substr($saved_code,0,3) == 'wps') {
		...
		if ($days < 366) {
			return array($days, '<p>'.__('Your current "Bronze" activation code expires on', WPS_TEXT_DOMAIN).' '.@date('l d F Y', $code).' ('.sprintf('%d days, %d:%s:%s', $days, $hours, $minutes, $seconds).')'.', '.__('<a href="http://www.wpsymposium.com/membership" target="_new">get a new activation code</a> before then to reset for another 365 days.', WPS_TEXT_DOMAIN).'<br />'.sprintf(__('Note that this may not tie in with your %s expiry payment date.', WPS_TEXT_DOMAIN), WPS_WL).'</p>');
		} else {
