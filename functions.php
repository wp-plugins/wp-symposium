						...
						$html .= "<div class='topic-post-header'>";
							...
							if ( ($post->topic_owner == $current_user->ID) || (current_user_can('level_10')) ) {
								...
								if ( (get_option(WPS_OPTIONS_PREFIX.'_forum_lock')==0) || ($post->topic_started > $now) || (current_user_can('level_10')) ) {
									$html .= "<a href='javascript:void(0)' title='".$post->tid."' id='edit-this-topic' class='edit_topic edit label' style='cursor:pointer'><div class='topic-edit-icon'><img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/edit.png' /></div></a>";
								} else {
									//$html .= "locked";
								}
							}
				...
				if ($can_reply_switch) {
					...
					$html .= '<div id="reply-topic-bottom" name="reply-topic-bottom" style="padding:0; width:100%;">';
						...
						if (get_option(WPS_OPTIONS_PREFIX.'_use_wp_editor')) {
							...
							// xxx
							$html .= '<div id="__wps__reply_text_parent" style="width:'.get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_width').'; height:'.get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_height').';"><textarea class="textarea_Editor reply-topic-text'.$elastic.'" id="__wps__reply_text" style="width:100%;" ></textarea></div>';
							$html .= '<div class="sep"></div>';
						}
	...
	if ( $continue ) {
		...
		if ($limit_from == 0) {
			...
			if (is_user_logged_in()) {
				...
				if ( ($can_edit) && (current_user_can('level_10') || $allow_new == 'on') ) {
...
function __wps__show_profile_menu_tabs($uid1, $uid2) {
	...
	global $wpdb, $current_user;
		...
		if ( ($uid1 == $uid2) || (is_user_logged_in() && strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'public') || (strtolower($privacy) == 'friends only' && $is_friend) || __wps__get_current_userlevel() == 5) {
			...
			// Filter for additional menu items 
			$menu .= '<div style="float:right; width:250px;text-align:right;">'.apply_filters ( '__wps__profile_menu_tabs_filter', $menu, $uid1, $uid2, $privacy, $is_friend, $extended, $share, '' ).'</div>';
			...
			foreach($str_arr as $item) {
				...
				if (strpos($item, '=') !== false) {
					...
					case 'events':
						if ($uid1 == $uid2 || __wps__get_current_userlevel() == 5)
							$i = '<li id="menu_events" class="__wps__profile_menu" href="javascript:void(0)">'.$title.'</li>';
						break;
					case 'mygroups' :
					...
					if ($show_add_comment) {
						...
						$html .= '<div id="symposium_add_status">';
							...
							if (get_option(WPS_OPTIONS_PREFIX."_activity_images")) {
								$html .= '<div id="activity_file_upload_button">';
									$html .= '<input id="activity_file_upload" name="file_upload" type="file" />';
								$html .= '</div>';
								$html .= '<div id="activity_file_upload_file" style="margin-bottom:10px;display:none"></div>';
							}
											...
											if (file_exists($directory)) {
												...
												while ($image = readdir($handler)) {
													...
													if ($path_parts['filename'] == $comment->cid) {
														...
														$text .= "<a target='_blank' href='".$directoryURL."' rev='1' rel='symposium_activity_images_".$comment->cid."' data-owner='".$comment->author_uid."' data-name='".$title_bar."' data-iid='".$comment->cid."' class='wps_gallery_album'>";
...
function has_bronze_plug_actived() {
	...
	if (function_exists('__wps__news_main') || 
		...
		function_exists('__wps__group') || 
		function_exists('__wps__add_notification_bar') ||
