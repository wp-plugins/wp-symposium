			...
			if ($can_reply) {
				...
				if ($wpdb->get_var($wpdb->prepare("SELECT allow_replies FROM ".$wpdb->prefix."symposium_topics WHERE tid = %d", $post->tid)) == "on")
				{
					...
					if (get_option(WPS_OPTIONS_PREFIX.'_use_wp_editor')) {
						...
						$html .= '<textarea class="textarea_Editor reply-topic-text'.$elastic.'" style="width:'.get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_width').'; height:'.get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_height').'" id="__wps__reply_text"></textarea><br />';
						if (get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg') == 'on') { $html .= '<br />'; }
					}
		...
		if ($limit_from == 0) {
			...
			if (is_user_logged_in()) {
				...
				if (get_option(WPS_OPTIONS_PREFIX.'_forum_login') == "on") {
					$new_topic_form .= "<div>".__("Until you login, you can only view the forum.", WPS_TEXT_DOMAIN);
...
function __wps__update_meta($uid, $meta, $value) {
	...
	if ($meta != 'profile_avatar') {
		...
		if ($wpdb->get_var($wpdb->prepare("SELECT meta_key FROM ".$wpdb->base_prefix."usermeta WHERE meta_key = 'symposium_extended_city' AND user_id = ".$uid)) != 'symposium_extended_city') __wps__create_usermeta($uid);
						...
						$html .= '<div id="symposium_add_status">';
							...
							if (get_option(WPS_OPTIONS_PREFIX."_activity_images")) {
								$html .= '<div class="uploader" id="symposium_activity_uploader" style="display:none;">';
							        $html .= '<img src="'.get_option(WPS_OPTIONS_PREFIX.'_images').'/add_small.png" /> <a id="pickfiles" href="#">'.__('Attach an image', 'wp-sympsium').'</a>';
							...
							if (get_option(WPS_OPTIONS_PREFIX.'_show_buttons')) {
								$html .= '<input id="__wps__add_comment" type="submit" style="width:75px;" class="__wps__button" value="'.__('Post', WPS_TEXT_DOMAIN).'" /><br />';
							} else {
								$html .= '<br /><br />';
							}
									...
									if  ( ($comment->type != 'poke') || ($comment->type == 'poke' && ($comment->author_uid == $uid2 || $comment->subject_uid == $uid2 )) ) {	
										...
										$html .= "<div class='wall_post_div' id='post_".$comment->cid."'>";
											...
											if ($comment->is_group) {
												...
												$html .= __("Group post in", WPS_TEXT_DOMAIN)." <a href='".$url."'>".$group_name."</a>: ".__wps__make_url(stripslashes($text));
											} else {
				...
				if ($__wps__recent_desc == 'on') {
					$html .= "<div class='__wps__new_members_row'>";		
						...
						$html .= "<div class='__wps__new_members_row_member'>";
							...
							if ($__wps__recent_show_mail == 'on' && __wps__friend_of($member->ID, $current_user->ID) ) {
						...
						if ($__wps__friends_desc == 'on') {
							$html .= "<div class='__wps__new_members_row'>";		
								...
								$html .= "<div class='__wps__new_members_row_member'>";
									...
									$html .= __('last active', WPS_TEXT_DOMAIN)." ";
									$html .= __wps__time_ago($member->last_activity).".";
									if ($__wps__friends_show_mail == 'on') {
										$html .= " <a title='".$member->display_name."' href='".$mail_url.$q."view=compose&to=".$member->ID."'>".__('Send Mail', WPS_TEXT_DOMAIN)."</a>";
									}
								$html .= "</div>";
							$html .= "</div>";
...
// Recent forum posts
function __wps__do_Forumrecentposts_Widget($postcount,$preview,$cat_id,$show_replies,$incl_cat,$incl_parent) {
	...
	if ($posts) {
		...
		$html .= "<div id='__wps__latest_forum'>";
			...
			foreach ($posts as $post)
			{
					if ($post->topic_group == 0 || (__wps__member_of($post->topic_group) == "yes") || ($wpdb->get_var($wpdb->prepare("SELECT content_private FROM ".$wpdb->prefix."symposium_groups WHERE gid = ".$post->topic_group)) != "on") ) {
...
function __wps__bronze_countdown() {
	...
	if (substr($saved_code,0,3) == 'vip' || substr($saved_code,0,3) == 'wps') {
		...
		return '<p>'.__('Your current activation code will not expire', WPS_TEXT_DOMAIN).'.</p>';
		...
		return '<p>'.__('Your current activation code expires on', WPS_TEXT_DOMAIN).' '.date('l d F Y', $code).' ('.sprintf('%d days, %d:%s:%s', $days, $hours, $minutes, $seconds).')'.'.</p>';
