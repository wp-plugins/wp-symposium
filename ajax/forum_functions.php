	...
	if (is_user_logged_in()) {
		...
		if (__wps__safe_param($tid)) {
			...
			if ($answers) {
				...
				$row_array['message'] = __('You have change the answer. If a better answer is posted, you can change your selection again.', WPS_TEXT_DOMAIN);
	...
	if (is_user_logged_in()) {
		...
		if ($reply_text != '') {
			...
			if (get_option(WPS_OPTIONS_PREFIX.'_img_db') == "on") {
				...
				if (WPS_DEBUG && $show_debug) echo 'Looking for images in '.get_option(WPS_OPTIONS_PREFIX.'_img_path').'/forum<br />';
				if (file_exists($tmp_path)) {
					if (WPS_DEBUG && $show_debug) echo 'FILE EXISTS: '.$tmp_path.'<br />';
					mkdir($to_path, 0777, true);
					// copy tmp files to new location
					$handler = opendir($tmp_path);
					while ($file = readdir($handler)) {
			...
			foreach ($posts as $post)
			{
				...
				if ($gid > 0 || strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {				
					...
					$html .= "<div style='clear:both'>";
						...
						$html .= "</div>";
						if ($post->topic_date > $previous_login && $post->topic_owner != $current_user->ID && is_user_logged_in() && get_option(WPS_OPTIONS_PREFIX.'_forum_stars')) {
...
if ($_POST['action'] == 'getEditDetails') {
	...
	if (is_user_logged_in()) {
		...
		$post_text = stripslashes($details->topic_post);
		...
		if ($details) {
			echo stripslashes($subject)."[split]".stripslashes($post_text)."[split]".$details->topic_parent."[split]".$details->tid."[split]".$details->topic_category;
		} else {
