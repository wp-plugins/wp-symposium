	...
	$html .= "'>";		
		...
		$html .= "<div id='symposium_wall'>";
			...
			if (is_user_logged_in() && symposium_member_of($uid1) == "yes" && $limit_from == 0) {
					...
					$html .= 'id="symposium_group_comment"  onblur="this.value=(this.value==\'\') ? \''.$whatsup.'\' : this.value;" onfocus="this.value=(this.value==\''.$whatsup.'\') ? \'\' : this.value;">';
					$html .= $whatsup;
					$html .= '</textarea>';

					if (get_option('symposium_show_buttons')) {
						$html .= '<br /><input id="symposium_group_add_comment" type="submit" class="symposium-button" style="width:75px;" value="'.__('Post', 'wp-symposium').'" /> ';
					}
							...
							$html .= "<div class='wall_post'>";
								...
								if (is_user_logged_in() && symposium_member_of($uid1) == "yes") {
									$html .= '<div>';
												...
												$html .= '<textarea id="symposium_reply_'.$comment->cid.'" title="'.$comment->cid.'" class="input-field reply_field';
												...
												if (get_option('symposium_show_buttons')) {
													$html .= '<br /><input title="'.$comment->cid.'" id="symposium_reply_'.$comment->cid.'" type="submit" style="width:75px" class="symposium-button reply_field-button" value="'.__('Add', 'wp-symposium').'" />';
												}
