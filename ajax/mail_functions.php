...
if ($_POST['action'] == 'getMailMessage') {
	...
	if (is_user_logged_in()) {
		...
		if ( ($tray == "in" && $mail->mail_to == $current_user->ID) || ($tray != "in" && $mail->mail_from == $current_user->ID) || (__wps__get_current_userlevel() == 5) ) {
			...
			$msg = str_replace('[]', '', get_option(WPS_OPTIONS_PREFIX.'_template_mail_message'));
			...
			if (get_option(WPS_OPTIONS_PREFIX.'_use_styles') == "on") {
				$mail_style = "style='font-family:".get_option(WPS_OPTIONS_PREFIX.'_headingsfamily')."; font-size:".get_option(WPS_OPTIONS_PREFIX.'_headingssize')."px; font-weight:bold;'";
			}
			$msg = str_replace("[mail_subject]", "<span ".$mail_style.">".stripslashes(__wps__bbcode_replace($mail->mail_subject))."</span>", $msg);						
