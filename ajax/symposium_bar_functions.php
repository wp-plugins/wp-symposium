...
if ($_POST['action'] == 'symposium_getfriendsonline') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT f.*, m.last_activity, m.status, u.display_name, u.ID FROM ".$wpdb->base_prefix."symposium_friends f 
		...
		WHERE f.friend_accepted = 'on' AND f.friend_from = ".$me." ORDER BY last_activity DESC";
			...
			$return .= "<div style='clear:both; margin-top:4px; overflow: auto;'>";		
				$return .= "<div style='float: left; width:15px; padding-left:4px;'>";
					if ($last_active_minutes >= $offline) {
						$return .= "<img src='".WPS_IMAGES_URL."/loggedout.gif' alt='Logged Out'>";
					} else {
						$friends_online++;
						if ($last_active_minutes >= $inactive) {
							$return .= "<img src='".WPS_IMAGES_URL."/inactive.gif' alt='Inactive'>";
						} else {
							$return .= "<img src='".WPS_IMAGES_URL."/online.gif' alt='Online'>";
						}
					}
				$return .= "</div>";
				$return .= "<div>";
					if ( $use_chat != 'on' || WPS_LITE ) {
...
if ($_POST['action'] == 'symposium_getchat') {
	...
	if (is_user_logged_in()) {
		...
		$chats = $wpdb->get_results($wpdb->prepare($sql, $me, $me));
	...
	if (is_user_logged_in()) {
		...
		if ($chatlist) {
			...
			foreach ($chatlist as $chat) {
				...
				$last_active_minutes = floor(($time_now-$last_active_minutes)/60);			
				if ($last_active_minutes >= $offline) {
					$status_img = WPS_IMAGES_URL.'/loggedout.gif';
				} else {
					if ($last_active_minutes >= $inactive) {
						$status_img = WPS_IMAGES_URL.'/inactive.gif';
					} else {
						$status_img = WPS_IMAGES_URL.'/online.gif';
					}
				}
