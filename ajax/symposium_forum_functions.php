		...
		if ($post) {
						...
						$html .= "<div class='topic-post-header'>";
							...
							if (is_user_logged_in()) {
								if (strpos(get_symposium_meta($current_user->ID, 'forum_favs'), "[".$post->tid."]") === FALSE) { 
									...
									$html .= "<img title='".$post->tid."' id='fav_link' src='".WPS_IMAGES_URL."/fav-on.gif' style='height:22px; width:22px; cursor:pointer;' alt='".__("Click to remove to favorites", "wp-symposium")."' />";						
								}
							}
			...
			if ($can_edit) {
				...
				if ($wpdb->get_var($wpdb->prepare("SELECT allow_replies FROM ".$wpdb->prefix."symposium_topics WHERE tid = %d", $post->tid)) == "on")
				{
					...
					if ($config->use_answers == 'on' && symposium_get_current_userlevel() == 5) {
				...
				foreach ($query as $topic) {
					...
					$html .= $color_check.'">';
						...
						if (strpos($favs, "[".$topic->tid."]") === FALSE ) { } else {
							$topic_title_html .= "<img src='".WPS_IMAGES_URL."/fav-on.gif' class='floatleft' style='height:18px; width:18px; margin-right:4px; margin-top:4px' />";						
						}								
