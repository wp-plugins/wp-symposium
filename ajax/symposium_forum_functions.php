...
if ($_POST['action'] == 'getForum') {
	...
	if ($cat_id == '' || $cat_id == '0' || $cat_title) {
							...
							foreach($categories as $category) {
								...
								if ($cnt&1) {
									...
									if (!WPS_LITE) {
										...
										if ($post_count > 0) { 
											$post_count_html = "<div class='post_count' style='color:".WPS_TEXT_COLOR.";'>".$post_count."</div>";
												$post_count_html .= "<div style='color:".WPS_TEXT_COLOR.";' class='post_count_label'>";
												if ($post_count > 1) {
									...
									if (!WPS_LITE) {
										...
										if ($topic_count > 0) {
											$topic_count_html = "<div class='post_count' style='color:".WPS_TEXT_COLOR.";'>".$topic_count."</div>";
											$topic_count_html .= "<div style='color:".WPS_TEXT_COLOR.";' class='post_count_label'>";
											if ($topic_count != 1) {
		...
		$html = __('Sorry, this category is no longer available.', 'wp-symposium');
	}
			...
			if ($topics) {
				foreach ($topics as $topic) {	
					...
					if (strpos($shown, $topic->topic_parent.",") === FALSE) { 
						...
						if ( strlen($text) > $snippet_length ) { $text = substr($text, 0, $snippet_length)."..."; }
						$html .= $text;
						if (WPS_USER_ANSWERS == 'on' && $topic->topic_answer == 'on') {
