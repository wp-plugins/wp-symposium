...
if ($_POST['action'] == 'removeUploadedImage') {
	...
	if (WPS_IMG_DB == "on") {
		...
		if ($blog_id > 1) {			
			$src = WPS_IMG_PATH.'/'.$blog_id.'/forum/'.$folder.'/'.$file;
		} else {
			$src = WPS_IMG_PATH.'/forum/'.$folder.'/'.$file;
		}
...
if ($_POST['action'] == 'forumNewPost') {
	...
	if (is_user_logged_in()) {
		...
		if (WPS_MODERATION == "on" && symposium_get_current_userlevel() < 5) {
			$topic_approved = "";
		} else {
			$topic_approved = "on";
		}
		...
		if (WPS_IMG_DB == "on") {
			...
			if ($blog_id > 1) {
				$tmp_path = WPS_IMG_PATH.'/'.$blog_id.'/forum/0_'.$current_user->ID.'_tmp';
				$to_path = WPS_IMG_PATH.'/'.$blog_id.'/forum/'.$new_tid;
			} else {
				$tmp_path = WPS_IMG_PATH.'/forum/0_'.$current_user->ID.'_tmp';
				$to_path = WPS_IMG_PATH.'/forum/'.$new_tid;
			}
...
if ($_POST['action'] == 'forumNewPostEmails') {
	...
	if (is_user_logged_in()) {
		...
		if ($topic->topic_approved == 'on') {
			...
			if ($group_id == 0) {
				...
				if ($query) {					
					...
					foreach ($query as $user) {
						...
						// Send mail
						symposium_sendmail($user->user_email, $subject, $body);						
					}						
				}
			...
			do_action('symposium_forum_newtopic_hook', $current_user->ID, $current_user->display_name, $current_user->ID, $post, 'forum');
...
if ($_POST['action'] == 'getTopic') {
	...
	if ($can_view) {
		...
		if ($post) {
				...
				$html .= "<div id='top_of_first_post'>";
					...
					$html .= "<div class='avatar' style='margin-bottom:0px; margin-top:6px;'>";
						...
						if (WPS_FORUM_INFO) {
							...
							if ($post->topic_started > $previous_login && $post->topic_owner != $current_user->ID && is_user_logged_in() && WPS_FORUM_STARS) {
					...
					$html .= "<div class='topic-post-header-with-fav'>";
						...
						$html .= "<div class='topic-post-header'>";
							...
							if ( ($post->topic_owner == $current_user->ID) || (current_user_can('level_10')) ) {
								$now = date('Y-m-d H:i:s', time() - WPS_FORUM_LOCK * 60);
								if ( (WPS_FORUM_LOCK==0) || ($post->topic_started > $now) || (current_user_can('level_10')) ) {
							...
							foreach ($images as $file) {
								...
								if ($ext[sizeof($ext)-1]=='gif' || $ext[sizeof($ext)-1]=='jpg' || $ext[sizeof($ext)-1]=='png' || $ext[sizeof($ext)-1]=='jpeg') {
									...
									if ($ext[sizeof($ext)-1]=='mp4') {
										...
										// Document
										if ($blog_id > 1) {
											$url = WPS_IMG_URL.'/'.$blog_id.'/forum/'.$post->tid.'/'.$file->filename;
										} else {
											$url = WPS_IMG_URL.'/forum/'.$post->tid.'/'.$file->filename;
										}
										$url = WP_CONTENT_URL."/plugins/wp-symposium/get_attachment.php?tid=".$post->tid."&filename=".$file->filename;
					...
					foreach ($child_query as $child) {
						...
						$reply_html .= "<div id='reply".$child->tid."' class='child-reply";
							...
							$reply_html .= "<div class='avatar'>";
								...
								if ($child->topic_date > $previous_login && $child->topic_owner != $current_user->ID && is_user_logged_in() && WPS_FORUM_STARS) {
									$reply_html .= "<img src='".WPS_IMAGES_URL."/new.gif' alt='New!' /> ";
								}
							...
							$reply_html .= "<div style='padding-left: 85px;'>";
								...
								if ( (WPS_USE_VOTES_REMOVE == $score) && (symposium_get_current_userlevel() < 5) && ($score != 0) ) {
										...
										$reply_html .= "<div class='floatright forum_post_score' style='width: 24px; text-align:center;'>";
											$reply_html .= "<div style='line-height:16px;'>";
												if ($post_count >= WPS_USE_VOTES_MIN) {
													$reply_html .= "<img id='".$child->tid."' class='forum_post_score_change' title='plus' src='".WPS_IMAGES_URL."/smilies/good.png' style='cursor:pointer;width:24px; height:24px;' />";
												} else {
													$reply_html .= "<img id='".$child->tid."' class='forum_post_score_change' title='novote' src='".WPS_IMAGES_URL."/smilies/good.png' style='cursor:pointer;width:24px; height:24px;' />";
												}
											$reply_html .= "</div>";
											...
											$reply_html .= "<div>";
												if ($post_count >= WPS_USE_VOTES_MIN) {
													$reply_html .= "<img id='".$child->tid."' class='forum_post_score_change' title='minus' src='".WPS_IMAGES_URL."/smilies/bad.png' style='cursor:pointer;width:24px; height:24px;' />";
												} else {
													$reply_html .= "<img id='".$child->tid."' class='forum_post_score_change' title='novote' src='".WPS_IMAGES_URL."/smilies/bad.png' style='cursor:pointer;width:24px; height:24px;' />";
												}
											$reply_html .= "</div>";
										$reply_html .= "</div>";
									}
									...
									if (WPS_USE_ANSWERS == 'on') {
										$reply_html .= "<div class='floatright'>";
											if ($child->topic_answer == 'on') {
												$reply_html .= "<img id='symposium_accepted_answer' src='".WPS_IMAGES_URL."/tick.png' style='cursor:pointer;margin-top:3px;width:20px; height:20px;' />";
											} else {
									...
									$reply_html .= "<div class='topic-edit-delete-icon'>";
										...
										if (WPS_ALLOW_REPORTS == 'on') {
											$reply_html .= "<a href='javascript:void(0)' class='floatright link_cursor symposium_report' style='display:none' title='reply_".$child->tid."'><img src='".WPS_IMAGES_URL."/warning.png' /></a>";
										}
										if ( ($child->topic_owner == $current_user->ID) || (current_user_can('level_10')) ) {
											$now = date('Y-m-d H:i:s', time() - WPS_FORUM_LOCK * 60);
											if ( (WPS_FORUM_LOCK==0) || ($child->topic_started > $now) || (current_user_can('level_10')) ) {
												$reply_html .= "<a href='javascript:void(0)' class='floatright link_cursor delete_forum_reply' style='display:none' id='".$child->tid."'><img src='".WPS_IMAGES_URL."/delete.png' /></a>";
												$reply_html .= "<a href='javascript:void(0)' class='floatright link_cursor edit_forum_reply' style='display:none; margin-right: 5px' id='".$child->tid."'><img src='".WPS_IMAGES_URL."/edit.png' /></a>";
											}
										}
									$reply_html .= "</div>";
								...
								if (WPS_IMG_DB == 'on') {
									...
									foreach ($images as $file) {
										...
										$reply_html .= "<a target='_blank' href='".$url."'";
										$ext = explode('.', $file->filename);
										if ($ext[sizeof($ext)-1]=='gif' || $ext[sizeof($ext)-1]=='jpg' || $ext[sizeof($ext)-1]=='png' || $ext[sizeof($ext)-1]=='jpeg') {
											$reply_html .= " rel='symposium_forum_images-".$post->tid."'";
										}
										...
										$reply_html .= '</a> ';
										$reply_html .= '<img id="'.$child->tid.'" title="'.$file->filename.'" class="remove_forum_image link_cursor" src="'.WPS_IMAGES_URL.'/delete.png" /> ';
										$reply_html .= '</div>';	
									}
									...
									if (file_exists($targetPath)) {
										...
										while ($file = readdir($handler)) {
											...
											if ($file != "." && $file != ".." && $file != ".DS_Store") {
												...
												if ($blog_id > 1) {
													$url = WPS_IMG_URL.'/'.$blog_id.'/forum/'.$post->tid.'/'.$child->tid.'/'.$file;
												} else {
													$url = WPS_IMG_URL.'/forum/'.$post->tid.'/'.$child->tid.'/'.$file;
												}
												$ext = explode('.', $file);
												if (strpos(WPS_IMAGE_EXT, $ext[sizeof($ext)-1]) > 0) {
													// Image
													$reply_html .= "<a target='_blank' href='".$url."' rel='symposium_forum_images-".$post->tid."'";
													$reply_html .= ' title="'.$file.'">';
													if (WPS_FORUM_THUMBS == 'on') {
														//list($width, $height, $type, $attr) = getimagesize(get_bloginfo('url').$url);
														list($width, $height, $type, $attr) = getimagesize(parse_url(get_bloginfo('url'),PHP_URL_SCHEME)."://".parse_url(get_bloginfo('url'),PHP_URL_HOST).$url);
														$max_width = WPS_FORUM_THUMBS_SIZE;
													...
													if (strpos(WPS_VIDEO_EXT, $ext[sizeof($ext)-1]) > 0) {
														...
														$reply_html .= '		file: "'.$url.'",';
														$reply_html .= '		width: "'.WPS_FORUM_THUMBS_SIZE.'px",';
												...
												if ($child->topic_owner == $current_user->ID || symposium_get_current_userlevel($current_user->ID) == 5) {
													$reply_html .= '<img id="'.$post->tid.'/'.$child->tid.'" title="'.$file.'" class="remove_forum_image link_cursor" src="'.WPS_IMAGES_URL.'/delete.png" /> ';
												}
												$reply_html .= '</div>';
			...
			if ($can_edit) {
				...
				if ($group_id == 0) {
					...
					if (WPS_FORUM_LOGIN_FORM) {
						$html .= wp_login_form(array(
   					 		'echo' => false,
						    'redirect' => WPS_FORUM_URL.'#cid='.$post->topic_category.',tid='.$post->tid,
		...
		$html .= 'You do not have permission to view this topic, sorry.';
		echo $html;
	}
			...
			if (is_user_logged_in()) {
				...
				if ( ($can_edit) && ($allow_new == 'on') ) {
					...
					$new_topic_form .= '<div name="new-topic" id="new-topic" style="display:none;">';
						...
						if (WPS_ELASTIC == 'on' && WPS_WYSIWYG != 'on') { $elastic = ' elastic'; } else { $elastic = ''; }
						$new_topic_form .= '<textarea class="new-topic-subject-text'.$elastic.'" id="new_topic_text"></textarea>';
						if (WPS_WYSIWYG == 'on') { $new_topic_form .= '<br />'; }
							...
							if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {
								...
								if ($cnt&1) {
									...
									if (is_user_logged_in()) {
										...
										if ($recursive_new && WPS_FORUM_STARS) {
					...
					foreach ($query as $topic) {
						...
						$html .= $color_check.'">';
							...
							$topic_title_html .= '<div class="row_link_div">';
								...
								if ($topic->for_info == "on") { $topic_title_html .= '<img src="'.WPS_IMAGES_URL.'/info.png" alt="'.__('Information only', 'wp-symposium').'" /> '; }
		...
		if ($reply_text != '') {
			...
			if (is_user_logged_in()) {
				...
				if (WPS_IMG_DB == "on") {
					...
					// File system
					if ($blog_id > 1) {
						$to_path = WPS_IMG_PATH.'/'.$blog_id.'/forum/'.$tid.'/'.$new_id;
						$tmp_path = WPS_IMG_PATH.'/'.$blog_id.'/forum/'.$tid.'_'.$current_user->ID.'_tmp';
					} else {
						$to_path = WPS_IMG_PATH.'/forum/'.$tid.'/'.$new_id;
						$tmp_path = WPS_IMG_PATH.'/forum/'.$tid.'_'.$current_user->ID.'_tmp';
					}
					if (file_exists($tmp_path)) {
						mkdir($to_path, 0777, true);
						// copy tmp files to new location
				...
				if ($topic_approved == "on") {
					...
					if ($query) {						
						foreach ($query as $user) {	
							...
							// Send mail
							if (strpos(WPS_FORUM_REPLY, '[topic]') !== FALSE) {
								$subject = str_replace("[topic]", $parent, WPS_FORUM_REPLY);
							} else {
								$subject = WPS_FORUM_REPLY;
							}
							symposium_sendmail($user->user_email, $subject, $body);							
						}
					}						
			...
			foreach ($posts as $post)
			{
				...
				if ($gid > 0 || strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {				
					...
					$html .= "<div style='clear:both'>";
						...
						$html .= "</div>";
						if ($post->topic_date > $previous_login && $post->topic_owner != $current_user->ID && is_user_logged_in() && WPS_FORUM_STARS) {
