...
if ($_POST['action'] == 'getTopic') {
	...
	if ( (get_option('symposium_viewer') == "Guest")
	 || (get_option('symposium_viewer') == "Subscriber" && $level >= 1)
	 || (get_option('symposium_viewer') == "Contributor" && $level >= 2)
	 || (get_option('symposium_viewer') == "Author" && $level >= 3)
	 || (get_option('symposium_viewer') == "Editor" && $level >= 4)
	 || (get_option('symposium_viewer') == "Administrator" && $level == 5) ) {
		$can_view = true;
	 } else {
		$can_view = false;
	 }
	...
	if ( (get_option('symposium_forum_editor') == "Guest")
	 || (get_option('symposium_forum_editor') == "Subscriber" && $level >= 1)
	 || (get_option('symposium_forum_editor') == "Contributor" && $level >= 2)
	 || (get_option('symposium_forum_editor') == "Author" && $level >= 3)
	 || (get_option('symposium_forum_editor') == "Editor" && $level >= 4)
	 || (get_option('symposium_forum_editor') == "Administrator" && $level == 5) ) {
		$can_edit = true;
	...
	if ($can_view) {
		...
		if ($post) {
			...
			if ($can_edit) {
				...
				if ($group_id == 0) {
					$html .= "<p>".__("The minimum level to reply on this forum is", "wp-symposium")." ".get_option('symposium_forum_editor')."</p>";				
					
					// Show login form, and redirect back here
					if (get_option('symposium_forum_login')) {
			...
			if (is_user_logged_in()) {
				...
				if ( ($can_edit) && ($allow_new == 'on') ) {
					...
					if ($group_id == 0 && $allow_new == 'on') {
						$new_topic_form = "<p>".__("The minimum level to start a topic is", "wp-symposium")." ".get_option('symposium_forum_editor')."</p>";	
					}
			...
			if ($group_id > 0 || strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {		
				...
				if ($limit_from == 0) {
					...
					foreach ($query as $topic) {
						...
						$closed_word = strtolower(get_option('closed_word'));
