	...
	if ($can_view) {
		...
		if ($post) {
				...
				$html .= "<div id='top_of_first_post'>";
					...
					$html .= "<div class='avatar' style='margin-bottom:0px; margin-top:6px;'>";
						...
						if ($config->forum_info) {
							...
							$html .= __('Posts:', 'wp-symposium').' '.$count.'<br />';
						
							if ($config->use_votes == 'on') {
								$sql = "SELECT SUM(score) FROM ".$wpdb->prefix."symposium_topics_scores WHERE uid = %d";
								$count = $wpdb->get_var($wpdb->prepare($sql, $post->topic_owner));
								if ($count == NULL) { $count = 0; }
								$html .= __('Rating:', 'wp-symposium').' '.$count.'<br />';
							}
			...
			$sql = "SELECT t.tid, (SELECT SUM(score) FROM ".$wpdb->prefix."symposium_topics_scores s WHERE s.tid = t.tid) AS score, topic_subject, topic_approved, topic_post, t.topic_date, topic_owner, display_name, topic_answer, ID
				FROM ".$wpdb->prefix."symposium_topics t INNER JOIN ".$wpdb->base_prefix."users u ON t.topic_owner = u.ID 
				WHERE (t.topic_approved = 'on' OR t.topic_owner = %d) AND t.topic_parent = %d ORDER BY t.topic_answer DESC, t.tid";
										...
										$html .= "<div class='floatright'>";
											if ($child->topic_answer == 'on') {
												...
												if ($post->topic_owner == $current_user->ID || symposium_get_current_userlevel() == 5) {
													$html .= "<a id=".$child->tid." class='forum_post_answer' href='javascript:void(0);' style='margin-right:10px'>".__('Accept answer', 'wp-symposium')."</a>";
												}
											}
										$html .= "</div>";
									}
...
if ($_POST['action'] == 'getForum') {
	...
	$sql = "SELECT tid, topic_subject, topic_approved, topic_post, topic_owner, topic_category, topic_date, display_name, topic_sticky, allow_replies, 
			...
			foreach ($query as $topic) {
				...
				$html .= $color_check.'">';
					...
					if (strpos($favs, "[".$topic->tid."]") === FALSE ) { } else {
						$topic_title_html .= "<img src='".$plugin."images/star-on.gif' class='floatleft' style='height:18px; width:18px; margin-right:4px; margin-top:4px' />";						
					}								
