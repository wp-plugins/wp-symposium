...
if ($_POST['action'] == 'reply') {
	...
	if ($reply_text != '') {
		...
		if (is_user_logged_in()) {
			...
			$owner_name = $wpdb->get_var($wpdb->prepare("SELECT display_name FROM ".$wpdb_prefix->."users WHERE ID = ".$current_user->ID));
			...
			$body .= "<p>".$reply_text."</p>";
			$body .= "<p>".symposium_get_url('forum')."cid=".$cat_id."&show=".$tid."</p>";
