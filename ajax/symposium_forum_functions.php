					...
					if ($query) {						
						foreach ($query as $user) {	
							...
							if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {	 
								...
								// Send mail
								symposium_sendmail($user->user_email, WPS_FORUM_NEW, $body);							
							}
									...
									if ( ($file != "." && $file != ".." && $file != ".DS_Store") && (!is_dir($targetPath.'/'.$file)) ) {
										...
										if (strpos(WPS_IMAGE_EXT, $ext[sizeof($ext)-1]) > 0) {
											...
											if (WPS_FORUM_THUMBS == 'on') {
												...
												// Heena list($width, $height, $type, $attr) = getimagesize(parse_url(get_bloginfo('url'),PHP_URL_SCHEME)."://".parse_url(get_bloginfo('url'),PHP_URL_HOST).$url);
												$max_width = WPS_FORUM_THUMBS_SIZE;
												if ($width > $max_width) {
													$height = $height / ($width / $max_width);
													$width = $max_width;
