...
if ($_POST['action'] == 'replycommentemails') {
	...
	if (is_user_logged_in()) {
		...
		$body .= "<p>".$owner_name." ".__('commented', 'wp-symposium')."...</p>";
		$body .= "<p>".$comment_text."</p>";
		$body .= "<p>".__('The reply commented on is', 'wp-symposium')."...</p>";
		$body .= "<p>".$reply_text."</p>";
		if (get_option('symposium_permalink_structure') && $group_id == 0) {
			$perma_cat = symposium_get_forum_category_part_url($cat_id);
			$url = $forum_url.'/'.$perma_cat.$stub;
		...
		if (function_exists('symposium_mailinglist')) { 
			$subject_add = ' #TID='.$reply->topic_parent.' ['.__('do not edit', 'wp-symposium').']'; 
		...
		if ($query) {						
			foreach ($query as $user) {	
				...
				// Filter to allow further actions to take place
				if (WPS_DEBUG) echo 'Applying symposium_forum_newreplycomment_filter: '.$user->ID.','.$current_user->ID.','.$current_user->display_name.','.$url.'<br />';
				apply_filters ('symposium_forum_newreplycomment_filter', $user->ID, $current_user->ID, $current_user->display_name, $url);
	...
	if (is_user_logged_in()) {
		...
		if ($topic_approved == "on") {
			...
			$query = $wpdb->get_results("
				...
				FROM ".$wpdb->base_prefix."users u 
				RIGHT JOIN ".$wpdb->prefix."symposium_subs ON ".$wpdb->prefix."symposium_subs.uid = u.ID 
				WHERE u.ID != ".$current_user->ID." AND tid = ".$parent_tid);
					...
					if ($topic->topic_group == 0 || symposium_member_of($topic->topic_group) == 'yes') {
						...
						if ($topic->topic_group == 0) {
							...
							if (get_option('symposium_permalink_structure') && $group_id == 0) {
								...
								$html .= "<a href='".$url."'>".symposium_bbcode_remove(stripslashes($topic->topic_subject))."</a> ";
							} else {
					...
					if ($topic->topic_group == 0 || symposium_member_of($topic->topic_group) == 'yes') {
						...
						if (strpos($shown, $topic->topic_parent.",") === FALSE) { 
							...
							if ($topic->topic_group == 0) {
								...
								if (get_option('symposium_permalink_structure') && $group_id == 0) {
									$stub = $wpdb->get_var($wpdb->prepare("SELECT stub FROM ".$wpdb->prefix."symposium_topics WHERE tid = ".$topic->topic_parent));
									$perma_cat = symposium_get_forum_category_part_url($topic->topic_category);
									$url = $thispage.'/'.$perma_cat.$stub;							
									$html .= "<a href='".$url."'>".symposium_bbcode_remove(stripslashes($topic->topic_subject))."</a> ";
								} else {
...
if ($_POST['action'] == 'getAllActivity') {
	...
	$html = '<div id="forum_activity_div">';
		...
		$sql = "
			...
			LEFT JOIN ".$wpdb->prefix."symposium_topics p ON t.topic_parent = p.tid 
			WHERE t.topic_approved = 'on' AND t.topic_date > '".$include."' AND t.topic_group = ".$gid." 
			AND (p.topic_parent = 0 || p.topic_subject != '')
			ORDER BY t.tid DESC LIMIT 0,".$postcount;
					...
					$html .= "<div style='clear:both'>";
						...
						$html .= "<div style='margin-bottom: 3px; float:left;'>";
							if ($post->topic_parent > 0) {
								...
								if (get_option('symposium_permalink_structure') && $group_id == 0) {
									$stub = $wpdb->get_var($wpdb->prepare("SELECT stub FROM ".$wpdb->prefix."symposium_topics WHERE tid = ".$post->topic_parent));
								...
								if (get_option('symposium_permalink_structure') && $group_id == 0) {
									$stub = $wpdb->get_var($wpdb->prepare("SELECT stub FROM ".$wpdb->prefix."symposium_topics WHERE tid = ".$post->tid));
								...
								$html .= symposium_time_ago($post->topic_date).".<br>";
...
function showThreadChildren($parent, $level, $gid, $previous_login) {
	...
	$posts = $wpdb->get_results("
		...
		FROM ".$wpdb->prefix.'symposium_topics'." t INNER JOIN ".$wpdb->base_prefix.'users'." u ON t.topic_owner = u.ID 
		LEFT JOIN ".$wpdb->prefix."symposium_topics p ON t.topic_parent = p.tid
		WHERE t.topic_approved = 'on' AND t.topic_parent = ".$parent." AND t.topic_group = ".$gid." AND t.topic_date > '".$include."' 
