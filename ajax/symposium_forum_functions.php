		...
		if ($post) {
							...
							if (file_exists($targetPath)) {
								...
								while ($file = readdir($handler)) {
									if ( ($file != "." && $file != ".." && $file != ".DS_Store") && (!is_dir($targetPath.'/'.$file)) ) {
										...
										$ext = explode('.', $file);
										if (strpos($config->image_ext, $ext[sizeof($ext)-1]) > 0) {
											// Image
											$html .= "<a target='_blank' href='".$url."' rel='symposium_forum_images-".$post->tid."'";
											$html .= ' title="'.$file.'">';
											if ($config->forum_thumbs == 'on') {
											...
											// Video
											if (strpos($config->video_ext, $ext[sizeof($ext)-1]) > 0) {
												...
												$html .= '		file: "'.$url.'",';
												$html .= '		width: "'.$config->forum_thumbs_size.'px",';
												$html .= '		height: "250px"';
												$html .= '	});';
												$html .= '</script>';	
			...
			if ($can_edit) {
				...
				if ($wpdb->get_var($wpdb->prepare("SELECT allow_replies FROM ".$wpdb->prefix."symposium_topics WHERE tid = %d", $post->tid)) == "on")
				{
					...
					if ($config->elastic == 'on') { $elastic = ' elastic'; } else { $elastic = ''; }
							...
							if ($cnt&1) {
								...
								if ($last_topic) {
									$reply = $wpdb->get_row($wpdb->prepare("
										SELECT tid, topic_subject, topic_approved, topic_post, topic_owner, topic_date, topic_category, ".$wpdb->base_prefix."users.display_name
										FROM ".$wpdb->prefix."symposium_topics, ".$wpdb->base_prefix."users  
										WHERE (topic_approved = 'on' OR topic_owner = %d) AND topic_parent = %d ORDER BY topic_date DESC LIMIT 0,1", $current_user->ID, $last_topic->tid)); 
								...
								$sql = "SELECT COUNT(t.tid)
										...
										INNER JOIN ".$wpdb->prefix."symposium_topics u ON u.topic_parent = t.tid 
										WHERE t.topic_parent = 0 
										  AND (t.topic_approved = 'on' OR t.topic_owner = %d) 
								...
								if ($new_posts) {
									...
									foreach ($new_posts AS $new_post) {
										$sql = "SELECT COUNT(*) FROM ".$wpdb->prefix."symposium_topics WHERE topic_parent = %d";
										$reply_count = $wpdb->get_var($wpdb->prepare($sql, $new_post->tid));
...
if ($_POST['action'] == 'getSearch') {
	...
	$html = '<div id="forum_activity_div">';
		...
		$sql = "SELECT t.*, p.tid AS parent_tid, u2.display_name as parent_display_name, p.topic_subject AS parent_topic_subject, p.topic_started AS parent_topic_started, u.display_name FROM ".$wpdb->prefix."symposium_topics t LEFT JOIN ".$wpdb->base_prefix."users u ON t.topic_owner = u.ID LEFT JOIN ".$wpdb->prefix."symposium_topics p ON t.topic_parent = p.tid LEFT JOIN ".$wpdb->base_prefix."users u2 ON p.topic_owner = u2.ID WHERE t.topic_approved = 'on' && (t.topic_subject LIKE '%\%s%' OR t.topic_post LIKE '%".$term."%') ORDER BY t.topic_started DESC LIMIT 0,40";
...
function show_sharing_icons($cat_id, $topic_id, $sharing, $group_id) {
	...
	$html = "<div id='share_link' style='text-align:right;width:180px;'>";
		...
		if (!(strpos($sharing, "pl") === FALSE)) {
			...
			$html .= "<img id='share_permalink' title='".$pageURL."' src='".$plugin."images/link-icon.gif' style='cursor:pointer; margin-left:3px; height:22px; width:22px' alt='Permalink icon' />";
		}
		// Email
		if (!(strpos($sharing, "em") === FALSE)) {
			...
			$html .= "<img src='".$plugin."images/email-icon.gif' style='margin-left:3px; height:22px; width:22px;' alt='Email icon' /></a>";
		}
		// Facebook
		if (!(strpos($sharing, "fb") === FALSE)) {
			...
			$html .= "<img src='".$plugin."images/facebook-icon.gif' style=margin-left:3px; 'height:22px; width:22px' alt='Facebook icon' /></a>";
		}
		// Twitter
		if (!(strpos($sharing, "tw") === FALSE)) {
			...
			$html .= "<img src='".$plugin."images/twitter-icon.gif' style='margin-left:3px; height:22px; width:22px' alt='Twitter icon' /></a>";
		}
		// Bebo
		if (!(strpos($sharing, "be") === FALSE)) {
			...
			$html .= "<img src='".$plugin."images/bebo-icon.gif' style=margin-left:3px; 'height:22px; width:22px' alt='Bebo icon' /></a>";
		}
		// LinkedIn
		if (!(strpos($sharing, "li") === FALSE)) {
			...
			$html .= "<img src='".$plugin."images/linkedin-icon.gif' style='margin-left:3px; height:22px; width:22px' alt='LinkedIn icon' /></a>";
		}
		// MySpace
		if (!(strpos($sharing, "ms") === FALSE)) {
			...
			$html .= "<img src='".$plugin."images/myspace-icon.gif' style='margin-left:3px; height:22px; width:22px' alt='MySpace icon' /></a>";
		}
