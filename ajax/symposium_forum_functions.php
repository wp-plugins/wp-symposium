...
if ($_POST['action'] == 'updateTopicScore') {
	...
	if (is_user_logged_in()) {
		...
		if (symposium_safe_param($tid)) {
			$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->base_prefix."symposium_topics SET score = %d WHERE tid = %d", $new_score, $tid ));
		}
...
if ($_POST['action'] == 'getTopic') {
	...
	if ($can_view) {
		...
		if ($post) {
			...
			// Replies
			$sql = "SELECT tid, score, topic_subject, topic_approved, topic_post, topic_date, topic_owner, display_name, ID
				FROM ".$wpdb->prefix."symposium_topics t INNER JOIN ".$wpdb->base_prefix."users u ON t.topic_owner = u.ID 
				...
				if ($child_query) {
					...
					foreach ($child_query as $child) {
						...
						$html .= "<div id='reply".$child->tid."' class='child-reply";
							...
							if ( ($config->use_votes_remove == $child->score) && (symposium_get_current_userlevel() < 5) && ($child->score != 0) ) {
								$html .= '<p>'.__('This post has been voted off the forum', 'wp-symposium').'</p>';
							} else {
							
								if ( ($config->use_votes_remove == $child->score) && ($child->score != 0) ) {
									$html .= '<p>'.__('This post has been voted off the forum (only visible to site admins) with a score of', 'wp-symposium').' '.$child->score.'.</p>';
								}
								if ($config->use_votes == 'on') {
					...
					if ($group_id == 0) {
						...
						if ($config->forum_uploads) {
							...
							while ($file = readdir($handler)) {
								if ($file != "." && $file != ".." && $file != ".DS_Store") {
									$new_topic_form .= '<div>';
									$new_topic_form .= '<a href="'.WP_CONTENT_URL.'/wps-content/forum/'.$post->tid.'_'.$current_user->ID.'_tmp/'.$file.'"';
									$ext = explode('.', $file);
									if ($ext[sizeof($ext)-1]=='gif' || $ext[sizeof($ext)-1]=='jpg' || $ext[sizeof($ext)-1]=='png' || $ext[sizeof($ext)-1]=='jpeg') {
										$new_topic_form .= ' rel="symposium_forum_images-'.$post->tid.'"';
									} else {
										$new_topic_form .= ' target="_blank"';
									}
									$new_topic_form .= ' title="'.$file.'">'.$file.'</a> ';
									$new_topic_form .= '<img id="'.$post->tid.'_'.$current_user->ID.'_tmp" title="'.$file.'" class="remove_forum_image link_cursor" src="'.WP_CONTENT_URL.'/plugins/wp-symposium/images/delete.png" /> ';
									$new_topic_form .= '</div>';
								}
							}			
							closedir($handler);
