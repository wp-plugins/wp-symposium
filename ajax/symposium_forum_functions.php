...
if ($_POST['action'] == 'forumNewPost') {
	...
	if (is_user_logged_in()) {
		...
		$new_topic_subject = strip_tags($_POST['subject']);
		$new_topic_text = strip_tags($_POST['text']);
...
if ($_POST['action'] == 'updateEditDetails') {
	...
	if (is_user_logged_in()) {
		...
		$tid = $_POST['tid'];	
		$topic_subject = addslashes(strip_tags($_POST['topic_subject']));	
		$topic_post = addslashes(strip_tags($_POST['topic_post']));	
