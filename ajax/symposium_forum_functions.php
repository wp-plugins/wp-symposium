...
if ($_POST['action'] == 'forumReplyEmails') {
	...
	if (is_user_logged_in()) {
		...
		$forum_url = symposium_get_url('forum');
		$q = symposium_string_query($forum);
...
if ($_POST['action'] == 'getAllActivity') {
	...
	$html = '<div id="forum_activity_div">';
		...
		$sql = "
			...
			WHERE t.topic_approved = 'on' AND t.topic_date > '".$include."' AND t.topic_group = ".$gid." ORDER BY t.tid DESC LIMIT 0,".$postcount;
...
function showThreadChildren($parent, $level, $gid, $previous_login) {
	...
	$posts = $wpdb->get_results("
		...
		FROM ".$wpdb->prefix.'symposium_topics'." t INNER JOIN ".$wpdb->base_prefix.'users'." u ON t.topic_owner = u.ID 
		LEFT JOIN ".$wpdb->prefix."symposium_topics p ON t.topic_parent = p.tid
		WHERE t.topic_approved = 'on' AND t.topic_parent = ".$parent." AND t.topic_group = ".$gid." AND t.topic_date > '".$include."' ORDER BY t.tid ".$desc." LIMIT 0,".$postcount); 
