...
if ($_POST['action'] == 'getBox') {
	...
	if (is_user_logged_in()) {
		...
		if ($tray == "in") {
			$sql = "SELECT m.*, u.display_name FROM ".$wpdb->base_prefix."symposium_mail m LEFT JOIN ".$wpdb->base_prefix."users u ON m.mail_from = u.ID WHERE mail_in_deleted != 'on' %s AND mail_to = %d AND (u.display_name LIKE '%%%s%%' OR mail_subject LIKE '%%%s%%' OR mail_message LIKE '%%%s%%') ORDER BY mail_mid DESC LIMIT %d, %d";
		} else {
			$sql = "SELECT m.*, u.display_name FROM ".$wpdb->base_prefix."symposium_mail m LEFT JOIN ".$wpdb->base_prefix."users u ON m.mail_to = u.ID WHERE mail_sent_deleted != 'on' %s AND mail_from = %d AND (u.display_name LIKE '%%%s%%' OR mail_subject LIKE '%%%s%%' OR mail_message LIKE '%%%s%%') ORDER BY mail_mid DESC LIMIT %d, %d";
		}

		$mail = $wpdb->get_results($wpdb->prepare($sql, $unread, $uid, $term, $term, $term, $start, $length));
		$sql = $wpdb->last_query;
