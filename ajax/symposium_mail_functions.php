...
if ($_POST['action'] == 'getMailMessage') {
	...
	if (is_user_logged_in()) {
		...
		if ( ($tray == "in" && $mail->mail_to == $current_user->ID) || ($tray != "in" && $mail->mail_from == $current_user->ID) || (symposium_get_current_userlevel() == 5) ) {
			...
			$unread = "?!";
			if ($tray == "in") {
				$unread = $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix.'symposium_mail'." WHERE mail_to = ".$mail->mail_to." AND mail_".$tray."_deleted != 'on' AND mail_read != 'on'");
			} else {
				$unread = $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix.'symposium_mail'." WHERE mail_from = ".$mail->mail_from." AND mail_".$tray."_deleted != 'on' AND mail_read != 'on'");
			}
