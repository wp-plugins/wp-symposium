	...
	if (is_user_logged_in()) {
		...
		if (!$recipient) {
			...
			if ( get_symposium_meta($recipient->ID, 'notify_new_messages') ) {
				...
				$body = "<h1>".$subject."</h1>";
				$body .= "<p><a href='".symposium_get_url('mail')."'>".__(sprintf("Go to %s Mail", symposium_get_url('mail')), "wp-symposium")."...</a></p>";
				$body .= "<p>";
				$body .= $message;
				$body .= "</p>";
				$body .= "<p><em>";
	...
	if (is_user_logged_in()) {
		...
		if ($mail) {
			foreach ($mail as $item)
			{
				...
				//$row_array['message'] = symposium_bbcode_remove($message).' ::: '.$sql;
