...
if ($_POST['action'] == 'getMailMessage') {
	...
	if (is_user_logged_in()) {
		...
		if ( ($tray == "in" && $mail->mail_to == $current_user->ID) || ($tray != "in" && $mail->mail_from == $current_user->ID) || (symposium_get_current_userlevel() == 5) ) {
			...
			$template = "<div id='message_header'><div id='message_header_delete'>[reply_button][delete_button]</div><div id='message_header_avatar'>[avatar,44]</div>[mail_subject]<br />[mail_recipient] [mail_sent]</div></div><div id='message_mail_message'>[message]</div>";
			...
			if ($tray == 'in') {
				if ($mail->mail_to == $current_user->ID) {		
					$msg = str_replace("[reply_button]", '<img src="'.get_option('symposium_images').'/reply.png" id='.$mail->mail_from.' title='.$mail_mid.' rel="'.stripslashes($mail->display_name).'" class="message_reply" />', $msg);
				} else {
					$msg = str_replace("[reply_button]", '', $msg);
				}
			} else {
				if ($mail->mail_from == $current_user->ID) {		
					$msg = str_replace("[reply_button]", '<img src="'.get_option('symposium_images').'/reply.png" id='.$mail->mail_to.' title='.$mail_mid.' rel="'.stripslashes($mail->display_name).'" class="message_reply" />', $msg);
				} else {
					$msg = str_replace("[reply_button]", '', $msg);
				}
			}
