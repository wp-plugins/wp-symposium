...
if ($_POST['action'] == 'get_comments') {
	...
	$max_items = get_option("symposium_lounge_max_rows")+0;
	...
	foreach ($comment_list as $comment) {
		...
		$add = array (	
			...
			'ID' => $comment->ID,
			'last_activity' => get_symposium_meta($comment->ID, 'last_activity'),
			'status' => get_symposium_meta($member->ID, 'status')
		);
		
		array_push($comment_array, $add);		
	}
	$comments = sub_val_sort($comment_array, 'lid', false);
	...
	if ($comments) {
		foreach ($comments as $comment) {
			...
			if ($comment['last_activity'] && $comment['status'] != 'offline') {
				$last_active_minutes = convert_datetime($comment['last_activity']);
			...
			$row_array['comment'] = stripslashes($comment['comment']);
			...
			$row_array['added'] = symposium_time_ago($comment['added']);
