...
function add_to_create_activity_feed($aid) {
	...
	// Work out message
	$msg = __("Added to", "wp-symposium").' '.$name.'[]'.$aid.'[]added';
...
if ($_POST['action'] == 'symposium_reorder_photos') {
	...
	if (is_user_logged_in()) {
		...
		for($i=0;$i < sizeof($order);$i++){
			$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->prefix."symposium_gallery_items SET photo_order = %d WHERE iid = %d AND gid = %d AND owner = %d", ($i+1), $order[$i], $album_id, $current_user->ID  ) );  
		};
		echo __('Order saved, reload page to view new order.', 'wp-symposium');
...
if ($_POST['action'] == 'symposium_add_photo_comment') {
	...
	if (is_user_logged_in()) {
		...
		// Get name of photo
		$sql = "SELECT gid,title FROM ".$wpdb->prefix."symposium_gallery_items WHERE iid = %d";
		$photo = $wpdb->get_row($wpdb->prepare($sql, $photo_id));
	
		// Work out message
		$msg = __("Commented on", "wp-symposium").' '.$photo->title.'[]'.$photo->gid.'[]comment[]'.$photo_id.'[]'.$comment;
			               	...
			               	$html .= '<div class="symposium_photo_outer">';
			           			$html .= '<div class="symposium_photo_inner">';
			      					$html .= '<div class="symposium_photo_cover">';
										$html .= '<a class="symposium_photo_cover_action wps_gallery_album" data-owner="'.$album->owner.'" data-iid="'.$photo->iid.'" data-name="'.stripslashes($photo->title).'" href="'.$img_src.'" rev="'.$cnt.'" rel="symposium_gallery_photos_'.$album->gid.'" title="'.stripslashes($album->name).'">';
...
if ($_POST['action'] == 'menu_gallery_select_cover') {
	...
	$sql = "SELECT gid FROM ".$wpdb->prefix."symposium_gallery_items WHERE iid = %d";
	$gid = $wpdb->get_var($wpdb->prepare($sql, $item_id));

	if ($item_id > 0 && $gid > 0) {
		$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->prefix."symposium_gallery_items SET cover = 'on' WHERE gid = %d AND iid = %d", $gid, $item_id  ) );  
		$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->prefix."symposium_gallery_items SET cover = '' WHERE gid = %d AND iid != %d", $gid, $item_id  ) );  
		echo 'OK';
	} else {
...
if ($_POST['action'] == 'menu_gallery_manage_delete') {
    ...
    if ($item_id != 0) {
		...
		// remove comments
		$wpdb->query( $wpdb->prepare( "DELETE FROM ".$wpdb->prefix."symposium_comments WHERE subject_uid = %d AND type = 'photo'", $item_id  ) );  
	
		// Rebuild activity entry
		add_to_create_activity_feed($photo->gid);
		
		echo __('Photo deleted.', 'wp-symposium');
...
if ($_POST['action'] == 'menu_gallery_manage_rename') {
    ...
    if ($item_id != 0 && $new_name != '') {
      $wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->prefix."symposium_gallery_items SET title = %s WHERE iid = %d", $new_name, $item_id  ) );  
      echo 'OK';
    } else {
...
if ($_POST['action'] == 'menu_gallery') {
	...
	if ($album_id > 0) {
		...
		$html .= '<div id="symposium_gallery_breadcrumb">';
			...
			if ($album_id != 0) {
				$html .= '<div style="float:right"><a href="javascript:void(0);" rel="'.$album_id.'" type="submit" class="symposium_photo_delete_all">'.__('Delete this album', 'wp-symposium').'</a>';
				$html .= '<br /><a href="javascript:void(0);" class="symposium_new_album_button">'.__("Create sub album", "wp-symposium").'</a></div>';	
	   	  	}
   	...
   	if ($albums) {
		...
		$html = apply_filters('symposium_gallery_header', $html);
       	...
       	foreach ($albums as $album) {
			...
			// check for privacy
			if ( ($album->owner == $current_user->ID) || (strtolower($album->sharing) == 'public') || (is_user_logged_in() && strtolower($album->sharing) == 'everyone') || (strtolower($album->sharing) == 'friends only' && symposium_friend_of($album->owner, $current_user->ID)) || symposium_get_current_userlevel() == 5) {
			...
			$cnt=0;
	       	...
	       	foreach ($photos as $photo) {
	            ...
	            $html .= '<div class="symposium_photo_outer">';
           			$html .= '<div class="symposium_photo_inner">';
						$html .= '<div class="symposium_photo_cover">';
						$html .= '<a class="symposium_photo_cover_action wps_gallery_album" data-owner="'.$owner.'" data-iid="'.$photo->iid.'" data-name="'.stripslashes($photo->title).'" href="'.$img_src.'" rev="'.$cnt.'" rel="symposium_gallery_photos_'.$album_id.'" title="'.stripslashes($this_album->name).'">';
							$html .= '<img class="symposium_photo_image" style="width:'.$thumbnail_size.'px; height:'.$thumbnail_size.'px;" src="'.$thumb_src.'" />';
						$html .= '</a>';
						$html .= '</div>';
					$html .= '</div>';
	...
	$html .= "<div id='symposium_create_gallery'>";
		...
		if ($this_id > 0) {
			...
			$html .= "<input type='checkbox' title='".$this_id."' id='symposium_create_sub_gallery_select' CHECKED> ".__("Create as a sub-album of ".$this_album, "wp-symposium");
			$html .= "</div>";
		}
