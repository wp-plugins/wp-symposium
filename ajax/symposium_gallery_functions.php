...
if ($_POST['action'] == 'symposium_get_photo_comments') {
	...
	foreach ($comments as $comment) {
		$add = array (
			...
			'comment' => symposium_smilies(symposium_make_url(stripslashes($comment->comment))),
...
if ($_POST['action'] == 'symposium_add_photo_comment') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT g.name,i.title, i.gid FROM ".$wpdb->prefix."symposium_gallery_items i LEFT JOIN ".$wpdb->prefix."symposium_gallery g ON i.gid = g.gid WHERE iid = %d";
