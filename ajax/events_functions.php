...
if ($_POST['action'] == 'confirmAttendee') {
	...
	$msg = '<p>'.sprintf(__('You have confirmed the booking for %s, for %d ticket(s) for event (%s) ID:', WPS_TEXT_DOMAIN), $display_name, $howmany, $event_name).$eid.'<br />';
...
if ($_POST['action'] == 'removeAttendee') {
	...
	$subject = sprintf(__('Attendee removal for Event (%s) ID:', WPS_TEXT_DOMAIN), $event_name).$eid;
...
if ($_POST['action'] == 'register_event') {
	...
	if (is_user_logged_in()) {
		...
		// Inform the organiser
		$msg = '<p>'.$current_user->display_name.sprintf(__(' has booked %d ticket(s) for event (%s) ID:', WPS_TEXT_DOMAIN), $howmany, $event->event_name).$eid.'<br />';
		$msg .= __('If payment is required, please follow this up.', WPS_TEXT_DOMAIN).'</p>';
				...
				foreach ($events as $event) {
					$html .= '<div class="__wps__event_list_item row">';
						...
						if ($event->event_more) {
							...
							$html .= '<div id="symposium_more_'.$event->eid.'" title="'.stripslashes($event->event_name).'" class="__wps__dialog_content">'.$content.'</div>';
							$html .= '<input type="submit" id="symposium_event_more" rel="symposium_more_'.$event->eid.'" class="symposium-dialog __wps__button" value="'.__("More info", WPS_TEXT_DOMAIN).'" /><br />';
						}
