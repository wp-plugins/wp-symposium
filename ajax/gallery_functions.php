...
if ($_POST['action'] == 'menu_gallery') {
	...
	if ($album_id > 0) {
    	...
    	if ($user_page == $user_id) {
			...
			$html .= '<div class="__wps__menu_gallery_alert"><input type="file" id="menu_gallery_file_upload" name="file_upload" /></div>';
		}
			...
			$cnt=0;
	       	...
	       	foreach ($photos as $photo) {
				...
				if (get_option(WPS_OPTIONS_PREFIX.'_img_db') == "on") {
					...
					if (file_exists(get_option(WPS_OPTIONS_PREFIX.'_img_path').'/members/'.$user_page.'/media/'.$album_id.'/thumb_'.$photo->name)) {
	...
	$html .= "<div id='__wps__create_gallery'>";
		...
		if ($this_id > 0) {
			...
			$html .= "<input type='checkbox' title='".$this_id."' id='__wps__create_sub_gallery_select' CHECKED> ".__("Create as a sub-album of ".$this_album, "wp-symposium");
			$html .= "</div>";
		}
