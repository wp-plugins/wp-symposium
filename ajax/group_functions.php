...
if ($_POST['action'] == 'acceptGroup') {
	...
	if (is_user_logged_in()) {
		...
		// Tell other members
		$html = __wps__inform_members($group->name, $group->gid, $group->new_member_emails);	
	...
	if (is_user_logged_in()) {
		...
		if (__wps__group_admin($gid) == "yes" || __wps__get_current_userlevel($current_user->ID) == 5 ) {
			...
			$html .= "<div id='profile_left_column'>";
				...
				$html .= '<input type="submit" id="updateGroupSettingsButton" name="Submit" class="__wps__button" value="'.__('Save', WPS_TEXT_DOMAIN).'" /> ';
				$html .= '</p> ';
					...
					$html .= "<div id='".$comment->cid."' class='wall_post_div'>";
						...
						$html .= "<div class='wall_post_entry'>";
							$html .= "<div class='wall_post'>";
								...
								$html .= __wps__make_url(stripslashes($comment->comment));
