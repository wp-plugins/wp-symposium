...
if ($_POST['action'] == 'getMembers') {
	...
	$quick_ver = get_option('symposium_dir_full_ver');
	
	if (!$quick_ver && get_option('symposium_use_distance') && function_exists('symposium_profile_plus') && is_user_logged_in() && ($lat = get_symposium_meta($current_user->ID, 'plus_lat')) != '') {
		...
		foreach ($members as $member) {
			...
			if ($include) {	
				...
				if ($skip < $start) {
					...
					if (!$friends || $friends && symposium_friend_of($member->uid, $current_user->ID)) {
									...
									$html .= "<div style='padding-left: 75px;'>";						
										...
										if (function_exists('symposium_profile_plus') && is_user_logged_in() && $member->distance < 99999 && $member->uid != $current_user->ID) {
											...
											if ( (strtolower($share) == 'everyone') 
												...
												$html .= '<br />'.__('Location is set to private', 'wp-symposium');
											}
										}
