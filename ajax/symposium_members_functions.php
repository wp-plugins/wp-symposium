...
if ($_POST['action'] == 'getMembers') {
	...
	$extended = ($_POST['extended'] != '') ? $_POST['extended'] : '';
    	...
    	switch ($orderby)
	    {
	        ...
	        case 'distance': $order = ''; break;
	        default: return '';  break;
	    }
	
		if (is_user_logged_in() && function_exists('symposium_profile_plus')) {
			...
			$sql = "
			SELECT m.uid, u.display_name, m.city, m.country, m.share, m.last_activity, m.share, m.wall_share, 0 as distance 
			FROM ".$wpdb->base_prefix."symposium_usermeta m 
			LEFT JOIN ".$wpdb->base_prefix."users u ON m.uid = u.ID 					
			WHERE ( (lower(u.display_name) LIKE '".$term."%') 
			    OR (lower(u.display_name) LIKE '% %".$term."%') 
		...
		if ($extended) {
			$sql .= 'AND (1 ';
			foreach($extended as $search) {
				$item = explode('|',$search);
				$ext = $item[0].'[]'.$item[1];
				$sql .= "AND (lower(m.extended) LIKE '%".strtolower($ext)."[|]%') ";
			}
			$sql .= ') ';
		}
		$sql .= "ORDER BY ".$orderby." ".$order." LIMIT ".$start.",".WPS_DIR_PAGE_LENGTH;
		...
		if ($members) {
						...
						$html .= "<div class='members_row";
							...
							$html .= "<div class='members_info'>";
								...
								$html .= "<div style='padding-left: 71px;'>";						
									...
									if ( ($member->uid == $me) || (is_user_logged_in() && strtolower($member->share) == 'everyone') || (strtolower($member->share) == 'public') || (strtolower($member->share) == 'friends only' && $is_friend) ) {
									...
									// Distance
									if (is_user_logged_in() && $member->distance < 99999 && $member->uid != $current_user->ID) {
			...
			$html .= '<div style="clear:both;margin-top:10px;">'.__('No members found', 'wp-symposium').".</div>";
		}
