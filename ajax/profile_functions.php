...
// Clear Subscriptions
if ($_POST['action'] == 'deleteTmpAttachments') {
	global $current_user;
	if (is_user_logged_in()) {
		
		set_time_limit(0); // to avoid spike in CPU usage on Windows
		usleep(5000000); // wait for 5 seconds so temporary image has time to be displayed on the activity
		$targetDir = get_option(WPS_OPTIONS_PREFIX.'_img_path').'/tmp/'.$current_user->ID;
		__wps__rrmdir_tmp($targetDir);
...
if ($_POST['action'] == 'facebook_id') {
	...
	if (is_user_logged_in()) {
		__wps__update_meta($current_user->ID, 'facebook_id', $_POST['facebook_id']);
		...
		if ($_POST['facebook_id'] != '') {
			...
			$app_id = get_option(WPS_OPTIONS_PREFIX.'_facebook_api');
			$app_secret = get_option(WPS_OPTIONS_PREFIX.'_facebook_secret');
	...
	if (is_user_logged_in()) {
		...
		if (get_option(WPS_OPTIONS_PREFIX.'_img_db') == 'on') {
			...
			if ( imagecopyresampled($dst_r,$img_r,0,0,$x,$y,$targ_w,$targ_h,$w,$h) ) {
				...
				$err .= ' crop failed: '.$src.','.$dst_r.','.$img_r.','.$wpdb->last_query;
...
if ($_POST['action'] == 'addStatus') {
	...
	if (is_user_logged_in()) {
		...
		if (is_user_logged_in()) {
			...
			if ( ($text != __(addslashes(get_option(WPS_OPTIONS_PREFIX.'_status_label')), "wp-symposium")) && ($text != '') ) {
				...
				$directory = get_option(WPS_OPTIONS_PREFIX.'_img_path').'/tmp/'.$current_user->ID;
				if (file_exists($directory)) {
					$handler = opendir($directory);
					$done_one = false;
					while ($image = readdir($handler)) {
						if ($image != "." && $image != ".." && $image != ".DS_Store" && !$done_one) {
							$targetDir = get_option(WPS_OPTIONS_PREFIX.'_img_path')."/members/".$current_user->ID;
							$targetActivityDir = $targetDir.'/activity';
					...
					if ($done_one) {
						echo get_option(WPS_OPTIONS_PREFIX.'_img_url').'/tmp/'.$current_user->ID.'/'.$image_filename;
					}
				}
				...
				if ($facebook == 1 && function_exists('__wps__facebook')) {
					$facebook_id = __wps__get_meta($author_uid, 'facebook_id');
					if ($facebook_id != '') {
...
if ($_POST['action'] == 'menu_activity') {
	...
	$html = __wps__buffer(__wps__profile_body($uid1, $uid2, $post, "friends_activity", $limit_from));
...
if ($_POST['action'] == 'menu_extended') {
	...
	if (is_user_logged_in() || $share == 'public') {
			...
			if ( ($uid1 == $uid2) || (is_user_logged_in() && strtolower($share) == 'everyone') || (strtolower($share) == 'public') || (strtolower($share) == 'friends only' && __wps__friend_of($uid1, $current_user->ID)) ) {
				...
				// Name
				$html .= "<p class='__wps__profile_heading'>".$display_name."</p>";
				...
				if (get_option(WPS_OPTIONS_PREFIX.'_show_dob') == 'on') {
					...
					$dob_day = __wps__get_meta($uid1, 'dob_day');
					$dob_month = __wps__get_meta($uid1, 'dob_month');
					$dob_year = __wps__get_meta($uid1, 'dob_year');
					
					if ($dob_day > 0 && $dob_month > 0 && $dob_year > 0) {
						
						$monthname = __wps__get_monthname($dob_month);

						$html .= sprintf(__("Born %s %s %s", WPS_TEXT_DOMAIN), $monthname, $dob_day, $dob_year);
					...
					foreach ($ext_rows as $row) {
						if ($row['type'] == 'Checkbox' && !$row['value'] && get_option(WPS_OPTIONS_PREFIX.'_profile_show_unchecked') != 'on') { 
							...
							if ($row['type'] != 'Checkbox') {
								...
								$html .= "<div>".stripslashes(wpautop(__wps__make_url($row['value'])))."</div>";
							} else {
	...
	if (is_user_logged_in()) {
		...
		if ($uid == $current_user->ID || __wps__get_current_userlevel($current_user->ID) == 5) {
			...
			$html .= '<div id="symposium_settings_table">';
				...
				$html .= '<input type="submit" id="updateSettingsButton" name="Submit" class="__wps__button" value="'.__('Save', WPS_TEXT_DOMAIN).'" /> ';
				$html .= '</div>';
...
if ($_POST['action'] == 'updateSettings') {
	...
	if (is_user_logged_in()) {
		...
		if (is_email($user_email)) {
			...
			$pwmsg = 'OK';
			$display_name_exists = $wpdb->get_var($wpdb->prepare("SELECT display_name FROM ".$wpdb->base_prefix."users WHERE ID != %d AND (replace(' ', '', lower(display_name)) = %s OR lower(user_login) = %s)", $uid, str_replace(' ', '', strtolower($display_name)), strtolower($display_name)));
	...
	if (is_user_logged_in()) {	
		...
		if ($uid == $current_user->ID || __wps__get_current_userlevel($current_user->ID) == 5) {
					...
					$html .= '<div>';
						$html .= '<select id="share" name="share">';
							...
							$html .= "<option value='Everyone'";
								...
								$html .= '>'.__('Everyone', WPS_TEXT_DOMAIN).'</option>';
			...
			$html .= '<br /><p class="submit"> ';
				$html .= '<input type="submit" id="updatePersonalButton" name="Submit" class="__wps__button" value="'.__('Save', WPS_TEXT_DOMAIN).'" /> ';
			$html .= '</p>';
...
if ($_POST['action'] == 'menu_groups') {
	...
	if (is_user_logged_in() || $share == 'public') {
		...
		echo $html;
			...
			echo __("Please login to view this member's groups.", WPS_TEXT_DOMAIN);
...
if ($_POST['action'] == 'addFriend') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT * FROM ".$wpdb->base_prefix."symposium_friends WHERE (friend_from = %d AND friend_to = %d OR friend_to = %d AND friend_from = %d)";
		if ( $wpdb->get_var($wpdb->prepare($sql, $friend_to, $current_user->ID, $friend_to, $current_user->ID)) ) {
			// already exists
		} else {
				...
				foreach ($requests as $request) {
					...
					$html .= "<div id='request_".$request->friend_from."' style='clear:right; margin-top:8px; overflow: auto; margin-bottom: 15px; width:95%;'>";		
						...
						$html .= "<div style='clear: both; float:right;'>";
							$html .= '<input type="submit" title="'.$request->friend_from.'" id="rejectfriendrequest" class="__wps__button" value="'.__('Reject', WPS_TEXT_DOMAIN).'" /> ';
						$html .= "</div>";
						$html .= "<div style='float:right;'>";
							$html .= '<input type="submit" title="'.$request->friend_from.'" id="acceptfriendrequest" class="__wps__button" value="'.__('Accept', WPS_TEXT_DOMAIN).'" /> ';
						$html .= "</div>";
					$html .= "</div>";
				}
