	...
	if (is_user_logged_in()) {
		...
		if (get_option(WPS_OPTIONS_PREFIX.'_img_db') == 'on') {
			...
			if ( imagecopyresampled($dst_r,$img_r,0,0,$x,$y,$targ_w,$targ_h,$w,$h) ) {
				...
				$r = 'crop failed: '.$profile_photo.','.$src.','.$dst_r.','.$img_r.','.$wpdb->last_query;
...
if ($_POST['action'] == 'addStatus') {
	...
	if (is_user_logged_in()) {
		...
		if ( ($text != __(addslashes(get_option(WPS_OPTIONS_PREFIX.'_status_label')), "wp-symposium")) && ($text != '') ) {
			...
			$directory = get_option(WPS_OPTIONS_PREFIX.'_img_path')."/members/".$author_uid."/activity/pending/";
			if (file_exists($directory)) {
				$handler = opendir($directory);
				$done_one = false;
				while ($image = readdir($handler)) {
					if (!$done_one && $image != "." && $image != ".." && $image != ".DS_Store") {
						$targetDir = get_option(WPS_OPTIONS_PREFIX.'_img_path')."/members/".$current_user->ID;
						$targetActivityDir = get_option(WPS_OPTIONS_PREFIX.'_img_path')."/members/".$author_uid."/activity";
				...
				if ($done_one) {
					echo WP_CONTENT_URL."/wps-content/members/".$author_uid."/activity/".$filename;
				}
			}
...
//					echo 'Posted to Facebook ('.$status.')';
				} else {
//					echo 'Failed to post to Facebook ('.$facebook_id.')';
				}
			} else {
//				echo 'No to Facebook';
			}
