...
if ($_POST['action'] == 'addStatus') {
	...
	if (is_user_logged_in()) {
		...
		if (file_exists($directory)) {
			...
			if ($done_one) {
				$attached_filename = WP_CONTENT_URL."/wps-content/members/".$author_uid."/activity/".$filename;
			}
		}
...
if ($_POST['action'] == 'menu_groups') {
	...
	if (is_user_logged_in() || $share == 'public') {
		...
		echo $html;
			...
			echo sprintf(__("Please <a href='%s'>login</a> to view this member's groups.", WPS_TEXT_DOMAIN), site_url().'/wp-login.php?redirect_to='.__wps__pageURL());		
