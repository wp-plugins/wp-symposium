...
if ($action == "deactivate_debug") {
	global $wpdb;
	if (is_user_logged_in() && symposium_get_current_userlevel()==5) {
		update_option('symposium_debug_mode', '');
	} else {
		echo __('Only site administrators can de-activate debug mode.', 'wp-symposium');
	}
	exit;
}
