	...
	if (is_user_logged_in()) {
		...
		if (is_user_logged_in()) {
			...
			if ( ($text != __(addslashes(get_option('symposium_status_label')), "wp-symposium")) && ($text != '') ) {
				...
				$directory = WP_CONTENT_DIR.'/wps-content/tmp/'.$current_user->ID;
				if (file_exists($directory)) {
					$handler = opendir($directory);
					$done_one = false;
					while ($image = readdir($handler)) {
						if ($image != "." && $image != ".." && $image != ".DS_Store" && !$done_one) {
							$targetDir = WP_CONTENT_DIR."/wps-content/members/".$current_user->ID;
							...
							rename($directory.'/'.$image, $targetActivityFile);
							$done_one = true;
						}
					}
					symposium_rrmdir_tmp($directory);
					echo WP_CONTENT_URL.'/wps-content/members/'.$current_user->ID.'/activity/'.$filename;
				}
