...
if ($_POST['action'] == 'deleteTmpAttachments') {
	...
	if (is_user_logged_in()) {
		...
		$targetDir = get_option('symposium_img_path').'/tmp/'.$current_user->ID;
		symposium_rrmdir_tmp($targetDir);
			...
			if (get_option('symposium_img_db') == 'on') {
				...
				if ( imagecopyresampled($dst_r,$img_r,0,0,$x,$y,$targ_w,$targ_h,$w,$h) ) {
					...
					if ( imagejpeg($dst_r,$to_file,$jpeg_quality) ) {
						...
						// update database
						update_symposium_meta($uid, 'profile_photo', "'".$filename."'");
						$r = '';
	...
	if (is_user_logged_in()) {
		...
		if (is_user_logged_in()) {
			...
			if ( ($text != __(addslashes(get_option('symposium_status_label')), "wp-symposium")) && ($text != '') ) {
				...
				$directory = get_option('symposium_img_path').'/tmp/'.$current_user->ID;
				if (file_exists($directory)) {
					$handler = opendir($directory);
					$done_one = false;
					while ($image = readdir($handler)) {
						if ($image != "." && $image != ".." && $image != ".DS_Store" && !$done_one) {
							$targetDir = get_option('symposium_img_path')."/members/".$current_user->ID;
					...
					if ($done_one) {
						echo get_option('symposium_img_url').'/tmp/'.$current_user->ID.'/'.$image_filename;
					}
				}
...
if ($_POST['action'] == 'menu_extended') {
	...
	$html = apply_filters ('symposium_profile_header_public', $html);
			...
			switch ($mention->type) {
				case 'forum': 
					if ($mention->parent_id > 0) {
						if (get_option('symposium_permalink_structure') && $mention->parent_topic_group == 0) {
						...
						if (get_option('symposium_permalink_structure') && $mention->topic_group == 0) {
...
if ($_POST['action'] == 'menu_settings') {
	...
	if (is_user_logged_in()) {
		...
		if ($uid == $current_user->ID || symposium_get_current_userlevel($current_user->ID) == 5) {
			...
			$notify_likes = get_symposium_meta($uid, 'notify_likes');
			...
			$html .= '<div id="symposium_settings_table">';
				...
				// First name
				$html .= '<div id="symposium_settings_firstname" class="symposium_settings_row">';
				...
				// Last name
				$html .= '<div id="symposium_settings_lastname" class="symposium_settings_row">';
				...
				// Display name
				$html .= '<div id="symposium_settings_displayname" class="symposium_settings_row">';
				...
				// Email address
				$html .= '<div id="symposium_settings_email" class="symposium_settings_row">';
				...
				// Signature (for forum)
				$html .= '<div id="symposium_settings_signature" class="symposium_settings_row">';
				...
				// Email notifications
				$html .= '<div id="symposium_settings_notify_new_messages" class="symposium_settings_row">';
				...
				// Email wall
				$html .= '<div id="symposium_settings_notify_new_wall" class="symposium_settings_row">';
				...
				if (get_option('symposium_activity_likes')) {
					$html .= '<div id="symposium_settings_notify_likes" class="symposium_settings_row">';
						$html .= '<input type="checkbox" name="notify_likes" id="notify_likes"';
							if ($notify_likes == "on") { $html .= "CHECKED"; }
							$html .= '/> ';
							$html .= __('Receive an email when you receive likes/dislikes?', 'wp-symposium');
					$html .= '</div>';
				}
														
				// Email all forum activity
				if (function_exists('symposium_forum')) {
					$html .= '<div id="symposium_settings_forum_all" class="symposium_settings_row">';
				...
				if (get_option('symposium_enable_password') == "on") {
					$html .= '<div id="symposium_settings_password" class="symposium_settings_row">';
...
if ($_POST['action'] == 'updateSettings') {
	...
	if (is_user_logged_in()) {
		...
		if (get_option('symposium_activity_likes')) $notify_likes = $_POST['notify_likes'];
		...
		if (is_email($user_email)) {
			...
			if (get_option('symposium_activity_likes')) update_symposium_meta($uid, 'notify_likes', "'".$notify_likes."'");

			// Update firstname and lastname
			wp_update_user( array ('ID' => $uid, 'first_name' => $user_firstname, 'last_name' => $user_lastname) );
		
			$pwmsg = 'OK';
			$display_name_exists = $wpdb->get_var("SELECT display_name FROM ".$wpdb->base_prefix."users WHERE ID != ".$uid." AND (replace(' ', '', lower(display_name)) = '".str_replace(' ', '', strtolower($display_name))."' OR lower(user_login) = '".strtolower($display_name)."')");
			if ( get_option("symposium_unique_display_name") && $display_name_exists ) {
			...
			if ($requests) {
				...
				foreach ($requests as $request) {
					...
					$html .= "<div id='request_".$request->friend_from."' style='clear:right; margin-top:8px; overflow: auto; margin-bottom: 15px; width:95%;'>";		
						...
						$html .= "</div>";
						$html .= "<div class='symposium_friend_request_info'>";
...
function symposium_rrmdir_tmp($dir) {
   if (is_dir($dir)) {
	 $objects = scandir($dir);
	 foreach ($objects as $object) {
	   if ($object != "." && $object != "..") {
		 if (filetype($dir."/".$object) == "dir") symposium_rrmdir_tmp($dir."/".$object); else unlink($dir."/".$object);
	   }
	 }
	 reset($objects);
	 rmdir($dir);
