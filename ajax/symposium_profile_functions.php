...
if ($_POST['action'] == 'send_poke') {
	...
	if (is_user_logged_in()) {
		...
		$config = $wpdb->get_row($wpdb->prepare("SELECT poke_label FROM ".$wpdb->prefix . 'symposium_config'));

		// Filter to allow further actions to take place
		apply_filters ('symposium_send_poke_filter', $recipient, $current_user->ID, $current_user->display_name, $config->poke_label );
			
		$subject = __('You have been sent a ', 'wp-symposium').$config->poke_label;
		$url = symposium_get_url('profile');
		$message = "<a href='".$url.symposium_string_query($url)."uid=".$current_user->ID."'>".$current_user->display_name."</a>".__(' has sent you a ', 'wp-symposium').$config->poke_label;
	...
	if (is_user_logged_in()) {
		...
		if (is_user_logged_in()) {
			...
			if ( ($text != __(addslashes("Write a comment..."), "wp-symposium")) && ($text != '') ) {
				...
				// Email the author of the parent (ie. first post) and want to be notified
				$sql = "SELECT u.user_email, m.notify_new_wall
					FROM ".$wpdb->base_prefix."users u 
					LEFT JOIN ".$wpdb->base_prefix."symposium_usermeta m ON u.ID = m.uid 
					WHERE ID = %d";
			
				$parent_post_recipient = $wpdb->get_row($wpdb->prepare($sql, $parent_post->author_uid));
				...
				if ($parent_post->subject_uid != $parent_post->author_uid) {
					...
					$parent_post_recipient = $wpdb->get_row($wpdb->prepare($sql, $parent_post->subject_uid));
				...
				$reply_recipients = $wpdb->get_results($wpdb->prepare($sql, $parent));
...
if ($_POST['action'] == 'menu_extended') {
	...
	if (is_user_logged_in() || $meta->share == 'public') {
		...
		$html = "";
			...
			if ( ($uid1 == $uid2) || (strtolower($meta->share) == 'everyone') || (strtolower($meta->share) == 'public') || (strtolower($meta->share) == 'friends only' && symposium_friend_of($uid1)) ) {
				...
				if ( ($city != '' || $country != '') && ($config->profile_google_map > 0) ){ 	
				...
				if ($fields) {
					...
					$ext_rows = subval_sort($ext_rows,'order');
					foreach ($ext_rows as $row) {
						...
						if ($row['type'] != 'Checkbox') {
							...
							if (strpos($row['value'], 'on') !== FALSE) { 
								$html .= "<img src='".WP_PLUGIN_URL."/wp-symposium/images/tick.png' />"; 
							} else {
								$html .= "<img src='".WP_PLUGIN_URL."/wp-symposium/images/cross.png' />"; 
							}
							$html .= "</div><br />";
						}
						$html .= "</div>";
				...
				if (!$has_map && !$has_info) {
					...
					$html .= '<p>'.__("This member has no personal information to show.", "wp-psymposium").'</p>';
	
				}
						
			} else {
			
				$html .= '<p>'.__("Sorry, this member has chosen not to share their personal information.", "wp-psymposium").'</p>';
...
if ($_POST['action'] == 'acceptFriend') {
	...
	if (is_user_logged_in()) {
		...
		$already_a_friend = $wpdb->get_var($sql, $friend_to, $current_user->ID, $friend_to, $current_user->ID);
