...
if ($_POST['action'] == 'menu_mentions') {
	...
	global $wpdb;
	$tag = strtolower(str_replace(' ', '', $_POST['display_name']));
	
	$html = "";	
	
	$sql = "SELECT * FROM 
	(
	SELECT t2.tid AS id, t2.topic_subject AS text, t.topic_post as moretext, t.topic_owner AS author, t.topic_date AS dated, 'forum' AS type FROM ".$wpdb->base_prefix."symposium_topics t INNER JOIN ".$wpdb->base_prefix."symposium_topics t2 ON t2.tid = t.topic_parent WHERE t.topic_post REGEXP '@".$tag."[[:>:]]'
	)
		...
		foreach ($mentions AS $mention) {
			...
			$html .= '<div class="symposium_mentions row">';
				$html .= '<div style="padding-left:70px;">';
					...
					$html .= "<a href='".$url."'>".str_replace('<br />', ' ', $mention->text).'</a><br />';
...
if ($_POST['action'] == 'updateSettings') {
	...
	if (is_user_logged_in()) {
		...
		if (is_email($user_email)) {
			...
			$pwmsg = 'OK';
			$display_name_exists = $wpdb->get_var("SELECT ID FROM ".$wpdb->base_prefix."users WHERE replace(' ', '', lower(display_name)) = '".str_replace(strtolower($display_name), ' ', '')."' OR lower(user_login) = '".strtolower($display_name)."'");
			if (!$display_name_exists || $display_name == $current_user->display_name) {
				...
				$pwmsg = __("Display name already exists, sorry.".$email_exists->ID, "wp-symposium");
			}
				...
				if ( 
					...
					if ($show_add_comment) {
						...
						if (get_option('symposium_show_buttons')) {
							$html .= '<br /><input id="symposium_add_comment" type="submit" style="width:75px" class="symposium-button" value="'.__('Post', 'wp-symposium').'" /> ';
						}
						$html .= '<br />';
					}
				}
