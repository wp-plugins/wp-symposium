...
if ($_POST['action'] == 'send_poke') {
	...
	if (is_user_logged_in()) {
		...
		// Add to activity
		symposium_add_activity_comment($current_user->ID, $current_user->display_name, $recipient, WPS_POKE_LABEL, 'poke');
		...
		apply_filters ('symposium_send_poke_filter', $recipient, $current_user->ID, $current_user->display_name, WPS_POKE_LABEL, $cid );
...
if ($_POST['action'] == 'addStatus') {
	...
	if (is_user_logged_in()) {
		...
		if (is_user_logged_in()) {
			...
			if ( ($text != __(addslashes(WPS_STATUS_POST), "wp-symposium")) && ($text != '') ) {
...
if ($_POST['action'] == 'menu_extended') {
	...
	if (is_user_logged_in() || $meta->share == 'public') {
		...
		$html = "";
			...
			if ( ($uid1 == $uid2) || (is_user_logged_in() && strtolower($meta->share) == 'everyone') || (strtolower($meta->share) == 'public') || (strtolower($meta->share) == 'friends only' && symposium_friend_of($uid1, $current_user->ID)) ) {
	...
	if (is_user_logged_in()) {
		...
		if ($uid == $current_user->ID || symposium_get_current_userlevel($current_user->ID) == 5) {
			...
			$html .= '<div id="symposium_settings_table">';
				...
				// First name
				$html .= '<div class="symposium_settings_row">';
					$html .= __('Your first name', 'wp-symposium');
					$html .= '<div>';
						$html .= '<input type="text" class="input-field" id="user_firstname" name="user_firstname" value="'.$current_user->user_firstname.'">';
					$html .= '</div>';
				$html .= '</div>';
			
				// Last name
				$html .= '<div class="symposium_settings_row">';
					$html .= __('Your last name', 'wp-symposium');
					$html .= '<div>';
						$html .= '<input type="text" class="input-field" id="user_lastname" name="user_lastname" value="'.$current_user->user_lastname.'">';
					$html .= '</div>';
				$html .= '</div>';
	...
	if (is_user_logged_in()) {
		...
		if (is_email($user_email)) {
			...
			if ($email_exists && $email_exists->user_email == $user_email && $email_exists->ID != $current_user->ID && symposium_get_current_userlevel($current_user->ID) < 5) {
				...
				$rows_affected = $wpdb->update( $wpdb->base_prefix.'users', array( 'display_name' => stripslashes($display_name), 'user_email' => $user_email ), array( 'ID' => $uid ), array( '%s', '%s' ), array( '%d' ) );
			}
...
if ($_POST['action'] == 'updatePersonal') {
	...
	if (is_user_logged_in()) {
		...
		$current_city = get_symposium_meta($uid, 'city');
		$current_country = get_symposium_meta($uid, 'country');
		...
		// if city and country have changed (or lat/long have not been set)		
		...
		// Now update WP user meta
		update_user_meta($uid, 'wps-dob_day', $dob_day);
		update_user_meta($uid, 'wps-dob_month', $dob_month);
		update_user_meta($uid, 'wps-dob_year', $dob_year);
		update_user_meta($uid, 'wps-city', $city);
		update_user_meta($uid, 'wps-country', $country);
		$extended = get_symposium_meta($uid, 'extended');
		$fields = explode('[|]', $extended);
		if ($fields) {
			$ext_rows = array();
			foreach ($fields as $field) {
				...
				if ( $split[0] != '') {			
					...
					if ($split[1] != '' || $extension->extended_type == 'Checkbox') {	
						...
						$value = str_replace('</p>', '', $value);						
						update_user_meta($uid, 'wps-'.$extension->extended_name, $value);
					}
				}
			}
		} 
