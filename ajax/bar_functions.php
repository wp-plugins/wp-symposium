...
if ($_POST['action'] == 'symposium_getfriendsonline') {
	...
	if (is_user_logged_in()) {
		...
		if (!get_option(WPS_OPTIONS_PREFIX.'_wps_panel_all')) {
				...
				$return .= "<div style='clear:both; margin-top:4px; overflow: auto;overflow-y:hidden;'>";		
					...
					$return .= "<div>";
						if ( $use_chat != 'on' || get_option(WPS_OPTIONS_PREFIX.'_wps_lite') ) {
							...
							$return .= "<span class='__wps__online_name' title='".$friend['ID']."'>".$friend['display_name']."</span>";
						}
					$return .= "</div>";
				$return .= "</div>";
			}
...
if ($_POST['action'] == 'symposium_getchat') {
	...
	if (is_user_logged_in()) {
		...
		$time_now = time();
	   	...
	   	// clear rogue chats
	   	$sql = "DELETE FROM ".$wpdb->base_prefix."symposium_chat WHERE chat_to = %d OR chat_from = %d";
		$rows_affected = $wpdb->query( $wpdb->prepare($sql, 0, 0) );
		...
		if ($chats_list) {
			...
			foreach ($chats_list as $chat) {
				...
				$add = array (	
					...
					'toname' => $chat->toname,
					'fromlast' => __wps__get_meta($chat->chat_from, 'last_activity'),
					'status' => __wps__get_meta($chat->chat_from, 'status'),
					'tolast' => __wps__get_meta($chat->chat_to, 'last_activity')
				);
				
				array_push($chat_array, $add);
			}
			$chats = __wps__sub_val_sort($chat_array, 'chid');
			...
			foreach ($chats as $chat) {
				...
				$last_message = $chat['chid']."[|]".$chat['chat_from'].'[|]'.$chat['chat_to'].'[|]';
					...
					if ($chat['chat_from'] == $current_user->ID) {
						$results .= __("You", WPS_TEXT_DOMAIN);
					} else {
						$results .= $chat['fromname'];
					}
					$results .= '</div>';
				...
				$last_message .= '<div style="color:#aaa">'.__('Last message', WPS_TEXT_DOMAIN).' '.__wps__time_ago($chat['chat_timestamp']).'.</div>[|]';
				

				if ($chat['chat_from'] == $me) {
					$last_active_minutes = __wps__convert_datetime($chat['tolast']);
				} else {

					if ($chat['fromlast'] && $chat['status'] != 'offline') {
						$last_active_minutes = __wps__convert_datetime($chat['fromlast']);
					} else {
						$last_active_minutes = 999999999;
					}
				}
...
if ($_POST['action'] == 'symposium_getchatroom') {
	...
	if (is_user_logged_in()) {
		...
		$use_chat = $_POST['use_chat'];	
	   	...
	   	// clear rogue chats
	   	$sql = "DELETE FROM ".$wpdb->base_prefix."symposium_chat WHERE chat_to = 0 OR chat_from = %d";
		$rows_affected = $wpdb->query( $wpdb->prepare($sql, 0) );
		...
		if ($chats_list) {
			...
			$chatlist = __wps__sub_val_sort($chat_array, 'chid');
		...
		if ($chatlist) {
			...
			foreach ($chatlist as $chat) {
				...
				if ($chat['fromlast'] && $chat['status'] != 'offline') {
					$last_active_minutes = __wps__convert_datetime($chat['from']);
				} else {
					$last_active_minutes = 999999999;
				}
				...
				$last_active_minutes = floor(($time_now-$last_active_minutes)/60);			
				if ($last_active_minutes >= $offline) {
					$status_img = get_option(WPS_OPTIONS_PREFIX.'_images').'/loggedout.gif';
				} else {
					if ($last_active_minutes >= $inactive) {
						$status_img = get_option(WPS_OPTIONS_PREFIX.'_images').'/inactive.gif';
					} else {
						$status_img = get_option(WPS_OPTIONS_PREFIX.'_images').'/online.gif';
					}
				}
				...
				$results .= __wps__make_url(stripslashes($chat['chat_message']));
				$results .= "</div>";

				$results .= "<div style='clear:both; float:right; color:#aaa; font-style:italic;'>";
				$results .= "<img src='".$status_img."' title='".$status_img."' />&nbsp;";
				$results .= $chat['fromname'];
				$results .= ", ".__wps__time_ago($chat['chat_timestamp']);
				$results .= '</div>';
...
if ($_POST['action'] == 'symposium_openchat') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT chid FROM ".$wpdb->base_prefix."symposium_chat WHERE (chat_from = %d AND chat_to = %d) OR (chat_from = %d AND chat_to = %d)";
		if ($wpdb->query( $wpdb->prepare($sql, $chat_from, $chat_to, $chat_from))) {
			...
			if ( $rows_affected = $wpdb->insert( $wpdb->base_prefix . "symposium_chat", array( 
				...
				$display_name = $wpdb->get_var($wpdb->prepare($sql, $chat_to));
