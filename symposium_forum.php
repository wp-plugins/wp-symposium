...
function symposium_forum() {	
	...
	if ( get_option('wps_permalink_structure') && get_query_var('stub')) {
	...
	if ( ( $topic_id == '' && $cat_id == '') || ( !get_option('symposium_forum_ajax') && !get_option('wps_permalink_structure') ) ) {
	...
	if ( $can_view || strpos($viewer, __('everyone', 'wp-symposium')) !== FALSE) {
		...
		if ( get_option('wps_permalink_structure') || !get_option('symposium_forum_ajax') ) {
			if ($topic_id == 0) {
...
function symposium_forum_latestposts_showThreadChildren($count, $parent, $level, $use_answers) {
	...
	$posts = $wpdb->get_results("
		...
		WHERE t.topic_parent = ".$parent." AND t.topic_group = 0 ORDER BY t.tid ".$desc." LIMIT 0,".$postcount); 
