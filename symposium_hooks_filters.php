...
function symposium_rewrite($wp_rewrite) {

	// Forum
	
	if ( ($r = get_option('symposium_rewrite_forum_single')) && ($t = get_option('symposium_rewrite_forum_single_target')) ) {
		$wp_rewrite->rules = array_merge( array ( $r => $t ), $wp_rewrite->rules);
	}
	if ( ($r = get_option('symposium_rewrite_forum_double')) && ($t = get_option('symposium_rewrite_forum_double_target')) ) {
		$wp_rewrite->rules = array_merge( array ( $r => $t ), $wp_rewrite->rules);
	}
	
	// Profile

	if ( ($r = get_option('symposium_rewrite_members')) && ($t = get_option('symposium_rewrite_members_target')) ) {
		$wp_rewrite->rules = array_merge( array ( $r => $t ), $wp_rewrite->rules);
	}
...
if (get_option('symposium_permalink_structure')) {
	add_filter('generate_rewrite_rules', 'symposium_rewrite');
	add_filter('query_vars', 'symposium_query_var' );
}

/* Hides plugins from Plugin Dashboard page, set in wp-options.php as constant */
/* If set, also hides the WP Symposium Admin menu */
function symposium_plugin_permissions($plugins) {

	global $current_user;
	if (WPS_HIDE_PLUGINS == 'WPS_HIDE_PLUGINS' || $current_user->user_login == WPS_SUPERUSER) return $plugins;
	
	$viewable_plugins = array();
	foreach ($plugins as $plugin) {
		if (strpos($plugin['Name'], WPS_HIDE_PLUGINS) === false)
...
add_filter('all_plugins', 'symposium_plugin_permissions');
	...
	global $wpdb,$current_user;
			...
			if ( ( get_option('symposium_menu_profile') == 'on') && ( ($uid1 == $uid2) || (is_user_logged_in() && strtolower($share) == 'everyone') || (strtolower($share) == 'public') || (strtolower($share) == 'friends only' && $is_friend) || symposium_get_current_userlevel() == 5) ) {
				if ($extended != '' || $uid1 == $uid2) {
					if ($uid1 == $uid2) {
						if (get_option('symposium_menu_profile'))
							$html .= '<div id="menu_extended" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_profile_text')) != '' ? $t :  __('My Profile', 'wp-symposium')).'</div>';
					} else {
						if (get_option('symposium_menu_profile_other'))
							$html .= '<div id="menu_extended" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_profile_other_text')) != '' ? $t :  __('Profile', 'wp-symposium')).'</div>';
					}
				}
			}
			...
			if  ( ($uid1 == $uid2) || (is_user_logged_in() && strtolower($privacy) == 'everyone') || (strtolower($share) == 'public') || (strtolower($privacy) == 'friends only' && $is_friend) || symposium_get_current_userlevel() == 5) {
				...
				if ($uid1 == $uid2) {
					if (get_option('symposium_menu_my_activity') == 'on') {
						$html .= '<div id="menu_wall" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_my_activity_text')) != '' ? $t :  __('My Activity', 'wp-symposium')).'</div>';
					}
					if (get_option('symposium_menu_friends_activity') == 'on') {
						if (strtolower($share) == 'public' && !(is_user_logged_in())) {
							...
							$html .= '<div id="menu_activity" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_friends_activity_text')) != '' ? $t :  __('My Friends Activity', 'wp-symposium')).'</div>';
						}
					}
				} else {
					if (get_option('symposium_menu_my_activity_other') == 'on') {
						$html .= '<div id="menu_wall" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_my_activity_other_text')) != '' ? $t :  __('Activity', 'wp-symposium')).'</div>';
					}
					if (get_option('symposium_menu_friends_activity_other') == 'on') {
						if (strtolower($share) == 'public' && !(is_user_logged_in())) {
							// don't show friends activity to public
						} else {
							$html .= '<div id="menu_activity" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_friends_activity_other_text')) != '' ? $t :  __('Friends Activity', 'wp-symposium')).'</div>';
						}
					}
				}
				...
				if (strtolower($share) == 'public' && !(is_user_logged_in())) {
					...
					if ($uid1 == $uid2) {
						if (get_option('symposium_menu_all_activity')) {
							$t = ($t = get_option('symposium_menu_all_activity_text')) != '' ? $t :  __('All Activity', 'wp-symposium');
							$html .= '<div id="menu_all" class="symposium_profile_menu">'.$t.'</div>';
						}
					} else {
						if (get_option('symposium_menu_all_activity_other')) {
							$t = ($t = get_option('symposium_menu_all_activity_other_text')) != '' ? $t :  __('All Activity', 'wp-symposium');
							$html .= '<div id="menu_all" class="symposium_profile_menu">'.$t.'</div>';
						}
					}					
				}
				if (function_exists('symposium_group')) {
					if ($uid1 == $uid2) {
						if (get_option('symposium_menu_groups'))
							$html .= '<div id="menu_groups" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_groups_text')) != '' ? $t :  __('My Groups', 'wp-symposium')).'</div>';
					} else {
						if (get_option('symposium_menu_groups_other'))
							$html .= '<div id="menu_groups" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_groups_other_text')) != '' ? $t :  __('Groups', 'wp-symposium')).'</div>';
					}
				}				
			}
			...
			if ( ($uid1 == $uid2) || (is_user_logged_in() && strtolower($share) == 'everyone') || (strtolower($share) == 'friends only' && $is_friend) || symposium_get_current_userlevel() == 5) {
				if (get_option('symposium_menu_friends') == 'on') {
					if ($uid1 == $uid2) {
						if (get_option('symposium_menu_friends')) {
							...
							$html .= '<div id="menu_friends" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_friends_text')) != '' ? $t :  __('My Friends', 'wp-symposium')).' '.$pending_friends.'</div>';
						}
					} else {
						if (get_option('symposium_menu_friends_other'))
							$html .= '<div id="menu_friends" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_friends_other_text')) != '' ? $t :  __('Friends', 'wp-symposium')).'</div>';
					}
				}
			}
...
function symposium_header() {
	...
	if (symposium_required()) {
		...
		if (WPS_DEBUG) {
			...
			echo '<strong>'.sprintf(__('%s Debug Mode', 'wp-symposium'), WPS_WL).'</strong><br />';
...
global $wpdb;
	...
	if ( ($city != '' || $country != '') && (get_option('symposium_profile_google_map') > 0) ){ 	
		...
		$html .= '</a><br /><span class="description"> '.sprintf(__("The Google map that will be displayed on top of your %s profile page, resulting from your personal data above.", "wp-symposium"), WPS_WL).'</span></td></tr>';
