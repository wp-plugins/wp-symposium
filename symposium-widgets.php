...
class Recentactivity_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Content of widget
		$posts = $wpdb->get_results("SELECT cid, author_uid, comment, comment_timestamp, display_name FROM ".$wpdb->base_prefix."symposium_comments c INNER JOIN ".$wpdb->base_prefix."users u ON c.author_uid = u.ID WHERE is_group != 'on' AND comment_parent = 0 AND author_uid = subject_uid ORDER BY cid DESC LIMIT 0,50");
...
class Symposium_friends_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if (is_user_logged_in()) {
			...
			$symposium_friends_count = apply_filters('widget_symposium_members_count', $instance['symposium_friends_count'] );
				...
				echo "<div id='symposium_new_members'>";
					...
					for ($l=1; $l<=$loop; $l++) {
						...
						foreach ($members as $member)
						{
							...
							$show = false;
							if ($symposium_friends_mode == 'online' && $last_active_minutes < $config->offline) { $show = true; }
							if ( ($symposium_friends_mode == 'split') && ( ($last_active_minutes < $config->offline && $l == 1) || ($last_active_minutes >= $config->offline && $l == 2) ) ) { $show = true; }
									...
									echo "<div class='symposium_new_members_row'>";		
										...
										echo "<div class='symposium_new_members_row_member'>";
											...
											if ($symposium_friends_show_light == 'on') {
												if ($last_active_minutes >= $config->offline) {
													echo '<img src="'.WP_PLUGIN_URL.'/wp-symposium/images/loggedout.gif"> ';
												} else {
													if ($last_active_minutes >= $config->online) {
														echo '<img src="'.WP_PLUGIN_URL.'/wp-symposium/images/inactive.gif"> ';
													} else {
														echo '<img src="'.WP_PLUGIN_URL.'/wp-symposium/images/online.gif"> ';
													}
												}
											}
...
class Forumrecentposts_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Content of widget
		$sql = "SELECT tid, topic_subject, topic_owner, topic_post, topic_started, topic_category, topic_date, display_name, topic_parent, topic_group FROM ".$wpdb->prefix.'symposium_topics'." t INNER JOIN ".$wpdb->base_prefix.'users'." u ON t.topic_owner = u.ID WHERE tid = tid AND topic_approved='on' ";
		if ($cat_id != '' && $cat_id > 0) {
									...
									if ($post->topic_parent > 0) {
										...
										if (is_user_logged_in()) {
											if ($post->topic_date > $previous_login && $post->topic_owner != $current_user->ID) {
												echo " <img src='".WP_CONTENT_URL."/plugins/wp-symposium/images/new.gif' alt='New!' />";
											}
										}
								echo "</div>";
							echo "</div>";
...
class symposium_summary_Widget extends WP_Widget {
		...
		echo "<div id='symposium_summary_widget'>";
			...
			if (is_user_logged_in()) {
				...
				if ($show_loggedout == 'on' && $show_form == '') {
					...
					echo ' (<a href="'.wp_lostpassword_url( get_bloginfo('url') ).'" title="'.__('Forgot Password?', 'wp-symposium').'">'.__('Forgot Password?', 'wp-symposium').'</a>)';
					echo "</li>";
					echo wp_register( "<li>", "</li>", true);
					echo "</ul>";
				}
				...
				if ($show_form == 'on') {
					echo '<div id="symposium-widget-login">';
					echo __('Username', 'wp-symposium').'<br />';
					echo '<input type="text" id="symposium_widget_login" /><br />';
					echo __('Password', 'wp-symposium').'<br />';
					echo '<input type="password" id="symposium_widget_password" /><br /><br />';
					echo '<input type="submit" id="symposium_widget_login_button" title="'.$login_url.'" value="'.__('Login', 'wp-symposium').'" /><br />';
					echo wp_register( "", "<br />", true);
					echo '<a href="'.wp_lostpassword_url( get_bloginfo('url') ).'" title="'.__('Forgot Password?', 'wp-symposium').'">'.__('Forgot Password?', 'wp-symposium').'</a>';
					echo '</div>';
				}
	...
	function form( $instance ) {
		...
		<br /><br />
			...
			<input id="<?php echo $this->get_field_id( 'login_url' ); ?>" name="<?php echo $this->get_field_name( 'login_url' ); ?>" value="<?php echo $login_url; ?>" />
		</p>
		<?php
	}
...
class Forumnoanswer_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$sql = "SELECT t.tid, t.topic_subject, t.topic_owner, t.topic_post, t.topic_category, t.topic_date, u.display_name, t.topic_parent, t.topic_group, t.topic_started, 
			(SELECT COUNT(*) FROM ".$wpdb->prefix."symposium_topics u WHERE u.topic_parent = t.tid) AS replies 
			FROM ".$wpdb->prefix."symposium_topics t INNER JOIN ".$wpdb->base_prefix.'users'." u ON t.topic_owner = u.ID
			WHERE t.topic_parent = 0 AND t.for_info != 'on' AND t.topic_approved = 'on' AND t.topic_started >= ( CURDATE() - INTERVAL ".$timescale." DAY ) 
			AND NOT EXISTS 
