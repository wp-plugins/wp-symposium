							...
							if ( (strtolower($share) == 'everyone') || (strtolower($share) == 'friends only' && $is_friend) ) {
								...
								echo "<div class='symposium_recent_activity_row'>";		
									...
									echo "<div class='symposium_recent_activity_row_post'>";
										...
										echo "<a href='".$thispage.$q."uid=".$post->author_uid."&post=".$post->cid."'>".$post->display_name."</a> ".$text." ".symposium_time_ago($post->comment_timestamp).".<br>";
									echo "</div>";
								echo "</div>";
...
class Forumrecentposts_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$posts = $wpdb->get_results("
			SELECT tid, topic_subject, topic_owner, topic_post, topic_category, topic_date, display_name, topic_parent, topic_group 
			FROM ".$wpdb->prefix.'symposium_topics'." t INNER JOIN ".$wpdb->base_prefix.'users'." u ON t.topic_owner = u.ID 
			ORDER BY tid DESC LIMIT 0,100"); 
		$count = 0;
							...
							echo "<div class='symposium_latest_forum_row'>";		
								...
								echo "<div class='symposium_latest_forum_row_post'>";
									if ($post->topic_parent > 0) {
										...
										echo symposium_profile_link($post->topic_owner)." ".__('started', 'wp-symposium')." <a href='".$url.$q."cid=".$post->topic_category."&show=".$post->tid."'>".$text."</a> ".symposium_time_ago($post->topic_date).".<br>";
									}
								echo "</div>";
							echo "</div>";
