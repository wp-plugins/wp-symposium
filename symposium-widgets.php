...
class Forumrecentposts_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Content of widget
		$sql = "SELECT tid, topic_subject, topic_owner, topic_post, topic_category, topic_date, display_name, topic_parent, topic_group FROM ".$wpdb->prefix.'symposium_topics'." t INNER JOIN ".$wpdb->base_prefix.'users'." u ON t.topic_owner = u.ID ";
		if ($cat_id != '' && $cat_id > 0) {
			$sql .= "WHERE topic_category = ".$cat_id." ";
		}
	...
	function form( $instance ) {
		...
		/* Set up some default widget settings. */
		$defaults = array( 'wtitle' => 'Recent Forum Posts', 'postcount' => '3', 'preview' => '30', 'cat_id' => '0' );
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>
