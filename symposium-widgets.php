...
class Forumnoanswer_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Content of widget
		$sql = "SELECT t.tid, t.topic_subject, t.topic_owner, t.topic_post, t.topic_category, t.topic_date, u.display_name, t.topic_parent, t.topic_group, 
			...
			WHERE t.topic_parent = 0 AND t.for_info != 'on' AND t.topic_date >= ( CURDATE() - INTERVAL ".$timescale." DAY ) 
			AND NOT EXISTS 
		...
		$sql .= "ORDER BY t.topic_date DESC LIMIT 0,".$postcount;
		$posts = $wpdb->get_results($sql); 
