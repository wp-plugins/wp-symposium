...
class symposium_summary_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if (is_user_logged_in()) {
			...
			if (function_exists('symposium_profile')) {
				...
				echo "<li>";
					$sql = "SELECT count(*) FROM ".$wpdb->base_prefix."symposium_friends WHERE friend_to = ".$current_user->ID." AND friend_accepted = 'on'";
					$current_friends = $wpdb->get_var($wpdb->prepare($sql));
					echo  "<a href='".$friends_url.$q."'>".__("Friends:", "wp-symposium")."</a> ".$current_friends;
					$sql = "SELECT count(*) FROM ".$wpdb->base_prefix."symposium_friends WHERE friend_to = ".$current_user->ID." AND friend_accepted != 'on'";
					$friend_requests = $wpdb->get_var($wpdb->prepare($sql));
	...
	function form( $instance ) {
		...
		<br /><br />
			...
			<?php if ($instance['show_form'] == 'on') { echo " CHECKED"; } ?>
			/>
		...
		<br /><br />
			...
			<input id="<?php echo $this->get_field_id( 'login_url' ); ?>" name="<?php echo $this->get_field_name( 'login_url' ); ?>" value="<?php echo $instance['login_url']; ?>" />
		</p>
		<?php
	}
