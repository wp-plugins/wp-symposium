...
class Forumrecentposts_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Content of widget
		$sql = "SELECT tid, topic_subject, topic_owner, topic_post, topic_category, topic_date, display_name, topic_parent, topic_group FROM ".$wpdb->prefix.'symposium_topics'." t INNER JOIN ".$wpdb->base_prefix.'users'." u ON t.topic_owner = u.ID WHERE tid = tid AND topic_approved='on' ";
		if ($cat_id != '' && $cat_id > 0) {
