...
class wps_user {
	...
	public function __construct($id='') {
		...
		$this->dob_day = get_symposium_meta($id, 'dob_day');				// Date of Birth (day)
		$this->dob_month = get_symposium_meta($id, 'dob_month');			// Date of Birth (month)
		$this->dob_year = get_symposium_meta($id, 'dob_year');				// Date of Birth (year)		
		$this->last_activity = get_symposium_meta($id, 'last_activity');	// When last active		
	...
	function set_city($value) {
    	$this->city = $value;
		update_symposium_meta($this->id, 'city', $value);
    }
    function get_city() {
		return $this->city;
    }
	...
	function set_country($value) {
    	$this->country = $value;
		update_symposium_meta($this->id, 'country', $value);
    }
    ...
    function get_latest_activity($as_row=false) {
        global $wpdb;
        if (!$as_row) {
	        $comment = stripslashes($wpdb->get_var( $wpdb->prepare("SELECT comment FROM ".$wpdb->base_prefix."symposium_comments WHERE subject_uid = ".$this->id." AND author_uid = ".$this->id." AND comment_parent = 0 ORDER BY cid DESC LIMIT 0,1")));
        } else {
            $comment = $wpdb->get_row( $wpdb->prepare("SELECT comment FROM ".$wpdb->base_prefix."symposium_comments WHERE subject_uid = ".$this->id." AND author_uid = ".$this->id." AND comment_parent = 0 ORDER BY cid DESC LIMIT 0,1"));
        }
		return $comment;
    }        
	...
	function set_activity_privacy($value) {
		...
		update_symposium_meta($this->id, 'wall_share', $value);
    }
	...
	function set_dob_day($value) {
    	$this->dob_day = $value;
		update_symposium_meta($this->id, 'dob_day', $value);
    }
	...
	function set_dob_month($value) {
		...
		update_symposium_meta($this->id, 'dob_month', $value);
    }
	...
	function set_dob_year($value) {
    	$this->dob_year = $value;
		update_symposium_meta($this->id, 'dob_year', $value);
    }
    ...
    function get_user_meta($uid, $slug) {
		return get_symposium_meta($uid, $slug);
    }
    ...
    function get_extended() {
		...
		$extended = get_symposium_meta($this->id, 'extended');					
		...
		if ($fields) {
			...
			foreach ($fields as $field) {
				...
				if ( ($split[0] != '') && ($split[1] != '') ) {
					...
					$extension = $wpdb->get_row($wpdb->prepare("SELECT extended_slug,extended_name,extended_order,extended_type FROM ".$wpdb->prefix."symposium_extended WHERE eid = ".$split[0]));
    ...
    function get_friends() {
	    ...
	    $sql = "SELECT f.friend_to AS id FROM ".$wpdb->base_prefix."symposium_friends f
	 	...
	 	$friends_list = $wpdb->get_results($wpdb->prepare($sql, $this->id));
...
function user_has_permission($id, $type) {
	...
	$prviacy = get_symposium_meta($id, $type);
