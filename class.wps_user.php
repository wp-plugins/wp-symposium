		...
		if ($fields) {
			...
			foreach ($fields as $field) {
				...
				if ( ($split[0] != '') && ($split[1] != '') ) {
					...
					$extension = $wpdb->get_row($wpdb->prepare("SELECT extended_slug,extended_name,extended_order,extended_type FROM ".$wpdb->prefix."symposium_extended WHERE eid = %d", $split[0]));
