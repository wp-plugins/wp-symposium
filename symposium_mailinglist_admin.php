...
<div id="icon-themes" class="icon32"><br /></div>
	...
	echo '<th style="font-size:1.2em" width="70%">'.__('Snippet', 'wp-symposium').'</th>';
	echo '</tr>';
	echo '</thead>';
	echo '<tbody>';	
	
	$sql = "SELECT l.*, u.display_name FROM ".$wpdb->prefix."symposium_mailinglist_log l
			LEFT JOIN ".$wpdb->base_prefix."users u ON l.uid = u.ID ORDER BY lid DESC
			LIMIT 0,50";
	$log = $wpdb->get_results($wpdb->prepare($sql));
	foreach ($log as $entry) {
		echo '<tr>';
		echo '<td><a href="'.get_option('symposium_profile_url').symposium_string_query(get_option('symposium_profile_url')).'uid='.$entry->uid.'">'.$entry->display_name.'</td>';
		echo '<td>'.$entry->added_date.'</td>';
		echo '<td><a href='.get_option('symposium_forum_url').symposium_string_query(get_option('symposium_forum_url')).'show='.$entry->tid.'>'.$entry->tid.'</a></td>';
		echo '<td>'.substr($entry->topic_post, 0, 100).'</td>';
		echo '</tr>';
	}
