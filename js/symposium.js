jQuery(document).ready(function() { 	
			...
			.data( "autocomplete" )._renderItem = function( ul, item ) {
				var member = "<a>";
					member += "<div style='height:40px; overflow:hidden'>";
						...
						member += item.avatar;
						member += "</div>";			
						member += "<div style=\'font-size:16px\'>" + item.label + "</div>";
						member += "<div style=\'font-size:10px\'>"+item.city + item.country + "</div>";
	...
	*/
			...
			jQuery.ajax({
				...
				success: function(str){
					...
					jQuery('#profile_file_upload').uploadify({
						...
						'auto'      : true,
						'scriptData' : {'user_login':user_login, 'user_email':user_email, 'uid':symposium.current_user_id}, 
	...
	jQuery("#updateSettingsButton").live('click', function() {
		...
		if (display_name == '') {
			jQuery("#display_name").css('border', '1px solid red').effect("highlight", {}, 4000);
		} else {

			if (user_email == '') {
				jQuery("#user_email").css('border', '1px solid red').effect("highlight", {}, 4000);
			} else {
	...
	*/
	...
	jQuery(".topic_subject").live('click', function() {
		...
		var topic_id = jQuery(this).attr("title");
		jQuery.ajax({
			url: symposium.plugin_url+"ajax/symposium_forum_functions.php", 
			type: "POST",
			data: ({
	...
	jQuery("#edit-this-topic").live('click', function() {
		...
		buttons: {
				"Update": function() {
					...
					if (parent == 0) {
						jQuery(".topic-post-header").html(topic_subject.replace(/\</g, "&lt;").replace(/\>/g, "&gt;").replace(/(<([^>]+)>)/ig, ''));
	...
	jQuery(".forum_post_score_change").live('click', function() {
		...
		if (symposium.current_user_id == 0) {
			...
			if (parseFloat(jQuery('#symposium_forum_vote_remove').html()) == new_score) {
					jQuery('#reply'+tid).html(jQuery('#symposium_forum_vote_remove_msg').html())
			}
		
			var full_score = new_score;
			if (full_score > 0) { full_score = '+'+full_score; }
			jQuery('#forum_score_'+tid).html(full_score);
			jQuery(this).removeClass('forum_post_score_change');
	...
	*/
	...
	// Templates
 	...
 	jQuery("#reset_template_forum_category").click(function() {
		if (confirm("Are you sure?")) {
			var reset = "<div class='row_startedby'>[]<div class='avatar avatar_last_topic'>[avatar,32]</div>[replied][subject][ago]</div>[]<div class='row_views'>[post_count]</div>[]<div class='row_topic row_replies'>[topic_count]</div>[]<div class='row_topic'>[category_title]<br />[category_desc]</div>";
			reset = reset.replace(/\[\]/g, String.fromCharCode(13));
	...
	if (jQuery("#hide_motd").length) {	
		jQuery('#hide_motd').click(function(){
			...
			jQuery.ajax({
				...
				async: true,
				success: function(str){
					window.location.href="admin.php?page=symposium_notes";
				}				
