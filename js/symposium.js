jQuery(document).ready(function() { 	
	...
	*/
	...
	jQuery("#setup_facebook").live('click', function() {
		...
		var str = '<p>Go to your Facebook Profile page and copy the long number from the web address (URL) shown afer id=<br /><br />This is an example:<br /> http://www.facebook.com/profile.php?id=12345432123</p>';
		str += '<input type="text" id="facebook_id" style="width:200px; height:19px;" />';
		str += '<input type="submit" id="facebook_id_submit" value="OK" class="symposium-button" style="width:50px; height:24px; margin-left: 3px;" />';
		
		jQuery("#dialog").html(str);
		jQuery("#dialog").dialog({ title: 'Please enter your Facebook ID' });
	});
	...
	jQuery("#cancel_facebook").live('click', function() {		
		...
		jQuery.ajax({
			...
			success: function(str){
				...
				jQuery("#dialog").dialog({ title: 'Facebook Connect Removed' });
			}
   		});
	});
	...
	jQuery("#showmore_wall").live('click', function() {
		...
		jQuery('#showmore_wall').html("<img src='"+symposium.plugin_url+"/images/busy.gif' />");
	...
	jQuery("#symposium_add_update").live('click', function() {
		...
		jQuery.ajax({
			...
			success: function(str){
				if (str != '' && str != 'Array') {
					jQuery("#dialog").html(str);
					jQuery("#dialog").dialog({ title: 'Alert' });
				}
				jQuery(".symposium_notice").fadeOut("slow");
			}
   		});
