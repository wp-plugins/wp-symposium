...
jQuery(document).ready(function() { 	
	...
	*/
	...
	jQuery('.symposium-dialog').live('click', function () {
		...
		jQuery("#dialog").html(jQuery('#'+id).html()).dialog({ title: title, width: 800, height: 500, modal: true,
		buttons: {
				"OK": function() {
	...
	*/
	...
	function symposium_add_comment_to_profile() {
		...
		jQuery("#symposium_comment").hide();
		if (jQuery("#symposium_add_comment").length) {
			jQuery("#symposium_add_comment").hide();
		}
	...
	if (!symposium.use_wp_editor) {
		...
		if (symposium.wps_wysiwyg == 'on' || editor_id == 'edit_event_text' || editor_id == 'edit_event_more') {
			...
			tinyMCE.init({
				// General options
				theme : "advanced",
				plugins : "fullscreen,safari,pagebreak,table,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,contextmenu,paste,directionality,visualchars,nonbreaking,xhtmlxtras",
				mode : "exact",
				elements : editor_id,
			    relative_urls : false,
