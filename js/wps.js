...
/*
	...
	*/
	...
	if ((jQuery("#profile_body").length) && (__wps__.embed != 'on')) {
		...
		jQuery.ajax({
			...
			success: function(str) {
				...
				// Activity file upload (default page version)
				if (jQuery('#activity_file_upload').length) {
					jQuery('#activity_file_upload').uploadify({
						'uploader': __wps__.plugin_url + 'uploadify/uploadify.swf',
						'buttonText': attachimage,
						'script': __wps__.plugin_url + 'uploadify/upload_activity_file.php',
						...
						'onError': function(event, ID, fileObj, errorObj, response, data) {
							alert("Error: " + errorObj.type + " " + errorObj.info);
						},
						'onComplete': function(event, queueID, fileObj, response, data) {
							jQuery('#activity_file_upload_file').html(response).show();
							jQuery('#activity_file_upload_button').hide();
						}
					});
				}
				...
				// Profile avatar upload (default page load)
	...
	jQuery("#setup_facebook").live('click', function() {
		var str = '<br /><input type="text" id="facebook_id" style="width:200px; height:16px; float:left;" />';
		str += '<input type="submit" id="facebook_id_submit" value="OK" class="__wps__button" style="width:50px; margin-left: 3px;" />';
		str += '<p>To find out your ID <a href="javascript:void(0)" id="get_facebook_id">click here</a>.</p>';
	...
	jQuery("#get_facebook_id").live('click', function() {
		var str = '<iframe scrolling="no" style="margin-top:-120px;margin-left:-10px;width:100%; height:260px;" src="http://findmyfacebookid.com/"></iframe>';
		jQuery("#dialog").html(str);
		jQuery("#dialog").dialog({
			title: 'Go to your Facebook timeline, and look in the browser address bar (URL)',
			width: 740,
			height: 230,
			modal: true,
			buttons: {},
	...
	jQuery(".__wps__profile_menu").click(function() {
		...
		jQuery.ajax({
			...
			success: function(str) {
				...
				// Activity file upload (page selected)
				if (jQuery('#activity_file_upload').length) {
					jQuery('#activity_file_upload').uploadify({
						'uploader': __wps__.plugin_url + 'uploadify/uploadify.swf',
						'buttonText': attachimage,
				...
				// Profile avatar upload (page selected)				
				jQuery('#profile_file_upload').uploadify({
					'uploader': __wps__.plugin_url + 'uploadify/uploadify.swf',
					'buttonText': browseforfile,
					'script': __wps__.plugin_url + 'uploadify/upload_profile_avatar.php',
	...
	function symposium_add_comment_to_profile() {
		...
		if (comment_text != '') {
			...
			setTimeout(function() {
				...
				jQuery('#activity_file_upload_file').html('').hide();
				jQuery('#activity_file_upload_button').show();
			}, 3000);
		...
		if (event.type == 'mouseenter' && __wps__.current_user_id > 0) {
			...
			if (rel == 'friend' || id == __wps__.current_user_id) {
				if (strpos(__wps__.mail_url, 'INVALID PLUGIN URL REQUESTED') === false && id != __wps__.current_user_id) {
					html += '<input id="symposium_plus_sendmail" ref="' + id + '" type="text" style="padding-left:18px; background: transparent url(\'' + __wps__.images_url + '/mail_small.png\') 2px 2px no-repeat;margin-top:4px;margin-bottom:8px;height:14px;font-size:12px;line-height:12px;width:90%;"  onblur = "this.value=(this.value==\'\') ? \'' + __wps__.sendmail + '\' : this.value;"  onfocus= "this.value=(this.value==\'' + __wps__.sendmail + '\') ? \'\' : this.value;"  value  = "' + __wps__.sendmail + '" />';
				} else {
					html += '<input id="symposium_plus_post" type="text" style="padding-left:18px; background: transparent url(\'' + __wps__.images_url + '/bubble.png\') 2px 2px no-repeat;margin-top:4px;margin-bottom:8px;height:14px;font-size:12px;line-height:12px;width:90%;"  onfocus= "this.value= \'\'"  value  = "' + __wps__.whatsup + '" />';
				}
			} else {
				if (rel == 'pending') {
					var q = symposium_q(__wps__.profile_url);
			...
			if (__wps__.gallery_url != '') {
				html += '<div style="float:left; margin-right:5px;"><img style="cursor:pointer" rel="' + display_name + '" id="symposium_gallery_search" src="' + __wps__.images_url + '/gallery.png" title="' + __wps__.gallerysearch + '" /></a></div>';
			}
		...
		jQuery.ajax({
			...
			success: function(str) {
				...
				jQuery('#menu_gallery_file_upload').uploadify({
					'uploader': __wps__.plugin_url + 'uploadify/uploadify.swf',
					'buttonText': browseforfile,
					'script': __wps__.plugin_url + 'uploadify/upload_menu_gallery.php',
					'cancelImg': __wps__.plugin_url + 'uploadify/cancel.png',
					'multi': true,
					'auto': true,
					'scriptData': {
