	...
	if (jQuery('#symposium_activity_uploader').length) {
	    ...
	    jQuery(function() {
	        ...
	        var uploader = new plupload.Uploader({
				...
				resize : {width : 700, height : 375, quality : 90}, 
...
jQuery(document).ready(function() { 	
	...
	*/
	...
	jQuery('.symposium-dialog').live('click', function () {
		...
		jQuery("#dialog").html(str).dialog({ title: title, width: 800, height: 500, modal: true,
		buttons: {
				"OK": function() {
	...
	*/
	...
	if (jQuery("#symposium_summary_Widget").length) {
		jQuery.ajax({
			...
			success: function(str){
				if (str.substring(0, 4) == 'FAIL') { jQuery("#dialog").html(str).dialog({ title: symposium.site_title+' debug', width: 800, height: 500, modal: true }); } else {
	...
	if (jQuery("#symposium_friends_Widget").length) {
		jQuery.ajax({
			...
			type: "POST", dataType: "html", async: false,
			success: function(str){
				if (str.substring(0, 4) == 'FAIL') { jQuery("#dialog").html(str).dialog({ title: symposium.site_title+' debug info', width: 800, height: 500, modal: true }); } else {
	...
	*/
	...
	if ( (jQuery("#profile_body").length) && (symposium.embed != 'on') ) {
			...
			if (symposium.post != '') { menu_id = 'menu_wall'; }

			if ( (menu_id == 'menu_extended') || 
	      		(menu_id == 'menu_wall') || 
	      		(menu_id == 'menu_activity') || 
	      		(menu_id == 'menu_all') || 
	      		(menu_id == 'menu_mentions') || 
	      		(menu_id == 'menu_groups') || 
	...
	function symposium_add_comment(comment_trigger) {
		...
		if (comment_text != '' && comment_text != '\n' && comment_text != write_a_comment && comment_text != write_a_comment+'\n') {
			...
			jQuery.ajax({
				...
				success: function(str){
					if (str != '') {
						jQuery("#dialog").html(str).dialog({ title: symposium.site_title+' debug info', width: 800, height: 500, modal: true });
					}
				}
	   		});
		} else {
			...
			if (user_email == '') {
				...
				jQuery.ajax({
					...
					success: function(str){
						...
						if (str != "OK") {
							jQuery("#dialog").html(str);
							jQuery("#dialog").dialog({ title: symposium.site_title, width: 300, height: 175, modal: true,
							buttons: {
									"OK": function() {
										jQuery("#dialog").dialog('close');
									}
	...
	*/
	...
	jQuery("#symposium_show_likes").live('click', function() {
		...
		jQuery("#dialog").html(str).dialog({ title: symposium.site_title, width: 600, height: 300, modal: true });
	...
	if (jQuery("input#symposium_member_small").length) {
		...
		.data( "autocomplete" )._renderItem = function( ul, item ) {
			...
			var group = "<a>";
				...
				group += "</div>";
			group += "</a>";
			return jQuery( '<li class="symposium_autocomplete_item"></li>' )
				.data( "item.autocomplete", item )
				.append( group )
				.appendTo( ul );
	...
	jQuery('.symposium-follow').live('mouseenter mouseleave', function(event) {
	  if (event.type == 'mouseenter' && symposium.current_user_id > 0) {
		...
		jQuery('#symposium-follow-box').css({'z-index':999999, 'position':'absolute', 'top':yPos,'left':xPos}).show();
	  }
	});
	...
	*/
	...
	if (jQuery("#symposium-forum-div").length) {
				...
				success: function(str){
					...
					if (str != 'DONTSHOW') {
						...
						jQuery('#forum_file_upload').uploadify({
						    ...
						    'cancelImg' : symposium.plugin_url+'uploadify/cancel.png',
						    'auto'      : true,
							'scriptData' : {'tid':show_tid, 'user_login':user_login, 'user_email':user_email, 'uid':symposium.current_user_id}, 
							'onError' 	: function(event, ID, fileObj, errorObj) {
											 	alert("Error: "+errorObj.type+" "+errorObj.info);
		      							  },
		      				'onComplete': function(event, queueID, fileObj, response, data) { 
						...
						if (symposium.wps_wysiwyg == 'on') {
							if (sub == 'getForum') {
								...
								if (typeof tinyMCE.get("new_topic_text") != 'undefined')
									tinyMCE.get('new_topic_text').setContent('');
							} else {
								tiny_mce_init('symposium_reply_text');							
								if (typeof tinyMCE.get("symposium_reply_text") != 'undefined')
		...
		// Enable file uploading
		if (symposium.show_tid == 0) { jQuery("#symposium_perma_topic_id").html() != '' ? symposium.show_tid = jQuery("#symposium_perma_topic_id").html() : 0; } // If not using AJAX
		var user_login = jQuery("#symposium_user_login").html();
		var user_email = jQuery("#symposium_user_email").html();
		jQuery('#forum_file_upload').uploadify({
		    'uploader'  : symposium.plugin_url+'uploadify/uploadify.swf',
			...
			jQuery.ajax({
				...
				success: function(str){
					...
					jQuery.ajax({
						...
						async: true,
						success: function(str){
							if (str != '') { jQuery("#dialog").html(str).dialog({ title: symposium.site_title+' debug info (send replies)', width: 800, height: 500, modal: true }); }
						},
						error:function (xhr, ajaxOptions, thrownError){
							if (show_js_errors) {
		                    	alert(xhr.status);
					...
					if (symposium.wps_forum_refresh == 'on') { location.reload(); }
				...
				jQuery.ajax({
					...
					success: function(str){
							...
							jQuery.ajax({
								...
								async: true,
								success: function(str){
									if (str != '') { jQuery("#dialog").html(str).dialog({ title: symposium.site_title+' debug info', width: 800, height: 500, modal: true }); }
								},
								error:function (xhr, ajaxOptions, thrownError){
									if (show_js_errors) {
				                    	alert(xhr.status);
...
return str;
	...
	*/
	...
	jQuery("#symposium_pay_event").live('click', function() {
		...
		jQuery.ajax({
			...
			success: function(str){
				jQuery(".symposium_pleasewait").hide();
				jQuery("#dialog").html(str).dialog({ title: symposium.site_title, width: 400, height: 300, modal: true, buttons: { "OK": function() { jQuery("#dialog").dialog('close'); } } });
			}
		});			
	});
	...
	jQuery("#symposium_book_event").live('click', function() {
		...
		jQuery("#dialog").html(str).dialog({ title: symposium.site_title, width: 400, height: 300, modal: true });
	});
	...
	jQuery(".symposium_edit_event").live('click', function() {
		...
		jQuery.ajax({
			...
			success: function(str){
				...
				jQuery("#dialog").dialog({ title: symposium.site_title+' Event ID:'+event_id, width: 850, height: 550, modal: true, buttons: {}, close: function(ev, ui) {
					jQuery(".symposium_pleasewait").inmiddle().show();		
					var q = symposium_q(symposium.profile_url);
	...
	jQuery(".symposium_event_update_button").live('click', function() {
		...
		if (symposium.events_user_places || symposium.current_user_level == 5) {
			...
			var enable_places = '';
			if (jQuery('#symposium_edit_event_enable_places:checked').val() == 'on') { enable_places = 'on'; }
			var show_max = '';
			if (jQuery('#symposium_edit_event_show_max:checked').val() == 'on') { show_max = 'on'; }
			var confirmation = '';
			if (jQuery('#symposium_edit_event_confirmation:checked').val() == 'on') { confirmation = 'on'; }
			var send_email = '';
			if (jQuery('#symposium_edit_event_send_email:checked').val() == 'on') { send_email = 'on'; }
		} else {
			var max_places = 0;
			var cost = 0;
		...
		jQuery.ajax({
			...
			async: true,
			success: function(str){
				if (symposium.debug) { jQuery("#dialog").html(str).dialog({ title: symposium.site_title+' debug info', width: 800, height: 500, modal: true }); }
				jQuery(".symposium_pleasewait").hide();
	...
	*/
	...
	function prepare_colorbox(t) {
		...
		var album_title = jQuery(t).attr("title");
		if (album_title == '' && jQuery(div_title).html() != '') { album_title = jQuery(div_title).html(); }
		...
		jQuery("a[rel='"+rel+"']").each(function(index) {
			if (jQuery.inArray(jQuery(this).attr("href"), list) == -1) {
				max_images++;
		...
		jQuery("#dialog").html("<img src='"+symposium.images_url+"/busy.gif' />");
		if (ids[0] == 0) {
			jQuery("#dialog").dialog({ zIndex: 10000, title: '<a href="" target="_blank" id="show_original">'+show_original+'</a>', width: 650, height: 600, modal: true, buttons: {}, open: function(event, ui){ jQuery('body').css('overflow-x','hidden'); jQuery('.ui-widget-overlay').css('width','100%'); }, close: function(event, ui){ jQuery('body').css('overflow','auto'); }  });
		} else {
			jQuery("#dialog").dialog({ zIndex: 10000, title: album_title, width: 1000, height: 600, modal: true, buttons: {}, open: function(event, ui){ jQuery('body').css('overflow-x','hidden'); jQuery('.ui-widget-overlay').css('width','100%'); }, close: function(event, ui){ jQuery('body').css('overflow','auto'); }  });
		}
	...
	function symposium_show_image(t, url, w, h) {
	    ...
	    if (h < photo_frame_height) { photo_margin_top = (photo_frame_height-h)/2; }
	...
	jQuery("#wps_gallery_comment_more").live('click', function() {
		jQuery(this).hide();
		jQuery(this).parent().find("#wps_comment_plus").css("overflow", "visible");
	});
