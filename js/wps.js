...
jQuery(document).ready(function() {
	...
	if (jQuery('#symposium_current_user_page').length) {
		symposium.current_user_page = jQuery('#symposium_current_user_page').html();
	}
...
/*
	...
	*/
	...
	var symposium_clear = jQuery("#symposium_clear").html();
	var symposium_update = jQuery("#symposium_update").html();
	var symposium_cancel = jQuery("#symposium_cancel").html();
	var write_a_comment = jQuery("#symposium_write_a_comment").html();
	var btn_add = jQuery("#symposium_add").html();
	var show_original = jQuery("#symposium_show_original").html();
	var add_a_comment = jQuery("#symposium_add_a_comment").html();
	...
	var symposium_next = jQuery('#symposium_next').html();
	var symposium_likes = jQuery('#symposium_likes').html();
	var symposium_dislikes = jQuery('#symposium_dislikes').html();
		...
		jQuery.ajax({
			...
			success: function(str) {
				if (str.substring(0, 4) == 'FAIL') {
					jQuery("#dialog").html(str).dialog({
						title: symposium.site_title + ' debug',
						width: 800,
						height: 500,
						modal: true
					});
	...
	if (jQuery("#symposium_friends_Widget").length) {
		jQuery.ajax({
			...
			success: function(str) {
				if (str.substring(0, 4) == 'FAIL') {
					jQuery("#dialog").html(str).dialog({
						title: symposium.site_title + ' debug info',
						width: 800,
						height: 500,
						modal: true
					});
	...
	if (jQuery("input#symposium_member").length) {
		jQuery("input#symposium_member").autocomplete({
			...
			select: function(event, ui) {
				...
				var q = symposium_q(symposium.profile_url);
				window.location.href = symposium.profile_url + q + 'uid=' + ui.item.value;
				return false;
			}
	...
	*/
	...
	if ((jQuery("#profile_body").length) && (symposium.embed != 'on')) {
		...
		if ((menu_id == 'menu_extended') || (menu_id == 'menu_wall') || (menu_id == 'menu_activity') || (menu_id == 'menu_all') || (menu_id == 'menu_mentions') || (menu_id == 'menu_groups') || (menu_id == 'menu_friends') || (menu_id == 'menu_avatar') || (menu_id == 'menu_personal') || (menu_id == 'menu_settings')) {
			...
			if ((ajax_part == 'gallery') || (ajax_part == 'mentions') || (ajax_part == 'plus') || (ajax_part == 'events') || (ajax_part == 'lounge')) {
		...
		if (menu_id == 'menu_') {
			menu_id = 'menu_wall';
			ajax_path = symposium.plugin_url + "ajax/symposium_profile_functions.php";
		}
			...
			success: function(str) {
				...
				jQuery('#profile_file_upload').uploadify({
					...
					'scriptData': {
						...
						'user_id': user_id,
						'page': symposium.current_user_page
					},
	...
	jQuery("#setup_facebook").live('click', function() {
		...
		jQuery("#dialog").dialog({
			title: 'Please enter your Facebook ID',
			width: 420,
			height: 200,
			modal: true,
			buttons: {}
	...
	jQuery("#cancel_facebook").live('click', function() {
		...
		jQuery.ajax({
			...
			success: function(str) {
				...
				jQuery("#dialog").dialog({
					title: 'Facebook Connect Removed',
					width: 400,
					height: 220,
					modal: true,
					buttons: {
	...
	jQuery(".symposium_profile_menu").click(function() {
		...
		if (!(jQuery("#profile_body").length)) {
			...
			var q = symposium_q(symposium.profile_url);
			window.location.href = symposium.profile_url + q + 'view=' + view;
		}
		...
		jQuery.ajax({
			...
			data: ({
				...
				menu_slug:menu_slug,
				uid1: symposium.current_user_page,
				display_name: symposium.current_user_display_name
			}),
			dataType: "html",
			async: true,
			success: function(str) {
	...
	jQuery('#symposium_comment').live('keypress', function(e) {
		if (e.keyCode == 13 && (!jQuery("#symposium_add_comment").length)) {
			symposium_add_comment_to_profile();
		}
	});
	...
	function symposium_add_comment_to_profile() {
		...
		if (comment_text != '') {
			...
			setTimeout(function() {
				...
				jQuery('#symposium_filelist').html("").show();
				jQuery('#symposium_add_status').slideDown('fast');
				jQuery('#symposium_activity_uploader').slideDown('fast');
			}, 3000);
			...
			var q = symposium_q(symposium.profile_url);
			comment = comment + '<a href="' + symposium.profile_url + q + 'uid=' + symposium.current_user_id + '">';
			comment = comment + symposium.current_user_display_name + '</a><br />';
			comment = comment + "<div id='myimage'></div>";
			...
			jQuery('#symposium_filelist').hide();
			jQuery('#symposium_old_upload').hide();

			var facebook_post = 0;
			if (jQuery("#post_to_facebook").length) {
				if (jQuery("#post_to_facebook").is(":checked")) {
					if (symposium.current_user_page == symposium.current_user_id) {
						facebook_post = 1;
					}
				}
			}
			...
			jQuery.ajax({
				...
				async: true,
				success: function(str) {
					jQuery('#myimage:first').html('<img src="' + str + '" width="100%" />');
					jQuery(".symposium_notice").fadeOut("slow");

					// Having displayed the tmp attachment, now clean up
					if (str != '') {
						jQuery.ajax({
							url: symposium.plugin_url + "ajax/symposium_profile_functions.php",
							type: "POST",
							data: ({
								action: "deleteTmpAttachments"
							}),
							dataType: "html",
							async: true,
							success: function(str) {}
	...
	function symposium_add_comment(comment_trigger) {
		...
		if (comment_text != '' && comment_text != '\n' && comment_text != write_a_comment && comment_text != write_a_comment + '\n') {
			...
			if (symposium.bg_color_2 != '') {
				comment = comment + " style='background-color:" + symposium.bg_color_2 + "'";
			}
			comment = comment + ">";
	...
	jQuery("#profile_send_mail_button").live('mousedown', function() {
		var q = symposium_q(symposium.mail_url);
		document.location = symposium.mail_url + q + 'view=compose&to=' + symposium.current_user_page;
	});
...
/*
	...
	*/
	...
	if (symposium.profile_menu_scrolls == 'on') {
		var el = jQuery('#profile_menu');
		if (el.length) {
			var elpos = el.offset().top;
			var t = el.position().top;
			var d = parseInt(symposium.profile_menu_delta);
			var a = parseInt(symposium.profile_menu_adjust);
			jQuery('.symposium-wrapper').addClass('symposium-wrapper-min-height');

			jQuery(window).scroll(function() {
				var y = jQuery(this).scrollTop();
				el.css('top', d).css('margin-left', '0');
				if (y > (elpos - d)) {
	...
	jQuery(".wall_add_like").live('click', function() {
		...
		jQuery("#symposium_likes_" + cid).html('<strong>' + jQuery(this).attr('title') + '</strong>').effect("slide");
		jQuery(this).parent().hide();
		...
		jQuery.ajax({
			...
			async: true,
			success: function(str) {
				if (str != 'OK') jQuery("#dialog").html(str).dialog({ title: symposium.site_title+' debug info', width: 800, height: 500, modal: true });
			}
		});
	})
	...
	jQuery("#symposium_show_likes").live('click', function() {
		var cid = jQuery(this).attr('rel');
		var str = '';
		var busy = "<img id='symposium_tmp' src='" + symposium.images_url + "/busy.gif' />";
		str += '<div id="symposium_who_dislikes" style="width:49%;float:right;"><strong>' + symposium_dislikes + '</strong><br /><br />' + busy + '</div>';
		str += '<div id="symposium_who_likes" style="width:49%;"><strong>' + symposium_likes + '</strong><br /><br />' + busy + '</div>';
		jQuery("#dialog").html(str).dialog({
			title: symposium.site_title,
			width: 600,
			height: 300,
		...
		jQuery.ajax({
			...
			success: function(str) {
				if (str != 'None') {
					...
					jQuery('#symposium_who_likes').html(likes);
				} else {
					jQuery('#symposium_who_likes').html('<strong>' + symposium_likes + '</strong>');
				}
			}
		});
		jQuery.ajax({
			...
			success: function(str) {
				if (str != 'None') {
					...
					jQuery('#symposium_who_dislikes').html(dislikes);
				} else {
					jQuery('#symposium_who_dislikes').html('<strong>' + symposium_dislikes + '</strong>');
				}
			}
		});
	})
	...
	if (jQuery("input#symposium_member_small").length) {
		...
		jQuery("input#symposium_member_small").autocomplete({
			source: symposium.plugin_url + "ajax/symposium_plus_functions.php",
			minLength: 1,
			focus: function(event, ui) {
				jQuery("input#symposium_member_small").val(ui.item.name);
				return false;
			},
			select: function(event, ui) {
				jQuery(".symposium_pleasewait").inmiddle().show().delay(3000).fadeOut("slow");
				switch (ui.item.type) {
				case 'topic':
					var q = symposium_q(symposium.forum_url);
					window.location.href = symposium.forum_url + q + 'show=' + ui.item.id;
					break;
				case 'post':
				...
				case 'amember':
					...
					break;
				case 'group':
					var q = symposium_q(symposium.group_url);
					window.location.href = symposium.group_url + q + 'gid=' + ui.item.value;
					break;
				case 'gallery':
					var q = symposium_q(symposium.profile_url);
					window.location.href = symposium.profile_url + q + 'uid=' + ui.item.owner + '&embed=on&album_id=' + ui.item.id;
					break;
				}
				return false;
			}
			...
			if (item.type == 'sep') {
				...
				return jQuery('<li class="symposium_autocomplete_sep"></li>').append(item.name).appendTo(ul);
				...
				group += "</div>";
				group += "</a>";
				return jQuery('<li class="symposium_autocomplete_item"></li>').data("item.autocomplete", item).append(group).appendTo(ul);
	...
	jQuery('.symposium-follow').live('mouseenter mouseleave', function(event) {
		if (event.type == 'mouseenter' && symposium.current_user_id > 0) {
			...
			var q = symposium_q(symposium.profile_url);
			html += '<a style="font-size:14px;" href="' + symposium.profile_url + q + 'uid=' + id + '">' + display_name + '</a><br />';
			if (rel == 'friend' || id == symposium.current_user_id) {
				...
				if (rel == 'pending') {
					var q = symposium_q(symposium.profile_url);
					html += '<div style="margin-top:4px;margin-bottom:8px;"><a href="' + symposium.profile_url + q + 'view=friends">' + jQuery('#symposium_friendpending').html() + '</a></div>';
				} else {
			...
			jQuery('#symposium-follow-box').css({
				'z-index': 999999,
				'position': 'absolute',
				'top': yPos,
				'left': xPos
	...
	jQuery("#symposium_plus_friends").live('click', function() {
		jQuery(".symposium_pleasewait").inmiddle().show();
		var q = symposium_q(symposium.profile_url);
		window.location = symposium.profile_url + q + 'uid=' + jQuery(this).attr("ref") + '&view=friends';
	});
	...
	jQuery("#symposium_plus_profile").live('click', function() {
		jQuery(".symposium_pleasewait").inmiddle().show();
		var q = symposium_q(symposium.profile_url);
		window.location = symposium.profile_url + q + 'uid=' + jQuery(this).attr("ref") + '&view=extended';
	});
	...
	jQuery("#symposium_following_who").live('click', function() {
		jQuery(".symposium_pleasewait").inmiddle().show();
		var q = symposium_q(symposium.profile_url);
		window.location = symposium.profile_url + q + 'view=plus';
	});
	...
	jQuery("#symposium_gallery_search").live('click', function() {
		jQuery(".symposium_pleasewait").inmiddle().show();
		var q = symposium_q(symposium.gallery_url);
		window.location = symposium.gallery_url + q + 'term=' + jQuery(this).attr("rel");
	});
		...
		if (e.keyCode == 13) {
			...
			jQuery.ajax({
				...
				success: function(str) {
					...
					jQuery("#dialog").dialog({
						title: jQuery('#symposium_privatemailsent').html(),
						width: 300,
						height: 250,
						modal: true,
						buttons: {
...
/*
	...
	*/
	...
	if (jQuery("#symposium-forum-div").length) {
		...
		if (jQuery("#symposium-forum-div").html() == '') {
			...
			if (jQuery("#symposium_perma_topic_id").length) {
				symposium.show_tid = jQuery("#symposium_perma_topic_id").html();
				if (jQuery("#symposium_perma_cat_id").length) {
					symposium.cat_id = jQuery("#symposium_perma_cat_id").html();
				}
				if (symposium.show_tid != '') {
					sub = "getTopic";
				} else {
					sub = "getForum";
				}
			}
			var show_tid = symposium.show_tid;
					...
					if (str != 'DONTSHOW') {
						...
						if (symposium.wps_wysiwyg == 'on') {
							if (sub == 'getForum') {
								...
								if (typeof tinyMCE.get("symposium_reply_text") != 'undefined') tinyMCE.get('symposium_reply_text').setContent('');
							}
						}
		...
		// Enable file uploading
		if (symposium.show_tid == 0) {
			jQuery("#symposium_perma_topic_id").html() != '' ? symposium.show_tid = jQuery("#symposium_perma_topic_id").html() : 0;
		} // If not using AJAX
		if (!symposium.show_tid && symposium.show_tid != 0) symposium.show_tid = 0;
		var user_login = jQuery("#symposium_user_login").html();
		var user_email = jQuery("#symposium_user_email").html();
		jQuery('#forum_file_upload').uploadify({
			'uploader': symposium.plugin_url + 'uploadify/uploadify.swf',
			...
			success: function(str) {
				...
				jQuery.ajax({
					...
					success: function(str) {
						if (str != '') {
							jQuery("#dialog").html(str).dialog({
								title: symposium.site_title + ' debug info (send replies)',
								width: 800,
								height: 500,
								modal: true
							});
	...
	jQuery("#fav_link").live('click', function() {
		...
		if (jQuery('#fav_link').attr('src') == symposium.images_url + '/fav-on.png') {
			jQuery('#fav_link').attr({
				src: symposium.images_url + '/fav-off.png'
			});
		} else {
			jQuery('#fav_link').attr({
				src: symposium.images_url + '/fav-on.png'
			});
		}
		jQuery.ajax({
			url: symposium.plugin_url + "ajax/symposium_forum_functions.php",
				...
				success: function(str) {
					...
					jQuery.ajax({
						...
						success: function(str) {
							...
							if (symposium.wps_forum_refresh == 'on' && symposium.debug != 'on') {
								location.reload();
							}
		...
		if ((symposium.bar_polling > 0) && (symposium.chat_polling > 0)) {
			...
			if (jQuery("#symposium-friends-box").css("display") != "none") {
				...
				jQuery("#symposium-friends-box").click(function() {
					var q = symposium_q(symposium.profile_url);
					window.location.href = symposium.profile_url + q + 'view=friends';
				});
			...
			if (symposium.wps_lite != 'on') {
				...
				jQuery("#symposium-chatroom_big").live('click', function() {
					...
					if (hc < 50) {
						...
						jQuery('#symposium-chatroom-box').removeClass('symposium-chatroom-new').addClass('symposium-chatroom-none');
						jQuery('#chatroom_messages').attr({
							scrollTop: jQuery('#chatroom_messages').attr('scrollHeight')
						});
						var objDiv = document.getElementById('chatroom_messages');
						objDiv.scrollTop = objDiv.scrollHeight;
						createCookie('wps_chatroom', 'show', 7);
				...
				if (symposium.use_chat == 'on') {
					...
					jQuery(".symposium_online_name").live('click', function() {
						...
						for (w = 1; w <= numChatWindows; w++) {
							if ((already_chatting == 0) && (jQuery('#chat' + w + '_to').html() == jQuery(this).attr("title"))) {
								already_chatting = w;
							}
						}
...
/*
	...
	*/
	...
	jQuery("#reset_mail_message").click(function() {
		if (confirm(areyousure)) {
			var reset = "<div id='message_header'><div id='message_header_delete'>[reply_button][delete_button]</div><div id='message_header_avatar'>[avatar,44]</div>[mail_subject]<br />[mail_recipient] [mail_sent]</div></div><div id='message_mail_message'>[message]</div>";
			reset = reset.replace(/\[\]/g, String.fromCharCode(13));
				...
				success: function(str) {
					if (str != '') {
						...
						for (i = 0; i < num_rows; i++) {
							...
							var chat_win = 0;
							for (w = 1; w <= numChatWindows; w++) {
								if (jQuery('#chat' + w + '_to').html() == other) {
									chat_win = w;
								}
		...
		A.extend(A.fn, {
			pstrength: function(B) {
				var B = A.extend({
					...
					scores: [10, 15, 30, 40],
					common: ["password", "sex", "god", "123456", "123", "welcome", "test", "qwerty", "admin"],
					minchar: 6
				}, B);
	...
	*/
	...
	if (jQuery("#symposium_alerts").length) {
		...
		jQuery('#symposium_clear_news').live('click', function(event) {
			jQuery(".symposium_news_item").remove();
			jQuery("#symposium_news_highlight").remove();
			jQuery.ajax({
				url: symposium.plugin_url + "ajax/symposium_news_functions.php",
				type: "POST",
...
/*
	...
	*/
	...
	jQuery("#symposium_cancel_event").live('click', function() {
		...
		if (answer) {
			jQuery(this).hide();
			jQuery('#symposium_pay_event').hide();
			jQuery(".symposium_pleasewait").inmiddle().show();
			jQuery.ajax({
				url: symposium.plugin_url + "ajax/symposium_events_functions.php",
				type: "POST",
	...
	jQuery("#symposium_book_event").live('click', function() {
		...
		// dialog from symposium_events.php
		var events_howmany = jQuery('#symposium_events_howmany').html();
		...
		str += '</select><br /><br />';
		str += '<input id="symposium_book_event_next" data-eid="' + jQuery(this).data('eid') + '" type="submit" class="symposium-button" value="' + symposium_next + ' &gt;" />';
		jQuery("#dialog").html(str).dialog({
			title: symposium.site_title,
			width: 400,
			height: 300,
				...
				jQuery.ajax({
					...
					success: function(str) {
						if (str == 'OK') {
							jQuery(".symposium_pleasewait").inmiddle().show();
							var q = symposium_q(symposium.profile_url);
							var reload_page = symposium.profile_url + q + "uid=" + symposium.current_user_page + "&embed=on&view=wps_events";
	...
	jQuery(".symposium_edit_event").live('click', function() {
		...
		// Get translated strings
		var events_max_places = jQuery("#symposium_events_max_places").html();
		var events_enable_places = jQuery("#symposium_events_enable_places").html();
		var events_show_max = jQuery("#symposium_events_show_max").html();
		var events_confirmation = jQuery("#symposium_events_confirmation").html();
		var events_tickets_per_booking = jQuery("#symposium_events_tickets_per_booking").html();
		var events_tab_1 = jQuery("#symposium_events_tab_1").html();
		var events_tab_2 = jQuery("#symposium_events_tab_2").html();
		var events_tab_3 = jQuery("#symposium_events_tab_3").html();
		var events_tab_4 = jQuery("#symposium_events_tab_4").html();
		var events_send_email = jQuery("#symposium_events_send_email").html();
		var events_pay_link = jQuery('#symposium_events_pay_link').html();
		var events_cost = jQuery('#symposium_events_cost').html();
		var events_replacements = jQuery('#symposium_events_replacements').html();
		var events_labels = jQuery('#symposium_events_labels').html().split('|');
		...
		jQuery.ajax({
			...
			success: function(str) {
				...
				jQuery.each(rows, function(i, row) {
					...
					html += '</li>';
					if (symposium.events_user_places || symposium.current_user_level == 5) {
						html += '<li>';
					...
					if (symposium.events_user_places || symposium.current_user_level == 5) {
						...
						html += "<div style='clear:both'>" + events_enable_places + " <input type='checkbox' style='float:right' id='symposium_edit_event_enable_places'";
						if (row.enable_places) {
							html += ' CHECKED';
						}
						html += "></div>";
						html += "<div style='clear:both;'>" + events_show_max + " <input type='checkbox' style='float:right' id='symposium_edit_event_show_max'";
						if (row.show_max) {
							html += ' CHECKED';
						}
						html += "></div>";
						html += "<div style='clear:both;'>" + events_confirmation + " <input type='checkbox' style='float:right' id='symposium_edit_event_confirmation'";
						if (row.confirmation) {
							html += ' CHECKED';
						}
						html += "></div>";
						html += "<div style='clear:both;'>" + events_send_email + " <input type='checkbox' style='float:right' id='symposium_edit_event_send_email'";
						if (row.send_email) {
							html += ' CHECKED';
						}
						html += "></div>";
						html += "<div style='clear:both;'>" + events_max_places + " <input type='text' id='symposium_edit_event_max_places' style='float:right; width:30px' value='" + row.max_places + "'></div>";
						html += "<div style='clear:both;'>" + events_tickets_per_booking + " <input type='text' id='symposium_edit_event_tickets_per_booking' style='float:right; width:30px' value='" + row.tickets_per_booking + "'></div>";
						html += "<div style='clear:both;'>" + events_cost + " <input type='text' id='symposium_edit_event_cost' style='float:right; width:30px' value='" + row.cost + "'></div>";
						html += "<div style='clear:both; '>" + events_pay_link + " <a href='javascript:void()' id='symposium_event_help'><img src='" + symposium.images_url + "/info.png' /></a><a style='display:none' href='javascript:void()' id='symposium_event_help_close'><img src='" + symposium.images_url + "/edit.png' /></a><br />";
						...
						html += '##unitcost##';
						html += '</div>';
						html += "<textarea id='symposium_edit_event_pay_link' style='font-size:11px;font-family:courier new;letter-spacing:-1px;width:370px; height:102px;'>" + row.pay_link + "</textarea>";
						html += "</div>";
						html += '</div>';
					}
					...
					html += '<input type="text" id="symposium_edit_event_start" style="width:100px" class="datepicker" value="';
					...
					html += '</select>';
					html += ' &rarr; ';
					html += '<input type="text" id="symposium_edit_event_end" style="width:100px" class="datepicker" value="';
					...
					html += '<div id="tabs-2">';
					if (symposium.events_use_wysiwyg) {
					...
					if (symposium.events_user_places || symposium.current_user_level == 5) {
						...
						jQuery.each(attendees, function(i, attendee) {
							...
							var q = symposium_q(symposium.profile_url);
							var url = symposium.profile_url + q + "uid=" + attendee.uid;
							...
							html += '<td>';
							html += '<a href="mailto:' + attendee.email + '"><img src="' + symposium.images_url + '/orange-tick.gif" /></a>&nbsp;&nbsp;';
							html += '<img id="symposium_events_resend_email" data-bid="' + attendee.bid + '" style="cursor:pointer" src="' + symposium.images_url + '/update.png" />&nbsp;&nbsp;';
							html += '<img id="symposium_events_remove_attendee" data-bid="' + attendee.bid + '" style="cursor:pointer" src="' + symposium.images_url + '/delete.png" />&nbsp;&nbsp;';
							if (!attendee.payment_processed) {
								html += '<img id="symposium_events_payment_processed" data-bid="' + attendee.bid + '" style="cursor:pointer" src="' + symposium.images_url + '/cash.png" />&nbsp;&nbsp;';
							}
							if (!attendee.confirmed) {
								html += '<img id="symposium_events_confirm_attendee" data-bid="' + attendee.bid + '" style="cursor:pointer; width:15px;height:15px" src="' + symposium.images_url + '/tick.png" />&nbsp;&nbsp;';
							}
							html += '</td>';
							html += '</tr>';
						...
						html += '<tr><td colspan="7" align="right"><br /><strong>' + events_labels[6] + ':</strong> ';
						html += '<img src="' + symposium.images_url + '/orange-tick.gif" /> ' + events_labels[8] + '&nbsp;&nbsp;&nbsp;';
						html += '<img src="' + symposium.images_url + '/update.png" /> ' + events_labels[9] + '&nbsp;&nbsp;&nbsp;';
						html += '<img src="' + symposium.images_url + '/delete.png" /> ' + events_labels[10] + '&nbsp;&nbsp;&nbsp;';
						html += '<img src="' + symposium.images_url + '/cash.png" /> ' + events_labels[11] + '&nbsp;&nbsp;&nbsp;';
						html += '<img style="width:15px;height:15px" src="' + symposium.images_url + '/tick.png" /> ' + events_labels[7] + '&nbsp;&nbsp;&nbsp;';
						html += '</td></tr>';
						html += '</table>';
						html += '</div>';
					...
					html += '<input type="submit" class="symposium_event_update_button symposium-button" value="' + symposium_update + '" />';
				...
				jQuery("#dialog").dialog({
					title: symposium.site_title + ' Event ID:' + event_id,
					width: 850,
					height: 550,
					modal: true,
					buttons: {},
					close: function(ev, ui) {
						jQuery(".symposium_pleasewait").inmiddle().show();
						var q = symposium_q(symposium.profile_url);
						var reload = symposium.profile_url + q + "uid=" + event_owner + "&embed=on&view=wps_events";
	...
	jQuery("#symposium_event_help").live('click', function() {
		jQuery('#symposium_edit_event_pay_link').hide();
	...
	jQuery("#symposium_event_help_close").live('click', function() {
		jQuery('#symposium_edit_event_pay_link').show();
	...
	jQuery(".symposium_event_update_button").live('click', function() {
		...
		if (symposium.events_user_places || symposium.current_user_level == 5) {
			...
			var max_places = jQuery('#symposium_edit_event_max_places').val();
			var cost = jQuery('#symposium_edit_event_cost').val();
			var pay_link = jQuery('#symposium_edit_event_pay_link').val();
			var tickets_per_booking = jQuery('#symposium_edit_event_tickets_per_booking').val();
			var enable_places = '';
			if (jQuery('#symposium_edit_event_enable_places:checked').val() == 'on') {
				enable_places = 'on';
			}
			var show_max = '';
			if (jQuery('#symposium_edit_event_show_max:checked').val() == 'on') {
				show_max = 'on';
			}
			var confirmation = '';
			if (jQuery('#symposium_edit_event_confirmation:checked').val() == 'on') {
				confirmation = 'on';
			}
			var send_email = '';
			if (jQuery('#symposium_edit_event_send_email:checked').val() == 'on') {
				send_email = 'on';
			}
		} else {
			var max_places = 0;
	...
	*/
	...
	if (jQuery("input#gallery_member").length) {
		jQuery("input#gallery_member").autocomplete({
			source: symposium.plugin_url + "ajax/symposium_gallery_functions.php",
			minLength: 3,
			focus: function(event, ui) {
	...
	function prepare_colorbox(t) {
		...
		// Build HTML			
		var html = '';
		html += '<div id="symposium_photo_gallery">';
		...
		html += '<input id="symposium_album_title" style="display:none" value="' + album_title + '" />';

		if (ids[0] != 0) {
			html += '<div id="symposium_photo_comments">';

			// Management
			if (owner == symposium.current_user_id && album_title != '') {
				html += '<div id="symposium_photo_management" style="margin-bottom:10px">';
				var gallery_labels = jQuery('#symposium_gallery_labels').html().split('|');

				html += '<div id="symposium_rename_photo" style="margin-bottom:5px"><input type="text" id="symposium_rename" value="" /><input type="submit" id="symposium_rename_button" class="symposium-button" value="' + gallery_labels[0] + '" /></div>';
				html += '<div id="symposium_rename_photo_confirm" style="margin-bottom:11px;display:none;">' + gallery_labels[1] + '</div>';
				html += '<div id="symposium_photo_order" style="clear:both">' + gallery_labels[2] + ' <a id="symposium_photo_order_save" style="text-decoration:underline" href="javascript:void(0);">' + gallery_labels[3] + '</a>.' + '</div>';
				html += '<div class="symposium_photo_delete" style="cursor:pointer; text-decoration:underline;">' + gallery_labels[4] + '</div>';
				html += '<div class="symposium_photo_select_cover_button" style="cursor:pointer; text-decoration:underline;">' + gallery_labels[5] + '</div>';
				html += '</div>';
			}

			// Actions
			if (symposium.current_user_id > 0 && album_title != '') {
				html += '<div id="symposium_photo_actions">';
			...
			if (symposium.current_user_id > 0 && album_title != '') {
				html += add_a_comment;
				html += '<textarea id="symposium_photo_add_comment" class="elastic"></textarea>';
				html += '<input id="symposium_photo_submit_comment" type="button" class="symposium-button" value="' + btn_add + '" />';
			}
			if (album_title != '') {
				html += '<div id="symposium_photo_comments_container"></div>';
			}

			html += '</div>';
		}

		html += '<div id="symposium_photo_container">';

		// Photo frame
		html += '<div id="symposium_photo_frame">';
		html += '</div>';
		...
		for (var i = 0; i < list.length; i++) {
			...
			var name = names[i];
			html += '<img id="thumb_' + i + '" title="' + name + '" rel="' + ids[i] + '" class="symposium_photo_thumbnail symposium_photo_thumbnail_border_off " src="' + url + '" />';
		}
		html += '</div>';
		...
		// Sortable thumbnails
		if (owner == symposium.current_user_id && album_title != '') {
			jQuery('#symposium_photo_thumbnails').sortable();
			jQuery('#symposium_photo_order_save').live('click', function() {
				var order = '';
				jQuery('#symposium_photo_thumbnails').children(jQuery('.symposium_photo_thumbnails')).each(function(i) {
					order += jQuery(this).attr('rel') + ',';
				});

				jQuery("#symposium_photo_order").html("<img src='" + symposium.images_url + "/busy.gif' />");
				jQuery.ajax({
					url: symposium.plugin_url + "ajax/symposium_gallery_functions.php",
					type: "POST",
					data: ({
					...
					success: function(str) {
						if (str != 'OK') {
							jQuery("#symposium_photo_order").html(str).effect("highlight", {}, 3000);
						}
					}
				});
			});
		...
		if (symposium.use_elastic == 'on' && jQuery(".elastic").length) {
			jQuery('.elastic').elastic();
		}
	...
	jQuery("#symposium_photo_submit_comment").live('click', function() {
		var comment = jQuery('#symposium_photo_add_comment').val();
		jQuery('#symposium_photo_add_comment').val('');
		jQuery('#symposium_photo_add_comment').css('height', '40px');

		if (comment != '') {

			var current = parseInt(jQuery("#symposium_current_image").val());
			var id = jQuery("#thumb_" + current).attr("rel");
			jQuery('#symposium_photo_comments_container').html("<img src='" + symposium.images_url + "/busy.gif' />");
	...
	jQuery(".symposium_photo_thumbnail").live('click', function() {
	...
	jQuery(".symposium_photo").live('click', function() {
		var current = parseInt(jQuery("#symposium_current_image").val());
	...
	jQuery("body").keydown(function(e) {
		...
		if (!jQuery('#symposium_photo_add_comment').is(':focus')) {

			if ((e.keyCode == 39 || e.keyCode == 13) && jQuery("#symposium_photo_gallery").length) {
				var current = parseInt(jQuery("#symposium_current_image").val());
				var max = parseInt(jQuery("#symposium_max_images").val());
				if (current < max) {
					current++;
			...
			if (e.keyCode == 37 && jQuery("#symposium_photo_gallery").length) {
				var current = parseInt(jQuery("#symposium_current_image").val());
				var max = parseInt(jQuery("#symposium_max_images").val());
				if (current > 0) {
					current--;
	...
	function display_gallery_item(t) {
		...
		// Show waiting...
		var middle_of_photo_frame_height = (jQuery("#symposium_photo_frame").height() / 2);
		jQuery("#symposium_photo_frame").html("<img style='margin-top:" + middle_of_photo_frame_height + "px' src='" + symposium.images_url + "/busy.gif' />");
	...
	function symposium_show_image(t, url, w, h) {
		...
		var photo_frame_width = jQuery("#symposium_photo_frame").width();
		var photo_frame_height = jQuery("#symposium_photo_frame").height();
		...
		var show_image = '<img class="symposium_photo" width=0 height=0 style="margin-top:' + photo_margin_top + 'px; width:' + w + 'px; height:' + h + 'px;" src="' + url + '" />';

		jQuery("#symposium_current_image").val(jQuery(t).attr("id").replace('thumb_', ''));
		jQuery("#symposium_photo_frame").html(show_image);

		jQuery(".symposium_photo_thumbnail").addClass("symposium_photo_thumbnail_border_off").removeClass("symposium_photo_thumbnail_border_on");
		jQuery(t).toggleClass("symposium_photo_thumbnail_border_off").toggleClass("symposium_photo_thumbnail_border_on");

		// Photo title
		var album_title = jQuery('#symposium_album_title').val();
		...
		jQuery('#symposium_rename').val(current_name);

		// Actions
		jQuery("#show_original").attr("href", url.replace('show_', ''));

		symposium_show_photo_comments(t);
		jQuery('.symposium_photo_delete').show();
		jQuery('.symposium_photo_select_cover_button').show();
		jQuery('#symposium_rename_photo').show();
		jQuery('#symposium_rename_photo_confirm').hide();
		...
		jQuery.ajax({
			...
			success: function(str) {
				if (str != '[]') {
					...
					var rows = jQuery.parseJSON(str);
					jQuery.each(rows, function(i, row) {
						html += '<div class="symposium_photo_comment">';
						html += '<div class="symposium_photo_comment_avatar">';
						html += row.avatar;
						html += '</div>';
						html += '<div>';
				...
				jQuery('#symposium_photo_comments_container').html(html);
	...
	jQuery(".symposium_photo_select_cover_button").live('click', function() {
		...
		jQuery('#symposium_photo_thumbnails').children(jQuery('.symposium_photo_thumbnails')).each(function(i) {
			if (cnt == current) {
				...
				jQuery(t).html("<img src='" + symposium.images_url + "/busy.gif' />");
	...
	jQuery(".symposium_photo_delete_all").live('click', function() {
		...
		if (confirm("Are you sure?")) {
			...
			symposium.album_id = jQuery(this).attr("rel");
			...
			jQuery.ajax({
				...
				success: function(str) {
					...
					if (str != 'OK') {
						...
						var q = symposium_q(symposium.profile_url);
						var reload_page = symposium.profile_url + q + "uid=" + symposium.current_user_page + "&view=gallery";
						window.location.href = reload_page;
					}
				}
			});
		...
		jQuery('#symposium_photo_thumbnails').children(jQuery('.symposium_photo_thumbnails')).each(function(i) {
			if (cnt == current) {
				if (confirm("Are you sure?")) {
					...
					var item_id = jQuery(this).attr("rel");
					jQuery(this).remove();
					jQuery("#symposium_photo_frame").children().fadeOut('slow');
	...
	// Rename photo
	jQuery("#symposium_rename_button").live('click', function() {

		var current = jQuery("#symposium_current_image").val();
		var t = this;

		jQuery("#symposium_rename_photo").hide();
		jQuery("#symposium_rename_photo_confirm").show().effect("highlight", {}, 3000);
		...
		jQuery('#symposium_photo_thumbnails').children(jQuery('.symposium_photo_thumbnails')).each(function(i) {
			if (cnt == current) {
				...
				// Delete this thumbnail and image
				var item_id = jQuery(this).attr("rel");
				var new_name = jQuery("#symposium_rename").val();
				jQuery('#thumb_' + current).attr('title', new_name);
				// Rename image on the page
				jQuery('a[data-iid="' + item_id + '"]').attr('data-name', new_name);
				// Update dialog title
				jQuery("#dialog").dialog('option', 'title', jQuery('#symposium_album_title').val() + ' - ' + new_name);
	...
	jQuery(".symposium_new_album_button").live('click', function() {
		...
		jQuery('.symposium_photo_delete_all').hide();
	...
	jQuery("#symposium_cancel_album").live('click', function() {
		...
		jQuery('.symposium_photo_delete_all').show();
	...
	jQuery("#symposium_new_album").live('click', function() {
		...
		jQuery.ajax({
			...
			data: ({
				...
				sub_album: jQuery("#symposium_create_sub_gallery_select").is(":checked"),
				parent: jQuery("#symposium_create_sub_gallery_select").attr("title")
			}),
			dataType: "html",
			success: function(str) {
				var q = symposium_q(symposium.profile_url);
				var reload_page = symposium.profile_url + q + "uid=" + symposium.current_user_page + "&embed=on&album_id=" + str;
				window.location.href = reload_page;
			}
		});
	});
			...
			jQuery.ajax({
				...
				success: function(str) {
					if (str != 'OK') {
						...
						var q = symposium_q(symposium.profile_url);
						var reload_page = symposium.profile_url + q + "uid=" + symposium.current_user_page;
						window.location.href = reload_page;
					}
				}
...
/*
	...
	*/
	...
	jQuery("#updateGroupSettingsButton").live('click', function() {
		...
		if (!complete) {
			jQuery("#dialog").html('Please complete group name and description').dialog({ title: symposium.site_title, width: 300, height: 200, modal: true, buttons: { "OK": function() { jQuery(this).dialog("close"); } } });
		} else {
	...
	jQuery("#create_group_button").click(function() {
		...
		if (name_of_group != '') {
			...
			jQuery.ajax({
				...
				success: function(str) {
					var q = symposium_q(symposium.group_url);
					window.location.href = symposium.group_url + q + 'gid=' + trim(str);
				},
				error: function(xhr, ajaxOptions, thrownError) {
					if (show_js_errors) {
						alert(xhr.status);
	...
	jQuery('#group_list_delete').live('click', function() {
		...
		if (answer) {
			...
			jQuery.ajax({
				...
				async: false,
				success: function(str) {
					window.location.href = 'admin.php?page=wp-symposium/symposium_groups_admin.php';
				}
			});
...
function tiny_mce_init(editor_id) {
	...
	if (!symposium.use_wp_editor) {
		...
		if (symposium.wps_wysiwyg == 'on' || editor_id == 'edit_event_text' || editor_id == 'edit_event_more') {
			...
			var my_width = symposium.wps_wysiwyg_width;
			var my_height = symposium.wps_wysiwyg_height;
			var toolbar1 = symposium.wps_wysiwyg_1;
			var toolbar2 = symposium.wps_wysiwyg_2;
			var toolbar3 = symposium.wps_wysiwyg_3;
			var toolbar4 = symposium.wps_wysiwyg_4;
			if (editor_id == 'edit_event_text' || editor_id == 'edit_event_more') {
				my_width = '565px';
				my_height = '355px';
				...
				if (symposium.debug) toolbar1 += ',code';
				toolbar2 = 'cut,copy,paste,pastetext,pasteword,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,image,|,forecolor,backcolor';
			...
			if (symposium.include_context == 'on') {
			...
			tinyMCE.init({
				// General options
				theme: "advanced",
				plugins: "fullscreen,safari,pagebreak,table,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace," + include_context + "paste,directionality,visualchars,nonbreaking",
				mode: "exact",
				elements: editor_id,
				relative_urls: false,
