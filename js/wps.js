	...
	if (jQuery("input#symposium_member").length) {
		jQuery("input#symposium_member").autocomplete({
			...
			select: function( event, ui ) {
				...
				window.location.href=symposium.profile_url+symposium.q.substring(0, 1)+'uid='+ui.item.value;
				return false;
			}
		})
	...
	if (jQuery("input#symposium_member_small").length) {
		...
		jQuery("input#symposium_member_small").autocomplete({
			...
			select: function( event, ui ) {
		        ...
		        switch (ui.item.type) {
		          case 'topic':
					window.location.href=symposium.forum_url+symposium.q.substring(0, 1)+'show='+ui.item.id;
		            break;
		          case 'post':
		          ...
		          case 'amember':
		            ...
		            break;
		          case 'group':
					window.location.href=symposium.group_url+symposium.q.substring(0, 1)+'gid='+ui.item.value;
		            break;
		          case 'gallery':
					window.location.href=symposium.profile_url+symposium.q.substring(0, 1)+'uid='+ui.item.owner+'&embed=on&album_id='+ui.item.id;
		            break;
		        }
				return false;
			}
		...
		jQuery.ajax({
			...
			success: function(str){
							...
							html += '<div id="tabs-4">';
						        ...
						        jQuery.each(attendees, function(i,attendee){
					            	html += '<tr id="attendee_row_'+attendee.bid+'">';
										...
										var url = symposium.profile_url+symposium.q.substring(0, 1)+"uid="+attendee.uid;
				...
				jQuery("#dialog").dialog({ title: symposium.site_title+' Event ID:'+event_id, width: 850, height: 550, modal: true, buttons: {}, close: function(ev, ui) {
					jQuery(".symposium_pleasewait").inmiddle().show();		
					var reload = symposium.profile_url+symposium.q.substring(0, 1)+"uid="+event_owner+"&embed=on&view=wps_events";
