...
/*
	...
	*/
	...
	var symposium_clear = jQuery("#__wps__clear").html();
	var symposium_update = jQuery("#__wps__update").html();
	var symposium_cancel = jQuery("#__wps__cancel").html();
	var write_a_comment = jQuery("#__wps__write_a_comment").html();
	var btn_add = jQuery("#__wps__add").html();
	var show_original = jQuery("#__wps__show_original").html();
	var add_a_comment = jQuery("#__wps__add_a_comment").html();
	var pleasewait = jQuery("#__wps__pleasewait").html();
	var saving = jQuery("#__wps__saving").html();
	var more = jQuery("#__wps__more").html();
	var browseforfile = jQuery("#__wps__browseforfile").html();
	var attachfile = jQuery("#__wps__attachfile").html();
	var whatsup = jQuery("#__wps__whatsup").html();
	var symposium_next = jQuery('#__wps__next').html();
	var symposium_likes = jQuery('#__wps__likes').html();
	var symposium_dislikes = jQuery('#__wps__dislikes').html();
...
/*
	...
	*/
	...
	if (jQuery("#symposium_friends_status_Widget").length) {
		jQuery.ajax({
			url: __wps__.plugin_url + "ajax/widget_functions.php",
			data: ({
				action: "friends_status_Widget",
				postcount: jQuery("#symposium_friends_status_postcount").html(),
				preview: jQuery("#symposium_friends_status_preview").html(),
				forum: jQuery("#symposium_friends_status_forum").html()
			}),
			type: "POST",
			dataType: "html",
			async: false,
			success: function(str) {
				if (str.substring(0, 4) == 'FAIL') {
					...
					jQuery("#symposium_friends_status_Widget").html(str);
				}
			}
		});
	}
	...
	*/
	...
	if ((jQuery("#profile_body").length) && (__wps__.embed != 'on')) {
		...
		if ((menu_id == 'menu_extended') || (menu_id == 'menu_wall') || (menu_id == 'menu_activity') || (menu_id == 'menu_all') || (menu_id == 'menu_mentions') || (menu_id == 'menu_groups') || (menu_id == 'menu_friends') || (menu_id == 'menu_avatar') || (menu_id == 'menu_personal') || (menu_id == 'menu_settings')) {
			...
			// added on menu item (ie. ID does not have menu_ at the start, instead ID = path to AJAX file)
			var ajax_path = __wps__.plugins+'/wp-symposium-'+__wps__.view+'/ajax/symposium_'+__wps__.view+'_functions.php';
			menu_id = 'wp-symposium-'+__wps__.view+'/ajax/symposium_'+__wps__.view+'_functions.php';
		...
		jQuery.ajax({
			...
			success: function(str) {
				...
				jQuery('#profile_file_upload').uploadify({
					'uploader': __wps__.plugin_url + 'uploadify/uploadify.swf',
					'buttonText': browseforfile,
					'script': __wps__.plugin_url + 'uploadify/upload_profile_avatar.php',
					'fileExt': '*.jpg;*.gif;*.png;*.jpeg;',
					'cancelImg': __wps__.plugin_url + 'uploadify/uploadify-cancel.png',
					'auto': true,
					'scriptData': {
						'user_login': user_login,
						'user_email': user_email,
						'user_id': user_id,
						'page': __wps__.current_user_page
					},
	...
	jQuery("#setup_facebook").live('click', function() {
		...
		str += '<input type="submit" id="facebook_id_submit" value="OK" class="__wps__button" style="width:50px; height:25px; margin-left: 3px;" />';
		...
		if (jQuery('#facebook_id').val() != '') {
			jQuery.ajax({
				...
				success: function(str) {
					if (str != "") {
						if (str == "OK") {
							window.location.href = __wps__.profile_url;
						} else {
	...
	jQuery(".__wps__profile_menu").click(function() {
		...
		if ((menu_id == 'menu_extended') || (menu_id == 'menu_wall') || (menu_id == 'menu_activity') || (menu_id == 'menu_all') || (menu_id == 'menu_mentions') || (menu_id == 'menu_groups') || (menu_id == 'menu_friends') || (menu_id == 'menu_avatar') || (menu_id == 'menu_personal') || (menu_id == 'menu_settings')) {
			...
			// added on menu item (ie. ID does not have menu_ at the start, instead ID = path to AJAX file)
			var ajax_path = __wps__.plugins+'/'+menu_id;
		...
		jQuery.ajax({
			...
			success: function(str) {
				...
				jQuery('#profile_file_upload').uploadify({
					...
					'script': __wps__.plugin_url + 'uploadify/upload_profile_avatar.php',
					'fileExt': '*.jpg;*.gif;*.png;*.jpeg;',
					'cancelImg': __wps__.plugin_url + 'uploadify/cancel.png',
					'auto': true,
					'scriptData': {
						'user_login': user_login,
						'user_email': user_email,
	...
	function addasfriend(id) {
		...
		jQuery.ajax({
			...
			async: true,
			success: function(str) {
				jQuery("#dialog").html(jQuery("#__wps__request_sent").html());
				jQuery("#dialog").dialog({
					title: __wps__.site_title,
					width: 600,
					height: 225,
...
/*
	...
	*/
    ...
    jQuery("ul.__wps__dropdown li").hover(function(){
        ...
        jQuery('ul:first',this).css('visibility', 'visible').hide().slideDown('fast');
	...
	if (__wps__.profile_menu_scrolls == 'on') {
		...
		if (el.length) {
			...
			var a = parseInt(__wps__.profile_menu_adjust);
		...
		if (event.type == 'mouseenter' && __wps__.current_user_id > 0) {
			...
			if (rel == 'friend' || id == __wps__.current_user_id) {
				if (strpos(__wps__.mail_url, 'INVALID PLUGIN URL REQUESTED') === false && id != __wps__.current_user_id) {
					html += '<input id="symposium_plus_sendmail" ref="' + id + '" type="text" style="padding-left:18px; background: transparent url(\'' + __wps__.images_url + '/mail_small.png\') 2px 2px no-repeat;margin-top:4px;margin-bottom:8px;height:14px;font-size:12px;line-height:12px;width:90%;"  onblur = "this.value=(this.value==\'\') ? \'' + jQuery('#__wps__sendmail').html() + '\' : this.value;"  onfocus= "this.value=(this.value==\'' + jQuery('#__wps__sendmail').html() + '\') ? \'\' : this.value;"  value  = "' + jQuery('#__wps__sendmail').html() + '" />';
				} else {
					html += '<input id="symposium_plus_post" type="text" style="padding-left:18px; background: transparent url(\'' + __wps__.images_url + '/bubble.png\') 2px 2px no-repeat;margin-top:4px;margin-bottom:8px;height:14px;font-size:12px;line-height:12px;width:90%;"  onfocus= "this.value= \'\'"  value  = "' + jQuery('#__wps__whatsup').html() + '" />';
				}
			} else {
				if (rel == 'pending') {
					var q = symposium_q(__wps__.profile_url);
					html += '<div style="margin-top:4px;margin-bottom:8px;"><a href="' + __wps__.profile_url + q + 'view=friends">' + jQuery('#__wps__friendpending').html() + '</a></div>';
				} else {
					html += '<input id="symposium_plus_addasafriend" title="' + id + '" type="text" style="padding-left:18px; background: transparent url(\'' + __wps__.images_url + '/add_small.png\') 2px 3px no-repeat;margin-top:4px;margin-bottom:8px;height:14px;font-size:12px;line-height:12px;width:90%;"  onblur = "this.value=(this.value==\'\') ? \'' + jQuery('#__wps__addasafriend').html() + '...\' : this.value;"  onfocus= "this.value=(this.value==\'' + jQuery('#__wps__addasafriend').html() + '...\') ? \'\' : this.value;"  value  = "' + jQuery('#__wps__addasafriend').html() + '..." />';
				}
			}
			html += "<img id='symposium_plus_profile' ref='" + id + "' title='" + jQuery('#__wps__profile_info').html() + "' style='float:left; margin-right:5px; cursor:pointer' src='" + __wps__.images_url + "/profile.png' />";
			html += "<img id='symposium_plus_friends' ref='" + id + "' title='" + jQuery('#__wps__plus_friends').html() + "' style='float:left; margin-right:5px; cursor:pointer' src='" + __wps__.images_url + "/friends.png' />";
			if (id == __wps__.current_user_id) {
				html += "<img id='symposium_following_who' title='" + jQuery('#__wps__plus_follow_who').html() + "' style='float:left; margin-right:5px; cursor:pointer' src='" + __wps__.images_url + "/fav-who.png' />";
				html += "<img id='symposium_plus_mail' title='" + jQuery('#__wps__plus_mail').html() + "' style='float:left; margin-right:5px; cursor:pointer' src='" + __wps__.images_url + "/mail.png' />";
			} else {
				if (rev == 'following') {
					html += "<img id='symposium_following' title='" + jQuery('#__wps__unfollow').html() + "' ref='" + id + "' style='float:left; margin-right:5px; cursor:pointer' src='" + __wps__.images_url + "/fav-on.png' />";
				} else {
					html += "<img id='symposium_following' title='" + jQuery('#__wps__follow').html() + "' ref='" + id + "' style='float:left; margin-right:5px; cursor:pointer' src='" + __wps__.images_url + "/fav-off.png' />";
				}
				if (__wps__.wps_use_poke) {
					html += "<img id='__wps__attention' title='" + jQuery('#__wps__attention').html() + "' ref='" + id + "' style='float:left; margin-right:5px; cursor:pointer' src='" + __wps__.images_url + "/attention.png' />";
				}
			}
			if (strpos(__wps__.forum_url, 'INVALID PLUGIN URL REQUESTED') === false) {
				html += "<img id='symposium_forum_search' title='" + jQuery('#__wps__forumsearch').html() + "' rel='" + display_name + "' ref='" + id + "' style='float:left; margin-right:5px; cursor:pointer' src='" + __wps__.images_url + "/search2.png' />";
			}
			if (__wps__.gallery_url != '') {
				html += '<div style="float:left; margin-right:5px;"><img style="cursor:pointer" rel="' + display_name + '" id="symposium_gallery_search" src="' + __wps__.images_url + '/gallery.png" title="' + jQuery('#__wps__gallerysearch').html() + '" /></a></div>';
			}
				...
				jQuery.ajax({
					...
					success: function(str) {
						...
						if (__wps__.current_user_page != __wps__.current_user_id || jQuery("#__wps__wall").length == 0) {
							jQuery("#dialog").html(jQuery('#__wps__whatsup_done').html() + '<br /><br />' + message);
							jQuery("#dialog").dialog({
								title: __wps__.site_title,
								width: 300,
								height: 250,
	...
	jQuery("#__wps__attention").live('click', function() {
		...
		jQuery("#dialog").html(avatar + jQuery('#__wps__sent').html());
		jQuery("#dialog").dialog({
			title: __wps__.site_title,
			width: 200,
			height: 175,
		...
		if (e.keyCode == 13) {
			...
			jQuery.ajax({
				...
				data: ({
					action: "sendMail",
					compose_recipient_id: recipient_id,
					compose_subject: jQuery('#__wps__privatemail').html(),
				...
				success: function(str) {
					...
					jQuery("#dialog").dialog({
						title: jQuery('#__wps__privatemailsent').html(),
						width: 300,
						height: 250,
						modal: true,
						buttons: {
...
/*
	...
	*/
				...
				success: function(str) {
					...
					if (str != 'DONTSHOW') {
						...
						jQuery('#forum_file_upload').uploadify({
							...
							'script': __wps__.plugin_url + 'uploadify/upload_forum_file.php',
							'fileExt': __wps__.permitted_ext,
							'cancelImg': __wps__.plugin_url + 'uploadify/cancel.png',
							'auto': true,
							'scriptData': {
								'tid': show_tid,
	...
	jQuery(".quick-comment-box-add").live('click', function() {
		...
		// Add to the page
		if (jQuery(".reply-comments-box").length) {
			jQuery(html).appendTo(jQuery(this).parent().parent().find(".reply-comments-box"));
		} else {
			jQuery(html).appendTo(jQuery(this).parent().parent().parent().find(".reply-comments"));
		}
...
/*
	...
	*/
	...
	if (jQuery("#__wps__notification_bar").length) {
		...
		if ((__wps__.bar_polling > 0) && (__wps__.chat_polling > 0)) {
			...
			if (jQuery("#__wps__friends_box").css("display") != "none") {
				...
				jQuery("#__wps__online_box").click(function() {
					jQuery('#__wps__who_online').toggle("fast");
				});
				jQuery("#__wps__who_online_close").click(function() {
					jQuery('#__wps__who_online').hide("fast");
				});
			}
			jQuery("#__wps__chatroom-box").click(function() {
				jQuery('#__wps__chatroom').show("fast");
				jQuery('#__wps__chatroom-box').removeClass('__wps__chatroom-new').addClass('__wps__chatroom-none');
				var objDiv = document.getElementById('chatroom_messages');
				objDiv.scrollTop = objDiv.scrollHeight;
				createCookie('wps_chatroom', 'show', 7);
			});
			jQuery("#__wps__chatroom_close").click(function() {
				jQuery('#__wps__chatroom').hide("fast");
				eraseCookie('wps_chatroom');
			});

			// Make DIVS draggable (there is a slight problem, but uncomment if you want this feature)
			if (jQuery("#__wps__chatroom").length) {
				//jQuery('#__wps__chatroom').draggable();
			}
			if (jQuery("#__wps__who_online").length) {
				//jQuery('#__wps__who_online').draggable();
			}
			if (jQuery(".__wps__chat_window").length) {
				//jQuery('.__wps__chat_window').draggable();
			}
			...
			if (__wps__.wps_lite != 'on') {
				...
				// Chatroom Clear ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				jQuery("#__wps__chatroom_clear").live('click', function() {
				...
				jQuery("#__wps__chatroom_big").live('click', function() {
					var hc = jQuery('#__wps__chatroom').height();
					if (hc < 50) {
						jQuery('#chatroom_messages').show();
						jQuery('#chatroom_typing_area').show();
						jQuery('#__wps__chatroom').css('margin-top', '0px');
						jQuery('#__wps__chatroom').height(hc + 214);
						jQuery('#__wps__chatroom_header').removeClass('__wps__unreadChat').addClass('__wps__readChat');
						jQuery('#__wps__chatroom_big').hide();
						jQuery('#__wps__chatroom_small').show();
						jQuery('#__wps__chatroom_max').show();
						jQuery('#__wps__chatroom_min').hide();
						jQuery('#__wps__chatroom-box').removeClass('__wps__chatroom-new').addClass('__wps__chatroom-none');
				...
				jQuery("#__wps__chatroom_small").live('click', function() {
					var hc = jQuery('#__wps__chatroom').height();
					if (hc > 50) {
						jQuery('#chatroom_messages').hide();
						jQuery('#chatroom_typing_area').hide();
						jQuery('#__wps__chatroom').css('margin-top', '214px');
						jQuery('#__wps__chatroom').height(hc - 214);
						jQuery('#__wps__chatroom_big').show();
						jQuery('#__wps__chatroom_small').hide();
						jQuery('#__wps__chatroom_max').hide();
						jQuery('#__wps__chatroom_min').hide();
				...
				jQuery(".__wps__chat_small").live('click', function() {
					var hc = jQuery(this).parent().parent().height();
					if (hc > 50) {
						jQuery(this).parent().parent().css('margin-top', '214px');
						jQuery(this).parent().find(".__wps__chat_messages").hide();
						jQuery(this).parent().find(".__wps__chat_message").hide();
						jQuery(this).parent().parent().height(hc - 214);
						jQuery(this).parent().find(".__wps__chat_big").show();
						jQuery(this).hide();
						// Cookie to store minimised state
						createCookie('wps_' + jQuery(this).parent().parent().attr("id"), 'min', 7);
					}
				});
				jQuery(".__wps__chat_big").live('click', function() {
					var hc = jQuery(this).parent().parent().height();
					if (hc < 50) {
						jQuery(this).parent().parent().css('margin-top', '0px');
						jQuery(this).parent().find(".__wps__chat_messages").show();
						jQuery(this).parent().find(".__wps__chat_message").show();
						jQuery(this).parent().parent().height(hc + 214);
						jQuery(this).parent().find(".__wps__chat_small").show();
						jQuery(this).hide();
						jQuery(this).parent().removeClass('__wps__unreadChat').addClass('__wps__readChat');
						// Remove cookies
				...
				// Type in Chat Window ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				jQuery('.__wps__chat_message').keypress(function(event) {
					if (event.which == 13) {
				...
				jQuery('#__wps__chatroom_textarea').keypress(function(event) {
					if (event.which == 13) {
						var msg = jQuery(this).val();
						msg = msg.replace(/</g, '&lt');
						msg = msg.replace(/>/g, '&gt');
						...
						if (msg != '') {
							...
							jQuery('#chatroom_messages').append('<div style=""><div style="clear:both;color:#006; font-style:normal;float: left;">' + msg + '</div><div style="clear:both; float:right; color:#aaa; font-style:italic;">' + __wps__.current_user_display_name + '</div><br style="clear:both;" /></div>');
							var objDiv = document.getElementById('chatroom_messages');
							objDiv.scrollTop = objDiv.scrollHeight;
				...
				// CHAT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				if (__wps__.use_chat == 'on') {

					var numChatWindows = 3;

					// ************** When clicking on a name to chat...
					jQuery(".__wps__online_name").live('click', function() {
	...
	function do_chat_check() {
		...
		if (chat_check_cookie == 'lock') {
			...
			var numChatWindows = 3; // Should equal number of chat windows set up in bar.php
			jQuery.ajax({
				url: __wps__.plugin_url + "ajax/bar_functions.php",
				type: "POST",
				data: ({
				...
				success: function(str) {
					if (str != '') {
						...
						for (i = 0; i < num_rows; i++) {
							...
							if (from == __wps__.current_user_id) {
								other = to;
							} else {
								other = from;
							}
							...
							if (chat_win == 0) {
								...
								for (w = 1; w <= numChatWindows; w++) {
									if ((jQuery('#chat' + w + '_to').html() == '') && (allocated == false)) {
										...
										jQuery('#chat' + w + '_display_name').html('<img src="' + __wps__.images_url + '/' + status + '_header.gif" /> ' + name);
										allocated_windows++;
										allocated = true;
										new_last_chid[w] = chid;
										new_last_from[w] = from;
						...
						for (w = 1; w <= numChatWindows; w++) {
							if (jQuery('#chat' + w + '_to').html() != '') {
								...
								if (message.indexOf('[closed-' + __wps__.current_user_id + ']') >= 0) {
									jQuery('#chat' + w + '_to').html('');
									jQuery('#chat' + w).hide();
								} else {
									...
									if (readCookie('chatwin' + chat_to) == 'hidden') {
										...
										if (readCookie('wps_chat' + w) == 'min') {
											...
											if (hc > 50) {
												...
												jQuery('#chat' + w).find(".__wps__chat_big").show();
												jQuery('#chat' + w).find(".__wps__chat_small").hide();
											}
										}
								...
								if (last_chid[w] != new_last_chid[w]) {
									if (jQuery('#chat' + w).height() < 50) {
										jQuery('#chat' + w + '_header').removeClass('__wps__readChat').addClass('__wps__unreadChat');
									} else {
	...
	function do_chatroom_check() {
		...
		if (chatroom_check_cookie == 'lock') {
			...
			if (!(jQuery('#__wps__chatroom').is(':visible'))) {
			...
			jQuery.ajax({
				...
				success: function(str) {
					...
					if (split[0] != '' && split[0] != __wps__.current_user_id && split[1] != readCookie('wps_chatroom_chid')) {
						if (jQuery('#__wps__chatroom').height() < 50) {
							jQuery('#__wps__chatroom_header').removeClass('__wps__readChat').addClass('__wps__unreadChat');
						}
						jQuery('#__wps__chatroom-box').removeClass('__wps__chatroom-none').addClass('__wps__chatroom-new');
					} else {
						if (show_chatroom == "show") {
							jQuery('#__wps__chatroom-box').removeClass('__wps__chatroom-new').addClass('__wps__chatroom-none');
						}
	...
	function chat_polling() {
		do_chat_check();
		do_chatroom_check();
		setTimeout(chat_polling, __wps__.chat_polling * 1000);
	}

	// Password strength (xxx is this needed?)
	(function(A) {
		A.extend(A.fn, {
...
/*
	...
	*/
	...
	if (jQuery("#__wps__alerts").length) {
...
/*
	...
	*/
	...
	jQuery("#symposium_book_event").live('click', function() {
		...
		var events_howmany = jQuery('#__wps__events_howmany').html();
	...
	jQuery(".__wps__edit_event").live('click', function() {
		...
		// Get translated strings
		var events_max_places = jQuery("#__wps__events_max_places").html();
		var events_enable_places = jQuery("#__wps__events_enable_places").html();
		var events_show_max = jQuery("#__wps__events_show_max").html();
		var events_confirmation = jQuery("#__wps__events_confirmation").html();
		var events_tickets_per_booking = jQuery("#__wps__events_tickets_per_booking").html();
		var events_tab_1 = jQuery("#__wps__events_tab_1").html();
		var events_tab_2 = jQuery("#__wps__events_tab_2").html();
		var events_tab_3 = jQuery("#__wps__events_tab_3").html();
		var events_tab_4 = jQuery("#__wps__events_tab_4").html();
		var events_send_email = jQuery("#__wps__events_send_email").html();
		var events_pay_link = jQuery('#__wps__events_pay_link').html();
		var events_cost = jQuery('#__wps__events_cost').html();
		var events_replacements = jQuery('#__wps__events_replacements').html();
		var events_labels = jQuery('#__wps__events_labels').html().split('|');
			...
			success: function(str) {
				...
				jQuery.each(rows, function(i, row) {
					...
					if (__wps__.events_user_places || __wps__.current_user_level == 5) {
						...
						html += '<img src="' + __wps__.images_url + '/orange-tick.gif" /> ' + events_labels[8] + '&nbsp;&nbsp;&nbsp;';
						html += '<img src="' + __wps__.images_url + '/update.png" /> ' + events_labels[9] + '&nbsp;&nbsp;&nbsp;';
						html += '<img src="' + __wps__.images_url + '/delete.png" /> ' + events_labels[10] + '&nbsp;&nbsp;&nbsp;';
						html += '<img src="' + __wps__.images_url + '/cash.png" /> ' + events_labels[11] + '&nbsp;&nbsp;&nbsp;';
						html += '<img style="width:15px;height:15px" src="' + __wps__.images_url + '/tick.png" /> ' + events_labels[7] + '&nbsp;&nbsp;&nbsp;';
						html += '</td></tr>';
						html += '</table>';
						html += '</div>';
		...
		jQuery.ajax({
			...
			success: function(str) {
				...
				jQuery('#menu_gallery_file_upload').uploadify({
					'uploader': __wps__.plugin_url + 'uploadify/uploadify.swf',
					'buttonText': browseforfile,
					'script': __wps__.plugin_url + 'upload_menu_gallery.php',
					'cancelImg': __wps__.plugin_url + 'uploadify/cancel.png',
					'multi': true,
					'auto': true,
					'scriptData': {
						'aid': __wps__.album_id,
						'user_login': user_login,
						'user_email': user_email,
						'uid': __wps__.current_user_id
					},
					...
					'onAllComplete': function(event, data) {
						...
						jQuery("#dialog").dialog({
							...
							buttons: {
								"OK": function() {
									jQuery("#dialog").dialog('close');
									window.location.href = __wps__.plugin_url + 'ajax/gallery_functions.php?href=redirect&num=' + data.filesUploaded + '&aid=' + __wps__.album_id;
								}
							}
						});
					}
	...
	function prepare_colorbox(t) {
		...
		if (ids[0] != 0) {
			...
			if ((owner == __wps__.current_user_id || __wps__.current_user_level == 5) && album_title != '') {
				...
				var gallery_labels = jQuery('#__wps__gallery_labels').html().split('|');
			...
			jQuery.ajax({
				...
				success: function(str) {
					if (str != 'OK') {
						...
						var q = symposium_q(__wps__.profile_url);
						var reload_page = __wps__.profile_url + q + "uid=" + __wps__.current_user_page;
						window.location.href = reload_page;
					}
				}
				...
				jQuery.ajax({
					...
					success: function(str) {
						...
						jQuery('#file_upload').uploadify({
							'uploader': __wps__.plugin_url + 'uploadify/uploadify.swf',
							'buttonText': browseforfile,
							'script': __wps__.plugin_url + 'uploadify/upload_group_avatar.php',
							'cancelImg': __wps__.plugin_url + 'uploadify/cancel.png',
							'auto': true,
							'fileExt': '*.jpg;*.gif;*.png;*.jpeg;',
							'scriptData': {
	...
	if (!__wps__.use_wp_editor) {
		...
		if (__wps__.wps_wysiwyg == 'on' || editor_id == '__wps__about_group_edit_textarea' || editor_id == 'edit_event_text' || editor_id == 'edit_event_more') {
			...
			tinyMCE.init({
				// General options
				theme: "advanced",
				plugins: "fullscreen,safari,pagebreak,table,emotions,inlinepopups,insertdatetime,preview,media,searchreplace," + include_context + "paste,directionality,visualchars,nonbreaking",
				mode: "exact",
				elements: editor_id,
				relative_urls: false,
