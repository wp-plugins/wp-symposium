	...
	if ($rss_share == 'on') {
		...
		foreach ($activities as $activity) {
			...
			echo '<item>';
				...
				echo '<link>'.$profile_url.symposium_string_query($profile_url).'uid='.$uid.'&amp;post='.$activity->cid.'</link>';
				echo '<guid>'.$profile_url.symposium_string_query($profile_url).'uid='.$uid.'&amp;post='.$activity->cid.'</guid>';
		...
		echo '<item>';
			echo '<title>'.__('This activity is now not available publicly.', 'wp-symposium').'</title>';
		echo '</item>';
