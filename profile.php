	...
	if ( is_user_logged_in() ) {
		...
		if ($uid1 == $uid2) {
			...
			if (__wps__friend_of($uid1, $current_user->ID)) {
				...
				// A friend
				// Send mail
				$actions .= '<input type="submit" class="__wps__button" id="profile_send_mail_button" value="'.__('Send Mail', WPS_TEXT_DOMAIN).'">';
				...
				if (__wps__pending_friendship($uid1)) {
					...
					if (get_option(WPS_OPTIONS_PREFIX.'_mail_all') == 'on') {
