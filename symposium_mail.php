		...
		if (WPS_CURRENT_USER_PAGE == $current_user->ID) {
			...
			$compose = '<div id="compose_form" style="display:none">';
				...
				$compose .= '<div id="compose_mail_to">';
					...
					$compose .= '<div class="send_button">';
					$compose .= '<img id="mail_send_button" src="'.get_option('symposium_images').'/send.png" alt="'.__('Send', 'wp-symposium').'" title="'.__('Send', 'wp-symposium').'" />';
					$compose .= '<img id="mail_cancel_button" style="float:right" src="'.get_option('symposium_images').'/close2.png" alt="'.__('Cancel', 'wp-symposium').'" title="'.__('Cancel', 'wp-symposium').'" />';
					$compose .= '</div>';
