...
function __wps__forum() {	
	...
	if ( ( $topic_id == '' && $cat_id == '') || ( !get_option(WPS_OPTIONS_PREFIX.'_forum_ajax') && !get_option(WPS_OPTIONS_PREFIX.'_permalink_structure') ) ) {
	...
	if ( $can_view || strpos($viewer, __('everyone', WPS_TEXT_DOMAIN)) !== FALSE) {
		...
		if (__wps__get_current_userlevel() == 5) $html .= sprintf(__('Permissions are set via the WordPress admin dashboard->%s->Forum.', WPS_TEXT_DOMAIN), WPS_WL_SHORT);
		...
		foreach ($posts as $post)
		{
			if ( ($post->topic_approved == 'on') || ($post->topic_approved != 'on' && ($post->topic_owner == $current_user->ID || current_user_can('level_10'))) ) {
				...
				$html .= "<div style='clear:both; overflow:auto; padding-left: ".($level*40)."px; margin-top:".$margin_top."px;'>";		
					$html .= "<div class='__wps__latest_forum_row_avatar' style='float:left'>";
