...
function __wps__plugin_menu() {
	...
	if (!WPS_HIDE_PLUGINS) {
		...
		if (__wps__is_wpmu()) {
			// WPMS
			add_menu_page(WPS_WL_SHORT,WPS_WL_SHORT.$count1, 'manage_options', 'symposium_debug', '__wps__plugin_debug', plugin_dir_url( __FILE__ ).'/images/logo_admin_icon.png', 7); 
...
function __wps__plugin_welcome() {
	...
	if ($file = @file_get_contents(WPS_WELCOME_MESSAGE)) {
		...
		?>
		<div id="wps-welcome-panel" class="welcome-panel" style="background-image: none; background-color: #ddf; margin: 30px 20px 0 0">
	...
	if ($file = @file_get_contents(WPS_WELCOME_MESSAGE)) {
		...
		<div id="wps-welcome-panel" class="welcome-panel" style="background-image: none; background-color: #ddf; margin: <?php echo $top_margin; ?>px 20px 0 0;">
			<div id="motd" class="welcome-panel-content" >
				...
				<?php echo sprintf(__( 'You may also want to reset your <a href="%s">templates</a>.', WPS_TEXT_DOMAIN ), "admin.php?page=symposium_templates"); ?>
			    </p>
 	...
 	$wpdb->show_errors();
  	...
  	echo '<div class="wrap">';
		...
		echo "<div style='margin-top:15px; margin-bottom:15px; '>";
				...
				echo '<td colspan='.$colspan.'>';
					...
					if (!WPS_HIDE_ACTIVATION) {
						...
						if (!get_option(WPS_OPTIONS_PREFIX.'_activation_code')) {
							...
							echo '<p>';
								echo __('The following features are "Bronze"/"Gold" member features. You can use them for free, but a polite notice will be shown at the top of your web pages.', WPS_TEXT_DOMAIN).' ';
								echo __('<strong>If you have purchased Bronze/Gold membership at www.wpsymposium.com</strong>, your activation code is shown at <a href="http://www.wpsymposium.com/membership" target="_new">http://www.wpsymposium.com/membership</a> (make sure you&apos;ve logged in!).', WPS_TEXT_DOMAIN);
							echo '</p>';					
						}
			...
			__wps__install_row('reply_by_email', __('Reply_by_Email', WPS_TEXT_DOMAIN), '', '__wps__mailinglist', '-', WPS_DIR.'/mailinglist.php', 'admin.php?page='.WPS_DIR.'/symposium_mailinglist_admin.php', '__wps__bronze__<a href="admin.php?page=wp-symposium/mailinglist_admin.php">Options</a>');
...
Gallery=gallery';
			...
			echo "<div class='updated slideaway'><p>".__('Saved', WPS_TEXT_DOMAIN).".</p></div>";
					...
					<td scope="row"><?php echo __('Current extended fields', WPS_TEXT_DOMAIN); ?></td><td>
						...
						$extensions = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix."symposium_extended ORDER BY extended_order, extended_name", ''));
