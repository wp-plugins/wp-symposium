...
function __wps__menu_manage() {
	...
	if ( (isset($_POST['delete_wps_user'])) && ($_POST['delete_wps_user'] != '') ) {
		$u = $_POST['delete_wps_user'];
		if (is_numeric($u)) {
			$id = $u;
			$user_info = get_userdata($id);
			$user_login = $user_info->user_login;
		} else {
			$user_info = get_userdatabylogin($u);
			$id = $user_info->ID;
			$user_login = $u;
		}
		if ($id && $user_login) {
			$user_info = get_userdatabylogin('nobody');
			if ($user_info) {
			...
			<div id="motd" class="welcome-panel-content">
				...
				<div class="welcome-panel-column-container">
					...
					<div class="welcome-panel-column">
						...
						<?php echo sprintf(__( 'It\'s <em>very important</em> that you read the <a href="%s" target="_blank">release notes</a>.', WPS_TEXT_DOMAIN ), "http://www.wpsymposium.com/release-information-for-v13-04/"); ?><br />
                    </div>
			...
			foreach ($posts as $post) {
				...
				if ($post->topic_parent == 0) {
					...
					if ($c) {
						...
						foreach ($c as $cat) {
							echo '<OPTION VALUE="'.$cat->cid.'">'.stripslashes($cat->title).'</OPTION>';
						}
						echo '</SELECT>';
	...
	wp_get_current_user();
 	...
 	$wpdb->show_errors();
  	...
  	echo '<div class="wrap">';
	  	...
	  	// ********** Backup reminder
	  	$reminder = '';
		$reminder .= '<div style="margin-top:10px; margin-bottom:10px; background-color:#9f9;padding:10px;border-radius:5px;border:1px solid #9a9;">';
			$reminder .= sprintf(__('Do you backup your site and database? If not, you really should. There are many <a href="%s">plugins</a> available, please take a moment to check them out.', WPS_TEXT_DOMAIN), 'http://wordpress.org/extend/plugins/search.php?q=backup+site+and+database');
		$reminder .= '</div>';
		$reminder = apply_filters( '__wps__menu_backup_filter', $reminder );
		echo $reminder;
		...
		echo "<div style='margin-top:15px; margin-bottom:15px; '>";
			...
			if ( is_super_admin() ) {
				...
				$bronze .= '<thead><tr>';
					$bronze .= '<th colspan='.$colspan.'>'.__('Bronze member features', WPS_TEXT_DOMAIN).'</th>';
				$bronze .= '</tr></thead>';
				...
				$bronze .= '<td colspan='.$colspan.'>';
					...
					if (!WPS_HIDE_ACTIVATION) {
						...
						if (!get_option(WPS_OPTIONS_PREFIX.'_activation_code')) {
							...
							$bronze .= '<p>';
								$bronze .= __('The following features are "Bronze"/"Gold" member features. You can use them for free, but a polite notice will be shown at the top of your web pages.', WPS_TEXT_DOMAIN).' ';
								$bronze .= __('<strong>If you have purchased Bronze/Gold membership at www.wpsymposium.com</strong>, your activation code is shown at <a href="http://www.wpsymposium.com/membership" target="_new">http://www.wpsymposium.com/membership</a> (make sure you&apos;ve logged in!).', WPS_TEXT_DOMAIN);
							$bronze .= '</p>';					
						}
				...
				echo "<div id='__wps__install_assist' style='margin-top:15px'>";
					...
					echo "<div style='width:49%; float:left;'>";
						...
						echo '<table class="widefat" style="margin-top:10px"><tr><td style="padding:0 10px 0 10px">';
							...
							echo '<input type="checkbox" name="symposium_ric_syn"> '.__('Syncronize WordPress user tables with WP Sympoium', WPS_TEXT_DOMAIN);
...
function __wps__show_manage_tabs_header($active_tab) {
	...
	if (get_option(WPS_OPTIONS_PREFIX.'_long_menu') == "on") {
		...
		echo '<div class="mail_tab nav-tab-'.$advertising_active.'"><a href="admin.php?page=symposium_advertising" class="nav-tab-'.$advertising_active.'-link">'.__('Advertising', WPS_TEXT_DOMAIN).'</a></div>';
