...
function __wps__plugin_menu() {
	...
	if (!get_option(WPS_OPTIONS_PREFIX.'_done_usermeta_migration')) {
		...
		if ($sum > $wum) {
			...
			echo '<div style="font-weight:bold; font-size:14px; margin-bottom:10px">'.__('User migration process - this must be completed, please let it finish!', WPS_TEXT_DOMAIN).'</div>';
			...
			foreach ($users as $user) {
				...
				update_user_meta($user->uid, 'symposium_forum_digest', __wps__get_meta($user->uid, 'forum_digest', true));
				update_user_meta($user->uid, 'symposium_notify_new_messages', __wps__get_meta($user->uid, 'notify_new_messages', true));
				update_user_meta($user->uid, 'symposium_notify_new_wall', __wps__get_meta($user->uid, 'forum_digest', true));
				update_user_meta($user->uid, 'symposium_extended_city', __wps__get_meta($user->uid, 'city', true));
				update_user_meta($user->uid, 'symposium_extended_country', __wps__get_meta($user->uid, 'country', true));
				update_user_meta($user->uid, 'symposium_dob_day', __wps__get_meta($user->uid, 'dob_day', true));
				update_user_meta($user->uid, 'symposium_dob_month', __wps__get_meta($user->uid, 'dob_month', true));
				update_user_meta($user->uid, 'symposium_dob_year', __wps__get_meta($user->uid, 'dob_year', true));
				update_user_meta($user->uid, 'symposium_share', __wps__get_meta($user->uid, 'share', true));
				update_user_meta($user->uid, 'symposium_last_activity', __wps__get_meta($user->uid, 'last_activity', true));
				update_user_meta($user->uid, 'symposium_status', __wps__get_meta($user->uid, 'status', true));
				update_user_meta($user->uid, 'symposium_visible', __wps__get_meta($user->uid, 'visible', true));
				update_user_meta($user->uid, 'symposium_wall_share', __wps__get_meta($user->uid, 'wall_share', true));
				update_user_meta($user->uid, 'symposium_widget_voted', __wps__get_meta($user->uid, 'widget_voted', true));
				update_user_meta($user->uid, 'symposium_profile_photo', __wps__get_meta($user->uid, 'profile_photo', true));
				update_user_meta($user->uid, 'symposium_forum_favs', __wps__get_meta($user->uid, 'forum_favs', true));
				update_user_meta($user->uid, 'symposium_trusted', __wps__get_meta($user->uid, 'trusted', true));
				update_user_meta($user->uid, 'symposium_facebook_id', __wps__get_meta($user->uid, 'facebook_id', true));
				update_user_meta($user->uid, 'symposium_last_login', __wps__get_meta($user->uid, 'last_login', true));
				update_user_meta($user->uid, 'symposium_previous_login', __wps__get_meta($user->uid, 'previous_login', true));
				update_user_meta($user->uid, 'symposium_forum_all', __wps__get_meta($user->uid, 'forum_all', true));
				update_user_meta($user->uid, 'symposium_signature', __wps__get_meta($user->uid, 'signature', true));
				update_user_meta($user->uid, 'symposium_rss_share', __wps__get_meta($user->uid, 'rss_share', true));
				update_user_meta($user->uid, 'symposium_plus_lat', __wps__get_meta($user->uid, 'plus_lat', true));
				update_user_meta($user->uid, 'symposium_plus_long', __wps__get_meta($user->uid, 'plus_long', true));
	 
				// Now update WP user meta for Extended Fields
				$fields = explode('[|]', __wps__get_meta($user->uid, 'extended', true));
			...
			if ($to < $max_uid) {
				...
				echo __('Progress', WPS_TEXT_DOMAIN).': '.(floor($to/$max_uid*100)).'%<br />';
				...
				echo '</div>';
				echo '<p><em>'.__('What is this migration?', WPS_TEXT_DOMAIN).'</em></p>';
				echo '<p>'.__('Information about members was previously stored in a dedicated table ('.$wpdb->base_prefix.'symposium_usermeta) in the WordPress database.', WPS_TEXT_DOMAIN).'</p>';
				echo '<p>'.__('After this migration, it will be stored in a WordPress table ('.$wpdb->base_prefix.'usermeta) which is more integrated and allows this information to be shared across your installation, with other plugins for example.', WPS_TEXT_DOMAIN).'</p>';
				echo '<p>'.__('In addition, configuration options have been moved from a dedicated table ('.$wpdb->prefix.'symposium_config) to the core Wordpress options table ('.$wpdb->prefix.'options).', WPS_TEXT_DOMAIN).'</p>';
				echo '<p>'.__('On WPMS installations, you should ensure that a network update is performed.', WPS_TEXT_DOMAIN).'</p>';
				echo '<p>'.__('Thank you for your patience...', WPS_TEXT_DOMAIN).'</p>';
				...
				// Done
				echo '<br />'.__('Migration complete. <a href="index.php">Close</a>', WPS_TEXT_DOMAIN);
		...
		update_option(WPS_OPTIONS_PREFIX.'_done_usermeta_migration', true);
	...
	if (!WPS_HIDE_PLUGINS) {
		...
		if (__wps__is_wpmu()) {
			// WPMS
			add_menu_page(WPS_WL_SHORT,WPS_WL_SHORT.$count1, 'manage_options', 'symposium_debug', '__wps__plugin_debug', '', 7); 
			...
			add_submenu_page('symposium_debug', __('Welcome message', WPS_TEXT_DOMAIN), __('Welcome message', WPS_TEXT_DOMAIN), 'manage_options', 'symposium_welcome', '__wps__plugin_welcome');
...
function __wps__menu_options() {
	...
	if (function_exists('__wps__add_notification_bar')) $show .= '<li><a href="admin.php?page=symposium_bar">'.__('Panel (notification bar)', WPS_TEXT_DOMAIN).'</a></li>';
	if (function_exists('__wps__mobile')) $show .= '<li><a href="admin.php?page=__wps__mobile_menu">'.__('Access for mobile devices', WPS_TEXT_DOMAIN).'</a></li>';

	if ($show) {
		echo '<h2>'.__('Options Tabs', WPS_TEXT_DOMAIN).'</h2>';
		echo '<p><em>'.__('The relevant plugin has to be <a href="plugins.php">activated</a>, for its tab above to appear.', WPS_TEXT_DOMAIN).'</em></p>';	
	...
	if ($show2) {
		echo '<h2>'.__('Bronze Plugin Tabs', WPS_TEXT_DOMAIN).'</h2>';
		echo '<ul style="list-style-type: square; margin: 10px 0 10px 30px;">';
		echo $show2;
		echo '</ul>';
	}
	...
	if (!$show && !$show2) {
		...
		echo '<p>'.sprintf(__('For the relevant option tabs to appear above, <a href="plugins.php">activate</a> some %s plugins.', WPS_TEXT_DOMAIN), WPS_WL).'</p>';
	}
...
function __wps__plugin_welcome() {
		...
		if ($file = @file_get_contents(WPS_WELCOME_MESSAGE)) {
			...
			<div id="wps-welcome-panel" class="welcome-panel" style="margin-top:40px">
				...
				<img src="<?php echo WPS_PLUGIN_URL; ?>/images/logo_small.png" /><br />
			    ...
			    echo ' '.sprintf(__('For more information and release notes, check out the <a href="%s" target="_blank">blog</a>.', WPS_TEXT_DOMAIN), 'http://www.wpsymposium.com/category/releases/');
				...
				<div class="welcome-panel-column-container">
					<div class="welcome-panel-column">
						<h4><span class="icon16 icon-page"></span> <?php _e('Highlights of this release', WPS_TEXT_DOMAIN) ?></h4>
						<p><?php echo sprintf(__( 'Full release notes on the <a href="%s" target="_blank">blog</a>...', WPS_TEXT_DOMAIN ), "http://www.wpsymposium.com/category/releases/"); ?></p>
						<ul>
						<li><?php echo sprintf(	__( '<a href="%s">Show who started and last replied on forum topics</a>' ), esc_url( admin_url('admin.php?page=symposium_templates') ) ); ?></li>
						<li><?php echo sprintf(	__( '<a href="%s">Set minimum forum rank per category</a>' ), esc_url( admin_url('admin.php?page=symposium_categories') ) ); ?></li>
						<li><?php _e('Set up a group welcome/splash page', WPS_TEXT_DOMAIN); ?></li>
						<li><?php echo sprintf(	__( '<a href="%s">Show multiple directories by WordPress roles</a>' ), esc_url( admin_url('admin.php?page=__wps__members_menu') ) ); ?></li>
						<li><?php _e('Edit comments on gallery photos', WPS_TEXT_DOMAIN); ?></li>
						<li><?php echo sprintf(	__( '<a href="%s">Option to show/hide "subscribe to all forum activity"</a>' ), esc_url( admin_url('admin.php?page=symposium_forum#more') ) ); ?></li>
						<li><?php _e('Forum images displayed in order uploaded', WPS_TEXT_DOMAIN); ?></li>
						<li><?php _e('Surname added as directory sort option', WPS_TEXT_DOMAIN); ?></li>
						<li><?php echo sprintf(	__( '<a href="%s">New thesaurus tab, initially to change "Friend(s)"</a>' ), esc_url( admin_url('admin.php?page=symposium_thesaurus') ) ); ?></li>
						<li><?php echo sprintf(	__( '<a href="%s">Target bulk mail to WordPress roles</a>' ), esc_url( admin_url('admin.php?page=__wps__mail_menu') ) ); ?></li>
						<li><?php _e('Alternative horizontal profile menu with Profile Plus', WPS_TEXT_DOMAIN); ?></li>
						<li><?php _e('Group forum "new topic" subscriptions', WPS_TEXT_DOMAIN); ?></li>
						<li><?php _e('@mentions included from all forums and activity posts', WPS_TEXT_DOMAIN); ?></li>
						<li><?php echo sprintf(	__( '<a href="%s">Dropdown list of categories for quick navigation</a>' ), esc_url( admin_url('admin.php?page=symposium_forum#display') ) ); ?></li>
						<li><?php _e('Admin\'s can label member profiles via Profile Details', WPS_TEXT_DOMAIN); ?></li>
                        </ul>
					</div>
					<div class="welcome-panel-column">
						<h4><span class="icon16 icon-settings"></span> <?php _e( 'Getting Started' ); ?></h4>
						<p><?php echo sprintf(__( 'First time setting up %s?', WPS_TEXT_DOMAIN ), WPS_WL); ?></p>
						<ul>
						<li><?php echo sprintf(	__( '<a href="%s">Activate some plugins</a>' ), esc_url( admin_url('plugins.php') ) ); ?></li>
						<li><?php echo sprintf( __( '<a href="%s">Add to your site pages</a>' ), esc_url( admin_url('admin.php?page=symposium_debug') ) ); ?></li>
						...
						<li><?php echo sprintf( __( '<a href="%s" target="_blank">Take a look at the videos</a>' ), esc_url('http://www.youtube.com/user/wpsymposium?feature=watch') ); ?></li>
						...
						<h4><span class="icon16 icon-appearance"></span> <?php _e( 'A little extra help needed?', WPS_TEXT_DOMAIN ); ?></h4>
						...
						<li><?php echo sprintf( __( '<a href="%s" target="_blank">Visit the Forum</a>' ), esc_url('http://www.wpsymposium.com/discuss') ); ?></li>
						<li><?php echo sprintf( __( '<a href="%s" target="_blank">Check out the tutorials</a>' ), esc_url('http://www.wpsymposium.com/tutorials') ); ?></li>
                        <li><?php echo sprintf(	__( '<a href="%s">Consider Silver Membership</a>' ), esc_url( 'http://www.wpsymposium.com/membership' ) ); ?></li>
						</ul>
                    </div>
					<div class="welcome-panel-column welcome-panel-last">
						<h4><span class="icon16 icon-appearance"></span> <?php _e( 'Admin Guide', WPS_TEXT_DOMAIN ); ?></h4>
						<p><?php _e( 'A thorough and more detailed 132 page reference guide covering installation, configuration and development tips.', WPS_TEXT_DOMAIN ); ?></p>
                        <img src="<?php echo get_option(WPS_OPTIONS_PREFIX.'_images'); ?>/wps_admin_guide.png" width="180" height="134" />
                        <p><?php echo sprintf(__('<a href="%s" target="_blank">Click here</a> to download (PDF, 17MB).', WPS_TEXT_DOMAIN), 'https://dl.dropbox.com/u/49355018/wps.pdf'); ?></p>                        
					</div>
				</div>
...
function __wps__plugin_templates() {
	...
	$template_group_forum_topic = str_replace("[]", chr(13), stripslashes(get_option(WPS_OPTIONS_PREFIX.'_template_group_forum_topic')));
  	...
  	echo '<div class="wrap">';
		...
		echo '<thead>';
		echo '<tr>';
		echo '<th style="font-size:1.2em">'.__('Import Template', WPS_TEXT_DOMAIN).'<input id="symposium_import_file_button" type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;margin-bottom:0px;" value="Import"><div id="symposium_import_file_pleasewait" style="width:15px; float:right;"></div></th>';
		echo '</tr>';
		echo '</thead>';
		echo '<tbody>';
		echo '<tr>';
		...
		echo '<div id="symposium_templates_values">';
			...
			echo '<input id="symposium_export_templates" type="submit" class="button" style="margin-top:10px;margin-right:6px;" value="'.__('Export', WPS_TEXT_DOMAIN).'">';
			echo '<input id="symposium_import_templates" type="submit" class="button" style="margin-top:10px;margin-right:6px;" value="'.__('Import', WPS_TEXT_DOMAIN).'">';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Profile Page Header', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;margin-bottom:0px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Profile Page Body', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Page Footer', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Mail Page: Tray Item', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Mail Page: Message', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Forum Header', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Forum Categories (list)', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Forum Topics (list)', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			echo '<td>';
				...
				echo '<tr>';
				echo '<td>[started]</td>';
				echo '<td>'.__('Ago of initial topic post', WPS_TEXT_DOMAIN).'</td>';
				echo '</tr>';
				echo '<tr>';
				echo '<td>[replied]</td>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Group Page', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			if (function_exists('__wps__groups')) {
				...
				echo $template_group;
				echo '</textarea>';
				echo '<br /><a id="reset_group" href="javascript:void(0)">'.__('Reset to default', WPS_TEXT_DOMAIN).'</a>';
			} else {
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Group Forum Categories (list)', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			echo '<td>';
				...
				echo $template_forum_category;
				echo '</textarea>';
				echo '<br /><a id="reset_template_group_forum_category" href="javascript:void(0)">'.__('Reset to default', WPS_TEXT_DOMAIN).'</a>';
			echo '</td>';
			echo '</tr>';
			echo '</tbody>';
			echo '</table>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Group Forum Topics (list)', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.sprintf(__('%s Emails', WPS_TEXT_DOMAIN), WPS_WL).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
  	...
  	echo '<div class="wrap">';
		...
		if ($posts) {
			...
			foreach ($posts as $post) {
				...
				echo '<td valign="top">';
				if ($post->topic_parent == 0) {
					echo '<strong>'.__('New Topic', WPS_TEXT_DOMAIN).'</strong><br />';
				} else {
					echo '<strong>'.__('New Reply', WPS_TEXT_DOMAIN).'</strong><br />';
				}
  	...
  	echo '<div class="wrap">';
		...
		if (!WPS_HIDE_ACTIVATION) {
			...
			if (get_option(WPS_OPTIONS_PREFIX.'_activation_code') && has_bronze_plug_actived()) {
		   	...
		   	echo __('Activation Code:', WPS_TEXT_DOMAIN).' <input type="text" name="symposium_validation_code" value="'.get_option(WPS_OPTIONS_PREFIX.'_activation_code').'" style="width:300px; background-color: #ff9; border:1px solid #333;">';
		   	echo '<p class="submit"><input type="submit" name="Submit" class="button-primary" value="'.__('Update', WPS_TEXT_DOMAIN).'" />';	   	
	
		   	if ($code=get_option(WPS_OPTIONS_PREFIX.'_activation_code')) {
		   	    if (($code != 'wps') && (substr($code,0,3) != 'vip')) {
			   	    $code =  preg_replace('#[^0-9]#','',$code);
			   	    if ($code > time()) {
						echo '<br /><br /> '.sprintf(__('This activation code expires on %s, <a href="http://www.wpsymposium.com/membership" target="_new">get a new activation code</a> before then to extend the date.', WPS_TEXT_DOMAIN), date('l d F Y', $code));
						echo '<br />'.sprintf(__('Note that this may not tie in with your %s expiry date, simply re-enter a new activation code at any time to reset to another 365 days.', WPS_TEXT_DOMAIN), WPS_WL);
			   	    } else {
						echo '<br /><br /> <strong>'.__('This activation code has expired! <a href="http://www.wpsymposium.com/membership" target="_new">Get a new activation code</a> to extend the date.', WPS_TEXT_DOMAIN).'</strong>';
			   	    }
		   	    } else {
		   	        if ($code=='wps') {
			   	        echo '<br /><br />'.__('This is a temporary activation code, it should not be used permenantly.', WPS_TEXT_DOMAIN);
		   	        }
		   	        if (substr($code,0,3) == 'vip') {
			   	        echo '<br /><br />'.__('This is a lifetime activation code.', WPS_TEXT_DOMAIN);
		   	        }
		   	    }
		   	} else {
		...
		// Status
		echo '<div id="icon-themes" class="icon32"><br /></div>';
		echo '<h2>' . __('Installed Plugins', WPS_TEXT_DOMAIN) . '</h2>';
		
		if (current_user_can('update_core') && (!WPS_HIDE_INSTALL_INFO)) {

			echo '<div style="float:right"><em>';
			echo __('Quick links:', WPS_TEXT_DOMAIN).' ';
			echo '<a href="#image">'.__('Image uploading', WPS_TEXT_DOMAIN).'</a> | ';
			echo '<a href="#ric">'.__('Integrity check', WPS_TEXT_DOMAIN).'</a> | ';
			echo '<a href="#purge">'.__('Database Purge Tool', WPS_TEXT_DOMAIN).'</a> | ';
			echo '<a href="#bbpress">'.__('bbPress migration', WPS_TEXT_DOMAIN).'</a> | ';
			echo '<a href="#mingle">'.__('Mingle migration', WPS_TEXT_DOMAIN).'</a>';
			echo '</em></div>';
		...
		echo "<div style='margin-top:15px; margin-bottom:15px; '>";
			...
			if (current_user_can('update_core'))
				echo '<th width="30px">'.__('Installed', WPS_TEXT_DOMAIN).'</th>';
			echo '<th width="30px">'.__('Activated', WPS_TEXT_DOMAIN).'</th>';
			*/
			echo '<th width="260px">'.__('Plugin', WPS_TEXT_DOMAIN).'</th>';
			echo '<th>'.__('WordPress page/URL Found', WPS_TEXT_DOMAIN).'</th>';
			echo '<th width="90px;">'.__('Status', WPS_TEXT_DOMAIN);
			if (current_user_can('update_core'))
			...
			echo '<tr>';
				/*
				echo '<td style="text-align:center"><img src="'.get_option(WPS_OPTIONS_PREFIX.'_images').'/tick.png" /></td>';
				if (current_user_can('update_core'))		
					echo '<td style="text-align:center"><img src="'.get_option(WPS_OPTIONS_PREFIX.'_images').'/tick.png" /></td>';
				*/
			...
			__wps__install_row('Forum', WPS_SHORTCODE_PREFIX.'-forum', '__wps__forum', get_option(WPS_OPTIONS_PREFIX.'_forum_url'), WPS_DIR.'/symposium_forum.php', 'admin.php?page=forum', __('The forum plugin must be installed in ', WPS_TEXT_DOMAIN).WPS_PLUGIN_DIR);
			__wps__install_row('Profile', WPS_SHORTCODE_PREFIX.'-profile', '__wps__profile', get_option(WPS_OPTIONS_PREFIX.'_profile_url'), WPS_DIR.'/profile.php', 'admin.php?page=profile', __('The profile plugin must be installed in ', WPS_TEXT_DOMAIN).WPS_PLUGIN_DIR);
			__wps__install_row('Mail', WPS_SHORTCODE_PREFIX.'-mail', '__wps__mail', get_option(WPS_OPTIONS_PREFIX.'_mail_url'), WPS_DIR.'/mail.php', '', __('The mail plugin must be installed in ', WPS_TEXT_DOMAIN).WPS_PLUGIN_DIR);
			__wps__install_row('Members', WPS_SHORTCODE_PREFIX.'-members', '__wps__members', get_option(WPS_OPTIONS_PREFIX.'_members_url'), WPS_DIR.'/members.php', 'admin.php?page=__wps__members_menu', __('The members directory plugin must be installed in ', WPS_TEXT_DOMAIN).WPS_PLUGIN_DIR);
			__wps__install_row('Panel', '', '__wps__add_notification_bar', '-', WPS_DIR.'/bar.php', 'admin.php?page=bar', __('The panel plugin must be installed in ', WPS_TEXT_DOMAIN).WPS_PLUGIN_DIR);
		...
		if (current_user_can('update_core') && (!WPS_HIDE_INSTALL_INFO)) {
			...
			echo "<div style='width:45%; float:left;'>";
		  		...
		  		echo '<div id="icon-themes" class="icon32"><br /></div>';
				echo '<h2>'.__('Core Information', WPS_TEXT_DOMAIN).'</h2>';
			...
			echo "<div style='width:50%; float:left; padding-bottom:15px;'>";
				...
				echo '<a name="perma"></a>';
			  	echo '<div id="icon-themes" class="icon32"><br /></div>';
			   	echo '<h2>'.sprintf(__('%s Permalinks', WPS_TEXT_DOMAIN), WPS_WL_SHORT).'</h2>';
...
//								}
							...
							echo '<input type="checkbox" name="symposium_permalinks_reset" /> '.__('Reset to default suggested values (if you have altered page names for example)', WPS_TEXT_DOMAIN).'<br /><br />';
						   	echo '<input type="submit" class="button-primary" value="'.__('Update', WPS_TEXT_DOMAIN).'" /> ';
							
						} else {
							echo '<input type="hidden" name="symposium_permalinks" value="Y">';
							echo '<input type="checkbox" name="symposium_permalinks_enable"> '.sprintf(__('Check to enable %s Permalinks', WPS_TEXT_DOMAIN), WPS_WL_SHORT).'<br /><br />';
						   	echo '<input type="submit" class="button-primary" value="'.__('Update', WPS_TEXT_DOMAIN).'" />';
						}
			...
			echo "<div style='width:45%; float:left;'>";
				...
				// ********** Reset database version
			  	echo '<div id="icon-themes" class="icon32"></div>';
			   	echo '<h2>'.sprintf(__('Refresh %s', WPS_TEXT_DOMAIN), WPS_WL).'</h2>';
			...
			echo "<div style='width:45%; float:left;'>";
				...
				if( isset($_POST[ 'symposium_testemail' ]) && $_POST[ 'symposium_testemail' ] == 'Y' ) {
			  	...
			  	echo '<div id="icon-themes" class="icon32"><br /></div>';
			   	echo '<h2>'.__('Send a test email', WPS_TEXT_DOMAIN).'</h2>';
				...
				echo '<input type="hidden" name="symposium_testemail" value="Y">';
			   	...
			   	echo '<p class="submit"><input type="submit" name="Submit" class="button-primary" value="'.__('Send email', WPS_TEXT_DOMAIN).'" /></p>';
			   	echo '</form>';
			...
			echo "<div style='width:45%; float:left;;'>";
				...
				echo '<a name="ric"></a>';
			  	echo '<div id="icon-themes" class="icon32"><br /></div>';
			   	echo '<h2>'.__('Integrity check', WPS_TEXT_DOMAIN).'</h2>';
				...
				if (isset($_POST['symposium_ric'])) {
					...
					if ($topics) {
						...
						$report .= sprintf( __("%d topics had missing categories and a parent", WPS_TEXT_DOMAIN), count($topics) )."<br />";
					}
					...
					echo __("Integrity check completed.", WPS_TEXT_DOMAIN)."<br />".$report;
					echo "</div>";
			...
			echo "<div style='width:45%; float:left;'>";
				...
				echo '<a name="image"></a>';
			  	echo '<div id="icon-themes" class="icon32"><br /></div>';
			   	echo '<h2>'.__('Image Uploading', WPS_TEXT_DOMAIN).'</h2>';
				...
				if (get_option(WPS_OPTIONS_PREFIX.'_img_db') == "on") {
					...
					if (file_exists(get_option(WPS_OPTIONS_PREFIX.'_img_path'))) {
						...
						if (!mkdir(get_option(WPS_OPTIONS_PREFIX.'_img_path'), 0777, true)) {
					...
					if (!file_exists($tmpDir)) {
						if (@mkdir($tmpDir)) {
							...
							echo __(', and cound not be created - please check permissions of this path.', WPS_TEXT_DOMAIN).$fail2.'</p>';
						}
					} else {
						...
						if (touch($targetTmpFile)) {
							...
							echo "<p>".sprintf(__('Temporary file (%s) created and removed okay.', WPS_TEXT_DOMAIN), $tmpFile)."</p>";
						} else {
			...
			echo "<div style='width:45%; float:left;'>";
				...
				echo "<a name='purge'></a>";
			  	echo '<div id="icon-themes" class="icon32"></div>';
			   	echo '<h2>'.__('Purge forum/chat', WPS_TEXT_DOMAIN).'</h2>';
				...
				echo '<p>'.__('Forum activity and chat purged are <strong>deleted</strong> - you cannot undo this! I recommend you take a backup first.', WPS_TEXT_DOMAIN).'</p>';
	
				echo '<form action="" method="post"><p>';
				echo __('Chat older than', WPS_TEXT_DOMAIN);
					echo ' <input type="text" size="3" name="purge_chat"> ';
					echo __('days', WPS_TEXT_DOMAIN)."<br />";
				echo __('Forum topics older than', WPS_TEXT_DOMAIN);
					echo ' <input type="text" size="3" name="purge_topics"> ';
					echo __('days', WPS_TEXT_DOMAIN)."</p>";
			...
			echo "<div style='width:50%; float:left;'>";
				...
				// ********** Daily Digest 
			  	...
			  	echo '<div id="icon-themes" class="icon32"><br /></div>';
			   	echo '<h2>'.__('Daily Digest', WPS_TEXT_DOMAIN).'</h2>';
				...
				if( isset($_POST[ 'symposium_dailydigest' ]) && $_POST[ 'symposium_dailydigest' ] == 'Y' ) {
					...
					if ($to_users == "on") {
						...
						$success = __wps__notification_do_jobs('send_admin_summary_only');
					}			
					echo $success;
					echo "Complete.<br />";
					if ($success == 'OK') {
			   	...
			   	echo '<p>'.__('It will also email an admin summary report. Choose below to optionally send Daily Digest immediately to all subscribed users', WPS_TEXT_DOMAIN).'.</p>';
			   	echo '<form method="post" action="">';
				echo '<input type="hidden" name="symposium_dailydigest" value="Y">';
			   	echo '<input type="checkbox" name="symposium_dailydigest_users" > '.__('Send Daily Digest to users', WPS_TEXT_DOMAIN);
			   	echo '<p class="submit"><input type="submit" name="Submit" class="button-primary" value="'.__('Send Daily Digest', WPS_TEXT_DOMAIN).'" /></p>';
			   	echo '</form>';
			...
			// ********** Stylesheets
		  	...
		  	echo '<div id="icon-themes" class="icon32"><br /></div>';
		   	echo '<h2>'.__('Stylesheets', WPS_TEXT_DOMAIN).'</h2>';
			...
			// ********** Javascript
		  	...
		  	echo '<div id="icon-themes" class="icon32"><br /></div>';
		   	echo '<h2>'.__('Javascript', WPS_TEXT_DOMAIN).'</h2>';
			...
			echo '<a name="bbpress"></a>';
		  	echo '<div id="icon-themes" class="icon32"><br /></div>';
		   	echo '<h2>'.__('bbPress Migration', WPS_TEXT_DOMAIN).'</h2>';
			...
			echo '<a name="mingle"></a>';
		  	echo '<div id="icon-themes" class="icon32"><br /></div>';
		   	echo '<h2>'.__('Mingle Migration', WPS_TEXT_DOMAIN).'</h2>';
...
function __wps__install_row($name, $shortcode, $function, $config_url, $plugin_dir, $settings_url, $install_help) {
		...
		echo '<tr>';
			...
			echo '<td style="height:30px">';
				...
				if ($ver != 'v'.WPS_VER && $ver != '') {
					...
					echo __('Upgrade Required', WPS_TEXT_DOMAIN).'</a>';
					$status = 'upgrade';
				}
			echo '</td>';
			...
			if ( ($pages) && ($shortcode != '') ) {
				...
				if ( ($status != 'cross') && ($status != 'notinstalled') && ($shortcode != '') ) {
					$status = 'add';
					echo '<div style="float:left; width:175px">'.sprintf(__('Add [%s] to:', WPS_TEXT_DOMAIN), $shortcode).'</div>';
					...
					if ($function == '__wps__add_notification_bar') {
						if (current_user_can('update_core'))
							echo ' [<a href="http://www.wpswiki.com/index.php?title=Chat_options" target="_blank">'.__('Read this!', WPS_TEXT_DOMAIN).'</a>]';
					}
			...
			echo '<td style="text-align:center">';
				...
				// Fix URL
				$fixed_url = false;
				$current_value = get_option(WPS_OPTIONS_PREFIX.'_'.strtolower($name).'_url');
					if ($current_value != $url) {
						update_option(WPS_OPTIONS_PREFIX.'_'.strtolower($name).'_url', $url);
						$fixed_url = true;
						if ($url != '') {
			...
			if (current_user_can('update_core')) {
				echo '<td class="symposium_url" style="background-color:#efefef">';
					...
					$value = get_option(WPS_OPTIONS_PREFIX.'_'.strtolower($name).'_url');
					if (!$value && $status != 'add') { 
						echo 'n/a';
					} else {
...
function __wps__plugin_bar() {
	...
	global $wpdb;
		...
		if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == '__wps__plugin_bar' ) {
			...
			update_option(WPS_OPTIONS_PREFIX.'_use_chatroom', isset($_POST[ 'use_chatroom' ]) ? $_POST[ 'use_chatroom' ] : '');
			...
			update_option(WPS_OPTIONS_PREFIX.'_use_wp_profile', isset($_POST[ 'use_wp_profile' ]) ? $_POST[ 'use_wp_profile' ] : '');
			...
			<tr valign="top"> 
			<td scope="row"><label for="use_chatroom"><?php echo __('Enable chatroom', WPS_TEXT_DOMAIN); ?></label></td>
			<td>
			<input type="checkbox" name="use_chatroom" id="use_chatroom" <?php if (get_option(WPS_OPTIONS_PREFIX.'_use_chatroom') == "on") { echo "CHECKED"; } ?>/>
			<span class="description"><?php echo __('Real-time chatroom (chat seen by all members)', WPS_TEXT_DOMAIN); ?></span></td> 
			</tr> 

			<tr valign="top"> 
			<td scope="row"><label for="chatroom_banned"><?php echo __('Banned chatroom words', WPS_TEXT_DOMAIN); ?></label></td> 
			<td><input name="chatroom_banned" type="text" id="chatroom_banned"  value="<?php echo get_option(WPS_OPTIONS_PREFIX.'_chatroom_banned'); ?>" /> 
			<span class="description"><?php echo __('Comma separated list of words not allowed in the chatroom', WPS_TEXT_DOMAIN); ?></td> 
			</tr> 
			...
			<span class="description"><?php echo __('Frequency of checks for new mail, friends online, etc, in seconds', WPS_TEXT_DOMAIN); ?></td> 
			</tr> 
			...
			<span class="description"><?php echo __('Frequency of chat window updates in seconds', WPS_TEXT_DOMAIN); ?></td> 
			</tr> 

			<tr valign="top"> 
			<td scope="row"><label for="use_wp_profile"><?php echo __('Profile Link', WPS_TEXT_DOMAIN); ?></label></td> 
			<td><input type="checkbox" name="use_wp_profile" id="use_wp_profile" <?php if (get_option(WPS_OPTIONS_PREFIX.'_use_wp_profile') == "on") { echo "CHECKED"; } ?>/>
			<span class="description"><?php echo __('Link to WordPress user profile page?', WPS_TEXT_DOMAIN); ?></td> 
			</tr> 
	...
	include_once( ABSPATH . 'wp-includes/formatting.php' );
		...
		// See if the user has posted profile settings
			...
			if (!get_option(WPS_OPTIONS_PREFIX.'_profile_menu_type')) {
				...
				update_option(WPS_OPTIONS_PREFIX.'_menu_profile_text', $_POST['menu_profile_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_my_activity_text', $_POST['menu_my_activity_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_friends_activity_text', $_POST['menu_friends_activity_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_all_activity_text', $_POST['menu_all_activity_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_friends_text', $_POST['menu_friends_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_mentions_text', $_POST['menu_mentions_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_groups_text', $_POST['menu_groups_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_events_text', $_POST['menu_events_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_gallery_text', $_POST['menu_gallery_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_following_text', $_POST['menu_following_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_followers_text', $_POST['menu_followers_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_lounge_text', $_POST['menu_lounge_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_avatar_text', $_POST['menu_avatar_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_details_text', $_POST['menu_details_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_settings_text', $_POST['menu_settings_text']);
	
				update_option(WPS_OPTIONS_PREFIX.'_menu_profile_other_text', $_POST['menu_profile_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_my_activity_other_text', $_POST['menu_my_activity_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_friends_activity_other_text', $_POST['menu_friends_activity_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_all_activity_other_text', $_POST['menu_all_activity_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_friends_other_text', $_POST['menu_friends_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_mentions_other_text', $_POST['menu_mentions_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_groups_other_text', $_POST['menu_groups_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_events_other_text', $_POST['menu_events_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_gallery_other_text', $_POST['menu_gallery_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_following_other_text', $_POST['menu_following_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_followers_other_text', $_POST['menu_followers_other_text']);
				update_option(WPS_OPTIONS_PREFIX.'_menu_lounge_other_text', $_POST['menu_lounge_other_text']);
...
Gallery=gallery';
			...
			echo "<div class='updated slideaway'><p>".__('Saved', WPS_TEXT_DOMAIN).".</p></div>";
					...
					<select name="wps_profile_default">
						...
						<option value='activity'<?php if (get_option(WPS_OPTIONS_PREFIX.'_wps_profile_default') == 'activity') { echo ' SELECTED'; } ?>><?php echo __('Friends activity (excludes my activity)', WPS_TEXT_DOMAIN); ?></option>
					...
					<td>
					<textarea name="menu_texthtml" id="menu_texthtml" rows="4" cols="30"><?php echo get_option(WPS_OPTIONS_PREFIX.'_menu_texthtml'); ?></textarea><br />
					...
					<td scope="row"><?php echo __('Current extended fields', WPS_TEXT_DOMAIN); ?></td><td>
						...
						<table style="margin-left:10px; margin-top:10px;">						
							...
							<tr><td>[<?php echo WPS_SHORTCODE_PREFIX; ?>-stream]</td>
								<td><?php echo __('Profile page, defaulting to all activity.', WPS_TEXT_DOMAIN); ?></td></tr>
...
function __wps__plugin_settings() {
	...
	global $wpdb;
		...
		if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == '__wps__plugin_settings' ) {
			...
			update_option(WPS_OPTIONS_PREFIX.'_img_path', $_POST[ 'img_path' ]);
			...
			echo "<p>".__('Saved', WPS_TEXT_DOMAIN).".</p></div>";
					...
					<span class="description"><?php echo __('Minimized loads faster, Normal can be edited', WPS_TEXT_DOMAIN); ?></span></td> 
					</tr> 		
...
?>					
					...
					<span class="description"><?php echo sprintf(__("Only for use by %s developers.", WPS_TEXT_DOMAIN), WPS_WL); ?></span></td> 
					</tr>
					...
					<tr valign="top"> 
					<td colspan="2"><hr /><?php echo __('The following can be disabled if clashes with other WordPress plugins are occuring', WPS_TEXT_DOMAIN); ?>:</td>
					</tr> 
...
function __wps__plugin_forum() {
	...
	global $wpdb;
		...
		if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == '__wps__plugin_forum' ) {
			...
			update_option(WPS_OPTIONS_PREFIX.'_preview1', $_POST[ 'preview1' ] != '' ? $_POST[ 'preview1' ] : 45);
			update_option(WPS_OPTIONS_PREFIX.'_preview2', $_POST[ 'preview2' ] != '' ? $_POST[ 'preview2' ] : 90);
			update_option(WPS_OPTIONS_PREFIX.'_chatroom_banned', $_POST[ 'chatroom_banned' ]);
			...
			update_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg', isset($_POST[ 'use_wysiwyg' ]) ? $_POST[ 'use_wysiwyg' ] : '');
			...
			update_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_2', $_POST[ 'use_wysiwyg_2' ]);
			update_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_3', $_POST[ 'use_wysiwyg_3' ]);
			update_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_4', $_POST[ 'use_wysiwyg_4' ]);
			update_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_css', $_POST[ 'use_wysiwyg_css' ]);
			update_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg_skin', $_POST[ 'use_wysiwyg_skin' ]);
		...
		?>
			...
			<?php if ( __wps__is_plus() ) { ?>
				...
				<?php if (get_option(WPS_OPTIONS_PREFIX.'_use_wysiwyg') == 'on') { ?>					
                    ...
                    <tr valign="top" style='border-bottom: 1px dashed #fff; border-right: 1px dashed #fff; border-left: 1px dashed #fff; '> 
				...
				echo __("To access WYSWIYG editor settings, <a href='admin.php?page=symposium_debug'>enter a valid activation code</a>.", WPS_TEXT_DOMAIN).'<br />';
				echo '<em>NB. Some themes cause layout problems with TinyMCE. The editor is verified with TwentyEleven and tested with many others, but if your editor toolbar layout is broken, check your theme stylesheets.</em>';
				echo '</td></tr>';
			...
			<tr valign="top"> 
			<td scope="row"><label for="forum_login_form"><?php echo __('Show login form', WPS_TEXT_DOMAIN); ?></label></td>
			<td>
			...
			<span class="description"><?php echo __('If a user has to log in, show the login form underneath the topic/replies.', WPS_TEXT_DOMAIN); ?></span></td> 
			</tr> 
			...
			<tr valign="top"> 
			<td scope="row"><label for="allow_subscribe_all"><?php echo __('Allow members to subscribe to all forum activity', WPS_TEXT_DOMAIN); ?></label></td>
			<td>
			...
			<span class="description"><?php echo __('Via Profile page, Profile Details. If you have a lot of users, consider switching this off to improve forum performance.', WPS_TEXT_DOMAIN); ?></span></td> 
			</tr> 
			...
			<span class="description"><?php echo __('Enable daily summaries to all members via email', WPS_TEXT_DOMAIN); ?></span></td> 
			</tr> 
			...
			<span class="description"><?php echo __('How many minutes before a forum topic/reply can no longer be edited, 0 for no limit.', WPS_TEXT_DOMAIN); ?></span></td> 
			</tr> 
	...
	if (isset($_POST['title'])) {
		...
		foreach ($range as $key) {
			...
			$wpdb->query( $wpdb->prepare( "
				...
				SET title = %s, cat_parent = %d, min_rank = %d, listorder = %s, allow_new = %s, cat_desc = %s, defaultcat = %s, level = %s
				WHERE cid = %d", 
				$title, $cat_parent, $min_rank, $listorder, $allow_new, $cat_desc, $defaultcat, serialize($level), $cid  ) );
	...
	if ($categories) {
		foreach ($categories as $category) {
			...
			echo '<option value=""';
				...
				echo '>'.__('No', WPS_TEXT_DOMAIN).'</option>';
			echo '</select>';
			echo '<span class="__wps__tooltip" title="'.__('Should users be able to start<br />new topics in this category?', WPS_TEXT_DOMAIN).'">?</span>';
			echo '</td>';
			echo '</td>';
			echo '<td>';
			echo '<a class="delete" href="?page=symposium_categories&action=delcid&all=0&cid='.$category->cid.'">'.__('Delete category', WPS_TEXT_DOMAIN).'</a><br />';
	...
	if (!current_user_can('manage_options'))  {
  	...
  	echo '<div class="wrap">';
		...
		if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == 'Y' ) {
			...
			update_option(WPS_OPTIONS_PREFIX.'_categories_background', $_POST['__wps__categories_background']);
			update_option(WPS_OPTIONS_PREFIX.'_categories_color', $_POST['categories_color']);
			update_option(WPS_OPTIONS_PREFIX.'_border_radius', $_POST['border_radius']);
			update_option(WPS_OPTIONS_PREFIX.'_bigbutton_background', $_POST['bigbutton_background']);
			update_option(WPS_OPTIONS_PREFIX.'_bigbutton_background_hover', $_POST['bigbutton_background_hover']);
			update_option(WPS_OPTIONS_PREFIX.'_bigbutton_color', $_POST['bigbutton_color']);
			update_option(WPS_OPTIONS_PREFIX.'_bigbutton_color_hover', $_POST['bigbutton_color_hover']);
			update_option(WPS_OPTIONS_PREFIX.'_bg_color_1', $_POST['bg_color_1']);
			update_option(WPS_OPTIONS_PREFIX.'_bg_color_2', $_POST['bg_color_2']);
			update_option(WPS_OPTIONS_PREFIX.'_bg_color_3', $_POST['bg_color_3']);
			update_option(WPS_OPTIONS_PREFIX.'_row_border_style', $_POST['row_border_style']);
			update_option(WPS_OPTIONS_PREFIX.'_row_border_size', $_POST['row_border_size']);
			update_option(WPS_OPTIONS_PREFIX.'_table_rollover', $_POST['table_rollover']);
			update_option(WPS_OPTIONS_PREFIX.'_table_border', $_POST['table_border']);
			update_option(WPS_OPTIONS_PREFIX.'_replies_border_size', $_POST['replies_border_size']);
			update_option(WPS_OPTIONS_PREFIX.'_text_color', $_POST['text_color']);
			update_option(WPS_OPTIONS_PREFIX.'_text_color_2', $_POST['text_color_2']);
			update_option(WPS_OPTIONS_PREFIX.'_link', $_POST['link']);
			update_option(WPS_OPTIONS_PREFIX.'_underline', $_POST['underline']);
			update_option(WPS_OPTIONS_PREFIX.'_link_hover', $_POST['link_hover']);
			update_option(WPS_OPTIONS_PREFIX.'_label', $_POST['label']);
			update_option(WPS_OPTIONS_PREFIX.'_closed_opacity', $_POST['closed_opacity']);
			update_option(WPS_OPTIONS_PREFIX.'_fontfamily', $_POST['fontfamily']);
			update_option(WPS_OPTIONS_PREFIX.'_fontsize', str_replace("px", "", strtolower($_POST[ 'fontsize' ])));
			update_option(WPS_OPTIONS_PREFIX.'_headingsfamily', $_POST['headingsfamily']);
			update_option(WPS_OPTIONS_PREFIX.'_headingssize', str_replace("px", "", strtolower($_POST[ 'headingssize' ])));
			update_option(WPS_OPTIONS_PREFIX.'_main_background', $_POST['main_background']);
		...
		echo '<div class="__wps__wrapper" style="margin-top:15px">';
			...
			echo '<div id="mail-main">';
				...
				if ($view == "css") {
					...
					echo '<thead>';
					echo '<tr>';
					echo '<th style="font-size:1.2em">'.__('CSS', WPS_TEXT_DOMAIN).'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', WPS_TEXT_DOMAIN).'"></th>';
					echo '</tr>';
					echo '</thead>';
					echo '<tbody>';
					echo '<tr>';
					...
					echo '<table style="float:right;width:39%">';
					echo '<tr>';
					echo '<td>'.__('Notes', WPS_TEXT_DOMAIN).'</td>';
					echo '</tr>';
					echo '<tbody>';
					echo '<tr><td>';
					...
					echo '</td></tr>';
					echo '<tr><td>';
					echo __('Refer to www.wpswiki.com for more help and examples.', WPS_TEXT_DOMAIN);
					echo '</td></tr>';
					echo '</tbody>';
					echo '</table>';
				...
				if ($view == "styles") {
					...
					<td><input name="headingsfamily" type="text" id="headingsfamily" value="<?php echo get_option(WPS_OPTIONS_PREFIX.'_headingsfamily'); ?>"/> 
		...
		if ($subject == '' || $message == '') {
			...
			if ($include_roles != '') {
		  	...
		  	$url = __wps__get_url('mail');	
				...
				echo "<div class='updated'><p>".sprintf(__('Bulk message sent to %d members', WPS_TEXT_DOMAIN), $cnt).".</p></div>";	
				$subject = '';
				$message = '';			
			} else {
...
function __wps__show_tabs_header($active_tab) {
	...
	if (get_option(WPS_OPTIONS_PREFIX.'_long_menu') == "on") {
		...
		if (function_exists('symposium_facebook')) 		echo '<div class="mail_tab nav-tab-'.$facebook_active.' bronze"><a href="admin.php?page='.WPS_DIR.'/facebook_admin.php" class="nav-tab-'.$facebook_active.'-link">'.__('Facebook', WPS_TEXT_DOMAIN).'</a></div>';
...
function __wps__show_manage_tabs_header($active_tab) {
	...
	if (get_option(WPS_OPTIONS_PREFIX.'_long_menu') == "on") {
		...
		include_once(dirname(__FILE__).'/show_tabs_style.php');
		?> <style> .wrap .mail_tab { width: 100px; } </style> <?php
		...
		echo '<div id="mail_tabs">';
		echo '<div class="mail_tab nav-tab-'.$manage_active.'"><a href="admin.php?page=symposium_manage" class="nav-tab-'.$manage_active.'-link">'.__('Manage', WPS_TEXT_DOMAIN).'</a></div>';
		echo '<div class="mail_tab nav-tab-'.$settings_active.'"><a href="admin.php?page=symposium_settings" class="nav-tab-'.$settings_active.'-link">'.__('Settings', WPS_TEXT_DOMAIN).'</a></div>';
		echo '<div class="mail_tab nav-tab-'.$thesaurus_active.'"><a href="admin.php?page=symposium_thesaurus" class="nav-tab-'.$thesaurus_active.'-link">'.__('Thesaurus', WPS_TEXT_DOMAIN).'</a></div>';
		if (function_exists('__wps__forum')) echo '<div class="mail_tab nav-tab-'.$categories_active.'"><a href="admin.php?page=symposium_categories" class="nav-tab-'.$categories_active.'-link">'.__('Categories', WPS_TEXT_DOMAIN).'</a></div>';
		if (function_exists('__wps__forum')) echo '<div class="mail_tab nav-tab-'.$posts_active.'"><a href="admin.php?page=symposium_moderation" class="nav-tab-'.$posts_active.'-link">'.sprintf(__('Forum Posts%s', WPS_TEXT_DOMAIN), $count2).'</a></div>';
		if (function_exists('__wps__mail')) echo '<div class="mail_tab nav-tab-'.$messages_active.'"><a href="admin.php?page=__wps__mail_messages_menu" class="nav-tab-'.$messages_active.'-link">'.__('Mail Messages', WPS_TEXT_DOMAIN).'</a></div>';
		echo '<div class="mail_tab nav-tab-'.$templates_active.'"><a href="admin.php?page=symposium_templates" class="nav-tab-'.$templates_active.'-link">'.__('Templates', WPS_TEXT_DOMAIN).'</a></div>';
		echo '</div>';
