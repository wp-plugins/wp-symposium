...
class wps_ui {
	...
	function get_breadcrumbs($current_catid=0, $param='catid', $show_home=true, $sep='&rarr;', $return_array, $url=WPS_FORUM_URL, $order=100) {
		...
		if (!$parent_cat) {
			...
			if ($parent_cat->cat_parent > 0) {
				$order--;
				$this->get_breadcrumbs($parent_cat->cat_parent, $param, $show_home, $sep, $return_array, $url, $order);
			} else {
				$trail = sub_val_sort($return_array,'order');
				$crumbs = '';
				$count = 0;
				foreach ($trail as $crumb) {
					if ($count++ < count($trail)-1) {
						$crumbs .= "<a href='".$url.symposium_string_query($url)."?".$param."=".$crumb['this_id']."'>".$crumb['cat_title']."</a> ".$sep." ";
					} else {
