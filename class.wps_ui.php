...
class wps_ui {
	...
	function friendship_cancel($id, $cancel_text='Cancel', $done_cancel_text='Cancelled', $button_class='') {
	...
	function poke_button($text='Hey!', $button_class='') {
	...
	function facebook_connect($id, $post_text="Post to Facebook", $connect_text="Connect to Facebook", $cancel_text="Cancel") {
		...
		if ( $facebook_id = __wps__get_meta($id, 'facebook_id') != '') {
			$html .= "<input type='checkbox' CHECKED id='post_to_facebook' /> ";
			$html .= $post_text;
			$html .= " (<a href='javascript:void(0)' id='cancel_facebook'>".$cancel_text."</a>)";
		} else {
			$html .= "<img src='".WPS_PLUGIN_URL."/images/logo_facebook.png' style='float:left; margin-right: 5px;' />";
