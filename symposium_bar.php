...
{  
	...
	if ( is_user_logged_in() ) {
		...
		$use_chatroom = WPS_USE_CHATROOM;
		if (WPS_LITE) { 
			$use_chat = ''; 
			$use_chatroom = '';
		}
		$bar_polling = (WPS_BAR_POLLING*1000);
		$chat_polling = (WPS_CHAT_POLLING*1000);
		$use_wp_profile = WPS_USE_WP_PROFILE;
		$inactive = WPS_ONLINE;
		$offline = WPS_OFFLINE;
		...
		<style>
			...
			#symposium-logout {
				background-image:url('<?php echo WPS_IMAGES_URL; ?>/logout.gif');
				border-radius: <?php echo $border_radius; ?>px;
				-moz-border-radius: <?php echo $border_radius; ?>px;
			}
			...
			.symposium-chatroom-new {
				background-image:url('<?php echo WPS_IMAGES_URL; ?>/chatroomnew.gif');
			}
			.symposium-chatroom-none {
				background-image:url('<?php echo WPS_IMAGES_URL; ?>/chatroom.gif');
			}
			...
			.symposium-email-box-read {
				background-image:url('<?php echo WPS_IMAGES_URL; ?>/email.gif');
			}
			.symposium-email-box-unread {
				background-image:url('<?php echo WPS_IMAGES_URL; ?>/emailunread.gif');
			}
			...
			.symposium-friends-box-none {
				background-image:url('<?php echo WPS_IMAGES_URL; ?>/friends.gif');
			}
			.symposium-friends-box-new {
				background-image:url('<?php echo WPS_IMAGES_URL; ?>/friendsnew.gif');
			}
			.corners {
				border-radius: <?php echo $border_radius; ?>px;
				-moz-border-radius: <?php echo $border_radius; ?>px;
		...
		echo "<!-- NOTIFICATION BAR -->";
			...
			if (is_user_logged_in()) {
				...
				echo "<div id='symposium-chatboxes'>";
					...
					echo "<div id='symposium-who-online'>";
						...
						echo "<div id='symposium-who-online_header' style='width:172px;height:24px;padding:4px 4px 0px 4px;background-color:#000;color:#fff;'>";
							echo "<div id='symposium-who-online_close' style='float:right;cursor:pointer;width:18px; text-align:center'><img src='".WPS_IMAGES_URL."/close.png' alt='".__("Close", "wp-symposium")."' /></div>";
					...
					if ($use_chatroom == 'on') {
						echo "<div id='symposium-chatroom'>";
							...
							echo "<div id='symposium-chatroom_header' class='symposium_readChat'>";
								echo "<div id='symposium-chatroom_close' style='float:right;cursor:pointer;width:18px; text-align:center'><img src='".WPS_IMAGES_URL."/close.png' alt='".__("Close", "wp-symposium")."' /></div>";
								echo "<div id='symposium-chatroom_small' style='margin-right:5px;float:right;cursor:pointer;'><img src='".WPS_IMAGES_URL."/min.gif' title='Minimize' /></div>";
								echo "<div id='symposium-chatroom_big' style='display:none; margin-right:5px;float:right;cursor:pointer;'><img src='".WPS_IMAGES_URL."/max.gif' title='Maximize' /></div>";
				...
				echo "<div id='symposium-logout-div'>";
					...
					echo "</div>";
					echo "<div id='symposium-online-status-div'>";
						echo "<img style='float: left; margin-left: 1px; margin-right: 5px;' src='".WPS_IMAGES_URL."/close.png' alt='".__("Logout", "wp-symposium")."' />";
...
function addChatWindow($id) {
	...
	echo "<div id='chat".$id."' title='chat".$id."' class='chat_window' style='display:none'>";
		...
		echo "<div id='chat".$id."_close' class='chat_close' style='float:right;cursor:pointer;width:18px; text-align:center'><img src='".WPS_IMAGES_URL."/close.png' alt='Close' /></div>";
		echo "<div class='symposium-chat_small' style='margin-right:5px;float:right;cursor:pointer;'><img src='".WPS_IMAGES_URL."/min.gif' title='Minimize' /></div>";
		echo "<div class='symposium-chat_big' style='display:none; margin-right:5px;float:right;cursor:pointer;'><img src='".WPS_IMAGES_URL."/max.gif' title='Maximize' /></div>";
