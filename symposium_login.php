...
function symposium_login() {	
	...
	$get_language = symposium_get_core_language();
	...
	$html = '<div id="symposium-wrapper" style="padding-top:10px">';
		...
		if (!is_user_logged_in()) {
			...
			$html .= '<div style="margin:0px; width:300px;">';

				$html .= '<form id="symposium_login" onsubmit="return validate_form(this)" method="post" action=""> ';
	
					$html .= '<div class="new-topic-subject label">Username</div>';
					...
					$html .= '<div id="symposium_forgotten_password_msg" style="clear:both;display:none;">';
						$html .= '<p>A new password has been sent to your email address.</p>';
					$html .= '</div>';
				
					$html .= '<div style="clear: both; float:right; text-align:right;">';
					$html .= '<a id="symposium_forgotten" href="javascript:void(0);">Forgotten Password</a><br />';
					if ($config->use_wp_register == "on") {
						$html .= '<a href="/wp-login.php?action=register">Register</a>';
					} else {
						$html .= '<a href="'.$config->register_url.'">Register</a>';
					}
					$html .= '</div>';
			
					$html .= '<div style="float:left;">';
					$html .= '<input id="symposium_login" type="submit" class="button" style="float: left; height:46px;" value="OK" />';
					$html .= '</div>';
