...
class wps_mail {
	...
	function get_inbox_count($unread_only=false) {
		global $wpdb;
		$unread = $unread_only ? " AND mail_read = ''" : "";
		$sql = "SELECT COUNT(mail_mid) FROM ".$wpdb->base_prefix."symposium_mail WHERE mail_to = %d AND mail_in_deleted != 'on'".$unread;
		return $wpdb->get_var($wpdb->prepare($sql, $this->id));
	}
	
	function get_inbox($count=10, $start=0, $avatar_size=40, $term="", $order=true, $message_len=75, $unread_only=false) {
	    global $wpdb;
	    $mail_count = 1;
    	$return_arr = array();
