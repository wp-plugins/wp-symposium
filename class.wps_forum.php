...
class wps_forum {
	...
	function delete_post($reply_id) {
		...
		if ($reply) {
			if ($reply->topic_owner == $current_user->ID || current_user_can('manage_options')) {
				...
				if ( (WPS_FORUM_LOCK==0) || ($reply->topic_started > $now) || (current_user_can('level_10')) ) {
	...
	function is_topic_locked($tid) {
		...
		if ( (WPS_FORUM_LOCK==0) || ($topic_started > $now) || (current_user_can('level_10')) ) {
			return false;
		} else {
			return true;
		}
	...
	function add_reply($tid, $reply_text) {
