...
class wps_forum {
	...
	function delete_post($reply_id) {
		...
		if ($reply) {
			if ($reply->topic_owner == $current_user->ID || current_user_can('manage_options')) {
				...
				if ( (get_option('symposium_forum_lock')==0) || ($reply->topic_started > $now) || (current_user_can('level_10')) ) {
	...
	function is_topic_locked($tid) {
		...
		if ( (get_option('symposium_forum_lock')==0) || ($topic_started > $now) || (current_user_can('level_10')) ) {
			return false;
		} else {
			return true;
		}
	...
	function add_reply($tid, $reply_text, $uid=0) {
		...
		if ($tid != '') {
			...
			if (!$duplicate) {
				...
				if (	
					...
					$owner_name = $wpdb->get_var($wpdb->prepare("SELECT display_name FROM ".$wpdb->base_prefix."users WHERE ID = ".$uid));
					...
					if ($topic_approved == "on") {
						...
						$query = $wpdb->get_results("
							...
							WHERE u.ID != ".$uid." AND tid = ".$tid);
							
						if ($query) {						
							foreach ($query as $user) {	
				
								// Filter to allow further actions to take place
								apply_filters ('symposium_forum_newreply_filter', $user->ID, $uid, $owner_name, $url);
					...
					//symposium_sendmail(get_bloginfo('admin_email'), __('POP3 insert failed', 'wp-symposium'), 'Query:'.$wpdb->last_query);
					return false;
				...
				//symposium_sendmail(get_bloginfo('admin_email'), __('POP3 insert failed', 'wp-symposium'), 'Duplicate skipped: '.$wpdb->last_query);
				return false;
			...
			//symposium_sendmail(get_bloginfo('admin_email'), __('POP3 insert failed', 'wp-symposium'), 'No tid passed');
			return false;
