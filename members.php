	...
	$html = '<div class="__wps__wrapper">';
		...
		$html .= "<div class='members_row' style='padding:0px'>";
			...
			if (is_user_logged_in()) {
				$html .= '<div style="clear:both;"><input type="checkbox" id="symposium_member_friends" /> '.__('Show only friends', WPS_TEXT_DOMAIN).'</div>';
			}
			$html .= '</div>';	
										...
										if (is_user_logged_in() && get_option(WPS_OPTIONS_PREFIX.'_show_dir_buttons') && $member->uid != $current_user->ID) {
											if (__wps__pending_friendship($member->uid)) {
												...
												if (!$is_friend) {
													...
													$html .= '<input title="'.$member->uid.'" id="addtext_'.$member->uid.'" type="text" class="addfriend_text input-field" onclick="this.value=\'\'" value="'.sprintf(__('Add as a %s...', WPS_TEXT_DOMAIN), get_option(WPS_OPTIONS_PREFIX.'_alt_friend')).'">';
