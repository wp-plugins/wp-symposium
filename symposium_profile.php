...
{  
	...
	$html = "<div class='symposium-wrapper'>";
	$html .= symposium_smilies(symposium_profile_body(0, $current_user->ID, 0, "stream_".$view, 0));
	$html .= "</div>";
...
{  
	...
	return symposium_show_profile(WPS_PROFILE_DEFAULT);
	exit;
		...
		if ($user) {
			...
			$html .= "<div class='symposium-wrapper'>";
				...
				if ($page != 'header') {
					...
					if ($page == '') { $page = WPS_PROFILE_DEFAULT; }
					
					$template = WPS_TEMPLATE_PROFILE_BODY;
					...
					// Put in busy image
					$template = str_replace("[page]", "<img src='".WPS_IMAGES_URL."/busy.gif' />", stripslashes($template));
