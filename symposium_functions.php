...
function symposium_make_url($text) {
	...
	if (strpos($text, 'youtube.com') === FALSE) {
		...
		$url = "<img src='".WP_PLUGIN_URL."/wp-symposium/images/video.gif' title='".$text."' alt='".$text."' />";;
	}
	...
	if ( is_user_logged_in() ) {
		...
		if ($uid1 == $uid2) {
			...
			if (symposium_friend_of($uid1)) {
				...
				// A friend
				// Send mail
				$actions .= '<input type="submit" class="symposium-button" id="profile_send_mail_button" onclick="document.location = \''.$url.symposium_string_query($url).'view=compose&to='.$uid1.'\';" value="'.__('Send Mail', 'wp-symposium').'">';
		...
		if (is_user_logged_in() || $privacy == 'public') {	
			...
			if ( ($uid1 == $uid2) || (strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'public') || (strtolower($privacy) == 'friends only' && $is_friend) || symposium_get_current_userlevel() == 5) {
					...
					$html .= "<div id='symposium_wall'>";
						...
						if ( ($uid1 != '') && ( ($uid1 != $uid2) || ($is_friend)) && (is_user_logged_in()) ) {
							// Post Comment Input
								...
								$html .= "<div id='".$comment->cid."' class='wall_post_div'>";
									...
									if ($replies) {
										...
										foreach ($replies as $reply) {
											...
											$html .= "<div id='".$reply->cid."' class='reply_div'>";
