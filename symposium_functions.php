...
function symposium_profile_header($uid1, $uid2, $url, $display_name) {
	...
	if ( is_user_logged_in() ) {
		...
		if ($uid1 == $uid2) {
			...
			// Status Input
			$actions .= '<input type="text" id="symposium_status" name="status" class="input-field"  onblur ="this.value=(this.value==\'\') ? \''.$whatsup.'\' : this.value;"  onfocus="this.value=(this.value==\''.$whatsup.'\') ? \'\' : this.value;"  value="'.$whatsup.'"';
			if (get_option('symposium_show_buttons')) $actions .= ' style="width:250px"';
			$actions .= ' />';
