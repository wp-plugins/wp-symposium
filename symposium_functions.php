				...
				$html .= "<div id='friend_".$friend->friend_to."' style='clear:right; margin-top:8px; overflow: auto; margin-bottom: 15px; '>";		
					...
					$html .= "<div style='padding-left:74px;'>";
						...
						$sql = "SELECT comment
								...
								WHERE author_uid = %d AND subject_uid = %d
								ORDER BY cid DESC
								LIMIT 0,1";
						$comment = $wpdb->get_var($wpdb->prepare($sql, $friend->friend_to, $friend->friend_to));
						$html .= symposium_make_url(stripslashes($comment));
					$html .= "</div>";
					...
					if ($friend->friend_accepted == 'on') {
						if ($uid == $current_user->ID) {
							...
							$html .= "<div style='margin-top:10px;'>";
								...
								$html .= "<div style='float:right;'>";
									$html .='<input type="submit" value="'.__('Send Mail', 'wp-symposium').'" class="symposium-button" onclick="document.location = \''.$mailpage.$q.'view=compose&to='.$friend->friend_to.'\';">';
								$html .= "</div>";
...
function symposium_profile_header($uid1, $uid2, $url, $display_name) {
	...
	if ($config->use_poke == 'on' && is_user_logged_in() && $uid1 != $uid2) {
		$poke = "Poke";
		$html = str_replace("[poke]", '<input type="submit" value="'.$config->poke_label.'" class="symposium-button poke-button">', $html);
	} else {
	...
	if ( ($uid1 == $uid2) || (strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'public') || (strtolower($privacy) == 'friends only' && symposium_friend_of($uid1)) ) {
		...
		if (is_user_logged_in() || $privacy == 'public') {	
			...
			if ( ($uid1 == $uid2) || (strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'public') || (strtolower($privacy) == 'friends only' && $is_friend) || symposium_get_current_userlevel() == 5) {
					...
					$html .= "<div id='symposium_wall'>";
						...
						if ( ($post == '') && ($uid1 != '') && ( ($uid1 != $uid2) || ($is_friend)) && (is_user_logged_in()) ) {
							// Post Comment Input
						...
						// See if author of one of the replies
						$in_reply = false;
						if ($post != '') {
							$sql = "SELECT author_uid FROM ".$wpdb->prefix."symposium_comments 
									WHERE comment_parent = %d
									  AND author_uid = %d";
							$replies = $wpdb->get_results($wpdb->prepare($sql, $post, $current_user->ID));
							if ($replies) { 
								$in_reply = true;
							}
						}	
						
						if ( ($in_reply && $post != '' && symposium_safe_param($post)) ) {
							...
							if ($version == "all_activity") {
								...
								FROM ".$wpdb->base_prefix."symposium_comments c 
								LEFT JOIN ".$wpdb->base_prefix."users u ON c.author_uid = u.ID 
								LEFT JOIN ".$wpdb->base_prefix."users u2 ON c.subject_uid = u2.ID WHERE c.comment_parent = 0 AND c.is_group != 'on' 
								ORDER BY c.comment_timestamp DESC LIMIT %d,%d";					
							...
							if ($version == "friends_activity") {
								...
								LEFT JOIN ".$wpdb->base_prefix."users u ON c.author_uid = u.ID 
								LEFT JOIN ".$wpdb->base_prefix."users u2 ON c.subject_uid = u2.ID 
								WHERE ( (c.subject_uid = %d) OR (c.author_uid = %d) 
							...
							if ($version == "wall") {
								...
								$sql = "SELECT c.*, u.display_name, u2.display_name AS subject_name 
										...
										LEFT JOIN ".$wpdb->base_prefix."users u ON c.author_uid = u.ID 
										LEFT JOIN ".$wpdb->base_prefix."users u2 ON c.subject_uid = u2.ID 
										WHERE ( (c.subject_uid = %d OR c.author_uid = %d) AND c.comment_parent = 0 AND c.is_group != 'on' )
						...
						if ($comments) {
							...
							foreach ($comments as $comment) {
								...
								if ( ($comment->author_uid == $uid1) || (strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'public') || (strtolower($privacy) == 'friends only' && symposium_friend_of($comment->author_uid)) ) {
