		...
		if ( ($uid1 == $uid2) || (strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'friends only' && $is_friend) || symposium_get_current_userlevel() == 5) {
				...
				$html .= "<div id='symposium_wall'>";
					...
					if ($post != '' && symposium_safe_param($post)) {
						...
						if ($version == "wall") {
							$sql = "SELECT c.*, u.display_name, u2.display_name AS subject_name FROM ".$wpdb->base_prefix."symposium_comments c LEFT JOIN ".$wpdb->base_prefix."users u ON c.author_uid = u.ID LEFT JOIN ".$wpdb->base_prefix."users u2 ON c.subject_uid = u2.ID WHERE (c.subject_uid = ".$uid1." OR c.author_uid = ".$uid1.") AND c.comment_parent = 0 AND c.is_group != 'on' ORDER BY c.comment_timestamp DESC LIMIT ".$limit_from.",".$limit_count;
						}
					...
					if ($comments) {
						foreach ($comments as $comment) {
							...
							$html .= "<div id='".$comment->cid."' class='wall_post_div'>";
								...
								{
									$html .= '<div>';
									$html .= '<input id="symposium_reply_'.$comment->cid.'" type="text" title="'.$comment->cid.'" class="input-field reply_field symposium_reply" onblur="this.value=(this.value==\'\') ? \''.__('Write a comment...', 'wp-symposium').'\' : this.value;" onfocus="this.value=(this.value==\''.__('Write a comment...', 'wp-symposium').'\') ? \'\' : this.value;" value="'.__('Write a comment...', 'wp-symposium').'" />';
