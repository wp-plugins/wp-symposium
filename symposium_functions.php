...
function symposium_getTopic($topic_id, $group_id=0) {
	...
	// Can view topic?
	$can_view = false;
	$viewer = strtolower(get_option('symposium_viewer'));
	if ($capabilities) {
		foreach ( $capabilities as $role => $name ) {
			if ($role) {
				...
				$html .= 'Checking role '.$role.' against '.$viewer.'<br />';
	...
	// Can create topic?
	$can_edit = false;
	$viewer = strtolower(get_option('symposium_forum_editor'));
	if ($capabilities) {
		foreach ( $capabilities as $role => $name ) {
			if ($role) {
				$role = strtolower($role);
	...
	$can_reply = false;
	$viewer = strtolower(get_option('symposium_forum_reply'));
	if ($capabilities) {
		foreach ( $capabilities as $role => $name ) {
			if ($role) {
				$role = strtolower($role);
	...
	if ( $can_view ) {
		...
		if ($post) {
					...
					if ($this_level) { 
						...
						if ($this_level->cat_parent == 0) { 
							if (get_option('symposium_forum_ajax') == 'on') { 
								...
								if (get_option('wps_permalink_structure') && $group_id == 0) {
			...
			if ($can_reply) {
				...
				if ($group_id == 0) {
					$html .= "<p>".__("You are not permitted to reply on this forum.", "wp-symposium")."</p>";				
			...
			if (is_user_logged_in()) {
				...
				if ( ($can_edit) && ($allow_new == 'on') ) {
					...
					if ($group_id == 0 && $allow_new == 'on') {
						$new_topic_form = "<p>".__("You are not permitted to start a new topic.", "wp-symposium")."</p>";	
					}
...
function symposium_create_stub($text) {
	...
	$sql = "SELECT COUNT(*) FROM ".$wpdb->prefix."symposium_topics WHERE stub LIKE '%".$stub."%'";
...
symposium_get_stub_id($cat, 'forum-cat');
