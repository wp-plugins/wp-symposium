...
function symposium_getTopic($topic_id, $group_id=0) {
	...
	$can_comment = false;
	$viewer = str_replace('_', '', str_replace(' ', '', strtolower(get_option('symposium_forum_reply_comment'))));
	if ($capabilities) {
		foreach ( $capabilities as $role => $name ) {
			if ($role) {
				$role = strtolower($role);
	...
	if (strpos($viewer, __('everyone', 'wp-symposium')) !== FALSE) $can_comment = true;
	if ($group_id > 0) {
		$sql = "SELECT COUNT(*) FROM ".$wpdb->prefix."symposium_group_members WHERE group_id=%d AND valid='on' AND member_id=%d";
		$member_count = $wpdb->get_var($wpdb->prepare($sql, $group_id, $current_user->ID));
	...
	if ($group_id > 0) {
		if (symposium_member_of($group_id) != "yes") $can_view = false;
	} else {
	...
	if ( $can_view ) {
		...
		if ($post) {
									...
									if ( ($file != "." && $file != ".." && $file != ".DS_Store") && (!is_dir($targetPath.'/'.$file)) ) {
										...
										if (strpos(get_option('symposium_image_ext'), $ext[sizeof($ext)-1]) > 0) {
											...
											if (get_option('symposium_forum_thumbs') == 'on') {
												...
												$html .= '<img src="'.$url.'" rev="'.$cnt_cont.'" rel="symposium_gallery_photos_'.$post->tid.'" class="wps_gallery_album" style="width:16px; height:16px" />';
												$html .= '</a> ';
												$cnt_cont++;
												$html .= $file;
											}
						...
						$reply_html .= "<div id='reply".$child->tid."' class='child-reply";
											...
											if ($file != "." && $file != ".." && $file != ".DS_Store") {
												...
												if (strpos(get_option('symposium_image_ext'), $ext[sizeof($ext)-1]) > 0) {
													...
													$reply_html .= '</a> ';
														...
														$reply_html .= '<img src="'.$url.'" rev="'.$cnt_cont.'" rel="symposium_gallery_photos_'.$post->tid.'" class="wps_gallery_album" style="width:16px; height:16px" />';
														$reply_html .= '</a> ';
														$reply_html .= $file;
														$cnt_cont++;
													}
										...
										$reply_html .= "<div id='comment".$comment->tid."' class='reply-comments-reply'>";
											...
											$reply_html .= "<div style='margin-left: 45px;'>";
												...
												$reply_html .= "<div class='topic-edit-delete-icon'>";
													// Report warning (if being used) 
													if (get_option('symposium_allow_reports') == 'on') {
														$reply_html .= "<a title='comment_".$comment->tid."' href='javascript:void(0);' style='padding:0px' class='report_post symposium_report reply_warning'><img src='".get_option('symposium_images')."/warning.png' style='width:14px;height:14px' /></a>";
													}
													// Delete comment
													if ( ($comment->topic_owner == $current_user->ID) || (current_user_can('level_10')) ) {
														$reply_html .= "<a href='javascript:void(0)' class='floatright link_cursor delete_forum_reply' style='display:none' id='".$comment->tid."'><img src='".get_option('symposium_images')."/delete.png' /></a>";
													}
												$reply_html .= "</div>";


												$reply_html .= "<div class='started-by'>".symposium_profile_link($comment->topic_owner)." ".__('commented', 'wp-symposium')." ".symposium_time_ago($comment->topic_date)."</div>";
												$reply_html .= symposium_buffer(stripslashes($comment->topic_post));
											$reply_html .= "</div>";
										$reply_html .= "</div>";
									}
									$reply_html .= "</div>";
							...
							if ($can_comment) {
								$reply_html .= "<div class='quick-comment-box-show'>";
									$reply_html .= '<a class="quick-comment-box-show-link" href="javascript:void(0);">'.__('Add a quick comment...', 'wp-symposium').'</a>';
								$reply_html .= "</div>";
							}
							...
							$reply_html .= "<div class='quick-comment-box'>";
								...
								$reply_html .= '<input type="submit" rel="'.$child->tid.'" class="quick-comment-box-add symposium-button" value="'.__("Add Comment", "wp-symposium").'" />';
							$reply_html .= "</div>";
			...
			if ($can_reply) {
				...
				$html .= '<div id="reply-topic-bottom" name="reply-topic-bottom" style="width:'.get_option('symposium_use_wysiwyg_width').'px">';
				if ($wpdb->get_var($wpdb->prepare("SELECT allow_replies FROM ".$wpdb->prefix."symposium_topics WHERE tid = %d", $post->tid)) == "on")
				{
					...
					if (get_option('symposium_use_wp_editor')) {
						// WordPress TinyMCE
						$settings = array(
						    'wpautop' => true,
						    'media_buttons' => false,
				...
				if ($group_id == 0) {
					...
					if (symposium_get_current_userlevel() == 5) $html .= sprintf(__('<br />Permissions are set via the WordPress admin dashboard->%s->Forum.', 'wp-symposium'), WPS_WL_SHORT);	
					$html .= "</p>";
		...
		if ($wpdb->get_var($wpdb->prepare($sql, $topic_id))) {
			...
			if (symposium_get_current_userlevel() == 5) $html .= '<br /><br />'.sprintf(__('Permissions are set via the WordPress admin dashboard->%s->Forum.', 'wp-symposium'), WPS_WL_SHORT);
		} else {
				...
				if ($categories) {
					...
					$html .= '<div id="symposium_table">';
						...
						foreach($categories as $category) {
							...
							if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos($cat_roles, $user_role.',') !== FALSE) {
	...
	global $wpdb, $current_user;
		...
		if ($uid1 > 0) {
			...
			if ($uid1 == $uid2 || symposium_get_current_userlevel() == 5) {
				if (get_option('symposium_profile_avatars') == 'on' && get_option('symposium_menu_avatar')) {
					$html .= '<div id="menu_avatar" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_avatar_text')) != '' ? $t :  __('Profile Photo', 'wp-symposium')).'</div>';
				}
				if (get_option('symposium_menu_details'))
					$html .= '<div id="menu_personal" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_details_text')) != '' ? $t :  __('Profile Details', 'wp-symposium')).'</div>';
				if (get_option('symposium_menu_settings'))
					$html .= '<div id="menu_settings" class="symposium_profile_menu">'.(($t = get_option('symposium_menu_settings_text')) != '' ? $t :  __('Community Settings', 'wp-symposium')).'</div>';
...
function symposium_sendmail($email, $subject, $msg)
{
	...
	$template =  str_replace('[powered_by_message]', sprintf(__('Powered by %s - Social Networking for WordPress', 'wp-symposium'), WPS_WL), $template);
								...
								if ($private_group != 'on') {
									...
									if  ( ($comment->type != 'poke') || ($comment->type == 'poke' && ($comment->author_uid == $uid2 || $comment->subject_uid == $uid2 )) ) {	
										...
										$html .= "<div class='wall_post_div' id='post_".$comment->cid."'>";
											...
											// Like/dislike icons
											if (get_option('symposium_activity_likes') && is_user_logged_in() && $comment->author_uid != $uid2 ) {
												...
												if (!$already_liked) {
													$html .= "<div class='wall_post_like delete_post_top'>";
														$html .= "<img class='wall_add_like' title='".__('You like this.', 'wp-symposium')."' data-action='like' rel='".$comment->cid."' style='width:20px;height:20px;' src='".get_option('symposium_images')."/smilies/good.png' />";
														$html .= "<img class='wall_add_like' title='".__('You do not like this.', 'wp-symposium')."'data-action='dislike' rel='".$comment->cid."' style='width:20px;height:20px' src='".get_option('symposium_images')."/smilies/bad.png' />";
													$html .= "</div>";
												}
											}	
											...
											// Likes/Dislikes
											if (get_option('symposium_activity_likes')) {
												...
												$html .= "<div id='symposium_likes_".$comment->cid."'>";
													if ($likes) {
														$link = '<a id="symposium_show_likes" href="javascript:void(0)" rel="'.$comment->cid.'">';
														$html .= "<img src='".get_option('symposium_images')."/smilies/good.png' style='width:16px;height:16px' /> ";
														if ($youlike) {
															$html .= __('You', 'wp-symposium');
															$likes--;
															if ($likes > 1) {
																$html .= ' '.sprintf(__('and %s%d others</a> like this.', 'wp-symposium'), $link, $likes);
															}
															if ($likes == 1) {
																$html .= ' '.sprintf(__('and %s1 other</a> person likes this.', 'wp-symposium'), $link);
															}
															if ($likes == 0) {
																$html .= ' '.__('like this.', 'wp-symposium');
															}
														} else {
															if ($likes > 1) {
																$html .= sprintf(__('%s%d people</a> like this.', 'wp-symposium'), $link, $likes);
															}
															if ($likes == 1) {
																...
																$html .= symposium_profile_link($uid).' '.__('likes this.', 'wp-symposium');
															}															
														}
													}
													if ($dislikes) {
														...
														$link = '<a id="symposium_show_likes" href="javascript:void(0)" rel="'.$comment->cid.'">';
														$html .= "<img src='".get_option('symposium_images')."/smilies/bad.png' style='width:16px;height:16px' /> ";
														if ($youdislike) {
															$html .= __('You', 'wp-symposium');
															$dislikes--;
															if ($dislikes > 1) {
																$html .= ' '.sprintf(__('and %s%d others</a> don\'t like this.', 'wp-symposium'), $link, $dislikes);
															}
															if ($dislikes == 1) {
																$html .= ' '.sprintf(__('and %s1 other</a> person don\'t like this.', 'wp-symposium'), $link);
															}
															if ($dislikes == 0) {
																$html .= ' '.__('don\'t like this.', 'wp-symposium');
															}
														} else {
															if ($dislikes > 1) {
																$html .= sprintf(__('%s%d people</a> don\'t like this.', 'wp-symposium'), $link, $dislikes);
															}
															if ($dislikes == 1) {
																$sql = "SELECT uid FROM ".$wpdb->base_prefix."symposium_likes WHERE cid = %d AND type = 'dislike'";
																$uid = $wpdb->get_var($wpdb->prepare($sql, $comment->cid));
																$html .= symposium_profile_link($uid).' '.__('doesn\'t like this.', 'wp-symposium');
															}															
														}
													}
												$html .= "</div>";
												...
												if ($photos) {
	   												foreach ($photos as $photo) {	
														...
														if (get_option('symposium_img_db') == "on") {
											    	        ...
											    	        $thumb_src = get_option('symposium_img_url').'/members/'.$photo->owner.'/media/'.$aid.'/thumb_'.$photo->name;
														}
														...
														if (($action == 'added' && $cnt == 1) || ($action == 'comment' && $iid == $single_iid)) {
															$image = preg_replace('/thumb_/', 'show_', $image, 1);														
										  					$text .= '<a class="symposium_photo_cover_action wps_gallery_album" data-name="'.stripslashes($title).'" data-iid="'.$iid.'" href="'.$image.'" rev="'.$cnt.'" rel="symposium_gallery_photos_'.$aid.'" title="'.$name.'">';
														...
														if ($action == 'added') {
															if (sizeof($photos) > 2) {
																...
																if ($cnt > 1 && $cnt <= sizeof($photos)) {
												  					$text .= '<a class="symposium_photo_cover_action wps_gallery_album" data-name="'.stripslashes($title).'" data-owner="'.$photo->owner.'" data-iid="'.$iid.'" href="'.$image.'" rev="'.$cnt.'" rel="symposium_gallery_photos_'.$aid.'" title="'.$name.'">';
													...
													$html .= "<div id='".$reply->cid."' class='reply_div' style='".$reply_style."'>";
														$html .= "<div class='wall_reply_div'>";
															$html .= "<div class='wall_reply'>";
																...
																if (get_option('symposium_activity_likes') && is_user_logged_in() && $reply->author_uid != $uid2 ) {
																	$sql = "SELECT COUNT(*) FROM ".$wpdb->base_prefix."symposium_likes WHERE cid = %d AND uid = %d";
																	...
																	if (!$already_liked) {
																		$html .= "<div class='wall_post_like delete_reply' style='margin:0;padding:0;'>";
																			$html .= "<img class='wall_add_like' title='".__('You like this.', 'wp-symposium')."' data-action='like' rel='".$reply->cid."' style='padding:0;width:20px;height:20px;' src='".get_option('symposium_images')."/smilies/good.png' />";
																			$html .= "<img class='wall_add_like' title='".__('You do not like this.', 'wp-symposium')."'data-action='dislike' rel='".$reply->cid."' style='padding:0;width:20px;height:20px' src='".get_option('symposium_images')."/smilies/bad.png' />";
																		$html .= "</div>";
																	}
																}	
																...
																if (get_option('symposium_activity_likes')) {
																	...
																	$html .= "<div id='symposium_likes_".$reply->cid."'>";
																		if ($likes) {
																			...
																			if ($youlike) {
																				...
																				if ($likes == 1) {
																					$html .= ' '.sprintf(__('and %s1 other person</a> likes this.', 'wp-symposium'), $link);
																				}
																				if ($likes == 0) {
																					$html .= ' '.__('like this.', 'wp-symposium');
																				}
							...
							$html .= "<div class='symposium_recent_activity_row'>";		
								...
								$html .= "<div class='symposium_recent_activity_row_post'>";
									...
									if (strpos($text, 'symposium_photo_image')) {
											...
											$html .= symposium_time_ago($post->comment_timestamp)." ".symposium_profile_link($post->author_uid)." ".$text."<br>";
									} else {
										$html .= symposium_profile_link($post->author_uid)." ".$text." ".symposium_time_ago($post->comment_timestamp).".<br>";
									}
								$html .= "</div>";
							$html .= "</div>";
...
// Recent forum posts
function do_Forumrecentposts_Widget($postcount,$preview,$cat_id,$show_replies,$incl_cat,$incl_parent) {
									...
									if ($post->topic_parent > 0) {
										...
										if ($preview > 0) {
											...
											if ($post->parent_parent == 0) {
												...
												$html .= " ".__('commented', 'wp-symposium');
											}
											if (get_option('symposium_permalink_structure') && $group_id == 0) {
											...
											if ($incl_parent) {
												if (get_option('symposium_permalink_structure') && $group_id == 0) {
													$perma_cat = symposium_get_forum_category_part_url($post->parent_category);
													$html .= ' '.__('to', 'wp-symposium')." <a href='".$forum_url.'/'.$perma_cat.$post->parent_stub."'>".strip_tags(stripslashes($post->parent_subject))."</a> ";
												} else {
													if ($post->parent_parent == 0) {
														$html .= ' '.__('to', 'wp-symposium')." <a href='".$forum_url.$forum_q."cid=".$post->parent_category."&show=".$post->topic_parent."'>".strip_tags(stripslashes($post->parent_subject))."</a> ";
													}
												}
											}
											if ($incl_cat) {
												if (get_option('symposium_permalink_structure') && $group_id == 0) {
													...
													$perma_cat = symposium_get_forum_category_part_url($post->parent_category);
													$html .= ' '.__('in', 'wp-symposium')." <a href='".$forum_url.'/'.$perma_cat."'>".$cat."</a> ";
												} else {
													$html .= ' '.__('in', 'wp-symposium')." <a href='".$forum_url.$forum_q."cid=".$post->parent_category."'>".$cat."</a> ";
												}
											}
										} else {
											$html .= "<br />";
										...
										if ($preview > 0) {
											...
											if ($incl_cat) {
												if (get_option('symposium_permalink_structure') && $group_id == 0) {
													...
													$html .= ' '.__('in', 'wp-symposium')." <a href='".$forum_url.$forum_q."cid=".$post->topic_category."'>".$cat."</a> ";
												}
											}
										}
										$html .= " ".symposium_time_ago($post->topic_started).".";
