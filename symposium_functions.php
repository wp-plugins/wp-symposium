	...
	if ( is_user_logged_in() ) {
		...
		if ($uid1 == $uid2) {
			...
			$actions .= '<input type="text" id="symposium_status" name="status" class="input-field" 
				onblur ="this.value=(this.value==\'\') ? \''.addslashes(WPS_STATUS_POST).'\' : this.value;" 
				onfocus="this.value=(this.value==\''.addslashes(WPS_STATUS_POST).'\') ? \'\' : this.value;" 
				value="'.stripslashes(WPS_STATUS_POST).'"';
			if (WPS_SHOW_BUTTONS) {
										...
										$html .= "<div id='".$reply->cid."' class='reply_div' style='".$reply_style."'>";
											$html .= "<div class='wall_reply_div'>";
												$html .= "<div class='wall_reply'>";
													...
													if (WPS_ALLOW_REPORTS == 'on') {
														$html .= " <a title='post_".$reply->cid."' href='javascript:void(0);' class='report_post symposium_report reply_warning'><img src='".WPS_IMAGES_URL."/warning.png' style='width:16px;height:16px' /></a>";
													}
...
function do_symposium_summary_Widget($show_loggedout,$show_form,$login_url) {
