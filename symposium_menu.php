...
function symposium_plugin_menu() {
	...
	if (wps_is_wpmu()) {
		...
		add_submenu_page('symposium_debug', __('Release Notes', 'wp-symposium'), __('Release Notes', 'wp-symposium'), 'manage_options', 'symposium_notes', 'symposium_plugin_notes');
	...
	global $wpdb;
	    ...
	    // See if the user has posted profile settings
			...
			if ( function_exists('symposium_profile_plus') ) { 
	    		if (isset($_POST[ 'wps_show_hoverbox' ])) 	{ $wps_show_hoverbox = $_POST['wps_show_hoverbox']; } 				else { $wps_show_hoverbox = ''; }
				$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix."symposium_config SET wps_show_hoverbox = '".$wps_show_hoverbox."'") );
			}
		...
		$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));					
				...
				echo '<div class="metabox-holder"><div id="toc" class="postbox">';
					...
					if ( function_exists('symposium_profile_plus') ) { 
						$wps_show_hoverbox = $config->wps_show_hoverbox;
					}
					...
					<?php if ( function_exists('symposium_profile_plus') ) { ?>
						...
						<tr valign="top"> 
						<td colspan=2><strong>Profile Plus</td>
						</tr> 
	
						<tr valign="top"> 
						<td scope="row"><label for="wps_show_hoverbox"><?php echo __('Enable hover box', 'wp-symposium'); ?></label></td>
						<td>
						<input type="checkbox" name="wps_show_hoverbox" id="wps_show_hoverbox" <?php if ($wps_show_hoverbox == "on") { echo "CHECKED"; } ?>/>
...
// yyy
			...
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET forum_lock = ".$forum_lock) );		
		...
		<div class="metabox-holder"><div id="toc" class="postbox"> 
			...
			<td><input name="forum_lock" style="width:50px" type="text" id="forum_lock"  value="<?php echo $forum_lock; ?>" /> 
...
function symposium_mail_messages_menu() {
	...
	if (isset($_GET['mail_mid_del'])) {
		...
		if (symposium_safe_param($_GET['mail_mid_del'])) {
			// Update
			$wpdb->query( $wpdb->prepare( "DELETE FROM ".$wpdb->prefix."symposium_mail WHERE mail_mid = %d", $_GET['mail_mid_del'] ) );
		} else {
	...
	echo '<div class="mail_message_dialog"></div>';
  	...
  	echo '<div class="wrap">';
	  	...
	  	$all = $wpdb->get_var("SELECT count(*) FROM ".$wpdb->prefix."symposium_mail"); 
		// Paging info
		$showpage = 0;
		$pagesize = 20;
		$numpages = floor($all / $pagesize);
...
function symposium_members_menu() {
    ...
    if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == 'symposium_members_menu' ) {
		...
		$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->prefix.'symposium_config'." SET dir_level = %s", serialize($level)  ) );
	...
	// Get values to show
	$config = $wpdb->get_row($wpdb->prepare("SELECT dir_level, show_dir_buttons, dir_page_length FROM ".$wpdb->prefix.'symposium_config'));
  	...
  	echo '<div class="wrap">';
		...
		<div class="metabox-holder"><div id="toc" class="postbox"> 
			...
			<?php
				...
				$dir_roles = unserialize($config->dir_level);
