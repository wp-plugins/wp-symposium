				...
				if (isset($_GET['tid'])) {
					...
					if (symposium_safe_param($_GET['tid'])) {
						...
						if ($post->topic_parent > 0) {						
							...
							do_action('symposium_forum_newtopic_hook', $post->topic_owner, $post->display_name, $post_url, 'forum');	
						}
	...
	wp_get_current_user();
 	...
 	$wpdb->show_errors();
  	...
  	echo '<div class="wrap">';
		...
		echo '<div id="admin_image_to_crop"></div><br />';
		echo "</div>";	 
		...
		echo "<div>";
		    ...
		    if( isset($_POST[ 'symposium_bbpress' ]) && $_POST[ 'symposium_bbpress' ] == 'Y' ) {
		    	...
		    	if ($cat_title != '') {
					...
					echo __('Please enter a new forum category title', 'wp-symposium');
					echo "</div>";		    	
		    	}
			}
			...
			if ($forums) {
			   	echo '<p>'.__('If you have bbPress v2 forums, you can migrate them to your WP Symposium forum as a category.', 'wp-symposium').'</p>';
			   	echo '<p>'.__('This migration works with the v2 WordPress bbPress plugin. If you are running a previous or stand-alone version of bbPress, you should upgrade your installation first.', 'wp-symposium').'</p>';
				...
				echo '</select><br />';
			   	...
			   	echo '<p><em>This feature is not yet available, currently being tested</em></p>';
	...
	global $wpdb;
		...
		$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));
				...
				echo '<div class="metabox-holder"><div id="toc" class="postbox">';
					...
					<tr valign="top">
					<td scope="row"><label for="wps_js_file">WPS JS file</label></td> 
					<td>
					...
					<tr valign="top">
					<td scope="row"><label for="wps_css_file">WPS CSS file</label></td> 
					<td>
