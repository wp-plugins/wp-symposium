...
function symposium_plugin_menu() {
	...
	// Build menu for admins but not super admins (on WPMU installations)
	if (!(current_user_can('update_core')) && current_user_can('edit_themes')) {
		add_menu_page('Symposium','Symposium'.$count1, 'read', 'symposium_wpmu_debug', 'symposium_plugin_debug', '', 7); 
//		add_submenu_page('symposium_wpmu_debug', __('Installation', 'wp-symposium'), __('Installation', 'wp-symposium'), 'read', 'symposium_debug', 'symposium_plugin_debug');
		add_submenu_page('symposium_wpmu_debug', __('Styles', 'wp-symposium'), __('Styles', 'wp-symposium'), 'read', 'symposium_styles', 'symposium_plugin_styles');
		if (function_exists('symposium_profile')) {
			add_submenu_page('symposium_wpmu_debug', __('Profile', 'wp-symposium'), __('Profile', 'wp-symposium'), 'read', 'symposium_profile', 'symposium_plugin_profile');
		}
		if (function_exists('symposium_forum')) {
			add_submenu_page('symposium_wpmu_debug', __('Forum', 'wp-symposium'), __('Forum', 'wp-symposium'), 'read', 'symposium_forum', 'symposium_plugin_forum');
			add_submenu_page('symposium_wpmu_debug', __('Forum Categories', 'wp-symposium'), __('Forum Categories', 'wp-symposium'), 'read', 'symposium_categories', 'symposium_plugin_categories');
			add_submenu_page('symposium_wpmu_debug', __('Forum Posts', 'wp-symposium'), sprintf(__('Forum Posts %s', 'wp-symposium'), $count2), 'read', 'symposium_moderation', 'symposium_plugin_moderation');
		}
		if (function_exists('add_notification_bar')) {
			add_submenu_page('symposium_wpmu_debug', __('Panel', 'wp-symposium'), __('Panel', 'wp-symposium'), 'read', 'symposium_bar', 'symposium_plugin_bar');
		}
		if (function_exists('symposium_members')) {
			add_submenu_page('symposium_wpmu_debug', __('Member Directory', 'wp-symposium'), __('Member Directory', 'wp-symposium'), 'read', 'symposium_members_menu', 'symposium_members_menu');
		}
	} else {
		// Build menu for admin's in single installs or if network administrator
...
function symposium_plugin_notes() {
  	...
  	echo '<div class="wrap">';
		...
		echo '<div style="clear:both; float:left; width:250px;">';
		
		$rss = fetch_feed( 'http://www.wpsymposium.com/feed/?cat=19' );
		...
		foreach ( $rss_items as $item ) {
			...
			$title = str_replace(" release notes", "", strtolower($title));
			$title = str_replace("v", "", $title);
			$title .= __(" release notes", "wp-symposium");
			$content = $item->get_content();
			$date = '';
			$link = esc_url( strip_tags( $item->get_link() ) );	
			
			echo '<a class="wps_show_notes" title="'.$cnt.'" href="javascript:void(0)">'.$title.'</a><br />';
			
			echo '<div id="wps_content_'.$cnt.'" style="display:none">';
			echo '<a href="'.$link.'" target="_blank">'.$link.'</a><br />'.$content;
			echo '</div>';
		...
		echo '<br />';
		echo __('View posts on the', 'wp-symposium');
		echo ' <a href="http://www.wpsymposium.com/blog" target="_blank">WPS Blog</a>:<br /><br />';

		$rss = fetch_feed( 'http://www.wpsymposium.com/feed/?cat=14' );
		$maxitems = $rss->get_item_quantity(0); 	    
		echo ' <a href="http://www.wpsymposium.com/category/article/" target="_blank">'.__("Articles", "wp-symposium").'</a> ('.$maxitems.')<br />';
	    	
		$rss = fetch_feed( 'http://www.wpsymposium.com/feed/?cat=21' );
		$maxitems = $rss->get_item_quantity(0); 	    
		echo ' <a href="http://www.wpsymposium.com/category/featured/" target="_blank">'.__("Featured", "wp-symposium").'</a> ('.$maxitems.')<br />';
	    	
		$rss = fetch_feed( 'http://www.wpsymposium.com/feed/?cat=19' );
		$maxitems = $rss->get_item_quantity(0); 	    
		echo ' <a href="http://www.wpsymposium.com/category/releases/" target="_blank">'.__("Release Notes", "wp-symposium").'</a> ('.$maxitems.')<br />';
	    	
		$rss = fetch_feed( 'http://www.wpsymposium.com/feed/?cat=15' );
		$maxitems = $rss->get_item_quantity(0); 	    
		echo ' <a href="http://www.wpsymposium.com/category/tips-and-tricks/" target="_blank">'.__("Tips &amp Tricks", "wp-symposium").'</a> ('.$maxitems.')<br />';
	    	
		$rss = fetch_feed( 'http://www.wpsymposium.com/feed/?cat=51' );
		$maxitems = $rss->get_item_quantity(0); 	    
		echo ' <a href="http://www.wpsymposium.com/category/tutorials/" target="_blank">'.__("Tutorials", "wp-symposium").'</a> ('.$maxitems.')<br />';
	    	
		$rss = fetch_feed( 'http://www.wpsymposium.com/feed/?cat=1' );
		$maxitems = $rss->get_item_quantity(0); 	    
		echo ' <a href="http://www.wpsymposium.com/category/uncategorized/" target="_blank">'.__("Uncategorized", "wp-symposium").'</a> ('.$maxitems.')<br />';
	    	

		echo '</div>';

		echo '<style>';
		echo '#wps_notes_display img {';
		echo 'box-shadow: 0 0 4px rgba(0, 0, 0, 0.4);';
		echo 'margin:6px;';
		echo 'padding:6px;';
		echo '} </style>';
		echo '<div id="wps_notes_display" style="width:600px; float:left;">';
	  	echo __('The articles listed here are fed from an RSS feed at', 'wp-symposium').' <a href="http://www.wpsymposium.com/blog" target="_blank">http://www.wpsymposium.com/blog</a>.';
		echo '</div>';
    ...
    $template_group_forum_topic = str_replace("[]", chr(13), stripslashes($config->template_group_forum_topic));
  	...
  	echo '<div class="wrap">';
		...
		echo '<div id="symposium_templates_values">';
			...
			echo "<strong>".__("BETA! If it doesn't work, use the 'Reset to default' links, then Save...", 'wp-symposium')."</strong>";
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="font-size:1.2em">'.__('Mail Page', 'wp-symposium').'<input type="submit" class="button-primary" style="float:right; padding:2px 6px 2px 6px;" value="'.__('Save', 'wp-symposium').'"></th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			echo '<tr>';
			echo '<td>';
				...
				echo '<tr>';
				echo '<td>[compose]</td>';
				echo '<td>'.__('Compose label', 'wp-symposium').'</td>';
				echo '</tr>';
				echo '<tr>';
				echo '<td>[inbox]</td>';
				echo '<td>'.__('In Box label', 'wp-symposium').'</td>';
				echo '</tr>';
				echo '<tr>';
				echo '<td>[sent]</td>';
				echo '<td>'.__('Sent label', 'wp-symposium').'</td>';
				echo '</tr>';
				echo '<tr>';
				echo '<td>[unread]</td>';
				echo '<td>'.__('Unread only checkbox', 'wp-symposium').'</td>';
				echo '</tr>';
				echo '</tbody>';
				echo '</table>';
  	...
  	echo '<div class="wrap">';
		...
		if (current_user_can('edit_themes')) {
			...
			echo "<div style='width:45%; float:left; margin-top:15px;'>";
		  		...
		  		echo '<div id="icon-themes" class="icon32"><br /></div>';
				echo '<h2>' . __('Available Versions', 'wp-symposium') . '</h2>';
			...
			if ($config->img_db == "on") {
				...
				if (file_exists($config->img_path)) {
				    echo "<p>";
					sprintf(__('The folder %s exists, where images uploaded will be placed.', 'wp-symposium'), $config->img_path);
					echo "</p>";
				} else {
				...
				if ($config->img_url == '') {
					...
					echo "<p>".__('The URL to your images folder is', 'wp-symposium')." <a href='".$config->img_url."'>".$config->img_url."</a>.</p>";
				}
			}
			...
			echo "<div>";
			    ...
			    if ( !file_exists($myJSfile) ) {
					echo $fail . sprintf(__('Javascript file (%s) not found, try de-activating and re-activating the core WPS plugin.', 'wp-symposium'), $myJSfile) . $fail2;
			    } else {
				...
				if ($forums) {
					...
					echo '</select><br />';
				   	...
				   	echo '<p><em>' . __("Although your bbPress forum is not altered, and only new categories/topics/replies are added, it is recommended that you backup your database first, this is still under BETA testing, so no guarantees!", 'wp-symposium') . '</em></p>';
...
function symposium_plugin_profile() {
	...
	global $wpdb;
	    ...
	    // See if the user has posted profile settings
	    	...
	    	if (isset($_POST[ 'profile_comments' ])) 		{ $profile_comments = $_POST['profile_comments']; } 				else { $profile_comments = ''; }
			...
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix."symposium_config SET profile_comments = '".$profile_comments."'") );
			...
			if ($_POST['new_name'] != '' && $_POST['new_name'] != __('New name', 'wp-symposium')) {
				$wpdb->query( $wpdb->prepare( "
		...
		$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));					
				...
				echo '<div class="metabox-holder"><div id="toc" class="postbox">';
					...
					$profile_comments = $config->profile_comments;
					...
					<select name="wps_profile_default">
						<option value='extended'<?php if ($wps_profile_default == 'extended') { echo ' SELECTED'; } ?>><?php echo __('Profile', 'wp-symposium'); ?></option>
						<option value='wall'<?php if ($wps_profile_default == 'wall') { echo ' SELECTED'; } ?>><?php echo __('My activity', 'wp-symposium'); ?></option>
						<option value='activity'<?php if ($wps_profile_default == 'activity') { echo ' SELECTED'; } ?>><?php echo __('Friends activity (includes my activity)', 'wp-symposium'); ?></option>
						<option value='all'<?php if ($wps_profile_default == 'all') { echo ' SELECTED'; } ?>><?php echo __('All activity', 'wp-symposium'); ?></option>
					</select> 
					...
					<tr valign="top"> 
					<td scope="row"><label for="show_wall_extras"><?php echo __('Profile Info On Wall', 'wp-symposium'); ?></label></td>
					<td>
					...
					<td>
					<input type="checkbox" name="profile_comments" id="profile_comments" <?php if ($profile_comments == "on") { echo "CHECKED"; } ?>/>
					...
					<tr valign="top"> 
					<td scope="row"><label for="offline"><?php echo __('Extended Fields', 'wp-symposium'); ?></label></td><td>
					...
					echo '<thead>';
					echo '<tr>';
					echo '<th>'.__('Order', 'wp-symposium').'</th>';
					echo '<th>'.__('Name', 'wp-symposium').'</th>';
					echo '<th>'.__('Type', 'wp-symposium').'</th>';
					...
					echo '<tr>';
						echo '<td><p>'.__('New extended field:', 'wp-symposium').'</p>';
						echo '<input type="text" name="new_order" onclick="javascript:this.value = \'\'" value="0" />';
						echo '</td>';
						echo '<td><p>&nbsp;</p>';
						echo '<input type="text" name="new_name" onclick="javascript:this.value = \'\'" value="'.__('New name', 'wp-symposium').'" />';
						echo '</td>';
						echo '<td><p>&nbsp;</p>';
						...
						echo '</td>';
					echo '</tr>';
					echo '<tr><td colspan="4"><span class="description">';
					...
					echo '<br />'.__('Members extended field values are not shown when they are left empty.', 'wp-symposium');

					echo '</td></tr></tbody></table></table>';
...
function symposium_plugin_settings() {
	...
	global $wpdb;
	    ...
	    if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == 'symposium_plugin_settings' ) {
	        ...
	        $subject_mail_new = $_POST[ 'subject_mail_new' ];
	        $subject_forum_new = $_POST[ 'subject_forum_new' ];
	        $subject_forum_reply = $_POST[ 'subject_forum_reply' ];
			...
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET subject_mail_new = '".$subject_mail_new."'") );						
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET subject_forum_new = '".$subject_forum_new."'") );						
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET subject_forum_reply = '".$subject_forum_reply."'") );						
		...
		$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));
				...
				echo '<div class="metabox-holder"><div id="toc" class="postbox">';
					...
					$subject_mail_new = $config->subject_mail_new;
					$subject_forum_new = $config->subject_forum_new;
					$subject_forum_reply = $config->subject_forum_reply;
					?>
					...
					<select name="wps_js_file">
						<option value='wps.min.js'<?php if ($wps_js_file == 'wps.min.js') { echo ' SELECTED'; } ?>><?php echo __('Minimized', 'wp-symposium'); ?></option>
						<option value='wps.js'<?php if ($wps_js_file == 'wps.js') { echo ' SELECTED'; } ?>><?php echo __('Normal', 'wp-symposium'); ?></option>
					</select> 
					...
					<select name="wps_css_file">
						<option value='wps.min.css'<?php if ($wps_css_file == 'wps.min.css') { echo ' SELECTED'; } ?>><?php echo __('Minimized', 'wp-symposium'); ?></option>
						<option value='wps.css'<?php if ($wps_css_file == 'wps.css') { echo ' SELECTED'; } ?>><?php echo __('Normal', 'wp-symposium'); ?></option>
					</select> 
					<span class="description"><?php echo __('Minimized loads faster, Normal can be edited', 'wp-symposium'); ?></span></td> 
					</tr> 		
						...
						<td></td><td style="border:1px dotted #999; background-color: #fff;">
							...
							<ul style="list-style-type: circle; margin: 10px 0 20px 30px;">
								...
								<?php if (function_exists('symposium_news_main')) { ?>
									<li><?php echo __('<a href="plugins.php?plugin_status=active">De-activate Alerts</a> or <a href="admin.php?page=wp-symposium-alerts/symposium_news_admin.php">set the polling interval</a> high, eg: at least 120 seconds.', 'wp-symposium'); ?></li>
								<?php } ?>
							</ul>
					...
					<?php if ($img_db != "on") { ?>
						...
						<span class="description"><?php echo __('URL to the images folder, Do not include http:// or your domain name eg: ', 'wp-symposium').' <a href="'.$url.'">'.$url.'</a>'; ?></td> 
						</tr> 					

						<tr valign="top" style='background-color: #ccc;'> 
						<td colspan=2>
							<?php echo __('From PHP.INI on your server, the PHP temporary upload folder is:', 'wp-symposium').' '.$img_tmp; ?>
					...
					<td><input name="subject_mail_new" type="text" id="subject_mail_new"  value="<?php echo $subject_mail_new; ?>" class="regular-text" /> 
					...
					<td><input name="subject_forum_new" type="text" id="subject_forum_new"  value="<?php echo $subject_forum_new; ?>" class="regular-text" /> 
					...
					<td><input name="subject_forum_reply" type="text" id="subject_forum_reply"  value="<?php echo $subject_forum_reply; ?>" class="regular-text" /> 
					...
					<select name="wp_alignment">
						<option value='Left'<?php if ($wp_alignment == 'Left') { echo ' SELECTED'; } ?>><?php echo __('Left', 'wp-symposium'); ?></option>
						<option value='Center'<?php if ($wp_alignment == 'Center') { echo ' SELECTED'; } ?>><?php echo __('Center', 'wp-symposium'); ?></option>
						<option value='Right'<?php if ($wp_alignment == 'Right') { echo ' SELECTED'; } ?>><?php echo __('Right', 'wp-symposium'); ?></option>
					</select> 
					...
					<tr valign="top"> 
					<td scope="row"><label for="colorbox"><?php echo __('Load Colorbox', 'wp-symposium'); ?></label></td>
					<td>
...
// yyy
	        ...
	        $forum_login_form = (isset($_POST[ 'forum_login_form' ])) ? $_POST[ 'forum_login_form' ] : '';	
	        $forum_info = (isset($_POST[ 'forum_info' ])) ? $_POST[ 'forum_info' ] : '';	
	        $forum_stars = (isset($_POST[ 'forum_stars' ])) ? $_POST[ 'forum_stars' ] : '';	
	        $forum_refresh = (isset($_POST[ 'forum_refresh' ])) ? $_POST[ 'forum_refresh' ] : '';	
			...
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET forum_login_form = '".$forum_login_form."'") );		
			...
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET forum_refresh = '".$forum_refresh."'") );		
		...
		$forum_login_form = $config->forum_login_form;
		$oldest_first = $config->oldest_first;
		$forum_info = $config->forum_info;
		$forum_stars = $config->forum_stars;
		$forum_refresh = $config->forum_refresh;
		...
		<div class="metabox-holder"><div id="toc" class="postbox"> 
			...
			<?php if ( symposium_is_plus() ) { ?>
				<tr valign="top"> 
				<td scope="row"><label for="use_wysiwyg"><?php echo __('WYSIWYG editor (BETA)', 'wp-symposium'); ?></label></td>
				<td>
			...
			<td>
			<input type="checkbox" name="forum_refresh" id="forum_refresh" <?php if ($forum_refresh == "on") { echo "CHECKED"; } ?>/>
			...
			<td>
			<input type="checkbox" name="forum_login_form" id="forum_login_form" <?php if ($forum_login_form == "on") { echo "CHECKED"; } ?>/>
			...
			<tr valign="top"> 
			<td scope="row"><label for="viewer"><?php echo __('View forum level', 'wp-symposium'); ?></label></td> 
			<td>
			<select name="viewer">
				<option value='Guest'<?php if ($viewer == 'Guest') { echo ' SELECTED'; } ?>><?php echo __('Guest', 'wp-symposium'); ?></option>
				<option value='Subscriber'<?php if ($viewer == 'Subscriber') { echo ' SELECTED'; } ?>><?php echo __('Subscriber', 'wp-symposium'); ?></option>
				<option value='Contributor'<?php if ($viewer == 'Contributor') { echo ' SELECTED'; } ?>><?php echo __('Contributor', 'wp-symposium'); ?></option>
				<option value='Editor'<?php if ($viewer == 'Editor') { echo ' SELECTED'; } ?>><?php echo __('Editor', 'wp-symposium'); ?></option>
				<option value='Administrator'<?php if ($viewer == 'Administrator') { echo ' SELECTED'; } ?>><?php echo __('Administrator', 'wp-symposium'); ?></option>
			</select> 
			<span class="description"><?php echo __('The minimum level a visitor has to be to view the forum', 'wp-symposium'); ?></span></td> 
			</tr> 
	
			<tr valign="top"> 
			<td scope="row"><label for="forum_editor"><?php echo __('Forum post level', 'wp-symposium'); ?></label></td> 
			<td>
			<select name="forum_editor">
				<option value='Guest'<?php if ($forum_editor == 'Guest') { echo ' SELECTED'; } ?>><?php echo __('Guest', 'wp-symposium'); ?></option>
				<option value='Subscriber'<?php if ($forum_editor == 'Subscriber') { echo ' SELECTED'; } ?>><?php echo __('Subscriber', 'wp-symposium'); ?></option>
				<option value='Contributor'<?php if ($forum_editor == 'Contributor') { echo ' SELECTED'; } ?>><?php echo __('Contributor', 'wp-symposium'); ?></option>
				<option value='Editor'<?php if ($forum_editor == 'Editor') { echo ' SELECTED'; } ?>><?php echo __('Editor', 'wp-symposium'); ?></option>
				<option value='Administrator'<?php if ($forum_editor == 'Administrator') { echo ' SELECTED'; } ?>><?php echo __('Administrator', 'wp-symposium'); ?></option>
			</select> 
				...
				if ($view == "styles") {
					...
					if ($style) {
						...
						<select name="underline" id="underline"> 
							<option <?php if ( $style->underline=='') { echo "selected='selected'"; } ?> value=''><?php echo __('No', 'wp-symposium'); ?></option> 
							<option <?php if ( $style->underline=='on') { echo "selected='selected'"; } ?> value='on'><?php echo __('Yes', 'wp-symposium'); ?></option> 
						</select> 
						...
						<select name="row_border_style" id="row_border_styledefault_role"> 
							<option <?php if ( $style->row_border_style=='dotted') { echo "selected='selected'"; } ?> value='dotted'><?php echo __('Dotted', 'wp-symposium'); ?></option> 
							<option <?php if ( $style->row_border_style=='dashed') { echo "selected='selected'"; } ?> value='dashed'><?php echo __('Dashed', 'wp-symposium'); ?></option> 
							<option <?php if ( $style->row_border_style=='solid') { echo "selected='selected'"; } ?> value='solid'><?php echo __('Solid', 'wp-symposium'); ?></option> 
						</select> 
						...
						$closed_word = $config->closed_word;
						?>
						<span class="description"><?php echo sprintf(__('Opacity of topics with [%s] in the subject (between 0.0 and 1.0)', 'wp_symposium'), $closed_word); ?></span></td> 
						</tr> 
	...
	$dir_page_length = $config->dir_page_length;
  	...
  	echo '<div class="wrap">';
		...
		<div class="metabox-holder"><div id="toc" class="postbox"> 
			...
			<tr valign="top"> 
			<td scope="row"><label for="show_admin"><?php echo __('Include admin in directory?', 'wp-symposium') ?></label></td>
			<td>
