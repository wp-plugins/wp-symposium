...
*/
		...
		if (current_user_can('update_core')) {
			...
			echo "<div style='width:50%; float:left; padding-bottom:15px;'>";
				...
				echo '<a name="perma"></a>';
			  	echo '<div id="icon-themes" class="icon32"><br /></div>';
			   	echo '<h2>'.__('Forum Permalinks', 'wp-symposium').' (BETA)</h2>';
			   	echo '<p style="font-weight:bold">'.__('This is work in progress, not complete, and not recommended for live sites! Backup your website and database before enabling!', 'wp-symposium').'</p>';
				...
				if (isset($_POST[ 'symposium_permalinks' ])) {
					if ( $_POST[ 'symposium_permalinks_enable' ] == 'on' ) {
				   	    update_option('wps_permalink_structure', 'on');
						if (isset($_POST[ 'symposium_permalinks_cats' ])) {
				   	    ...
				   	    delete_option('wps_permalink_structure');
					}
			   	}
				...
				if ( get_option('permalink_structure') != '' ) {
					...
					echo '<form method="post" action="">';
						...
						if ( get_option('wps_permalink_structure')  ) {
							...
							echo '<p>'.__('Forum "AJAX mode" has been disabled, as this is not compatible with permalinks.', 'wp-symposium').'</p>'; 
							...
							if ( (!$reset) && (isset($_POST['symposium_rewrite_forum_single']) || isset($_POST['symposium_rewrite_forum_double']) ) ) {
									
									$symposium_rewrite_forum_single = $_POST['symposium_rewrite_forum_single'] ? $_POST['symposium_rewrite_forum_single'] : '';
									$symposium_rewrite_forum_single_target = $_POST['symposium_rewrite_forum_single_target'] ? $_POST['symposium_rewrite_forum_single_target'] : '';
									$symposium_rewrite_forum_double = $_POST['symposium_rewrite_forum_double'] ? $_POST['symposium_rewrite_forum_double'] : '';
									$symposium_rewrite_forum_double_target = $_POST['symposium_rewrite_forum_double_target'] ? $_POST['symposium_rewrite_forum_double_target'] : '';
									
									if ($symposium_rewrite_forum_single && $symposium_rewrite_forum_single && $symposium_rewrite_forum_single && $symposium_rewrite_forum_single) {
										update_option('symposium_rewrite_forum_single', $symposium_rewrite_forum_single);
										update_option('symposium_rewrite_forum_single_target', $symposium_rewrite_forum_single_target);
										update_option('symposium_rewrite_forum_double', $symposium_rewrite_forum_double);
										update_option('symposium_rewrite_forum_double_target', $symposium_rewrite_forum_double_target);
										echo '<p style="color:green; font-weight:bold;">'.__('Re-write rules updated.', 'wp-symposium').'</p>'; 
									} else {
										echo '<p style="color:red; font-weight:bold;">'.__('Values must be entered in every field.', 'wp-symposium').'</p>'; 
									}										

							} else {
								
								// check that options exist if not put in defaults
								if ( ($reset) || ( !get_option('symposium_rewrite_forum_single') && !get_option('symposium_rewrite_forum_double') ) ) {
									...
									echo '<p style="color:green; font-weight:bold;">'.__('Re-write rules set to default.', 'wp-symposium').'</p>'; 

								}
							}
												
						    // Add forum rules to handle with or without cagetory
						    add_rewrite_rule(get_option('symposium_rewrite_forum_double'), get_option('symposium_rewrite_forum_double_target'), 'top');
						    add_rewrite_rule(get_option('symposium_rewrite_forum_single'), get_option('symposium_rewrite_forum_single_target'), 'top');
							...
							echo '<input type="text" name="symposium_rewrite_forum_double_target" style="width:400px" value="'.get_option('symposium_rewrite_forum_double_target').'" /><br /><br />';
							
							echo '<input type="hidden" name="symposium_permalinks" value="Y">';
							echo '<input type="checkbox" name="symposium_permalinks_enable"';
								if (get_option('wps_permalink_structure')) echo ' CHECKED';
								echo '> '.__('Enable WPS Permalinks', 'wp-symposium').'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
							echo '<input type="checkbox" name="symposium_permalinks_reset" /> '.__('Reset to default values', 'wp-symposium').'<br />';
							...
							echo '<input type="checkbox" name="symposium_permalinks_enable"';
								if (get_option('wps_permalink_structure')) echo ' CHECKED';
								echo '> '.__('Enable WPS Permalinks', 'wp-symposium').'<br /><br />';
						   	echo '<input type="submit" class="button-primary" value="'.__('Update', 'wp-symposium').'" />';
						}
...
function symposium_plugin_forum() {
	...
	global $wpdb;
		...
		<div class="metabox-holder"><div id="toc" class="postbox"> 
			...
			?>
			<span class="description"><?php echo __('The WordPress roles that can view the entire forum (over-rides forum categories)', 'wp-symposium'); ?></span></td> 
			</tr> 
