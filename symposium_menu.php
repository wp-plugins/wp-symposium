		...
		if ($posts) {
			...
			foreach ($posts as $post) {
				...
				if ($post->topic_approved != "on") {
					echo '<img src="'.get_site_url().'/'.WP_CONTENT_DIR.'/plugins/wp-symposium/images/forum_orange.png" alt="Unapproved" />';
				} else {
					echo '<img src="'.get_site_url().'/'.WP_CONTENT_DIR.'/plugins/wp-symposium/images/forum_green.png" alt="Unapproved" />';
				}
				echo '</td>';
				echo '<td valign="top">';
				if ($post->topic_parent == 0) {
		...
		$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));
				...
				echo '<div class="metabox-holder"><div id="toc" class="postbox">';
					...
					<?php if ($img_db != "on") { ?>
						...
						<span class="description"><?php echo __('URL to the images folder, eg: /'.WP_CONTENT_DIR.'/wps-content', 'wp-symposium'); ?></td> 
						</tr> 					
