	...
	global $wpdb;
		...
		$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));
				...
				echo '<div class="metabox-holder"><div id="toc" class="postbox">';
					...
					<tr valign="top"> 
					<td scope="row"><label for="jscharts">Load JScharts</label></td>
					<td>
					<input type="checkbox" name="jscharts" id="jscharts" <?php if ($jscharts == "on") { echo "CHECKED"; } ?>/>
					<span class="description"><?php echo __('Load JSCharts on non-admin pages, disable if causing problems', 'wp-symposium'); ?></span></td> 
					</tr>					
	...
	if ($categories) {
		foreach ($categories as $category) {
			...
			if (!inHaystack($included, $category->cid)) {
				...
				echo '<input type="checkbox" class="wps_forum_cat_'.$category->cid.'" name="level_'.$category->cid.'[]" value="'.__('Everyone', 'wp-symposium').'"';
				if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE) {
					echo ' CHECKED';
				}
				echo '> '.__('Everyone', 'wp-symposium').'<br />';
