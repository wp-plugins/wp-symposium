...
function symposium_plugin_templates() {
	...
	if (isset($_POST['template_mail_textarea'])) {
		update_option('symposium_template_mail', str_replace(chr(13), "[]", $_POST['template_mail_textarea']));
	}
	...
	$template_mail = str_replace("[]", chr(13), stripslashes(get_option('symposium_template_mail')));
		...
		if (current_user_can('update_core')) {
			...
			if (get_option('symposium_img_db') == "on") {
				...
				if (file_exists(get_option('symposium_img_path'))) {
					echo "<p>";
					sprintf(__('The folder %s exists, where images uploaded will be placed.', 'wp-symposium'), get_option('symposium_img_path'));
					echo "</p>";
				} else {
			...
			echo "<div>";
				...
				if (isset($_POST['symposium_ric'])) {
					...
					if(file_exists($path) && is_dir($path)) { 
						if ($handler = opendir($path)) {
							while (($sub = readdir($handler)) !== FALSE) {
								if ($sub != "." && $sub != ".." && $sub != "Thumb.db" && $sub != "Thumbs.db") {
									if (is_dir($path."/".$sub)) {
										...
										if (!$id) {
											$report .= 'User ID '.$sub.' not found, removing media from '.$path."/".$sub.'<br />';
							...
							$report .= __("The Symposium images/media path cannot be open, check rights and re-run the Integrity Check", "wp-symposium");
						}
					} else {
						$report .= __('The Symposium images/media path doesn\'t exist, you need to set this at the <a href="admin.php?page=symposium_settings">Settings</a> page, then re-run the Integrity Check', "wp-symposium");
					}
				...
				echo "<p>".__("If you are experiencing problems, <a href='http://www.wpsymposium.com/trythisfirst' target='_blank'>try this first</a>.", "wp-symnposium")."</p>";
				...
				echo '<a name="mingle"></a>';
			  	echo '<div id="icon-themes" class="icon32"><br /></div>';
			   	echo '<h2>'.__('Mingle Migration (BETA)', 'wp-symposium').'</h2>';
				...
				if($wpdb->get_var("show tables like '%".$wpdb->prefix."forum_forums%'") == $wpdb->prefix."forum_forums") {
					...
					if ($forums) {
						...
						echo '</select><br />';
					   	...
					   	echo '<p><em>' . __("Although your Mingle forum is not altered, and only new categories/topics/replies are added, it is recommended that you backup your database first, this is still under BETA testing, so no guarantees!", 'wp-symposium') . '</em></p>';
		...
		if (current_user_can('update_core')) {
			echo '<td style="text-align:center">';
				if ( file_exists(WP_PLUGIN_DIR.'/'.$plugin_dir) ) {
					$status = 'installed';
					echo '<img src="'.get_option('symposium_images').'/tick.png" />';
				} else {
					$status = 'notinstalled';
					echo '<img src="'.get_option('symposium_images').'/cross.png" />';
				}
			echo '</td>';
		}
...
function symposium_plugin_profile() {
	...
	get_currentuserinfo();
		...
		// See if the user has posted profile settings
			...
			if ($_POST['new_name'] != '' && $_POST['new_name'] != __('New label', 'wp-symposium') && $_POST['new_slug'] != '' && $_POST['new_slug'] != __('New slug', 'wp-symposium')) {

				$slug = strtolower(preg_replace("/[^A-Za-z0-9_]/", '', $_POST['new_slug']));
				if (in_array($slug, array( "city", "country" ))) $slug .= '_2';
			...
			// Hook to save items from the Profile settings page - deal with superglobal $_POST and save
			do_action ( 'symposium_save_profile_settings_hook' );
			...
			echo "<div class='updated slideaway'><p>".__('Saved', 'wp-symposium').".</p></div>";
				...
				echo '<div class="metabox-holder"><div id="toc" class="postbox">';
					...
					<select name="wps_profile_default">
						...
						<option value='activity'<?php if (get_option('symposium_wps_profile_default') == 'activity') { echo ' SELECTED'; } ?>><?php echo __('Friends activity (includes my activity)', 'wp-symposium'); ?></option>
...
<?php
					...
					echo apply_filters ( 'symposium_add_profile_settings_before_ef_hook', "" );
?>						
					<tr valign="top"> 
					<td scope="row"><?php echo __('Extended Fields', 'wp-symposium'); ?></td><td>
						...
						// Hook to add items to the Profile settings page
						echo apply_filters ( 'symposium_add_profile_settings_hook', "" );
...
function symposium_plugin_settings() {
	...
	global $wpdb;
		...
		if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == 'symposium_plugin_settings' ) {
			...
			update_option('symposium_image_ext', $_POST[ 'image_ext' ]);
			update_option('symposium_video_ext', $_POST[ 'video_ext' ]);
			update_option('symposium_doc_ext', $_POST[ 'doc_ext' ]);
			...
			update_option('symposium_colorbox', isset($_POST[ 'colorbox' ]) ? $_POST[ 'colorbox' ] : '');
			...
			// Hook to save items from the plugin settings page - deal with superglobal $_POST and save
			do_action ( 'symposium_save_plugin_settings_hook' );
...
<?php				
				...
				do_action ( 'symposium_add_plugin_settings_mail_title_hook' );
?>					
					<tr valign="top"> 
...
<?php				
				...
				echo apply_filters( 'symposium_add_plugin_settings_hook', "" );
?>					
					<tr valign="top"> 
					...
					<td>
					<input type="checkbox" name="colorbox" id="colorbox" <?php if (get_option('symposium_colorbox') == "on") { echo "CHECKED"; } ?>/>
...
function symposium_plugin_forum() {
	...
	global $wpdb;
		...
		if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == 'symposium_plugin_forum' ) {
			...
			update_option('symposium_viewer', $_POST[ 'viewer' ]);
			update_option('symposium_forum_editor', $_POST[ 'forum_editor' ]);
			update_option('symposium_chatroom_banned', $_POST[ 'chatroom_banned' ]);
		...
		<div class="metabox-holder"><div id="toc" class="postbox"> 
			...
			<td>
			<select name="viewer">
				<option value='Guest'<?php if (get_option('symposium_viewer') == 'Guest') { echo ' SELECTED'; } ?>><?php echo __('Guest', 'wp-symposium'); ?></option>
				<option value='Subscriber'<?php if (get_option('symposium_viewer') == 'Subscriber') { echo ' SELECTED'; } ?>><?php echo __('Subscriber', 'wp-symposium'); ?></option>
				<option value='Contributor'<?php if (get_option('symposium_viewer') == 'Contributor') { echo ' SELECTED'; } ?>><?php echo __('Contributor', 'wp-symposium'); ?></option>
				<option value='Editor'<?php if (get_option('symposium_viewer') == 'Editor') { echo ' SELECTED'; } ?>><?php echo __('Editor', 'wp-symposium'); ?></option>
				<option value='Administrator'<?php if (get_option('symposium_viewer') == 'Administrator') { echo ' SELECTED'; } ?>><?php echo __('Administrator', 'wp-symposium'); ?></option>
			</select> 
			...
			<select name="forum_editor">
				<option value='Guest'<?php if (get_option('symposium_forum_editor') == 'Guest') { echo ' SELECTED'; } ?>><?php echo __('Guest', 'wp-symposium'); ?></option>
				<option value='Subscriber'<?php if (get_option('symposium_forum_editor') == 'Subscriber') { echo ' SELECTED'; } ?>><?php echo __('Subscriber', 'wp-symposium'); ?></option>
				<option value='Contributor'<?php if (get_option('symposium_forum_editor') == 'Contributor') { echo ' SELECTED'; } ?>><?php echo __('Contributor', 'wp-symposium'); ?></option>
				<option value='Editor'<?php if (get_option('symposium_forum_editor') == 'Editor') { echo ' SELECTED'; } ?>><?php echo __('Editor', 'wp-symposium'); ?></option>
				<option value='Administrator'<?php if (get_option('symposium_forum_editor') == 'Administrator') { echo ' SELECTED'; } ?>><?php echo __('Administrator', 'wp-symposium'); ?></option>
			</select> 
		...
		echo '<div class="symposium-wrapper" style="margin-top:15px">';
			...
			echo '<div id="mail-main">';
				...
				if ($view == "styles") {
					...
					<td><input name="fontfamily" type="text" id="fontfamily" value="<?php echo get_option('symposium_fontfamily'); ?>"/> 
					...
					<td><input name="main_background" type="text" id="main_background" class="iColorPicker" value="<?php echo get_option('symposium_main_background'); ?>"  /> 
					...
					<td><input name="label" type="text" id="label" class="iColorPicker" value="<?php echo get_option('symposium_label'); ?>"  /> 
					...
					<td><input name="text_color" type="text" id="text_color" class="iColorPicker" value="<?php echo get_option('symposium_text_color'); ?>"  /> 
					<span class="description"><?php echo __('Primary Text Colour', 'wp-symposium'); ?></span></td> 
					</tr> 

					<tr valign="top"> 
					<td scope="row"><label for="text_color_2"></label></td> 
					<td><input name="text_color_2" type="text" id="text_color_2" class="iColorPicker" value="<?php echo get_option('symposium_text_color_2'); ?>"  /> 
					...
					<td><input name="link" type="text" id="link" class="iColorPicker" value="<?php echo get_option('symposium_link'); ?>"  /> 
					<span class="description"><?php echo __('Link Colour', 'wp-symposium'); ?></span></td> 
					</tr> 

					<tr valign="top"> 
					<td scope="row"><label for="link_hover"</label></td> 
					<td><input name="link_hover" type="text" id="link_hover" class="iColorPicker" value="<?php echo get_option('symposium_link_hover'); ?>"  /> 
					...
					<td><input name="bigbutton_background" type="text" id="bigbutton_background" class="iColorPicker" value="<?php echo get_option('symposium_bigbutton_background'); ?>"  /> 
					...
					<td><input name="bigbutton_background_hover" type="text" id="bigbutton_background_hover" class="iColorPicker" value="<?php echo get_option('symposium_bigbutton_background_hover'); ?>"  /> 
					...
					<td><input name="bigbutton_color" type="text" id="bigbutton_color" class="iColorPicker" value="<?php echo get_option('symposium_bigbutton_color'); ?>"  /> 
					<span class="description"><?php echo __('Text Colour', 'wp-symposium'); ?></span></td> 
					</tr> 

					<tr valign="top"> 
					<td scope="row"><label for="bigbutton_color_hover"></label></td> 
					<td><input name="bigbutton_color_hover" type="text" id="bigbutton_color_hover" class="iColorPicker" value="<?php echo get_option('symposium_bigbutton_color_hover'); ?>"  /> 
					...
					<td><input name="bg_color_1" type="text" id="bg_color_1" class="iColorPicker" value="<?php echo get_option('symposium_bg_color_1'); ?>"  /> 
					<span class="description"><?php echo __('Primary Colour', 'wp-symposium'); ?></span></td> 
					</tr> 

					<tr valign="top"> 
					<td scope="row"><label for="bg_color_2"></label></td> 
					<td><input name="bg_color_2" type="text" id="bg_color_2" class="iColorPicker" value="<?php echo get_option('symposium_bg_color_2'); ?>"  /> 
					<span class="description"><?php echo __('Row Colour', 'wp-symposium'); ?></span></td> 
					</tr> 

					<tr valign="top"> 
					<td scope="row"><label for="bg_color_3"></label></td> 
					<td><input name="bg_color_3" type="text" id="bg_color_3" class="iColorPicker" value="<?php echo get_option('symposium_bg_color_3'); ?>"  /> 
					...
					<td><input name="table_rollover" type="text" id="table_rollover" class="iColorPicker" value="<?php echo get_option('symposium_table_rollover'); ?>"  /> 
					...
					<td><input name="categories_background" type="text" id="categories_background" class="iColorPicker" value="<?php echo get_option('symposium_categories_background'); ?>"  /> 
					...
					<td><input name="categories_color" type="text" id="categories_color" class="iColorPicker" value="<?php echo get_option('symposium_categories_color'); ?>"  /> 
					<span class="description"><?php echo __('Text Colour', 'wp-symposium'); ?></span></td> 
					</tr> 
					...
					<td><input name="closed_opacity" type="text" id="closed_opacity" class="iColorPicker" value="<?php echo get_option('symposium_closed_opacity'); ?>"  /> 
					<?php
