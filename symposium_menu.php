...
function symposium_plugin_menu() {
	...
	add_submenu_page('symposium_debug', __('Settings', 'wp-symposium'), __('Settings', 'wp-symposium'), 'edit_themes', 'symposium_options', 'symposium_plugin_options');
 	...
 	$wpdb->show_errors();
  	...
  	echo '<div class="wrap">';
		...
		echo "<div style='margin-top:15px'>";
			...
			install_row(__('Forum', 'wp-symposium'), 'symposium_forum', $config->forum_url, 'wp-symposium/symposium_forum.php', __('The forum plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium.');
			install_row(__('Profile', 'wp-symposium'), 'symposium_profile', $config->profile_url, 'wp-symposium/symposium_profile.php', __('The profile plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium.');
			install_row(__('Mail', 'wp-symposium'), 'symposium_mail', $config->mail_url, 'wp-symposium/symposium_mail.php', __('The mail plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium.');
			install_row(__('Members', 'wp-symposium'), 'symposium_members', $config->members_url, 'wp-symposium/symposium_members.php', __('The members directory plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium.');
			install_row(__('Panel', 'wp-symposium'), 'add_notification_bar', '-', 'wp-symposium/symposium_bar.php', __('The panel plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium.');
			install_row(__('Groups', 'wp-symposium'), 'symposium_groups', $config->groups_url, 'wp-symposium-groups/symposium_groups.php', __('The groups plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium-groups.'.chr(10).chr(10).'Download from http://www.wpsymposium.com/downloadinstall.');
			install_row(__('Group', 'wp-symposium'), 'symposium_group', $config->group_url, 'wp-symposium-groups/symposium_groups.php', __('The groups plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium-groups.'.chr(10).chr(10).'Download from http://www.wpsymposium.com/downloadinstall.');
			install_row(__('Facebook Connect', 'wp-symposium'), 'symposium_facebook', '-', 'wp-symposium-facebook/symposium-facebook.php', __('The Facebook plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium-facebook.'.chr(10).chr(10).'Once activated, see further instructions on the WP Symposium -> Settings -> Facebook page.');
			install_row(__('Mobile', 'wp-symposium'), 'symposium_mobile', '-', 'wp-symposium-mobile/symposium-mobile.php', __('The Mobile plugin must be installed in ', 'wp-symposium').WP_PLUGIN_DIR.'/wp-symposium-facebook.'.chr(10).chr(10).'Once activated, see further instructions on the WP Symposium -> Settings -> Mobile page.');
		...
		echo '<div id="admin_image_to_crop"></div>';
	  	...
	  	echo '<h3 style="clear:both">'.__('Table Structures', 'wp-symposium').'</h3><p>';
		...
		// ********** Test Updating a Value   		
	   	echo '<h3>'.__('Test updating the database', 'wp-symposium').'</h3>';
...
function install_row($name, $function, $config_url, $plugin_dir, $install_help) {

	global $wpdb;
	$shortcode = "[".str_replace('_', '-', $function)."]";
	...
	echo '<tr>';
		...
		// Shortcode on a page?
		$sql = "SELECT ID FROM ".$wpdb->prefix."posts WHERE lower(post_content) LIKE '%".$shortcode."%' AND post_status = 'publish';";
		...
		if ($pages) {
			...
			echo '<td>';
				...
				if ($status == 'tick') {
					if ($function == 'symposium_forum') {
						echo '[<a href="admin.php?page=symposium_options&view=forum">'.__('Configure', 'wp-symposium').'</a>]';
					}
					if ($function == 'symposium_profile') {
						echo '[<a href="admin.php?page=symposium_options&view=profile">'.__('Configure', 'wp-symposium').'</a>]';
					}
					if ( ($function == 'symposium_group') || ($function == 'symposium_groups') ) {
						echo '[<a href="admin.php?page=symposium_options&view=groups">'.__('Configure', 'wp-symposium').'</a>]';
					}
				}
			echo '</td>';
			if ($url != $config_url && $status != 'cross') {
				$status = 'error';
			}
		} else {
			echo '<td>';
			if ( ($status != 'cross') && ($status != 'notinstalled') && ($function != 'symposium_facebook') && ($function != 'add_notification_bar') && ($function != 'symposium_mobile') && ($function != 'symposium_gallery') ) {
				$status = 'add';
				...
				if ($status == 'tick') {
					if ($function == 'symposium_facebook') {
						echo '[<a href="admin.php?page=symposium_options&view=facebook">'.__('Configure', 'wp-symposium').'</a>]';
					}
					if ($function == 'symposium_mobile') {
						echo '[<a href="admin.php?page=symposium_options&view=mobile">'.__('Configure', 'wp-symposium').'</a>]';
					}
					if ($function == 'add_notification_bar') {
						echo '[<a href="admin.php?page=symposium_options&view=bar">'.__('Configure', 'wp-symposium').'</a>]';
					}
				}
				if ($function == 'add_notification_bar') {
	...
	$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));
  	...
  	echo '<div class="wrap">';
		...
		echo '<div class="symposium-wrapper" style="margin-top:15px">';
			...
			echo '<div class="mail_tab nav-tab-'.$settings_active.'"><a href="admin.php?page=symposium_options&view=settings" class="nav-tab-'.$settings_active.'-link">'.__('Options', 'wp-symposium').'</a></div>';
			if (function_exists('symposium_forum')) {
