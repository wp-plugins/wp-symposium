  	...
  	echo '<div class="wrap">';
		...
		echo '<div id="admin_image_to_crop"></div>';
	  	...
	  	// Options
	   	...
	   	if($wpdb->get_var("show tables like '$table_name'") != $table_name) {
			...
			if (!symposium_field_exists($table_name, 'forum_login')) { $status = "X"; }	
			...
			if (!symposium_field_exists($table_name, 'template_group')) { $status = "X"; }	
			if (!symposium_field_exists($table_name, 'template_forum_category')) { $status = "X"; }	
			if (!symposium_field_exists($table_name, 'template_forum_topic')) { $status = "X"; }	
			if (!symposium_field_exists($table_name, 'template_group_forum_category')) { $status = "X"; }	
			if (!symposium_field_exists($table_name, 'template_group_forum_topic')) { $status = "X"; }	
			if (!symposium_field_exists($table_name, 'facebook_api')) { $status = "X"; }	
			if (!symposium_field_exists($table_name, 'facebook_secret')) { $status = "X"; }	
			if (!symposium_field_exists($table_name, 'css')) { $status = "X"; }	
			if (!symposium_field_exists($table_name, 'mobile_topics')) { $status = "X"; }	
	  	...
	  	// Usermeta
	   	...
	   	if($wpdb->get_var("show tables like '$table_name'") != $table_name) {
			...
			if (!symposium_field_exists($table_name, 'facebook_id')) { $status = "X"; }
		...
		echo '<h2>'.__('Version Numbers and URLs', 'wp-symposium').'</h2>';
	  	...
	  	echo "<p>";
			...
			echo '<p>'.__('Site domain name', 'wp-symposium').': '.symposium_get_siteURL().'</p>';
			...
			if ($warning == true) {
				echo $fail."You must update your plugin URLs on the <a href='admin.php?page=symposium_options&view=settings'>options page</a>.".$fail2;
			  	...
			  	if (function_exists('symposium_mobile')) { 
			  		echo "&nbsp;&middot;&nbsp;".__('the mobile plugin is activated', 'wp-symposium')."<br />";
			  	} else {
			  		echo "&nbsp;&middot;&nbsp;<strong>".__('Mobile/SEO/Accessibility plugin is available to Bronze members at <a href="http://www.wpsymposium.com">WP Symposium</a>', 'wp-symposium')."</strong><br />";				
				}
			  	if (function_exists('symposium_groups')) { 
			  		...
			  		echo "&nbsp;&middot;&nbsp;<strong>".__('Facebook Connect is available to Bronze members at <a href="http://www.wpsymposium.com">WP Symposium</a>', 'wp-symposium')."</strong><br />";
				}
	    ...
	    if( isset($_POST[ 'symposium_testemail' ]) && $_POST[ 'symposium_testemail' ] == 'Y' ) {
			...
			if (symposium_sendmail($to, "WP Symposium Test Email", __("This is a test email sent from", "wp-symposium")." ".symposium_get_siteURL())) {
	...
	$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));
  	...
  	echo '<div class="wrap">';
	    ...
	    if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == 'CSS' ) {
			$sql = "UPDATE ".$wpdb->prefix."symposium_config SET css = '".addslashes(str_replace(chr(13), "[]", $_POST['css']))."'";
			$wpdb->query( $wpdb->prepare($sql) );
  	    }
...
function symposium_plugin_options() {
	...
	$config = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'symposium_config'));
  	...
  	echo '<div class="wrap">';
	    ...
	    // See if the user has posted Facebook settings
	    if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == 'FB' ) {
	    	    	        
	        $facebook_api = $_POST[ 'facebook_api' ];
	        $facebook_secret = $_POST[ 'facebook_secret' ];
	
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET facebook_api = '".$facebook_api."'") );					
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET facebook_secret = '".$facebook_secret."'") );					

	        // Put an settings updated message on the screen
			echo "<div class='updated'><p>".__('Facebook options saved', 'wp-symposium').".</p></div>";

	    }
			
	    // See if the user has posted Mobile settings
	    if( isset($_POST[ 'symposium_update' ]) && $_POST[ 'symposium_update' ] == 'M' ) {
	    	    	        
	        $mobile_topics = $_POST[ 'mobile_topics' ];
	        
			$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET mobile_topics = ".$mobile_topics) );					
	        // Put an settings updated message on the screen
			echo "<div class='updated'><p>".__('Mobile options saved', 'wp-symposium').".</p></div>";
		...
		$facebook_active = 'inactive';
		$profile_active = 'inactive';
		$mobile_active = 'inactive';
		$view = "settings";
		...
		if (isset($_GET['view']) && $_GET['view'] == "facebook") {
			$notes_active = 'inactive';
			$settings_active = 'inactive';
			$forum_active = 'inactive';
			$register_active = 'inactive';
		...
		echo '<div class="symposium-wrapper" style="margin-top:15px">';
			...
			if (function_exists('symposium_facebook')) {
				echo '<div class="mail_tab nav-tab-'.$facebook_active.'"><a href="admin.php?page=symposium_options&view=facebook" class="nav-tab-'.$facebook_active.'-link">'.__('Facebook', 'wp-symposium').'</a></div>';
			}
			if (function_exists('symposium_mobile')) {
				echo '<div class="mail_tab nav-tab-'.$mobile_active.'"><a href="admin.php?page=symposium_options&view=mobile" class="nav-tab-'.$mobile_active.'-link">'.__('Mobile', 'wp-symposium').'</a></div>';
			}
			echo '</div>';
			...
			echo '<div id="mail-main">';
				...
				// Insert all the default pages?
				if (isset($_POST['symposium_update']) && $_POST['symposium_update'] == "Q") {
					
					// Add all the pages, with shortcodes
					if ( $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."posts WHERE post_content like '%[symposium-forum]%'") == 0 ) { 
						$wpdb->query( $wpdb->prepare( "INSERT INTO ".$wpdb->prefix."posts ( post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, post_mime_type, comment_count )
						VALUES ( %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %d, %s, %s, %d )", 
				        array( $current_user->ID,  date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "[symposium-forum]", __("Forum", "wp-symposium"), "", "publish", "closed", "closed", "", "forum", "", "", date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "", 0, "WPS_TMP", 0, "page", "", 0 ) 
				        ) );
					}

					if ( $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."posts WHERE post_content like '%[symposium-mail]%'") == 0 ) { 
						$wpdb->query( $wpdb->prepare( "INSERT INTO ".$wpdb->prefix."posts ( post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, post_mime_type, comment_count )
							VALUES ( %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %d, %s, %s, %d )", 
				        	array( $current_user->ID,  date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "[symposium-mail]", __("Mail", "wp-symposium"), "", "publish", "closed", "closed", "", "mail", "", "", date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "", 0, "WPS_TMP", 0, "page", "", 0 ) 
				        	) );
					}
				
					if ( $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."posts WHERE post_content like '%[symposium-members]%'") == 0 ) { 
						$wpdb->query( $wpdb->prepare( "INSERT INTO ".$wpdb->prefix."posts ( post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, post_mime_type, comment_count )
							VALUES ( %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %d, %s, %s, %d )", 
					        array( $current_user->ID,  date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "[symposium-members]", __("Members", "wp-symposium"), "", "publish", "closed", "closed", "", "members", "", "", date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "", 0, "WPS_TMP", 0, "page", "", 0 ) 
					        ) );
					}

					if ( $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."posts WHERE post_content like '%[symposium-profile]%'") == 0 ) { 
						$wpdb->query( $wpdb->prepare( "INSERT INTO ".$wpdb->prefix."posts ( post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, post_mime_type, comment_count )
							VALUES ( %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %d, %s, %s, %d )", 
					        array( $current_user->ID,  date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "[symposium-profile]", __("Profile", "wp-symposium"), "", "publish", "closed", "closed", "", "profile", "", "", date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "", 0, "WPS_TMP", 0, "page", "", 0 ) 
					        ) );
					}

					// Update URLs
					$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET forum_url = '/forum'") );
					$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET mail_url = '/mail'") );
					$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET members_url = '/members'") );
					$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET profile_url = '/profile'") );

					if (function_exists('symposium_group')) {

						if ( $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."posts WHERE post_content like '%[symposium-groups]%'") == 0 ) { 
							$wpdb->query( $wpdb->prepare( "INSERT INTO ".$wpdb->prefix."posts ( post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, post_mime_type, comment_count )
								VALUES ( %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %d, %s, %s, %d )", 
						        array( $current_user->ID,  date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "[symposium-groups]", __("Groups", "wp-symposium"), "", "publish", "closed", "closed", "", "groups", "", "", date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "", 0, "WPS_TMP", 0, "page", "", 0 ) 
						        ) );
						}

						if ( $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."posts WHERE post_content like '%[symposium-group]%'") == 0 ) { 
							$wpdb->query( $wpdb->prepare( "INSERT INTO ".$wpdb->prefix."posts ( post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, post_mime_type, comment_count )
								VALUES ( %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %d, %s, %s, %d )", 
						        array( $current_user->ID,  date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "[symposium-group]", __("Group", "wp-symposium"), "", "publish", "closed", "closed", "", "group", "", "", date("Y-m-d H:i:s"), gmdate("Y-m-d H:i:s"), "", 0, "WPS_TMP", 0, "page", "", 0 ) 
						        ) );
						}

						$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET groups_url = '/groups'") );
						$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET group_url = '/group'") );
						
					} else {
						
						$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET groups_url = 'none'") );
						$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix.'symposium_config'." SET group_url = 'none'") );

					}
				
				
					// Update all GUID's
					$sql = "UPDATE ".$wpdb->prefix."posts SET guid = concat('".get_site_url()."/?p=', ID) WHERE guid = 'WPS_TMP'";
					$wpdb->query( $wpdb->prepare($sql) );
					
					// Show notice
				    echo "<div class='updated'><strong>".__("WP Symposium - Quick Setup", "wp-symposium")."</strong><br /><div style='padding:4px;'>";

				    echo "<p style='line-height:15px'>";
				    echo "Quick Setup complete. You can now review the <a href='admin.php?page=symposium_debug'>Health Check</a>, change the default <a href='admin.php?page=symposium_styles'>styles</a>, set up the <a href='admin.php?page=symposium_categories'>forum categories</a>, have a look at the new <a href='admin.php?page=symposium_options&view=settings'>settings</a> or just <a href='".get_site_url()."'>view your social network</a>.";
				    echo "</p>";
					...
					<div class="postbox-container side" style="width:34%; float:right"> 
						<div class="metabox-holder">	
							...
							<div id="toc" class="postbox"> 
								<h3><?php _e("Quick Setup", "wp-symposium") ?></h3> 
								<div class="inside"> 
									<p><?php _e("This will quickly add WordPress pages with the WP Symposium shortcodes and set up the URL's on the <a href='admin.php?page=symposium_options&view=settings'>settings page</a>.", "wp-symposium") ?></p>
									
									<p><?php _e("You may still need to set up your menu, for example the 'Group' page should not be shown.", "wp-symposium") ?></p>

									<form method="post" action=""> 
									<input type="hidden" name="symposium_update" value="Q">
									<input type="submit" name="Submit" class="button-primary" value="<?php echo __('Run Quick Setup', 'wp-symposium'); ?>" /> 
									</form>
...
					</select> 
				...
				if ($view == "profile") {
					...
					<span class="description"><?php echo __('Enable this feature (coming soon.....)', 'wp-symposium'); ?></span></td> 
					</tr> 
				...
				if ($view == "facebook") {
					...
					?>
					<p><?php _e("A Facebook application is used to post messages to Facebook Walls - you need to create a Facebook application for your website:", "wp-symposium-facebook") ?></p>

					<ol>
						<li>Log in to Facebook</li>
						<li>Go <a target='_blank' href='http://www.facebook.com/developers/apps.php'>here</a>.</li>
						<li>Click on Set Up New App button</li>
						<li>Enter an Application Name (that will appear under Facebook Wall posts), and enter security test</li>
						<li>Click on Website and enter your website URL (eg: http://www.example.com/)</li>
						<li>Copy the Application ID and Application Secret below</li>
						<li>Save Changes</li>
					</ol>
					...
					<form method="post" action=""> 
					<input type="hidden" name="symposium_update" value="FB">
				
					<table class="form-table"> 
				
						<tr valign="top"> 
						<th scope="row"><label for="facebook_api"><?php _e('Facebook Application ID', 'wp-symposium'); ?></label></th> 
						<td><input name="facebook_api" type="text" id="facebook_api"  value="<?php echo $facebook_api; ?>" style="width:250px" /> 
						<span class="description"><?php echo __('Also called your OAuth client_id'); ?></td> 
						</tr> 

						<tr valign="top"> 
						<th scope="row"><label for="facebook_secret"><?php _e('Facebook Application Secret', 'wp-symposium'); ?></label></th> 
						<td><input name="facebook_secret" type="text" id="facebook_secret"  value="<?php echo $facebook_secret; ?>" style="width:250px" /> 
						<span class="description"><?php echo __('Also called your OAuth client_secret'); ?></td> 
						</tr> 
				...
				if ($view == "mobile") {
				    ...
				    // Get values from database  
					$mobile_topics = $config->mobile_topics;
					...
					<table class="form-table"> 
						...
						<tr valign="top"> 
						<th scope="row"><label for="mobile_topics"><?php _e('Maximum number of topics', 'wp-symposium'); ?></label></th> 
						<td><input name="mobile_topics" type="text" id="mobile_topics"  value="<?php echo $mobile_topics; ?>" style="width:50px" /> 
						<span class="description"><?php echo __('Threads view is also limited to last 7 days'); ?></td> 
						</tr> 
					...
					<p><?php _e("To install the Mobile/SEO/Accessibility plugin on your site:", "wp-symposium-facebook") ?></p>
					...
					<ol>
						...
						<li>Copy the <strong>contents</strong> of the downloaded plugin ZIP file <strong>into</strong> this new folder.</li>
					</ol>
