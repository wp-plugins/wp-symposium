...
<?php
echo '<div id="header">'.get_bloginfo('name').'<div class="home_link"><a href="index.php?'.$a.'">'.__('Home', 'wp-symposium').'</a></div></div>';

$viewer = get_option('symposium_viewer');
...
if ( ($viewer == "Guest")
 ...
 || ($viewer == "Administrator" && $level == 5) ) {
	...
	if ($topic_id == 0) {
		...
		$postcount = get_option('symposium_mobile_topics');
		if ($postcount < 1) { $postcount = 20; }
		
		// Show login link?
		if ( !is_user_logged_in() ) {
			echo '<ul><li><a href="login.php'.$a.'">'.__('Login', 'wp-symposium').'</a></li></ul>';
		}
		...
		if ($categories) {
			...
			if ($cat_id == 0) {
				echo '<div class="subheading">'.__('Categories', 'wp-symposium').'</div>';
			}
			if ($cat_id > 0) {
				...
				echo "<ul>";
				if ($cat_parent > 0) {
					echo "<li><a href='?cat_id=".$cat_parent."&".$a."'>".__('Up a level', 'wp-symposium')."</a></li>";
				} else {
					echo "<li><a href='?cat_id=".$cat_parent."&".$a."'>".__('Top level', 'wp-symposium')."</a></li>";
				}
				if ($forum) {
					echo "<li><a href='#topics?".$a."'>".__('Jump to topics', 'wp-symposium')."</a></li>";
				}
				echo "</ul>";
			...
			if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {		
				if ( is_user_logged_in() ) {
					echo '<li><a href="forum_new_topic.php?cat_id='.$cat_id.'&'.$a.'">'.__('Add new topic', 'wp-symposium').'</a></li>';
				}
			}
		...
		if ($forum) {
			...
			if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {					
				...
				// Show topics
				echo '<a name="topics" />';
				echo '<div class="subheading">'.__('Topics', 'wp-symposium').'</div>';
				echo '<ul>';
	...
	if ($topic_id > 0) {
		...
		if ($topic) {
			...
			if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {					
				...
				// Show login link?
				if ( !is_user_logged_in() ) {
					echo '<li><a href="login.php?".$a.">'.__('Login', 'wp-symposium').'</a></li>';
				}
				echo "<li><a href='?cat_id=".$topic->topic_category."&".$a."'>Back to ".$cat_name."...</a></li>";
				if ( is_user_logged_in() ) {
					echo "<li><a href='forum_new_reply.php?cat_id=".$topic->topic_category."&topic_id=".$topic_id."&".$a."'>".__('Post a reply', 'wp-symposium')."</a></li>";
				}
				echo "</ul>";
				...
				echo __("Topic started:", "wp-symposium")." ".symposium_time_ago(stripslashes($topic->topic_date));
				echo " ".__("by", "wp-symposium")." ".$topic->display_name;
				echo "</div>";
				...
				if ($replies) {
					echo '<div class="subheading">'.__('Replies', 'wp-symposium').'</div>';
					foreach ($replies as $reply) {
						echo "<div class='line'>";
						echo str_replace(chr(13), "<br />", stripslashes($reply->topic_post));
						echo "</div>";				
						echo "<div class='line reply'>";
						echo symposium_time_ago(stripslashes($reply->topic_date));
						echo " ".__("by", "wp-symposium")." ".$reply->display_name;
						echo "</div>";
					}
				}				
				...
				if ( is_user_logged_in() ) {
					echo "<ul>";
					echo "<li><a href='forum_new_reply.php?topic_id=".$topic_id."&".$a."'>".__('Post a reply', 'wp-symposium')."</a></li>";
					echo "</ul>";
				}
			}			
		}
	...
	echo '<div class="line">'.__('You need to login to view the forum.', 'wp-symposium').'</div>';

}

if ( is_user_logged_in() ) {
	echo '<div class="line">'.__('Logged in as', 'wp-symposium').' '.$current_user->display_name.' - <a href="login.php?action=logout">'.__('Logout', 'wp-symposium').'</a></div>';
}
