...
if ($_POST['new_reply_post'] != '') {
	...
	if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {					
		...
		if (get_option('symposium_moderation') == "on") {
			$topic_approved = "";
		} else {
			$topic_approved = "on";
		}
...
echo '<div class="subheading">'.__('Post a reply', 'wp-symposium').'</div>';
...
echo "<ul><li><a href='forum.php?cat_id=".$cat_id."&".$a."'>".__('Back to', 'wp-symposium')." ".$cat_name."...</a></li></ul>";
...
if ( is_user_logged_in() ) {
	...
	echo '<div class="form">';
		...
		echo __('Reply text', 'wp-symposium').'<br />';
		echo '<textarea name="new_reply_post"></textarea><br />';
		echo '<input type="submit" class="submit" value="'.__('Post reply', 'wp-symposium').'" />';
		echo '</form>';
	echo '</div>';
