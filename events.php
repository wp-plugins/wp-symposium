...
function __wps__events_main() {
	...
	$html = '<div class="__wps__wrapper">';
		...
		$html .= '<div id="__wps__events_list" style="width:95%">';
			...
			if ($include) {
				$sql = "SELECT e.*, u.ID, u.display_name FROM ".$wpdb->prefix."symposium_events e LEFT JOIN ".$wpdb->base_prefix."users u ON event_owner = ID WHERE ".$hide." event_owner IN (".$include.") AND event_live = %s ORDER BY event_start";
			} else {
				$sql = "SELECT e.*, u.ID, u.display_name FROM ".$wpdb->prefix."symposium_events e LEFT JOIN ".$wpdb->base_prefix."users u ON event_owner = ID WHERE ".$hide." event_live = %s ORDER BY event_start";
			}
