...
if ( ( get_option("symposium_version") != WPS_VER && is_admin()) || (isset($_GET['force_create_wps']) && $_GET['force_create_wps'] == 'yes' && is_admin())) {
	...
	if (!$wpdb->get_var($wpdb->prepare("SELECT option_name FROM ".$wpdb->prefix."options WHERE option_name = 'symposium_categories_background'"))) {
		...
		update_option('symposium_viewer', 'Guest');
		update_option('symposium_forum_editor', 'Subscriber');
		...
		update_option('symposium_forum_ajax', 'on');
		...
		update_option('symposium_template_mail', '');
		...
		update_option('symposium_colorbox', 'on');
	...
	if (get_option('symposium_template_mail') == '') {
		update_option('symposium_template_mail', "[compose_form][]<div id='mail_office'><div id='mail_sent_message'></div>[]<input id='compose_button' class='symposium-button' type='submit' value='[compose]'>[]<div id='trays'>[]<div style='float:left; margin-right:10px;'><input type='radio' id='in' class='mail_tray' name='tray' checked> [inbox]<span id='in_unread'></span></div>[]<div style='float:left; margin-right:10px;'><input type='radio' id='sent' class='mail_tray' name='tray'> [sent]</div>[]<div style='float:left;'>[unread]</div>[]</div>[]<div id='symposium_search'>[]<input id='search_inbox' type='text' style='width: 160px'>[]<input id='search_inbox_go' class='symposium-button message_search' type='submit' style='margin-left:10px;' value='Search'>[]</div>[]<div id='mailbox'>[]<div id='mailbox_list'></div>[]<div id='messagebox'></div>[]</div></div>");
	}
	...
	if (get_option('symposium_template_mail_message') == '') {
		update_option('symposium_template_mail_message', "<div id='message_header'><div id='message_header_avatar'>[avatar,44]</div>[mail_subject]<br />[mail_recipient] [mail_sent]</div>[]<div id='message_header_delete'>[delete_button]</div><div id='message_header_reply'>[reply_button]</div>[]<div id='message_mail_message'>[message]</div>");
	}
...
function symposium_uninstall() {
   	...
   	$wpdb->query("DROP TABLE IF EXISTS ".$wpdb->base_prefix."symposium_events");
...
function symposium_lastactivity() {
	...
	if (WPS_DEBUG) {
		echo '<div id="symposium_debug" style="width:90%; text-align:left; margin:10px; border: 1px solid #000; background-color: #fff; border-radius:3px; padding:6px;">';
		echo '<strong>'.__('WP Symposium Debug Mode', 'wp-symposium').'</strong><br /><br />';
		echo '</div>';
	}
...
function symposium_scriptsAction() {
	...
	if (!is_admin()) {
		...
		// Load Symposium JS
	 	...
	 	// Load Colorbox?
		/*
	 	if (get_option('symposium_colorbox')) {
...
function add_symposium_stylesheet() {
	...
	if (!is_admin()) {
		...
		/*
	 	if (get_option('symposium_colorbox')) {
	 	    if (get_option('symposium_wps_css_file') == 'wps.css') {
