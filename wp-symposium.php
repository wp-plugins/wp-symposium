...
if ( ( get_option(WPS_OPTIONS_PREFIX."_version") != WPS_VER && is_admin()) || (isset($_GET['force_create_wps']) && $_GET['force_create_wps'] == 'yes' && is_admin())) {
	...
	if (false) { // Only relevant to old upgrades, don't run but leave in case needed
...
function __wps__js_init() {
	...
	if (!is_admin()) {
		...
		if (get_option(WPS_OPTIONS_PREFIX.'_jqueryui') == "on") {
			wp_enqueue_script('jquery-ui-custom', $plugin.'/js/jquery-ui-1.9.2.custom.min.js', array('jquery'));	
		    wp_register_style('__wps__jquery-ui-css', WPS_PLUGIN_URL.'/css/jquery-ui-1.9.2.custom.css');
