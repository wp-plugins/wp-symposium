<?php
/*
Plugin Name: WP Symposium Social Network
...
if ( ( get_option("symposium_version") != WPS_VER && is_admin()) || (isset($_GET['force_create_wps']) && $_GET['force_create_wps'] == 'yes' && is_admin())) {
	...
	symposium_alter_table("extended", "MODIFY", "extended_name", "varchar(128)", "NOT NULL", "'New field'");
...
function symposium_widget() {
	...
	echo '<img src="'.WPS_IMAGES_URL.'/logo_small.gif" alt="WP Symposium logo" style="float:right; width:100px;height:120px;" />';
...
function add_symposium_stylesheet() {
	...
	if (!is_admin()) {
		...
		// Notices
		echo "<div class='symposium_notice' style='display:none; z-index:999999;'><img src='".WPS_IMAGES_URL."/busy.gif' /> ".__('Saving...', 'wp-symposium')."</div>";
		echo "<div class='symposium_pleasewait' style='display:none; z-index:999999;'><img src='".WPS_IMAGES_URL."/busy.gif' /> ".__('Please Wait...', 'wp-symposium')."</div>";	
...
function symposium_scriptsAction() {
	...
	/*
	define('WPS_SHOW_CATEGORIES', isset($config->show_categories) ? $config->show_categories : '');
	define('WPS_LANGUAGE', isset($config->language) ? $config->language : '');
	define('WPS_SEO', isset($config->seo) ? $config->seo : '');
	define('WPS_BAR_POSITION', isset($config->bar_position) ? $config->bar_position : '');
	define('WPS_BAR_LABEL', isset($config->bar_label) ? $config->bar_label : '');
	define('WPS_ALLOW_PERSONAL_SETTINGS', isset($config->allow_personal_settings) ? $config->allow_personal_settings : '');
	define('WPS_USE_WP_LOGIN', isset($config->use_wp_login) ? $config->use_wp_login : '');
	define('WPS_CUSTOM_LOGIN_URL', isset($config->custom_login_url) ? $config->custom_login_url : '');
	define('WPS_CUSTOM_LOGOUT_URL', isset($config->custom_logout_url) ? $config->custom_logout_url : '');
	define('WPS_LOGIN_REDIRECT', isset($config->login_redirect) ? $config->login_redirect : '');
	define('WPS_VISITORS', isset($config->visitors) ? $config->visitors : '');
	define('WPS_LOGIN_REDIRECT_URL', isset($config->login_redirect_url) ? $config->login_redirect_url : '');
	define('WPS_LOGOUT_REDIRECT', isset($config->logout_redirect) ? $config->logout_redirect : '');
	define('WPS_LOGOUT_REDIRECT_URL', isset($config->logout_redirect_url) ? $config->logout_redirect_url : '');
	define('WPS_ENABLE_REDIRECTS', isset($config->enable_redirects) ? $config->enable_redirects : '');
	define('WPS_AUDIT_WIPE', isset($config->audit_wipe) ? $config->audit_wipe : '');
	define('WPS_USE_WP_REGISTER', isset($config->use_wp_register) ? $config->use_wp_register : '');
	define('WPS_CUSTOM_REGISTER_URL', isset($config->custom_register_url) ? $config->custom_register_url : '');
	define('WPS_REGISTER_USE_SUM', isset($config->register_use_sum) ? $config->register_use_sum : '');
	define('WPS_REGISTER_USE_CAPTCHA', isset($config->register_use_captcha) ? $config->register_use_captcha : '');
	define('WPS_REGISTER_URL', isset($config->register_url) ? $config->register_url : '');
	define('WPS_CAPTCHA_PUBLIC', isset($config->captcha_public) ? $config->captcha_public : '');
	define('WPS_CAPTCHA_PRIVATE', isset($config->captcha_private) ? $config->captcha_private : '');
	define('WPS_LOGIN_URL', isset($config->login_url) ? $config->login_url : '');
	define('WPS_AVATAR_URL', isset($config->avatar_url) ? $config->avatar_url : '');
	define('WPS_REGISTER_MESSAGE', isset($config->register_message) ? $config->register_message : '');
	define('WPS_SHOW_PROFILE_MENU', isset($config->show_profile_menu) ? $config->show_profile_menu : '');
	define('WPS_CHAT_PURGE', isset($config->chat_purge) ? $config->chat_purge : '');
	define('WPS_PROFILE_PAGE_CONTENT', isset($config->profile_page_content) ? $config->profile_page_content : '');
	define('WPS_IMG_TMP', isset($config->img_tmp) ? $config->img_tmp : '');
	*/
	...
	if (!is_admin()) {
		...
		wp_enqueue_script('jwplayer', WP_PLUGIN_URL.'/wp-symposium/jwplayer/jwplayer.js', array('jquery'));
		if (WPS_ELASTIC == 'on') {
