...
add_action('admin_init', 'symposium_admin_header', 10);
...
if ( ( get_option("symposium_version") != WPS_VER && is_admin()) || (isset($_GET['force_create_wps']) && $_GET['force_create_wps'] == 'yes' && is_admin())) {
	...
	symposium_alter_table("config", "ADD", "img_url", "varchar(128)", "NOT NULL", "'/'.WP_CONTENT_DIR.'/wps-content'");
...
function symposium_admin_warnings() {
    ...
    if ($wpdb->get_var("SELECT motd FROM ".$wpdb->prefix.'symposium_config') != 'on') {
	    ...
	    echo __("Please visit the WP Symposium Release notes page for important release notes and what's new.", "wp-symposium");
	    echo "</p>";
