...
function symposium_scriptsAction() {
	...
	if (!is_admin()) {
		...
		if ($page_gid > 0) {
			$show_forum_default = $wpdb->get_var($wpdb->prepare("SELECT show_forum_default FROM ".$wpdb->prefix."symposium_groups WHERE gid = %d", $page_gid));
			if ($show_forum_default == 'on') {
				$cat_id = $show_forum_default;
			}
		}
