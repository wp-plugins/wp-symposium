...
function symposium_smilies($buffer){ // $buffer contains entire page
	...
	if (!get_option('symposium_wps_lite') && !strpos($buffer, "<rss") ) {
		...
		global $wpdb;
		$emoticons = get_option('symposium_emoticons');
...
function js_init() {
	...
	if (get_option('symposium_jqueryui') == "on" || is_admin()) {
	...
	if (!is_admin()) {
		...
		*/
		...
		wp_localize_script( 'symposium', 'symposium', array(
			...
			'wps_wysiwyg' => function_exists('symposium_profile_plus') ? get_option('symposium_use_wysiwyg') : '',
