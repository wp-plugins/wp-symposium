...
if ( ( get_option("symposium_version") != WPS_VER && is_admin()) || (isset($_GET['force_create_wps']) && $_GET['force_create_wps'] == 'yes' && is_admin())) {
	...
	symposium_alter_table("config", "ADD", "forum_refresh", "varchar(2)", "NOT NULL", "''");
	symposium_alter_table("config", "ADD", "subject_mail_new", "varchar(128)", "NOT NULL", "'New Mail Message: [subject]'");
	symposium_alter_table("config", "ADD", "subject_forum_new", "varchar(128)", "NOT NULL", "'New Forum Topic'");
	symposium_alter_table("config", "ADD", "subject_forum_reply", "varchar(128)", "NOT NULL", "'New Forum Reply'");
	symposium_alter_table("config", "ADD", "profile_comments", "varchar(2)", "NOT NULL", "'on'");
	symposium_alter_table("config", "ADD", "forum_login_form", "varchar(2)", "NOT NULL", "'on'");
	symposium_alter_table("config", "ADD", "forum_lock", "int(11)", "NOT NULL", "'30'");
	symposium_alter_table("config", "ADD", "use_wysiwyg_3", "varchar(256)", "NOT NULL", "''");
	symposium_alter_table("config", "ADD", "dir_level", "varchar(256)", "", "'s:60:\"Everyone,Administrator,Editor,Author,Contributor,Subscriber,\";'");
	...
	if ($wpdb->get_var("SELECT template_mail FROM ".$wpdb->prefix.'symposium_config') == '') {
		$wpdb->query("UPDATE ".$wpdb->prefix."symposium_config SET template_mail = \"[compose_form][]<div id='mail_office'><div id='mail_sent_message'></div>[]<input id='compose_button' class='symposium-button' type='submit' value='[compose]'>[]<div id='trays'>[]<div style='float:left; margin-right:10px;'><input type='radio' id='in' class='mail_tray' name='tray' checked> [inbox]<span id='in_unread'></span></div>[]<div style='float:left; margin-right:10px;'><input type='radio' id='sent' class='mail_tray' name='tray'> [sent]</div>[]<div style='float:left;'>[unread]</div>[]</div>[]<div id='symposium_search'>[]<input id='search_inbox' type='text' style='width: 160px'>[]<input id='search_inbox_go' class='symposium-button message_search' type='submit' style='margin-left:10px;' value='Search'>[]</div>[]<div id='mailbox'>[]<div id='mailbox_list'></div>[]<div id='messagebox'></div>[]</div></div>\""); 
	}
...
function symposium_scriptsAction() {
	...
	// Set admin page
	define('WPS_ADMIN_PAGE', 'symposium_debug');
	...
	define('WPS_FORUM_REFRESH', $config->forum_refresh);	
	define('WPS_MAIL_NEW', $config->subject_mail_new);	
	define('WPS_FORUM_NEW', $config->subject_forum_new);	
	define('WPS_FORUM_REPLY', $config->subject_forum_reply);	
	define('WPS_PROFILE_COMMENTS', $config->profile_comments);	
	define('WPS_FORUM_LOGIN_FORM', $config->forum_login_form);
	define('WPS_FORUM_LOCK', $config->forum_lock);
	...
	if (!is_admin()) {
		...
		wp_localize_script( 'symposium', 'symposium', array(
			...
			'wps_forum_refresh' => WPS_FORUM_REFRESH,
