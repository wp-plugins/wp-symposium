...
if ( ( get_option("symposium_version") != WPS_VER && is_admin()) || (isset($_GET['force_create_wps']) && $_GET['force_create_wps'] == 'yes' && is_admin())) {
	...
	$img_url = str_replace(get_bloginfo('url'), '', $img_url); 
...
function add_symposium_stylesheet() {
	...
	if (!is_admin()) {
		...
		// Information for Javascript (such as translations)
		...
		echo "<div id='symposium_whatsup' style='display:none'>".__("What's up?", "wp-symposium")."</div>";
	}
