...
function symposium_send_poke($message_to, $message_from, $from_name, $poke) {
	$url = symposium_get_url('profile');
	$message = $from_name.__(' has sent you a ', 'wp-symposium').$poke;
	symposium_news_add($message_from, $message_to, "<a href='".$url.symposium_string_query($url)."uid=".$message_from."'>".$message."</a>");
}
add_filter('symposium_send_poke_filter', 'symposium_send_poke', 10, 4);
...
function symposium_news_add_message($message_to, $message_from, $from_name, $mail_id) {
	...
	symposium_news_add($message_from, $message_to, "<a href='".$url.symposium_string_query($url)."mid=".$mail_id."'>".__("You have a new message from", "wp-symposium-news")." ".$from_name."</a>");
}
...
function symposium_news_add_friendrequest($message_to, $message_from, $from_name) {
	$url = symposium_get_url('profile');
	symposium_news_add($message_from, $message_to, "<a href='".$url.symposium_string_query($url)."view=friends'>".__("New friend request from", "wp-symposium-news")." ".$from_name."</a>");
}
...
function symposium_news_add_friendaccepted($message_to, $message_from, $from_name) {
	$url = symposium_get_url('profile');
	symposium_news_add($message_from, $message_to, "<a href='".$url.symposium_string_query($url)."view=friends'>".__("Friend request accepted by", "wp-symposium-news")." ".$from_name."</a>");
}
...
function symposium_news_add_newtopic($message_to, $from_id, $from_name, $url) {
	symposium_news_add($from_id, $message_to, "<a href='".$url."'>".__("Subscribed forum topic by", "wp-symposium-news")." ".$from_name."</a>");
}
...
function symposium_news_add_newreply($message_to, $message_from, $from_name, $url) {
	if ($message_to != $message_from) {
		symposium_news_add($message_from, $message_to, "<a href='".$url."'>".__("Subscribed forum reply by", "wp-symposium-news")." ".$from_name."</a>");
	}
}
...
function symposium_news_add_wall_newpost($post_to, $post_from, $from_name) {
	if ($post_to != $post_from) {
		symposium_news_add($post_from, $post_to, "<a href='".symposium_get_url('profile')."'>".$from_name." ".__("has posted on your profile.", "wp-symposium-news")."</a>");
	}
}
...
function symposium_news_add_wall_reply($first_post_subject, $first_post_author, $from_id, $from_name, $url) {
	...
	if ($first_post_subject != $current_user->ID) {
		symposium_news_add($from_id, $first_post_subject, "<a href='".$url."'>".$from_name." ".__("has replied to a post on your profile", "wp-symposium-news")."</a>");
	} else {
		if ($first_post_author != $current_user->ID) {
			symposium_news_add($from_id, $first_post_author, "<a href='".$url."'>".$from_name." ".__("has replied to a post you started", "wp-symposium-news")."</a>");
		}
	}
...
function symposium_news_add_wall_reply_involved_in($post_to, $post_from, $from_name, $url) {
	if ($post_to != $post_from) {
		symposium_news_add($post_from, $post_to, "<a href='".$url."'>".$from_name." ".__("has replied to a post you are involved in", "wp-symposium-news")."</a>");
	}
}
...
if ( ( get_option("symposium_version") != WPS_VER && is_admin()) || ($_GET['force_create_wps'] == 'yes' && is_admin())) {
	...
	if ($wpdb->get_var("SELECT template_forum_category FROM ".$wpdb->prefix.'symposium_config') == '') {
		$wpdb->query("UPDATE ".$wpdb->prefix."symposium_config SET template_forum_category = \"<div class='row_startedby'>[]<div class='avatar avatar_last_topic'>[avatar,32]</div>[replied][subject][ago]</div>[]<div class='row_views'>[post_count]</div>[]<div class='row_topic row_replies'>[topic_count]</div>[]<div class='row_topic'>[category_title]<br />[category_desc]</div>\""); 
	}
	...
	symposium_alter_table("topics", "ADD", "score", "int(11)", "", "0");
...
function symposium_admin_warnings() {
    ...
    if ($wpdb->get_var("SELECT motd FROM ".$wpdb->prefix.'symposium_config') != 'on') {
	    ...
	    echo __("Please visit the WP Symposium Settings page for important release notes.", "wp-symposium");
	    echo "</p>";
...
function symposium_admin_check() {
	...
	if ($missing_users) {
		foreach ($missing_users as $missing) {
			...
			$sql = "DELETE FROM ".$wpdb->base_prefix."symposium_friends WHERE friend_from = ".$missing->uid." or friend_to = %d";
			$wpdb->query($wpdb->prepare($sql, $missing->uid)); 
		}
	}	
...
function js_init() {
	...
	if ($jquery->jqueryui == "on" || is_admin()) {
