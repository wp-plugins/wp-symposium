...
function __wps__header() {
	...
	if (__wps__required()) {
		...
		if ($has_bronze_plug_actived && !__wps__is_plus()) {
			...
			echo sprintf(__('You have activated a <a href="'.get_admin_url().'admin.php?page=symposium_debug">Bronze/Gold %s feature</a>, but you have not entered your <a href="'.get_admin_url().'admin.php?page=symposium_debug">Activation Code</a>.', WPS_TEXT_DOMAIN), WPS_WL).' ';
...
add_action("personal_options_update", "__wps__save_metadata", 10, 0);

// Runs when the page data is edited by an admin. Action function argument: user ID.
add_action("edit_user_profile_update", "__wps__save_metadata", 10, 0);
