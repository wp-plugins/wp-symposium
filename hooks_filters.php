...
{  
	global $wpdb,$current_user;
			...
			if ( ( get_option(WPS_OPTIONS_PREFIX.'_menu_profile') == 'on') && ( ($uid1 == $uid2) || (is_user_logged_in() && strtolower($share) == 'everyone') || (strtolower($share) == 'public') || (strtolower($share) == 'friends only' && $is_friend) || __wps__get_current_userlevel() == 5) ) {
...
function __wps__header() {
	...
	if (__wps__required()) {
		...
		if ($has_bronze_plug_actived && !__wps__is_plus()) {
			...
			echo sprintf(__('You have activated a <a href="'.get_admin_url().'plugins.php">Bronze %s plugin</a>, but you have not entered your <a href="'.get_admin_url().'admin.php?page=symposium_debug">Activation Code</a>.', WPS_TEXT_DOMAIN), WPS_WL).' ';
			echo sprintf(__('You can get your Activation Code on the <a href="http://www.wpsymposium.com/membership" target="_blank">Membership page</a> on the %s website.', WPS_TEXT_DOMAIN), WPS_WL);
			echo '</div>';
		}		
...
add_action("__wps__show_user_profile", "__wps__show_metadata", $user, 10, 1);
...
add_action("__wps__edit_user_profile", "__wps__show_metadata", $user, 10, 1);
...
function __wps__save_metadata($uid) {
	...
	if ( $_POST["action"] == 'update' ) {
		...
		__wps__update_meta($uid, 'facebook_id', isset($_POST["facebook_id"]) ? $_POST["facebook_id"] : "");
		...
		if ($extensions) {
			...
			foreach ($extensions as $extension) {
				...
				if ( $extension->wp_usermeta == '' ) {
					...
					__wps__update_meta($uid, $slug, $value);
				}
			}
		}
	}
...
add_action("__wps__personal_options_update", "__wps__save_metadata", $uid, 10, 1);
// Runs when the page data is edited by an admin. Action function argument: user ID.
add_action("__wps__edit_user_profile_update", "__wps__save_metadata", $uid, 10, 1);
