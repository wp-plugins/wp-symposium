...
class __wps__recent_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if (get_option(WPS_OPTIONS_PREFIX.'_ajax_widgets') == 'on') {
			...
			echo "<img src='".get_option(WPS_OPTIONS_PREFIX.'_images')."/busy.gif' />";
			echo '<div id="symposium_recent_Widget_count" style="display:none">'.$__wps__recent_count.'</div>';
			echo '<div id="symposium_recent_Widget_desc" style="display:none">'.$__wps__recent_desc.'</div>';
			echo '<div id="symposium_recent_Widget_show_light" style="display:none">'.$__wps__recent_show_light.'</div>';
			echo '<div id="symposium_recent_Widget_show_mail" style="display:none">'.$__wps__recent_show_mail.'</div>';
			echo '</div>';	
...
class __wps__forumrecentposts_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if (get_option(WPS_OPTIONS_PREFIX.'_ajax_widgets') == 'on') {
			...
			__wps__do_Forumrecentposts_Widget($postcount,$preview,$cat_id,$show_replies,$incl_cat,$incl_parent);			
		}
...
class __wps__summary_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if (get_option(WPS_OPTIONS_PREFIX.'_ajax_widgets') == 'on') {
			...
			__wps__do_summary_Widget($show_loggedout,$show_form,$login_url,$show_avatar,$login_username,$login_password,$login_remember_me,$login_button,$login_forgot,$login_register);			
		}
	...
	function form( $instance ) {
		...
		<br /><br />
			...
			/>
			<label for="<?php echo $this->get_field_id( 'show_avatar' ); ?>"><?php echo __('Show avatar', WPS_TEXT_DOMAIN); ?></label>
		<br />
