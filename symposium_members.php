	...
	$html .= '<div class="symposium-wrapper">';
		...
		$html .= '<br /><div id="symposium_members_orderby_div">';
			...
			$html .= '<select id="symposium_members_orderby">';
				...
				if (get_option('symposium_use_distance') && function_exists('symposium_profile_plus') && !get_option('symposium_hide_location')) {
		...
		$html .= '<div id="symposium_members_atoz">';
			for ($i = 65; $i <= 90; $i++) { 
				if (chr($i) != strtoupper($term)) {
					...
					$member_url = symposium_get_url('members');
					$q = symposium_string_query($member_url);
		...
		$html .= '<div id="symposium_members">';
			...
			if (get_option('symposium_use_distance') && $lat != 0 && is_user_logged_in() && function_exists('symposium_profile_plus')) {
							...
							$html .= "<div class='members_row";
								...
								$html .= "<div class='members_info'>";
									...
									$html .= "<div style='padding-left: 75px;'>";						
										...
										$html = apply_filters ( 'symposium_directory_member_filter', $html, $member->uid);
		...
		$html .= '</div>'; // symposium_members
		
	$html .= '</div>'; // symposium-wrapper
