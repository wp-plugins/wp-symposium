	...
	$dbpage = $plugin.'/symposium_members_db.php';
		...
		if ($ver == '') {
			...
			$html = '<div class="symposium-wrapper">';
				...
				if ( (isset($_POST['member']) && $_POST['member'] != '') || (isset($_GET['term']) && $_GET['term'] != '' ) ) {
					...
					$term = "";
					if (isset($_POST['member'])) { $term .= $_POST['member']; }
					...
					$sql = "SELECT u.ID, m.last_activity, m.city, m.country, m.share, m.wall_share, m.extended, 
						(SELECT comment 
							FROM ".$wpdb->base_prefix."symposium_comments 
							WHERE author_uid = u.ID AND subject_uid = author_uid AND comment_parent = 0 
							ORDER BY cid DESC LIMIT 0,1)
							AS latest_comment, 
						(SELECT COUNT(*) 
							FROM ".$wpdb->base_prefix."symposium_friends 
							WHERE friend_from = ".$me." AND friend_to = u.ID) 
							AS is_friend 
