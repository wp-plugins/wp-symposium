<?php
/**
 * Template Name: WPS demo forum page 4
...
<div id="primary">
	...
	if (isset($_POST['symposium_reply_text']) && $_POST['symposium_reply_text']) {
		$wps_forum->add_reply($tid, $_POST['symposium_reply_text']);
	}
	...
	if ($tid) {
		...
		if ($topic) {
			...
			echo '<div id="my-forum-topic">';
				...
				echo 'Started by '.$topic->display_name.' '.symposium_time_ago($topic->topic_started);
				echo '</div>';
				echo '<div id="my-topic-first-post">';
					$post_text = symposium_make_url(stripslashes($topic->topic_post));
					$post_text = symposium_bbcode_replace($post_text);
					if (!get_option('symposium_use_wysiwyg')) $post_text = str_replace(chr(13), "<br />", $post_text);
					echo $post_text;
				echo '</div>';
			echo '</div>';
			...
			// Show avatar
				echo '<div class="my-forum-reply">';
					...
					echo '<div class="my-reply-author">';
						echo 'Reply by '.$reply->display_name.' '.symposium_time_ago($reply->topic_started);
					echo '</div>';
					$post_text = symposium_make_url(stripslashes($reply->topic_post));
					$post_text = symposium_bbcode_replace($post_text);
				...
				foreach ($topics as $topic) {
					echo '<div class="my-topic-row">';
						...
						echo '<div class="my-topic-row-title">';
							...
							echo "<a href='".$wps->get_forum_url().symposium_string_query($wps->get_forum_url())."catid=".$catid."&tid=".$topic->tid."'>".$topic_subject."</a>";
						echo '</div>';
						// Last reply
