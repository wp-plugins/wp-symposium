 ...
 * Template Name: WPS demo mail page 7

 * Description: A Mail Page Template to demonstrate using WPS classes
...
First we over-ride WPS settings for mail page to ensure links to this page across WPS go to
	...
	if ( ($action == 'reply' && $mail_id) || ($action == 'inbox' || $action == 'sent') ) { 
		...
		if ($action == 'reply') {
			...
			if ($original) {
				...
				$header .= 'Date: '.$d[2].' '.substr(wps_get_monthname($d[1]),0,3).' '.$d[0].', '.$t[0].':'.$t[1].chr(13);
		...
		echo '<div '.$override_hidden_state.' id="my-compose-form">';
			...
			echo '<div id="my-compose-form-to">';
				...
				$friends_array=sub_val_sort($friends_array, 'display_name');
				...
				foreach($inbox as $mail) {
					...
					echo '<div class="my-inbox-row '.$mail_unread.'">';
						...
						echo '<div class="my-inbox-sent">';
							...
							echo $d[2].' '.substr(wps_get_monthname($d[1]),0,3).' '.$d[0].', '.$t[0].':'.$t[1].'<br />';
							...
							echo '<a href="'.$wps->get_mail_url().'?start='.$start.'&action=delete&id='.$mail['mail_id'].'"><img src="'.get_option('symposium_images').'/delete.png" /></a>';
		...
		if ($pages > 1) {
			...
			if ($current_page > 1) echo '<a href="'.$wps->get_mail_url().'?action='.$paging_action.'&start='.((($current_page-1)*$page_length)-$page_length+1).$term_parameter.'">Previous</a> ';
			...
			for ($p=1; $p<=$pages; $p++) {
				...
				if ($p==$current_page) {
					...
					echo ' <a href="'.$wps->get_mail_url().'?action='.$paging_action.'&start='.(($p*$page_length)-$page_length).$term_parameter.'">'.$p.'</a>';

				}

			}

			if ($current_page < $pages) echo ' <a href="'.$wps->get_mail_url().'?action='.$paging_action.'&start='.((($current_page+1)*$page_length)-$page_length).$term_parameter.'">Next</a>';

			if ($current_page < $pages) echo ' <a href="'.$wps->get_mail_url().'?action='.$paging_action.'&start='.(($pages*$page_length)-$page_length).$term_parameter.'">Last</a>';
		...
		if ($term) {
			...
			echo '<a href="'.$wps->get_mail_url().'">Clear search</a>.';
		...
		$id = isset($_GET['mail_id']) ? $_GET['mail_id'] : $_GET['mid']; // To ensure WPS compatibility
		...
		if ($message) {
			...
			echo '<div id="my-mail">';
				...
				echo '<div id="my-mail-sent"><div class="my-mail-label">Received:</div> '.$d[2].' '.substr(wps_get_monthname($d[1]),0,3).' '.$d[0].', '.$t[0].':'.$t[1].'</div>';

				$mail_message = symposium_make_url(stripslashes($message->mail_message));
