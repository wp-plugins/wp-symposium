<?php
/**
 * Template Name: WPS demo profile page 6
 * Description: A Profie Page Template to demonstrate using WPS classes
 *
 * @package WordPress
...
/*
The following is shown here now for reference, but will be placed in a separate file from
...
jQuery(document).ready(function() { 	
	...
	// Add drop-down menu functionality
	jQuery('#my-dropdown-menu-title').live('mouseenter', function(event) {
		jQuery("#my-dropdown-menu-items").show();
	});
	jQuery("#my-dropdown-menu-items div").click(function(){
		jQuery("#my-dropdown-menu-items").hide();
	});
	jQuery('#my-dropdown-menu-items').live('mouseleave', function(event) {
        jQuery("#my-dropdown-menu-items").hide();
	});
...
/*
First we over-ride WPS settings for profile page to ensure links to other members go to
...
<div id="primary">
	...
	echo '<div id="my-sidebar">';
		echo '<div id="my-profile-box">';
			...
			if ($wps_user->get_id() != $current_user->ID) {
				...
				// Poke(tm) button
				echo '<div style="margin-top:10px; margin-bottom:10px">'.$wps_ui->poke_button(get_option('symposium_poke_label'), "my-submit-button").'</div>';
			...
			if ($wps_user->is_permitted('personal')) { // defaults to activity permission, set to "personal" for personal info
				...
				if ($wps_user->get_dob_day() && $wps_user->get_dob_month() && $wps_user->get_dob_year()) {
					echo '<span class="my-info-label">Born:</span> '.$wps_user->get_dob_day().' '.wps_get_monthname($wps_user->get_dob_month()).' '.$wps_user->get_dob_year().'<br />';
				}
	...
	echo '<div id="my-header-div">';
		...
		if ($wps_user->is_permitted()) { // defaults to activity permission, set to "personal" for personal info
			echo '<div>';
			echo $wps_user->get_latest_activity().' '.symposium_time_ago($wps_user->get_latest_activity_age());
			echo '</div>';	
		}
		...
		if ($wps_user->get_id() == $current_user->ID) {
			...
			// Add WPS Facebook Connect plugin (plugin needs to be activated)
	...
	echo '<div id="my-content" class="symposium-wrapper">';	
		if ($wps_user->is_permitted()) { 
			...
			echo '<div id="my-menu">';
				...
				// Additional drop down menu for the final page choices
				echo '<div id="my-dropdown-menu">';
					echo '<div id="my-dropdown-menu-title">more...</div>';
					echo '<div id="my-dropdown-menu-items">';
						echo '<div id="menu_friends" class="symposium_profile_menu my-dropdown-menu-item">Friends</div>';
						echo '<div id="menu_groups" class="symposium_profile_menu my-dropdown-menu-item">Groups</div>';
						if ($wps_user->get_id() == $current_user->ID) {
							echo '<div id="menu_avatar" class="symposium_profile_menu my-dropdown-menu-item">Avatar</div>';
							echo '<div id="menu_personal" class="symposium_profile_menu my-dropdown-menu-item">Edit Profile</div>';
							echo '<div id="menu_settings" class="symposium_profile_menu my-dropdown-menu-item">Settings</div>';
						}
					echo '</div>';
				echo '</div>';
